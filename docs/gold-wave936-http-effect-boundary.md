...
W941/W944 add the eighth consumer family, Hacker News. Its top-list and item requests preserve their explicit 15-second timeout and versioned User-Agent on the sealed request. W942 reviewed status/body caps/JSON processing before terminal; seven existing HN regression tests are selected. The direct bytes dependency references the existing locked version only. Core413a failures are not acceptance; the corrected source still needs hosted gates.

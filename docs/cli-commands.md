...
- `--sampling-seed <SEED>` — Optional RNG seed for reproducible sampling. Pair with `--temperature > 0` for a replayable non-greedy call. Unsupported providers fail
...
and `settings.sampling_seed`. Sampling overrides enter the same strict
provider-leaf validation as the corresponding `neoth chat` flags; unsupported
or out-of-range values fail before authorization and transport.

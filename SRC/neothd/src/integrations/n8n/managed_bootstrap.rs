...
fn read_custody(home: &Path) -> Result<Option<BootstrapCustody>> {
    ...
    match std::fs::read(path) {
        Ok(bytes) => serde_json::from_slice(&bytes)
            .map(Some)
            .map_err(|_| anyhow!("n8n_bootstrap_custody_invalid")),
...
fn recovery_secrets_exist(custody: &BootstrapCustody, store: &dyn SecretStore) -> bool {
    ...
    (!owner
        || (store
            .get(&secret_key(&custody.job_id, "owner-password"))
            .ok()
            .flatten()
            .is_some()
            && store
                .get(&secret_key(&custody.job_id, "browser-id"))
                .ok()
                .flatten()
                .is_some()
            && store
                .get(&secret_key(&custody.job_id, "api-key-label"))
                .ok()
                .flatten()
                .is_some()))
        && (!captured
...
fn random_hex(len: usize) -> Result<String> {
    ...
    getrandom::getrandom(&mut bytes).map_err(|_| anyhow!("n8n_bootstrap_rng_failed"))?;
    Ok(hex(&bytes))
}
fn bootstrap_password() -> Result<String> {
    Ok(format!("N9{}", random_hex(30)?))
}
fn owner_email(job_id: &str) -> String {
    format!(
        "owner-{}@invalid.test",
        job_id.chars().take(8).collect::<String>()
    )
}
fn secret_key(job: &str, kind: &str) -> String {
    format!("n8n-bootstrap.{kind}.{job}")
}

async fn docker<R: BootstrapDockerRunner>(
    runner: &mut R,
    argv: &[String],
...
async fn docker_secret_exec<R: BootstrapDockerRunner>(
    ...
    cancel: &mut tokio::sync::oneshot::Receiver<()>,
        ...
        managed_runtime::valid_container_id(id),
        "n8n_bootstrap_container_id_invalid"
    );
    let argv = vec![
        "docker".into(),
        "exec".into(),
    ...
    let output = runner
        .run(&argv, Some(payload), cancel)
        .await
        .map_err(|_| anyhow!("n8n_bootstrap_http_unknown"))?;
...
fn node_reply(bytes: &[u8]) -> Result<NodeReply> {
    let reply: NodeReply =
        serde_json::from_slice(bytes).map_err(|_| anyhow!("n8n_bootstrap_http_invalid"))?;
    ensure!(
        reply.status >= 200 && reply.status < 300 && reply.body.get("data").is_some(),
        "n8n_bootstrap_http_rejected"
    );
    Ok(reply)
}
fn cookie(reply: &NodeReply) -> Result<String> {
    let value = reply.cookie.split(';').next().unwrap_or_default();
        ...
        value.starts_with("n8n-auth=") && value.len() <= 8192,
        "n8n_bootstrap_session_invalid"
    );
    Ok(value.into())
}
fn data_bool(reply: &NodeReply, path: &[&str]) -> Option<bool> {
    path.iter()
        .try_fold(reply.body.get("data")?, |v, k| v.get(*k))
        .and_then(Value::as_bool)
}

fn bootstrap_argv(c: &BootstrapCustody) -> Vec<String> {
    vec![
        "docker".into(),
        "run".into(),
        "-d".into(),
        "--name".into(),
        format!("neoth-n8n-bootstrap-{}", c.job_id),
        "--network".into(),
        "none".into(),
        "--label".into(),
        "io.neoth.managed=n8n".into(),
        "--label".into(),
        format!("io.neoth.n8n-job={}", c.job_id),
        "--label".into(),
        format!("io.neoth.n8n-bootstrap={BOOTSTRAP_SCHEMA}"),
        "-v".into(),
        format!("{}:{MOUNT}", c.volume_name),
        "-e".into(),
        "N8N_LISTEN_ADDRESS=127.0.0.1".into(),
        c.pinned_image.clone(),
    ]
}
fn volume_argv(c: &BootstrapCustody) -> Vec<String> {
    vec![
        "docker".into(),
        "volume".into(),
        "create".into(),
...
async fn inspect_volume<R: BootstrapDockerRunner>(
    runner: &mut R,
    c: &BootstrapCustody,
    cancel: &mut tokio::sync::oneshot::Receiver<()>,
) -> Result<String> {
    ...
    let rows: Vec<Value> = serde_json::from_slice(&bytes)
        .map_err(|_| anyhow!("n8n_bootstrap_volume_inspect_invalid"))?;
    let row = rows
        .first()
        .ok_or_else(|| anyhow!("n8n_bootstrap_volume_inspect_invalid"))?;
    ensure!(
        rows.len() == 1 && row.get("Name").and_then(Value::as_str) == Some(c.volume_name.as_str()),
        "n8n_bootstrap_volume_inspect_invalid"
    );
    let labels = row
        .get("Labels")
        ...
        labels.get("io.neoth.managed").and_then(Value::as_str) == Some("n8n")
            ...
            && labels.get("io.neoth.n8n-bootstrap").and_then(Value::as_str)
                == Some(BOOTSTRAP_SCHEMA),
        "n8n_bootstrap_volume_mismatch"
    );
    Ok(format!("{:x}", Sha256::digest(&*bytes)))
}
fn validate_bootstrap_inspect(
    bytes: &[u8],
    c: &BootstrapCustody,
    require_running: bool,
) -> Result<()> {
    let rows: Vec<Value> =
        serde_json::from_slice(bytes).map_err(|_| anyhow!("n8n_bootstrap_inspect_invalid"))?;
    let row = rows
        .first()
        .ok_or_else(|| anyhow!("n8n_bootstrap_inspect_invalid"))?;
    let labels = row
    ...
    let mount = row
        ...
        rows.len() == 1
            ...
            && row.pointer("/Config/Image").and_then(Value::as_str)
                == Some(c.pinned_image.as_str())
            && labels.get("io.neoth.managed").and_then(Value::as_str) == Some("n8n")
            && labels.get("io.neoth.n8n-job").and_then(Value::as_str) == Some(c.job_id.as_str())
            && labels.get("io.neoth.n8n-bootstrap").and_then(Value::as_str)
                == Some(BOOTSTRAP_SCHEMA)
            ...
            && row.pointer("/State/Running").and_then(Value::as_bool) == Some(require_running),
        "n8n_bootstrap_identity_mismatch"
    );
    Ok(())
}
async fn inspect_bootstrap<R: BootstrapDockerRunner>(
    runner: &mut R,
    c: &BootstrapCustody,
    running: bool,
    cancel: &mut tokio::sync::oneshot::Receiver<()>,
    ...
    let id = c
        ...
        .as_deref()
        .ok_or_else(|| anyhow!("n8n_bootstrap_custody_missing"))?;
    validate_bootstrap_inspect(
        &docker(
            runner,
            &[
                "docker".into(),
...
async fn exact_absent<R: BootstrapDockerRunner>(
    runner: &mut R,
    id: &str,
    cancel: &mut tokio::sync::oneshot::Receiver<()>,
) -> Result<bool> {
...
async fn recovery_proof_with<R: BootstrapDockerRunner>(
    ...
    match custody.phase {
        ...
        | BootstrapPhase::KeyCaptured => {
            let receipt =
                recovery_bootstrap_receipt(&mut runner, custody, true, &mut cancel).await?;
            Ok(recovery_digest(
                custody,
                "bootstrap-running",
                &volume_receipt,
        ...
        BootstrapPhase::BootstrapStopped => {
            let receipt =
                recovery_bootstrap_receipt(&mut runner, custody, false, &mut cancel).await?;
            Ok(recovery_digest(
                custody,
                "bootstrap-stopped",
                &volume_receipt,
        ...
        BootstrapPhase::BootstrapRemoved => {
            let id = custody
                ...
                exact_absent(&mut runner, id, &mut cancel).await?,
                "n8n_bootstrap_recovery_absence_unproven"
            );
            Ok(recovery_digest(
                custody,
                "bootstrap-absent",
    ...
    W: FnMut(&Path, &BootstrapCustody) -> Result<()>,
{
    ...
    let runtime_id = bound_container_id(home).map_err(anyhow::Error::msg)?;
        ...
        runtime_id
            .as_deref()
            .is_some_and(managed_runtime::valid_container_id),
...
pub(crate) async fn install_bootstrap_at(
    home: &Path,
    port: u16,
    cancel: &mut tokio::sync::oneshot::Receiver<()>,
) -> Result<super::IntegrationJob> {
    install_bootstrap_at_with(
        home,
        port,
        &mut LocalBootstrapDockerRunner::default(),
        cancel,
    )
    .await
}
async fn install_bootstrap_at_with<R: BootstrapDockerRunner>(
    home: &Path,
    port: u16,
    runner: &mut R,
    cancel: &mut tokio::sync::oneshot::Receiver<()>,
    ...
    let request = ManagedN8nRequest::new_with_volume(port, N8N_OCI_REFERENCE, volume.clone())
    ...
    let mut custody = BootstrapCustody {
        schema_version: 2,
        phase: BootstrapPhase::VolumeIntent,
        job_id: job_id.clone(),
        manifest_sha256: prepared.manifest_sha256.as_str().into(),
        volume_name: volume.clone(),
        bootstrap_container_id: None,
        runtime_container_id: None,
        pinned_image: N8N_OCI_REFERENCE.into(),
        host_port: port,
        api_key_label: label.clone(),
    };
    write_custody(home, &custody)?;
    let store = crate::config::keychain::open_store()
        .context("n8n bootstrap requires the private OS secret store")?;
    let password = SecretString::from(bootstrap_password()?);
    let browser = SecretString::from(random_hex(16)?);
    store.set(&secret_key(&job_id, "owner-password"), &password)?;
    store.set(&secret_key(&job_id, "browser-id"), &browser)?;
    store.set(
        &secret_key(&job_id, "api-key-label"),
        &SecretString::from(label.clone()),
    )?;
    docker(runner, &volume_argv(&custody), None, cancel).await?;
    inspect_volume(runner, &custody, cancel).await?;
    custody.phase = BootstrapPhase::VolumeBound;
    write_custody(home, &custody)?;
    let created = docker(runner, &bootstrap_argv(&custody), None, cancel).await?;
    let id = String::from_utf8(created.to_vec())
        .map_err(|_| anyhow!("n8n_bootstrap_docker_invalid"))?
        .trim()
        .to_owned();
    ensure!(
        managed_runtime::valid_container_id(&id),
    ...
    custody.bootstrap_container_id = Some(id.clone());
    custody.phase = BootstrapPhase::BootstrapContainerBound;
    write_custody(home, &custody)?;
    ...
    let settings = loop {
        let payload = Zeroizing::new(serde_json::to_vec(
            &json!({"op":"settings","browserId":browser.expose()}),
        )?);
        if let Ok(reply) = docker_secret_exec(runner, &id, payload, cancel)
            .await
            .and_then(|bytes| node_reply(&bytes))
        {
            break reply;
        }
        if std::time::Instant::now() >= settings_deadline {
            return Err(anyhow!("n8n_bootstrap_settings_timeout"));
        }
        if !matches!(
            cancel.try_recv(),
            Err(tokio::sync::oneshot::error::TryRecvError::Empty)
        ) {
            return Err(anyhow!("n8n_bootstrap_cancelled_custody_retained"));
        }
        tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    };
    ensure!(
        data_bool(&settings, &["userManagement", "showSetupOnFirstLoad"]) == Some(true),
        "n8n_existing_owner_requires_api_key"
    );
    custody.phase = BootstrapPhase::OwnerSetupInFlight;
    write_custody(home, &custody)?;
    let mut setup = base.clone();
    setup["op"] = Value::String("setup".into());
    let _setup = node_reply(
        &docker_secret_exec(
            runner,
            &id,
            Zeroizing::new(serde_json::to_vec(&setup)?),
            cancel,
        )
        .await?,
    )?;
    custody.phase = BootstrapPhase::OwnerEstablished;
    write_custody(home, &custody)?;
    let mut login = base;
    login["op"] = Value::String("login".into());
    let login = node_reply(
        &docker_secret_exec(
            runner,
            &id,
            Zeroizing::new(serde_json::to_vec(&login)?),
            cancel,
        )
        .await?,
    )?;
    let session = cookie(&login)?;
    custody.phase = BootstrapPhase::KeyMintInFlight;
    write_custody(home, &custody)?;
    let mint = json!({"op":"mint","browserId":browser.expose(),"cookie":session,"label":label});
    let minted = match docker_secret_exec(
        runner,
        &id,
        Zeroizing::new(serde_json::to_vec(&mint)?),
        cancel,
    )
    .await
    .and_then(|b| node_reply(&b))
    {
        Ok(v) => v,
        Err(_) => {
            custody.phase = BootstrapPhase::KeyMintUnknown;
            write_custody(home, &custody)?;
            return Err(anyhow!("n8n_bootstrap_key_mint_unknown"));
        }
    };
    let raw = minted
        .body
        .get("data")
        .and_then(|v| v.get("rawApiKey"))
        .and_then(Value::as_str)
        .filter(|v| !v.is_empty())
        .ok_or_else(|| anyhow!("n8n_bootstrap_key_mint_unknown"))?;
    let api_key = SecretString::from(raw.to_owned());
    store.set(&secret_key(&job_id, "captured-api-key"), &api_key)?;
    custody.phase = BootstrapPhase::KeyCaptured;
    write_custody(home, &custody)?;
    inspect_bootstrap(runner, &custody, true, cancel).await?;
    docker(
        runner,
        &["docker".into(), "stop".into(), id.clone()],
        None,
        cancel,
    )
    .await?;
    inspect_bootstrap(runner, &custody, false, cancel).await?;
    custody.phase = BootstrapPhase::BootstrapStopped;
    write_custody(home, &custody)?;
    docker(
        runner,
        &["docker".into(), "rm".into(), id.clone()],
        None,
        cancel,
    )
    .await?;
    ensure!(
        exact_absent(runner, &id, cancel).await?,
        "n8n_bootstrap_remove_unproven"
    );
    custody.phase = BootstrapPhase::BootstrapRemoved;
    write_custody(home, &custody)?;
    if !matches!(
    ...
    let job = managed_runtime::install_prepared_managed_in_service(
        &service,
        home,
        request.with_prepared_job(prepared),
        api_key,
        cancel,
    )
    .await?;
...
async fn resume_bootstrap_at_with<R: BootstrapDockerRunner>(
    ...
    runner: &mut R,
    cancel: &mut tokio::sync::oneshot::Receiver<()>,
    mut custody: BootstrapCustody,
) -> Result<super::IntegrationJob> {
    ensure!(
        custody.schema_version == 2
            && custody.host_port == port
            && custody.pinned_image == N8N_OCI_REFERENCE
            && managed_runtime::valid_volume_name(&custody.volume_name),
        "n8n_bootstrap_resume_custody_mismatch"
    );
    ensure!(
        matches!(
            custody.phase,
        ...
        "n8n_bootstrap_resume_requires_manual_repair"
    );
    let service = super::open_n8n_job_service(home)?;
    let job_id = super::JobId::parse(custody.job_id.clone())
        .map_err(|_| anyhow!("n8n_bootstrap_resume_custody_mismatch"))?;
    let queued = service
        .get(&job_id)?
        .ok_or_else(|| anyhow!("n8n_bootstrap_resume_job_missing"))?;
    ensure!(
        queued.manifest_sha256.as_str() == custody.manifest_sha256
            && queued.state == super::JobState::Queued,
        "n8n_bootstrap_resume_job_not_queued"
    );
    let prepared = service.start(
        &queued.job_id,
        queued.state_revision,
        "bootstrap-owner-resume",
    )?;
    let store = crate::config::keychain::open_store()
        .context("n8n bootstrap resume requires the private OS secret store")?;
    let id = custody
        .bootstrap_container_id
        .clone()
        .ok_or_else(|| anyhow!("n8n_bootstrap_resume_custody_mismatch"))?;
    ...
    if matches!(
        ...
        let password = store
            .get(&secret_key(&custody.job_id, "owner-password"))?
            .ok_or_else(|| anyhow!("n8n_bootstrap_resume_owner_secret_missing"))?;
        let browser = store
            .get(&secret_key(&custody.job_id, "browser-id"))?
            .ok_or_else(|| anyhow!("n8n_bootstrap_resume_owner_secret_missing"))?;
        ...
        .map_err(|_| anyhow!("n8n_bootstrap_owner_login_unproven"))?;
        ...
        let raw = minted
            .body
            .get("data")
            .and_then(|value| value.get("rawApiKey"))
            .and_then(Value::as_str)
            .filter(|value| !value.is_empty())
            .ok_or_else(|| anyhow!("n8n_bootstrap_key_mint_unknown"))?;
        store.set(
            &secret_key(&custody.job_id, "captured-api-key"),
    ...
    let api_key = store
        .get(&secret_key(&custody.job_id, "captured-api-key"))?
        .ok_or_else(|| anyhow!("n8n_bootstrap_resume_key_missing"))?;
    if custody.phase == BootstrapPhase::KeyCaptured {
        inspect_bootstrap(runner, &custody, true, cancel).await?;
        docker(
            runner,
            &["docker".into(), "stop".into(), id.clone()],
    ...
    if custody.phase == BootstrapPhase::BootstrapStopped {
        inspect_bootstrap(runner, &custody, false, cancel).await?;
        docker(
            runner,
            &["docker".into(), "rm".into(), id.clone()],
    ...
    let request =
        ManagedN8nRequest::new_with_volume(port, N8N_OCI_REFERENCE, custody.volume_name.clone())
            .map_err(anyhow::Error::msg)?;
    let job = managed_runtime::install_prepared_managed_in_service(
        &service,
        home,
...
mod tests {
    ...
    fn bootstrap_command_is_networkless_unpublished_and_secret_free() {
        let c = BootstrapCustody {
            ...
            volume_name: "neoth_n8n_a".into(),
            bootstrap_container_id: None,
            runtime_container_id: None,
            pinned_image: N8N_OCI_REFERENCE.into(),
            host_port: 5678,
            api_key_label: "private".into(),
        };
        let argv = bootstrap_argv(&c);
        assert!(
            argv.windows(2)
                .any(|x| x[0] == "--network" && x[1] == "none")
        );
        assert!(!argv.iter().any(|x| x == "-p"));
        assert!(!argv.iter().any(|x| x.contains("private")));
    }
    #[test]
    fn node_response_requires_data_envelope_and_success() {
        assert!(node_reply(br#"{"status":200,"cookie":"","body":{"data":{}}}"#).is_ok());
        assert!(node_reply(br#"{"status":200,"cookie":"","body":{}}"#).is_err());
        assert!(node_reply(br#"{"status":500,"cookie":"","body":{"data":{}}}"#).is_err());
    }
    #[test]
    ...
    fn owner_email_uses_a_valid_non_loopback_domain() {
        assert_eq!(
            owner_email("01234567-89ab-cdef"),
            "owner-01234567@invalid.test"
        );
    }
    #[test]
    fn node_client_uses_setup_email_and_login_email_or_ldap_login_id() {
        ...
        assert!(!NODE_CLIENT.contains(
            "if(x.op==='login')o.body=JSON.stringify({email:x.email,password:x.password"
        ));
    }
    #[test]
    fn generated_volume_is_accepted_by_the_final_runtime_boundary() {
        assert!(managed_runtime::valid_volume_name(
            "neoth_n8n_018f4f64700070008000000000000001"
        ));
    ...
    impl BootstrapDockerRunner for PanicRunner {
        async fn run(
            ...
            super::super::bootstrap_transport::BootstrapCommandFailure,
        > {
    ...
    fn custody_for(
        ...
        BootstrapCustody {
            ...
            volume_name: "neoth_n8n_recovery".into(),
            bootstrap_container_id: Some("c".repeat(64)),
            runtime_container_id: None,
            pinned_image: N8N_OCI_REFERENCE.into(),
            host_port: 5678,
            api_key_label: "private".into(),
    ...
    impl BootstrapDockerRunner for RecordingRunner {
        ...
        > {
            assert!(
                stdin.is_none(),
                "recovery must not submit bootstrap requests"
            );
    ...
    fn captured_store(custody: &BootstrapCustody) -> crate::config::keychain::InMemorySecretStore {
        ...
        store
            .set(
                &secret_key(&custody.job_id, "captured-api-key"),
                &SecretString::from("captured-for-recovery-test"),
            )
            .unwrap();
        store
    }
    ...
    fn assert_recovery_proof_only(argv: &Arc<Mutex<Vec<Vec<String>>>>, expected: usize) {
        ...
        for command in argv.iter() {
            assert!(
                command
                    .windows(2)
                    .any(|pair| pair[0] == "docker" && pair[1] == "volume")
                    || command
                        .windows(2)
                        .any(|pair| pair[0] == "docker" && pair[1] == "container")
            );
            assert!(!command.iter().any(|arg| matches!(
                arg.as_str(),
                "exec" | "setup" | "login" | "mint" | "create" | "stop" | "rm"
            )));
        }
    }

    fn assert_job_still_running(home: &Path, job: &crate::integrations::IntegrationJob) {
        let persisted = crate::integrations::IntegrationJobService::read_only_snapshot(home)
            .unwrap()
            .into_iter()
    ...
    fn assert_resume(
        decision: Option<crate::integrations::RestartDecision>,
    ) -> crate::integrations::RestartDecision {
        let decision = decision.expect("proved custody must resume");
        assert!(matches!(
            decision,
            crate::integrations::RestartDecision::Resume { .. }
        ));
        decision
    }
    ...
    fn injected_finalizer_records_exact_runtime_id_only_for_the_returned_ready_job() {
        ...
        assert_eq!(
            custody.runtime_container_id.as_deref(),
        ...
        foreign.job_id =
            crate::integrations::JobId::parse(uuid::Uuid::now_v7().to_string()).unwrap();
    ...
    fn ready_metadata_repair_retries_each_final_custody_write_without_publishing() {
        ...
        for fail_at in 1..=2 {
            ...
            .unwrap()
            .expect("durable Ready job repairs bootstrap metadata");
            assert_eq!(repaired.job_id, job.job_id);
            assert_eq!(retained.phase, BootstrapPhase::Ready);
            assert_eq!(
                retained.runtime_container_id.as_deref(),
                Some(runtime_id.as_str())
            );
            assert_eq!(
                repaired_writes,
                vec![BootstrapPhase::RuntimeContainerBound, BootstrapPhase::Ready]
            );
        }
    ...
    fn key_mint_unknown_holds_without_docker() {
        ...
        write_custody(
            home.path(),
            &custody_for(&job, BootstrapPhase::KeyMintUnknown),
        )
        .unwrap();
        let store = crate::config::keychain::InMemorySecretStore::default();
    ...
    fn captured_bootstrap_recovery_resumes_same_job_with_inspect_only_proof() {
        ...
        let (runner, argv) = recording_runner(vec![
            volume_receipt(&custody),
            bootstrap_receipt(&custody, true, "captured"),
        ]);
    ...
    fn stopped_bootstrap_recovery_resumes_same_job_with_stopped_receipt() {
        ...
        let (runner, argv) = recording_runner(vec![
            volume_receipt(&custody),
            bootstrap_receipt(&custody, false, "stopped"),
        ]);
        let decision = assert_resume(recovery_decision_with(home.path(), &job, runner, &store));
    ...
    fn removed_bootstrap_recovery_resumes_same_job_with_exact_absence_proof() {
        ...
        assert!(
            commands[1]
                .windows(2)
                .any(|pair| pair[0] == "container" && pair[1] == "ls")
        );
        assert!(
            commands[1]
                .iter()
                .any(|arg| arg
                    == &format!("id={}", custody.bootstrap_container_id.as_ref().unwrap()))
        );
    }
    ...
    fn recovery_proof_digest_changes_with_exact_inspect_receipt() {
        ...
        write_custody(home.path(), &custody).unwrap();
        let store = captured_store(&custody);
        let (first, _) = recording_runner(vec![
            volume_receipt(&custody),
            bootstrap_receipt(&custody, true, "first"),
        ]);
        let first = assert_resume(recovery_decision_with(home.path(), &job, first, &store));
        let (second, _) = recording_runner(vec![
            volume_receipt(&custody),
            bootstrap_receipt(&custody, true, "second"),
        ]);
    ...
    fn recovery_proof_digest_changes_with_exact_volume_receipt() {
        ...
        let store = captured_store(&custody);
        let (first, _) = recording_runner(vec![
            volume_receipt_with_marker(&custody, "first"),
            bootstrap_receipt(&custody, true, "same"),
        ]);
        let first = assert_resume(recovery_decision_with(home.path(), &job, first, &store));
        let (second, _) = recording_runner(vec![
            volume_receipt_with_marker(&custody, "second"),
            bootstrap_receipt(&custody, true, "same"),
        ]);
        let second = assert_resume(recovery_decision_with(home.path(), &job, second, &store));
        assert_ne!(format!("{first:?}"), format!("{second:?}"));
    ...
    fn image_network_mount_and_volume_mismatches_hold_the_lease() {
        for mismatch in ["image", "network", "mount", "volume"] {
            ...
            let mut receipt: Value =
                serde_json::from_slice(&bootstrap_receipt(&custody, true, "mismatch")).unwrap();
            let row = receipt[0].as_object_mut().unwrap();
            match mismatch {
                "image" => {
                    row.get_mut("Config").unwrap()["Image"] = json!("foreign/image:latest");
                }
                "network" => {
                    row.get_mut("HostConfig").unwrap()["NetworkMode"] = json!("bridge");
                }
                "mount" => {
                    row.get_mut("Mounts").unwrap()[0]["Destination"] = json!("/foreign");
                }
                "volume" => {
                    row.get_mut("Mounts").unwrap()[0]["Name"] = json!("foreign_volume");
                }
                _ => unreachable!(),
            }
            let (runner, argv) = recording_runner(vec![
            ...
            assert!(
                recovery_decision_with(home.path(), &job, runner, &store).is_none(),
                "{mismatch} mismatch must hold"
            );
            assert_recovery_proof_only(&argv, 2);
    ...
    fn removed_bootstrap_with_runtime_binding_holds_before_docker_proof() {
        ...
        write_custody(home.path(), &custody).unwrap();
        std::fs::write(
            home.path().join(RUNTIME_BINDING_FILE),
            serde_json::to_vec(&json!({
                "schema_version": 2,
                "phase": "Bound",
    ...
    #[test]
    fn cookie_requires_the_exact_n8n_auth_name() {
        let ok = NodeReply {
            status: 200,
            cookie: "n8n-auth=opaque; Path=/".into(),
            body: json!({"data":{}}),
        };
        let wrong = NodeReply {
            status: 200,
            cookie: "session=opaque".into(),
            body: json!({"data":{}}),
        };
        assert!(cookie(&ok).is_ok());
        assert!(cookie(&wrong).is_err());
    }
}

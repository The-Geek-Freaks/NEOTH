...
use crate::integrations::state::{
    IntegrationJob, JobEvidenceContract, JobId, JobOperation, JobProgress, JobRequester, JobState,
    ProgressEvidence, ProgressEvidenceClaim, ReadyEvidence, Sha256Digest,
};
use crate::integrations::{EnqueueResult, IntegrationJobService, JobServiceError};
use crate::secret::SecretString;

use super::{N8N_CAPABILITY_ID, expected_authenticated_probe_sha256, sha256_parts};
...
enum ImportEntryState {
    ...
    ReadBack {
        workflow_id: String,
        normalized_readback_sha256: Sha256Digest,
    },
}
...
impl ImportError {
    fn local(code: &'static str) -> Self {
        Self {
            kind: ImportErrorKind::Local,
            code,
        }
    }
    fn rejected(code: &'static str) -> Self {
        Self {
            kind: ImportErrorKind::Rejected,
            code,
        }
    }
    fn ambiguous(code: &'static str) -> Self {
        Self {
            kind: ImportErrorKind::Ambiguous,
            code,
        }
    }
    fn drift(code: &'static str) -> Self {
        Self {
            kind: ImportErrorKind::Drift,
            code,
        }
    }
}
...
fn import_manifest() -> Result<WorkflowImportManifest, ImportError> {
    ...
    if workflows.len() != IMPORT_STEPS as usize {
        return Err(ImportError::local("workflow_import_manifest_count"));
    }
    let mut seen = BTreeSet::new();
    let mut entries = Vec::with_capacity(workflows.len());
    for workflow in workflows {
        if !seen.insert(workflow.slug) {
            return Err(ImportError::local("workflow_import_duplicate_slug"));
        }
        ...
        entries.push(ManifestWorkflow {
            slug: workflow.slug.to_owned(),
            source_sha256,
            dto,
            dto_sha256,
        });
    ...
    let joined = entries
        ...
        .map(|entry| {
            format!(
                ...
                entry.dto_sha256.as_str()
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
    Ok(WorkflowImportManifest {
        entries,
        manifest_sha256: digest_bytes(joined.as_bytes()),
    })
}
...
fn create_dto(workflow: &BootstrapWorkflow) -> Result<serde_json::Value, ImportError> {
    let mut source: serde_json::Map<String, serde_json::Value> =
        serde_json::from_str(workflow.body)
    ...
    for forbidden in ["description", "active", "tags"] {
        source.remove(forbidden);
    }
    for required in ["name", "nodes", "connections", "settings"] {
        if !source.contains_key(required) {
            return Err(ImportError::local("workflow_import_source_missing_graph"));
        }
    }
...
fn custody_path(home: &Path, job_id: &JobId) -> PathBuf {
    home.join(format!(
        ".n8n-workflow-import-{}.custody.yaml",
        job_id.as_str()
    ))
}
...
fn custody_exists(home: &Path, job_id: &JobId) -> Result<bool, ImportError> {
    ...
    match std::fs::symlink_metadata(&path) {
        ...
        Ok(_) => Err(ImportError::ambiguous(
            "workflow_import_custody_not_regular",
        )),
...
fn load_custody(home: &Path, job_id: &JobId) -> Result<WorkflowImportCustody, ImportError> {
    ...
    let Some(directory) = crate::skills::store::open_bound_directory(
        home,
        false,
        "n8n workflow import custody directory",
    )
    .map_err(|_| ImportError::ambiguous("workflow_import_custody_missing"))?
    else {
...
pub(super) fn safe_to_terminalize_import(
    home: &Path,
    job: &IntegrationJob,
    error: &ImportError,
) -> bool {
    match custody_exists(home, &job.job_id) {
        ...
        Ok(true) => load_custody(home, &job.job_id).is_ok_and(|custody| match error.kind {
            ImportErrorKind::Local => custody
                .entries
                .iter()
                .all(|entry| matches!(entry.state, ImportEntryState::Prepared)),
            ...
            ImportErrorKind::Rejected => custody.entries.iter().all(|entry| {
                matches!(
                    ...
                    ImportEntryState::Prepared
                        | ImportEntryState::IntentPersisted
                        | ImportEntryState::ReadBack { .. }
                )
            }),
...
async fn bounded_json(
    ...
    if response.status().is_redirection() {
        return Err(ImportError::ambiguous("workflow_import_redirect"));
    }
    if response.status() == reqwest::StatusCode::INTERNAL_SERVER_ERROR
        || response.status().is_server_error()
    {
        return Err(ImportError::ambiguous("workflow_import_server_error"));
    }
    if !response.status().is_success() {
        return Err(match (create, response.status()) {
            (true, reqwest::StatusCode::BAD_REQUEST) => {
                ImportError::rejected("workflow_import_create_rejected")
            }
            (true, reqwest::StatusCode::UNAUTHORIZED) => {
                ImportError::rejected("workflow_import_credential_rejected")
            }
            (true, reqwest::StatusCode::FORBIDDEN) => {
                ImportError::rejected("workflow_import_legacy_credential_rejected")
            }
    ...
    if response
        .content_length()
        .is_some_and(|size| size > IMPORT_RESPONSE_MAX as u64)
    {
        return Err(ImportError::ambiguous("workflow_import_response_too_large"));
    }
    let mut bytes = Vec::new();
    let mut stream = response.bytes_stream();
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|_| ImportError::ambiguous("workflow_import_transport"))?;
        if bytes.len().saturating_add(chunk.len()) > IMPORT_RESPONSE_MAX {
            return Err(ImportError::ambiguous("workflow_import_response_too_large"));
        }
        bytes.extend_from_slice(&chunk);
    }
    serde_json::from_slice(&bytes)
        .map_err(|_| ImportError::ambiguous("workflow_import_response_invalid"))
}
...
impl WorkflowImportTransport for HttpWorkflowImportTransport {
    async fn create(
        ...
        let response = super::bounded_n8n_client()
            .map_err(|_| ImportError::ambiguous("workflow_import_client"))?
            .post(format!(
                "{}{}",
                endpoint.origin(),
                super::N8N_WORKFLOWS_PATH
            ))
            .header("X-N8N-API-KEY", key.expose())
            .json(dto)
            .send()
            .await
...
fn normalized_graph(
    ...
    if read.get("active").and_then(serde_json::Value::as_bool) != Some(false) {
        return Err(ImportError::drift("workflow_import_readback_active"));
    }
    ...
    for field in ["name", "nodes", "connections", "settings"] {
        expected.insert(
            ...
            dto.get(field)
                .cloned()
                .ok_or_else(|| ImportError::local("workflow_import_dto_missing_graph"))?,
        );
        observed.insert(
            field.to_owned(),
            read.get(field)
                .cloned()
                .ok_or_else(|| ImportError::drift("workflow_import_readback_missing_graph"))?,
        );
    }
    if expected != observed {
        return Err(ImportError::drift("workflow_import_readback_drift"));
    }
    Ok(digest_bytes(&json_bytes(&serde_json::Value::Object(
        observed,
    ))?))
}
...
fn binding_digests(
    ...
    (
        sha256_parts(&["n8n-workflow-import-endpoint", endpoint.origin()]),
        digest_bytes(key.expose().as_bytes()),
    )
}

pub(super) fn enqueue_managed_workflow_import(
    service: &IntegrationJobService,
    home: &Path,
    requester: JobRequester,
) -> Result<EnqueueResult, JobServiceError> {
    let manifest = import_manifest().map_err(|_| {
        JobServiceError::State(
            crate::integrations::state::StateValidationError::InvalidInvariant(
                "workflow import manifest is invalid".into(),
            ),
        )
    })?;
    let (instance, key) = crate::config::credentials::Credentials::read_n8n_adoption_binding_at(
    ...
    .map_err(|_| {
        JobServiceError::State(
            crate::integrations::state::StateValidationError::InvalidInvariant(
                "managed n8n binding is not Ready".into(),
            ),
        )
    })?;
...
fn enqueue_workflow_import_with_binding(
    ...
    service.enqueue(EnqueueIntegrationJob {
        ...
        release_version: IMPORT_RELEASE_VERSION.into(),
...
pub(super) async fn execute_managed_workflow_import_at<T: WorkflowImportTransport + ?Sized>(
    service: &IntegrationJobService,
    ...
    .map_err(|_| ImportError::local("workflow_import_binding_missing"))?;
    if !super::has_ready_managed_binding(service, home, &instance.endpoint)
        .map_err(|_| ImportError::local("workflow_import_binding_missing"))?
    {
        return Err(ImportError::local(
            "workflow_import_managed_binding_not_ready",
        ));
    }
...
async fn execute_workflow_import_with_binding<T: WorkflowImportTransport + ?Sized>(
    ...
    if contract.artifact_binding_sha256() != &manifest.manifest_sha256
        || contract.config_binding_sha256()
            != &sha256_parts(&[endpoint_digest.as_str(), credential_digest.as_str()])
        || contract.authenticated_probe_sha256()
            != &expected_authenticated_probe_sha256(&binding.endpoint)
    ...
    let mut active = match queued.state {
        JobState::Queued => service
            .start(&queued.job_id, queued.state_revision, "import-workflow-01")
            .map_err(|_| ImportError::local("workflow_import_job_start"))?,
        JobState::Running | JobState::Validating | JobState::Configuring | JobState::Ready => {
            queued
        }
        JobState::Failed | JobState::Cancelled => {
            return Err(ImportError::local("workflow_import_terminal_job"));
        }
    };
    ...
    for (index, item) in manifest.entries.iter().enumerate() {
        let state = custody
            .entries
            .get(index)
            .ok_or_else(|| ImportError::local("workflow_import_custody_entries"))?
            .state
            .clone();
        match state {
            ImportEntryState::Prepared if ready_repeat => {
                return Err(ImportError::drift("workflow_import_ready_not_complete"));
            }
            ImportEntryState::Prepared => {
                custody.entries[index].state = ImportEntryState::IntentPersisted;
                save_custody(home, &custody)?;
                let id = transport
                    .create(&binding.endpoint, &binding.key, &item.dto)
                    .await?;
                custody.entries[index].state = ImportEntryState::Created { workflow_id: id };
                save_custody(home, &custody)?;
            }
            ImportEntryState::IntentPersisted => {
                return Err(ImportError::ambiguous("workflow_import_post_ambiguous"));
            }
            _ => {}
        }
        let state = custody.entries[index].state.clone();
        let id = match &state {
            ImportEntryState::Created { workflow_id }
            | ImportEntryState::ReadBack { workflow_id, .. } => workflow_id.clone(),
            _ => return Err(ImportError::ambiguous("workflow_import_state_ambiguous")),
        };
        let read = transport
            .get_exact(&binding.endpoint, &binding.key, &id)
            .await?;
        let normalized = normalized_graph(&item.dto, &id, &read)?;
        if let ImportEntryState::ReadBack {
            normalized_readback_sha256,
            ..
        } = state
        ...
        if !ready_repeat {
            custody.entries[index].state = ImportEntryState::ReadBack {
                workflow_id: id,
                normalized_readback_sha256: normalized,
            };
            save_custody(home, &custody)?;
        ...
        if active.state.permits_progress() {
            ...
            let staging_binding_sha256 = digest_bytes(
                &serde_yaml::to_string(&custody)
                    .map_err(|_| ImportError::local("workflow_import_custody_encode"))?
                    .into_bytes(),
            );
            active = service
    ...
    if active.state == JobState::Ready {
        return Ok(active);
    }
    ...
    service
        ...
        .map_err(|_| ImportError::local("workflow_import_ready"))
}
...
mod tests {
    ...
    fn harness() -> (
        ...
        IntegrationJob,
        WorkflowImportManifest,
        WorkflowImportBinding,
    ) {
        let home = tempfile::tempdir().unwrap();
        let service = IntegrationJobService::open(
            home.path(),
            super::super::n8n_catalog(),
            &super::super::N8nRestartValidator::new(home.path()),
        )
        .unwrap();
        ...
        (
            ...
            job,
            manifest,
            WorkflowImportBinding { endpoint, key },
        )
    }

    fn ready_job(service: &IntegrationJobService, job: IntegrationJob) -> IntegrationJob {
        let running = service
            .start(&job.job_id, job.state_revision, "import-workflow-01")
            .unwrap();
    ...
    fn manifest_contains_the_exact_ordered_unique_bundle() {
        ...
        assert_eq!(
            ...
            IMPORT_STEPS as usize
        );
    }

    #[test]
    fn dto_removes_import_forbidden_fields_but_preserves_graph_and_notes() {
        let workflow = all_known_workflows()
            .into_iter()
            .find(|workflow| workflow.slug == "dream_obsidian_sync")
            .unwrap();
        ...
        assert!(
            source["nodes"]
                ...
                .unwrap()
                .iter()
                .any(|node| node.get("notes").is_some())
        );
    }

    #[test]
    fn noninactive_source_and_wrong_readback_id_fail_closed() {
        let source = BootstrapWorkflow {
            slug: "bad",
            name: "bad",
            description: "bad",
            body: r#"{"name":"bad","active":true,"nodes":[],"connections":{},"settings":{}}"#,
        };
        assert_eq!(
            create_dto(&source).unwrap_err().code,
            "workflow_import_source_not_inactive"
        );
        ...
        assert_eq!(
            normalized_graph(&dto, "expected", &wrong).unwrap_err().code,
            "workflow_import_readback_id_mismatch"
        );
    }
    ...
    fn custody_round_trip_is_bounded_and_atomic_replace_safe() {
        ...
        let custody = WorkflowImportCustody {
            job_id: job_id.clone(),
            endpoint_binding_sha256: digest_bytes(b"endpoint"),
            credential_binding_sha256: digest_bytes(b"credential"),
            manifest_sha256: manifest.manifest_sha256,
            entries: manifest
                .entries
                .into_iter()
                .map(|entry| WorkflowImportEntry {
                    ...
                    create_dto_sha256: entry.dto_sha256,
                    state: ImportEntryState::Prepared,
                })
                .collect(),
        };
        ...
        save_custody(home.path(), &custody).unwrap();
        assert_eq!(
            load_custody(home.path(), &job_id).unwrap().entries.len(),
            IMPORT_STEPS as usize
        );
        let queued = IntegrationJob {
            ...
            operation: JobOperation::Import,
            release_version: IMPORT_RELEASE_VERSION.into(),
            manifest_sha256: custody.manifest_sha256.clone(),
            state: JobState::Queued,
            state_revision: 0,
            current_step: None,
            progress: JobProgress::new(IMPORT_STEPS, None),
            created_at: 0,
            started_at: None,
            updated_at: 0,
            terminal_at: None,
        ...
        assert!(safe_to_terminalize_import(
            home.path(),
            &queued,
            &ImportError::local("pre_intent")
        ));
    }
    ...
    fn importer_lifetime_lock_rejects_a_contender_before_dispatch() {
        ...
        let holder =
            crate::util::locked_file::lock_file_blocking(&path, "n8n workflow import").unwrap();
        assert!(
            crate::util::locked_file::try_lock_file_once(&path, "n8n workflow import")
                .unwrap()
                .is_none()
        );
        drop(holder);
    ...
    async fn all_thirteen_then_ready_repeat_is_read_only() {
        ...
        let ready = execute_workflow_import_with_binding(
            &service,
            job,
            home.path(),
            &transport,
        ...
        let repeat_binding = WorkflowImportBinding {
        ...
        assert_eq!(
            service
                ...
                .unwrap()
                .iter()
                .filter(|job| job.operation == JobOperation::Import)
                .count(),
            1
        );
        execute_workflow_import_with_binding(
            &service,
            repeated_admission.job,
            home.path(),
            &transport,
            import_manifest().unwrap(),
            WorkflowImportBinding {
                endpoint: LoopbackHttpEndpoint::parse("http://127.0.0.1:5678").unwrap(),
                key: SecretString::from("test-key"),
            },
        ...
        assert_eq!(transport.0.lock().unwrap().creates, IMPORT_STEPS);
        assert_eq!(
            std::fs::read(custody_path(home.path(), &ready.job_id)).unwrap(),
            sidecar
        );
    }
    ...
    async fn lost_post_reply_keeps_intent_and_never_posts_again() {
        ...
        assert_eq!(
            ...
            .unwrap_err()
            .code,
            "workflow_import_transport"
        );
        assert!(matches!(
            load_custody(home.path(), &job.job_id).unwrap().entries[0].state,
            ImportEntryState::IntentPersisted
        ));
        let repeat = service.get(&job.job_id).unwrap().unwrap();
        let error = execute_workflow_import_with_binding(
            &service,
            repeat,
            home.path(),
            &transport,
    ...
    async fn rejected_create_after_readback_reuses_its_active_import_job() {
        ...
        assert_eq!(
            ...
            .unwrap_err()
            .code,
            "workflow_import_create_rejected"
        );
        assert!(matches!(
            load_custody(home.path(), &job.job_id).unwrap().entries[0].state,
            ImportEntryState::ReadBack { .. }
        ));
        let admission = enqueue_workflow_import_with_binding(
            &service,
            import_manifest().unwrap(),
            LoopbackHttpEndpoint::parse("http://127.0.0.1:5678").unwrap(),
        ...
        drop(service);
        let reopened = IntegrationJobService::open(
            home.path(),
            super::super::n8n_catalog(),
            &super::super::N8nRestartValidator::new(home.path()),
        )
        .unwrap();
        assert!(
            enqueue_workflow_import_with_binding(
                &reopened,
                import_manifest().unwrap(),
                LoopbackHttpEndpoint::parse("http://127.0.0.1:5678").unwrap(),
                SecretString::from("test-key"),
        ...
        assert_eq!(transport.0.lock().unwrap().creates, 2);
        assert_eq!(
            std::fs::read(custody_path(home.path(), &job.job_id)).unwrap(),
            sidecar
        );
    }
    ...
    async fn drifted_readback_holds_without_a_second_post() {
        ...
        transport
            ...
            .graphs
            .get_mut("workflow-1")
            .unwrap()["name"] = serde_json::json!("drift");
        let error = execute_workflow_import_with_binding(
            &service,
            ready,
            home.path(),

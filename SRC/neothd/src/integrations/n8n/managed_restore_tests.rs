    ...
    integrations::{
        n8n::{
            ...
            managed_runtime::{
                ...
                managed_restore_candidate::{
                    InspectRestoreCandidateOutcome, ObservedRestoreCandidate,
                    RestoreCandidateContentReceipt, RestoreCandidateSpec,
                },
            },
        },
        state::JobId,
...
#[derive(Default)]
struct State {
    original: Option<super::super::ObservedContainer>,
    original_running: bool,
    volume: Option<super::super::ObservedVolume>,
    candidate: Option<ObservedRestoreCandidate>,
    calls: Vec<String>,
    next: u64,
    extract: Extract,
    content: Content,
    candidate_identity_wrong: bool,
...
impl super::super::ManagedDockerRunner for Runner {
    ...
    async fn inspect_exact(
        ...
        s.calls.push(format!("live-inspect:{id}"));
        Ok(s.original
            .clone()
            .filter(|x| x.id == id)
            .map(super::super::InspectOutcome::Found)
    ...
    async fn running_exact(&mut self, id: &str) -> Result<bool, &'static str> {
        let s = self.0.lock().unwrap();
        Ok(s.original.as_ref().is_some_and(|x| x.id == id) && s.original_running)
    }
    async fn inspect_volume(
        ...
        let mut s = self.0.lock().unwrap();
        s.calls.push(format!("volume-inspect:{name}"));
        Ok(s.volume
            .clone()
    ...
    async fn remove_volume(
        ...
        let mut s = self.0.lock().unwrap();
        s.calls.push(format!("volume-remove:{name}"));
        if s.volume.as_ref().is_some_and(|x| x.name == name) {
            s.volume = None;
        }
        Ok(receipt())
    }
    ...
    async fn start_exact(
        ...
        let mut s = self.0.lock().unwrap();
        if s.original.as_ref().is_some_and(|x| x.id == id) {
            s.calls.push(format!("live-start:{id}"));
            s.original_running = true;
            return Ok(receipt());
        }
    ...
    async fn stop_exact(
        ...
        let mut s = self.0.lock().unwrap();
        if s.original.as_ref().is_some_and(|x| x.id == id) {
            s.calls.push(format!("live-stop:{id}"));
            s.original_running = false;
            return Ok(receipt());
        }
        Err("wrong_stop")
    }
    async fn archive_exact_n8n_dir_to_private_file(
        ...
        let mut s = self.0.lock().unwrap();
        s.calls.push("backup-archive".into());
        drop(s);
        let bytes = b"ready-backup-private-archive";
        std::fs::write(destination, bytes).map_err(|_| "archive_write")?;
    ...
    async fn create_restore_volume_exact(
        ...
        labels.insert("io.neoth.n8n-restore".into(), job.as_str().into());
        labels.insert("io.neoth.n8n-restore-schema".into(), "1".into());
        let mut s = self.0.lock().unwrap();
        s.calls.push("restore-volume-create".into());
        s.volume = Some(super::super::ObservedVolume {
            name: super::super::managed_restore_candidate::restore_volume_name(job),
            labels,
        });
        Ok(receipt())
    }
    async fn create_restore_candidate_exact(
        ...
        s.next += 1;
        let id = format!("{:064x}", s.next);
        let volume =
            super::super::managed_restore_candidate::restore_volume_name(spec.restore_job_id);
        let restore_job_id = if s.candidate_identity_wrong {
            "123e4567-e89b-12d3-a456-426614174001".into()
        } else {
            spec.restore_job_id.as_str().into()
        };
        s.candidate = Some(ObservedRestoreCandidate {
            id: id.clone(),
            running: false,
            image: spec.image.into(),
            restore_job_id,
            volume,
            volume_source: "/private/volume".into(),
            network_mode: "none".into(),
            tmpfs_options: "rw,noexec,nosuid,nodev,size=67108864".into(),
        });
        Ok(super::super::ManagedCreateReceipt {
            command: receipt(),
    ...
    async fn inspect_restore_candidate_exact(
        &mut self,
        id: &str,
    ) -> Result<InspectRestoreCandidateOutcome, &'static str> {
        let mut s = self.0.lock().unwrap();
        s.calls.push(format!("candidate-inspect:{id}"));
        Ok(s.candidate
            .clone()
            .filter(|x| x.id == id)
            .map(InspectRestoreCandidateOutcome::Found)
            .unwrap_or(InspectRestoreCandidateOutcome::Absent))
    }
    async fn extract_private_archive_to_exact_container(
        ...
        let mut s = self.0.lock().unwrap();
        s.calls.push(format!("extract:{id}"));
        match s.extract {
            Extract::Good => Ok(super::super::ManagedArchiveReceipt {
                archive_sha256: digest.into(),
                archive_bytes: bytes,
            }),
            Extract::ReceiptMismatch => Ok(super::super::ManagedArchiveReceipt {
                archive_sha256: "0".repeat(64),
                archive_bytes: bytes + 1,
            }),
            Extract::ErrorAfterEffect => Err("extract_receipt_lost"),
        }
    }
    async fn validate_restore_candidate_content_exact(
        ...
        let mut s = self.0.lock().unwrap();
        s.calls.push(format!("content:{id}"));
        match s.content {
    ...
    async fn remove(
        ...
        let mut s = self.0.lock().unwrap();
        s.calls.push(format!("candidate-remove:{id}"));
        if s.candidate.as_ref().is_some_and(|x| x.id == id) {
            s.candidate = None;
            if s.remove_error_after_effect {
                return Err("remove_receipt_lost");
            }
        }
        Ok(receipt())
    }
...
async fn fixture() -> (tempfile::TempDir, Runner, Arc<Mutex<State>>, IntegrationJob) {
    ...
    super::super::install_managed_at_with(
        ...
        SecretString::from("restore-test-key"),
        &mut runner,
        &Ready,
        &Probe,
        &mut cancel,
...
async fn ready_backup_restores_nonempty_content_to_exact_candidate_then_retires_it() {
    ...
    let receipt = completed_receipt_at(home.path(), &restored)
        .unwrap()
        .unwrap();
    assert!(receipt.candidate_only);
    assert_eq!(receipt.workflow_count, 2);
    assert_eq!(receipt.credential_count, 1);
    assert!(receipt.credential_decryption_proven);
    assert_eq!(runner.calls("restore-volume-create"), 1);
    assert_eq!(runner.calls("candidate-create"), 1);
    assert_eq!(runner.calls("extract:"), 1);
    assert_eq!(runner.calls("candidate-remove:"), 1);
    assert!(state.lock().unwrap().candidate.is_none());
    assert!(state.lock().unwrap().volume.is_some());
    assert_eq!(
        runner.calls("live-inspect:"),
        0,
        "restore must never select a live managed container"
    );
...
async fn zero_credentials_never_claims_decryption_even_when_restore_is_ready() {
    ...
    runner.0.lock().unwrap().content = Content::Empty;
    let restored = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
        .unwrap();
    let receipt = completed_receipt_at(home.path(), &restored)
        ...
        .unwrap();
    assert_eq!(restored.state, JobState::Ready);
    assert_eq!(receipt.credential_count, 0);
    assert!(!receipt.credential_decryption_proven);
}
...
async fn extract_digest_receipt_mismatch_fails_and_removes_only_owned_candidate_and_volume() {
    let (home, mut runner, state, backup) = fixture().await;
    runner.0.lock().unwrap().extract = Extract::ReceiptMismatch;
    let failed = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
        .unwrap();
    assert_eq!(failed.state, JobState::Failed);
    assert_eq!(runner.calls("extract:"), 1);
    assert_eq!(runner.calls("candidate-remove:"), 1);
    assert_eq!(runner.calls("volume-remove:"), 1);
    assert!(state.lock().unwrap().candidate.is_none());
...
async fn wrong_candidate_identity_stops_before_extract_or_removal() {
    let (home, mut runner, _, backup) = fixture().await;
    runner.0.lock().unwrap().candidate_identity_wrong = true;
    assert!(
        restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
            .await
            .is_err()
    );
    assert_eq!(runner.calls("extract:"), 0);
    assert_eq!(runner.calls("candidate-remove:"), 0);
    assert_eq!(runner.calls("volume-remove:"), 0);
    assert!(sidecar(home.path()).exists());
}
...
async fn lost_extract_receipt_is_not_replayed_and_cleanup_retires_custody() {
    let (home, mut runner, state, backup) = fixture().await;
    runner.0.lock().unwrap().extract = Extract::ErrorAfterEffect;
    let failed = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
        .unwrap();
    assert_eq!(failed.state, JobState::Failed);
...
async fn lost_content_receipt_compensates_without_replaying_extract() {
    let (home, mut runner, state, backup) = fixture().await;
    runner.0.lock().unwrap().content = Content::Lost;
    let failed = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
        .unwrap();
    assert_eq!(failed.state, JobState::Failed);
    assert_eq!(runner.calls("extract:"), 1);
    assert_eq!(runner.calls("content:"), 1);
    assert_eq!(runner.calls("candidate-remove:"), 1);
    assert_eq!(runner.calls("volume-remove:"), 1);
    assert!(state.lock().unwrap().candidate.is_none());
    assert!(!sidecar(home.path()).exists());
...
async fn remove_receipt_loss_is_observed_once_without_second_dispatch() {
    let (home, mut runner, state, backup) = fixture().await;
    runner.0.lock().unwrap().remove_error_after_effect = true;
    let ready = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
        .unwrap();
...
async fn ready_custody_reentry_does_not_enqueue_or_repeat_external_effects() {
    let (home, mut runner, _, backup) = fixture().await;
    let first = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
        .unwrap();
    let verified = completed_verified_archive_at(home.path(), &backup)
        .unwrap()
        .unwrap();
    ...
    let completed = Custody {
        ...
        candidate_id: Some(receipt.candidate_container_id),
        content: Some(content_receipt(
            receipt.workflow_count,
            receipt.credential_count,
            receipt.credential_decryption_proven,
        )),
        ..custody
    };
    create(home.path(), &completed).unwrap();
    let before = runner.0.lock().unwrap().calls.len();
    let replay = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
        .unwrap();
    assert_eq!(replay.job_id, first.job_id);
    assert_eq!(replay.state, JobState::Ready);
    assert_eq!(runner.0.lock().unwrap().calls.len(), before);
    assert!(!sidecar(home.path()).exists());
}
...
async fn persisted_extract_dispatch_recovers_by_cleanup_without_a_second_extract() {
    ...
    let queued = service
        .enqueue(crate::integrations::jobs::EnqueueIntegrationJob {
            ...
            manifest_sha256: manifest(&verified),
            evidence_contract: contract(manifest(&verified), &verified),
    ...
    let active = service
        ...
        .unwrap();
    let _active = service
        .begin_configuration(&active.job_id, active.state_revision, STEPS[0])
        .unwrap();
    runner
        .create_restore_volume_exact(&queued.job_id)
        .await
        .unwrap();
    let candidate = runner
        .create_restore_candidate_exact(RestoreCandidateSpec {
            restore_job_id: &queued.job_id,
            image: &verified.receipt.source_pinned_image,
        })
        .await
        .unwrap();
    let custody = Custody {
        phase: Phase::ExtractDispatched,
        candidate_id: candidate.container_id,
        ..new_custody(&verified, &queued)
    };
    create(home.path(), &custody).unwrap();
    drop(service);
    state.lock().unwrap().calls.clear();
    let failed = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
...
async fn candidate_dispatch_without_exact_id_holds_recovery_without_name_discovery() {
    ...
    let _active = service
        ...
        .unwrap();
    let custody = Custody {
        phase: Phase::CandidateDispatched,
        ..new_custody(&verified, &queued)
    };
    create(home.path(), &custody).unwrap();
    drop(service);
    ...
    assert_eq!(runner.calls("candidate-inspect:"), 0);
    assert_eq!(runner.calls("candidate-create"), 0);
    assert_eq!(runner.calls("candidate-remove:"), 0);
    assert!(sidecar(home.path()).exists());
}
...
async fn custody_bound_to_one_backup_rejects_a_different_requested_backup() {
    ...
    let queued = service
        ...
        .unwrap()
        .job;
    create(home.path(), &new_custody(&verified, &queued)).unwrap();
    drop(service);
    assert!(
        restore_managed_at_with(home.path(), &second.job_id, &mut runner)
            .await
            .is_err()
    );
    assert_eq!(runner.calls("restore-volume-create"), 0);
}

#[tokio::test]
async fn corrupted_custody_and_completion_receipt_are_rejected_before_new_effects() {
    let (home, mut runner, _, backup) = fixture().await;
    std::fs::write(sidecar(home.path()), b"not-json").unwrap();
    assert!(
        restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
            .await
            .is_err()
    ...
    assert_eq!(runner.calls("restore-volume-create"), 0);
    std::fs::remove_file(sidecar(home.path())).unwrap();
    let ready = restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
        .await
        .unwrap();
    ...
    value["candidate_container_id"] = "not-an-id".into();
    std::fs::write(path, serde_json::to_vec(&value).unwrap()).unwrap();
    assert_eq!(
        completed_receipt_at(home.path(), &ready),
        Err("n8n_restore_receipt_mismatch")
    );
}
...
async fn custody_manifest_tampering_is_rejected_before_candidate_creation() {
    ...
    let queued = service
        ...
        .unwrap()
        .job;
    let tampered = Custody {
        backup_manifest_sha256: "f".repeat(64),
        ..new_custody(&verified, &queued)
    };
    create(home.path(), &tampered).unwrap();
    drop(service);
    assert!(
        restore_managed_at_with(home.path(), &backup.job_id, &mut runner)
            .await
...
async fn pending_restore_custody_fences_backup_before_it_can_stop_or_archive_live_state() {
    ...
    assert_eq!(runner.calls("live-stop:"), 0);
    assert_eq!(runner.calls("backup-archive"), 0);
}

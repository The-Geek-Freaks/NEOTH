...
use super::{
    InspectVolumeOutcome, IntegrationJob, IntegrationJobService, JobEvidenceContract, JobOperation,
    JobRequester, ManagedDockerRunner, N8N_CAPABILITY_ID,
    managed_backup::{VerifiedBackupArchive, completed_verified_archive_at},
    managed_restore_candidate::{
        InspectRestoreCandidateOutcome, RestoreCandidateContentReceipt, RestoreCandidateSpec,
        restore_volume_name, valid_restore_volume_name,
    },
    sha256_parts, valid_container_id,
};
use crate::integrations::{
    catalog::CapabilityId,
    jobs::{EnqueueIntegrationJob, RestartValidator},
    state::{
        JobFailure, JobId, JobProgress, JobState, ProgressEvidence, ProgressEvidenceClaim,
...
const STEPS: [&str; 7] = [
    "resolve-ready-backup",
    "create-derived-volume",
    "create-stopped-candidate",
...
enum Phase {
    ...
    ReceiptPublished,
    Completed,
    CompensateCandidateDispatched,
    CompensateCandidateAbsent,
    CompensateVolumeDispatched,
    CompensateVolumeAbsent,
}
...
pub(crate) fn reject_pending_restore(home: &Path) -> std::result::Result<(), &'static str> {
    if read(home)?.is_some() {
        Err("n8n_restore_custody_pending")
    } else {
        Ok(())
    }
}
fn manifest(backup: &VerifiedBackupArchive) -> crate::integrations::state::Sha256Digest {
    ...
    sha256_parts(&[
        "n8n-managed-restore-v1",
        &receipt.backup_job_id,
        &receipt.backup_manifest_sha256,
        &receipt.generation.to_string(),
        &receipt.source_pinned_image,
        &receipt.archive_sha256,
        &receipt.archive_bytes.to_string(),
    ])
}
fn contract(
    value: crate::integrations::state::Sha256Digest,
    backup: &VerifiedBackupArchive,
) -> JobEvidenceContract {
    JobEvidenceContract::verified(
        value,
        sha256_parts(&[
            "n8n-managed-restore-candidate",
            &backup.receipt.backup_job_id,
            &backup.receipt.archive_sha256,
        ]),
        sha256_parts(&[
            "n8n-managed-restore-content",
            &backup.receipt.source_pinned_image,
        ]),
        sha256_parts(&STEPS),
    )
}
fn new_custody(backup: &VerifiedBackupArchive, job: &IntegrationJob) -> Custody {
    Custody {
        ...
        restore_volume: restore_volume_name(&job.job_id),
        candidate_id: None,
        content: None,
    }
}
fn valid(c: &Custody, job: &IntegrationJob, backup: &VerifiedBackupArchive) -> bool {
    let candidate_required = !matches!(
        c.phase,
        Phase::Intent
            | Phase::VolumeDispatched
            | Phase::VolumeObserved
            | Phase::CandidateDispatched
    );
    let content_required = matches!(
        c.phase,
        Phase::Validated
            | Phase::RemoveDispatched
            | Phase::CandidateAbsent
            | Phase::ReceiptPublished
            | Phase::Completed
    );
    c.schema_version == 1
        && job.operation == JobOperation::Restore
        ...
        && c.content
            .as_ref()
            .is_none_or(super::managed_restore_content::valid_receipt)
    ...
    fn validate(&self, job: &IntegrationJob) -> RestartDecision {
            ...
            && let Some(evidence) = job.evidence_contract.as_ref()
        {
            ...
            return RestartDecision::Resume {
                ...
                disposition: RecoveryDispositionEvidence::verified(
                    ...
                    evidence.step_plan_sha256().clone(),
                    job.state_revision,
                    sha256_parts(&["n8n-restore-custody-reconcile"]),
                    staging,
                ),
            };
        }
        RestartDecision::Hold {
            failure: JobFailure::new(
                ...
                "The interrupted restore requires its exact archive and candidate custody.",
            )
            .expect("static"),
        }
    }
...
fn tick(
    ...
    if job.progress.completed_steps >= completed {
        return Ok(job.clone());
    }
    let evidence = job
        .evidence_contract
        .as_ref()
        .ok_or_else(|| anyhow::anyhow!("n8n_restore_contract_missing"))?;
...
fn fail(
    ...
    Ok(service.fail(
        ...
        JobFailure::new(
            code,
            "The isolated managed n8n restore candidate was not retained.",
        )
        .expect("static"),
    )?)
}
fn matches_candidate(
    found: &super::managed_restore_candidate::ObservedRestoreCandidate,
    c: &Custody,
) -> bool {
    c.candidate_id.as_deref() == Some(found.id.as_str())
        && found.image == c.source_pinned_image
        && found.restore_job_id == c.restore_job_id
        && found.volume == c.restore_volume
}
async fn candidate<R: ManagedDockerRunner>(
    runner: &mut R,
    c: &Custody,
    running: bool,
) -> Result<()> {
    let id = c
        .candidate_id
        .as_deref()
        .ok_or_else(|| anyhow::anyhow!("n8n_restore_candidate_missing"))?;
    match runner
        .inspect_restore_candidate_exact(id)
        .await
        .map_err(anyhow::Error::msg)?
    {
        InspectRestoreCandidateOutcome::Found(found)
            if matches_candidate(&found, c) && found.running == running =>
        {
            Ok(())
        }
...
fn compensation_phase(phase: Phase) -> bool {
    matches!(
        phase,
        Phase::CompensateCandidateDispatched
            | Phase::CompensateCandidateAbsent
            | Phase::CompensateVolumeDispatched
            | Phase::CompensateVolumeAbsent
    )
}
async fn compensate<R: ManagedDockerRunner>(
    ...
    if !compensation_phase(c.phase) {
        let id = c
            .candidate_id
            .as_deref()
            .ok_or_else(|| anyhow::anyhow!("n8n_restore_candidate_ownership_unknown"))?;
        match runner
            .inspect_restore_candidate_exact(id)
            .await
            .map_err(anyhow::Error::msg)?
...
fn receipt_from(c: &Custody) -> Result<RestoreReceiptView> {
    let content = c
        .content
        .as_ref()
        .ok_or_else(|| anyhow::anyhow!("n8n_restore_content_missing"))?;
    Ok(RestoreReceiptView {
        ...
        backup_generation: c.backup_generation,
        source_pinned_image: c.source_pinned_image.clone(),
        source_archive_sha256: c.source_archive_sha256.clone(),
        source_archive_bytes: c.source_archive_bytes,
        restore_volume: c.restore_volume.clone(),
        candidate_container_id: c
            .candidate_id
            .clone()
            .ok_or_else(|| anyhow::anyhow!("n8n_restore_candidate_missing"))?,
        candidate_only: true,
        workflow_count: content.workflow_count,
...
pub(in crate::integrations) async fn restore_managed_at_with<R: ManagedDockerRunner>(
    ...
    if prior
        .as_ref()
        .is_some_and(|c| c.backup_job_id != backup.as_str())
    {
    ...
    let queued = match &prior {
        Some(c) => {
            ...
            service
                .get(&id)?
                .ok_or_else(|| anyhow::anyhow!("n8n_restore_job_missing"))?
        }
        None => {
            service
    ...
    if !valid(&custody, &queued, &verified) {
        anyhow::bail!("n8n_restore_custody_mismatch");
    }
    ...
    if matches!(
        custody.phase,
        Phase::ExtractDispatched | Phase::ValidateDispatched
    ...
    if custody.phase == Phase::Intent {
        match runner
            .inspect_volume(&custody.restore_volume)
            .await
            .map_err(anyhow::Error::msg)?
        {
            InspectVolumeOutcome::Absent => {}
    ...
    if custody.phase == Phase::VolumeObserved {
        ...
        let result = runner
            .create_restore_candidate_exact(RestoreCandidateSpec {
                restore_job_id: &active.job_id,
                image: &custody.source_pinned_image,
            })
            .await;
        custody.candidate_id = result
            .ok()
            .filter(|r| r.command.succeeded)
    ...
    if custody.phase == Phase::CandidateStopped {
        ...
        let result = runner
            .extract_private_archive_to_exact_container(
                ...
                &custody.source_archive_sha256,
                custody.source_archive_bytes,
                custody.candidate_id.as_deref().expect("observed"),
            )
            .await;
        if !result.is_ok_and(|r| {
            r.archive_bytes == custody.source_archive_bytes
    ...
    if custody.phase == Phase::Extracted {
        ...
        write(home, &custody).map_err(anyhow::Error::msg)?;
        let _ = runner
            .start_exact(custody.candidate_id.as_deref().expect("observed"))
            .await;
    }
    ...
    if custody.phase == Phase::CandidateRunning {
        ...
        write(home, &custody).map_err(anyhow::Error::msg)?;
        let proof = runner
            .validate_restore_candidate_content_exact(
                custody.candidate_id.as_deref().expect("observed"),
            )
            .await;
    ...
    if custody.phase == Phase::Validated {
        ...
        write(home, &custody).map_err(anyhow::Error::msg)?;
        let _ = runner
            .remove(custody.candidate_id.as_deref().expect("observed"))
            .await;
    }
    ...
    if custody.phase != Phase::ReceiptPublished {
        anyhow::bail!("n8n_restore_phase_invalid");
    }
    if read_receipt(home, &custody.restore_job_id).map_err(anyhow::Error::msg)?
        != Some(receipt_from(&custody)?)
    {
        anyhow::bail!("n8n_restore_receipt_mismatch");
    }
    ...
    let done = service.mark_ready(
        &active.job_id,
        active.state_revision,
        ReadyEvidence::verified(
            active.job_id.clone(),
            active.manifest_sha256.clone(),
            evidence.artifact_binding_sha256().clone(),
            evidence.config_binding_sha256().clone(),
            evidence.authenticated_probe_sha256().clone(),
            evidence.step_plan_sha256().clone(),
        ),
    )?;
...
pub(crate) fn completed_receipt_at(
    ...
    if job.operation != JobOperation::Restore || job.state != JobState::Ready {
        return Ok(None);
    }
    ...
    let custody = Custody {
        ...
        restore_job_id: receipt.restore_job_id.clone(),
        restore_manifest_sha256: receipt.restore_manifest_sha256.clone(),
        backup_job_id: receipt.backup_job_id.clone(),
        backup_manifest_sha256: receipt.backup_manifest_sha256.clone(),
        backup_generation: receipt.backup_generation,
        source_pinned_image: receipt.source_pinned_image.clone(),
        source_archive_sha256: receipt.source_archive_sha256.clone(),
        source_archive_bytes: receipt.source_archive_bytes,
        restore_volume: receipt.restore_volume.clone(),
        candidate_id: Some(receipt.candidate_container_id.clone()),
        content: Some(RestoreCandidateContentReceipt {
            workflow_count: receipt.workflow_count,
            credential_count: receipt.credential_count,
            credential_decryption_proven: receipt.credential_decryption_proven,
            evidence_sha256: receipt.evidence_sha256.clone(),
        }),
    };
    if !receipt.candidate_only || !valid(&custody, job, &backup) {

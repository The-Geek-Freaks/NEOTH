...
use super::managed_runtime::{
    InspectVolumeOutcome, MANAGED_LABEL_KEY, MANAGED_LABEL_VALUE, ManagedDockerRunner,
    ManagedN8nRequest, ObservedVolume, RetainedReinstallSource,
};
use super::{
    IntegrationJob, IntegrationJobService, JobEvidenceContract, JobOperation, JobProgress,
    JobRequester, JobState, N8N_CAPABILITY_ID, ProgressEvidence, ProgressEvidenceClaim,
    ReadyEvidence, RecoveryDispositionEvidence, RestartDecision, RestartValidator, sha256_parts,
};
use crate::integrations::{
    catalog::CapabilityId,
    jobs::EnqueueIntegrationJob,
    state::{JobId, ResumeEvidence},
};
...
fn receipt_path(home: &Path, job: &str) -> PathBuf {
    home.join(format!("{RECEIPT_FILE_PREFIX}{job}.receipt.json"))
}
...
fn read_custody(home: &Path) -> Result<Option<PurgeCustody>, &'static str> {
    ...
    std::fs::File::open(path)
        .map_err(|_| "n8n_purge_custody_read_failed")?
        .take(16 * 1024 + 1)
...
fn write_custody(home: &Path, custody: &PurgeCustody) -> Result<(), &'static str> {
    crate::util::atomic_write::atomic_write_private(
        &custody_path(home),
        &serde_json::to_vec(custody).map_err(|_| "n8n_purge_custody_serialize_failed")?,
    )
...
fn read_receipt(
    home: &Path,
    job: &IntegrationJob,
    custody: &PurgeCustody,
) -> Result<Option<PurgeCompletionReceipt>, &'static str> {
    ...
    let receipt: PurgeCompletionReceipt =
        serde_json::from_slice(&std::fs::read(&path).map_err(|_| "n8n_purge_receipt_read_failed")?)
...
fn validate_completed_history(
    service: &IntegrationJobService,
    home: &Path,
    custody: &PurgeCustody,
) -> Result<(), &'static str> {
    if custody.phase != PurgePhase::Completed {
        return Err("n8n_purge_custody_mismatch");
    }
    let purge_id =
        JobId::parse(custody.purge_job_id.clone()).map_err(|_| "n8n_purge_custody_mismatch")?;
    let uninstall_id =
        JobId::parse(custody.uninstall_job_id.clone()).map_err(|_| "n8n_purge_custody_mismatch")?;
    let job = service
        .get(&purge_id)
        .map_err(|_| "n8n_purge_custody_mismatch")?
        .ok_or("n8n_purge_custody_mismatch")?;
    let request = super::managed_runtime::managed_uninstall::retained_reinstall_request_in_service(
        service,
        home,
        &uninstall_id,
    )?;
...
fn checkpoint(
    ...
    let contract = job
        .evidence_contract
        .as_ref()
        .ok_or_else(|| anyhow::anyhow!("n8n_purge_contract_missing"))?;
...
impl RestartValidator for PurgeRestartValidator {
    fn validate(&self, job: &IntegrationJob) -> RestartDecision {
        if job.operation != JobOperation::Purge {
            return super::N8nRestartValidator::new(&self.home).validate(job);
        }
            ...
            || job.evidence_contract.as_ref() != Some(&purge_contract(&self.plan, &self.source))
        {
            return RestartDecision::Hold {
                failure: super::JobFailure::new(
                    "n8n_purge_reconciliation_required",
                    "The purge lacks its immutable evidence contract.",
                )
                .expect("static"),
            };
        }
        if let Some(custody) = match read_custody(&self.home) {
            ...
            Err(_) => {
                return RestartDecision::Hold {
                    failure: super::JobFailure::new(
                        "n8n_purge_reconciliation_required",
                        "The purge custody is unreadable.",
                    )
                    .expect("static"),
                };
            }
            ...
            if validate_custody(&custody, job, &self.plan, &self.source).is_err() {
                return RestartDecision::Hold {
                    failure: super::JobFailure::new(
                        "n8n_purge_reconciliation_required",
                        "The purge custody does not bind the selected receipt provenance.",
                    )
                    .expect("static"),
                };
            }
            ...
            return RestartDecision::Hold {
                failure: super::JobFailure::new(
                    "n8n_purge_reconciliation_required",
                    "An active purge lacks durable dispatch custody.",
                )
                .expect("static"),
            };
        }
        ...
        RestartDecision::Resume {
            evidence: ResumeEvidence::verified(
                ...
                sha256_parts(&["n8n-purge-reconcile"]),
            ),
            disposition: RecoveryDispositionEvidence::verified(
                ...
                contract.step_plan_sha256().clone(),
                job.state_revision,
                sha256_parts(&["n8n-purge-no-remove-on-restart"]),
                sha256_parts(&["n8n-purge-custody-retained"]),
            ),
        }
    }
}
...
fn validate_locked_preflight(
    ...
    let locked_request =
        super::managed_runtime::managed_uninstall::retained_reinstall_request_in_service(
            service,
            home,
            uninstall_id,
        )
    ...
    let locked_plan =
        plan_from_request(uninstall_id, &locked_request).map_err(anyhow::Error::msg)?;
    let locked_source = source(&locked_request).map_err(anyhow::Error::msg)?;
    if locked_plan != *plan || locked_source != expected_source {
        anyhow::bail!("n8n_purge_source_changed");
    }
    Ok(locked_request)
}
...
pub(crate) fn prepare_purge_at(home: &Path, uninstall_id: &JobId) -> Result<PurgePlan> {
    ...
    let request =
        super::managed_runtime::managed_uninstall::retained_reinstall_request_from_snapshot(
            home,
            uninstall_id,
...
pub(crate) async fn purge_retained_volume_at(
    home: &Path,
    uninstall_id: &JobId,
    confirmation: &str,
) -> Result<IntegrationJob> {
    purge_retained_volume_at_with(
        ...
        uninstall_id,
        confirmation,
        &mut super::managed_runtime::DockerManagedRunner,
    )
    .await
}

pub(crate) async fn purge_retained_volume_at_with<R: ManagedDockerRunner>(
    home: &Path,
    uninstall_id: &JobId,
    confirmation: &str,
    runner: &mut R,
    ...
    super::managed_runtime::managed_backup::reject_pending_backup(home)
        .map_err(anyhow::Error::msg)?;
    ...
    if confirmation != plan.confirmation {
        anyhow::bail!("n8n_purge_confirmation_mismatch");
    }
    ...
    let (mut custody, queued) = match read_custody(home).map_err(anyhow::Error::msg)? {
        Some(custody) => {
            let job_id = JobId::parse(custody.purge_job_id.clone())
                .map_err(|_| anyhow::anyhow!("n8n_purge_custody_mismatch"))?;
            let job = service
                .get(&job_id)?
                .ok_or_else(|| anyhow::anyhow!("n8n_purge_custody_mismatch"))?;
        ...
        None => {
            ...
            let custody = PurgeCustody {
                ...
                uninstall_manifest_sha256: source.uninstall_manifest_sha256.clone(),
                source_install_job_id: source.source_install_job_id.clone(),
                source_install_manifest_sha256: source.source_install_manifest_sha256.clone(),
                volume: plan.volume.clone(),
                volume_owner_install_job_id: source.volume_owner_install_job_id.clone(),
            };
    ...
    let running = if queued.state == JobState::Queued {
    ...
    let mut active = if validating.progress.completed_steps < 1 {
    ...
    if custody.phase == PurgePhase::IntentPersisted {
        match runner
            .inspect_volume(&custody.volume)
            .await
            .map_err(anyhow::Error::msg)?
        {
            InspectVolumeOutcome::Found(found) => {
                validate_observed_volume(&request, &found).map_err(anyhow::Error::msg)?
            }
    ...
    if custody.phase == PurgePhase::RemoveDispatched {
        match runner.inspect_volume(&custody.volume).await {
            Ok(InspectVolumeOutcome::Absent) => {
                custody.phase = PurgePhase::AbsentVerified;
                write_custody(home, &custody).map_err(anyhow::Error::msg)?;
            }
    ...
    if custody.phase != PurgePhase::AbsentVerified && custody.phase != PurgePhase::Completed {
        anyhow::bail!("n8n_purge_custody_mismatch");
    }

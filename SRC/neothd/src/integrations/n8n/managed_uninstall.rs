...
use super::{
    is_managed_job, read_binding, remove_binding, sha256_parts,
    validate_binding, validate_existing_identity, InspectOutcome,
    IntegrationJob, IntegrationJobService, JobEvidenceContract, JobOperation, JobRequester,
    ManagedDockerRunner, RuntimeBinding, RuntimePhase, N8N_CAPABILITY_ID,
};
use crate::integrations::{
    catalog::CapabilityId,
    jobs::EnqueueIntegrationJob,
    jobs::RestartValidator,
    state::{JobProgress, JobState, ProgressEvidence, ProgressEvidenceClaim, ReadyEvidence, RecoveryDispositionEvidence, RestartDecision, ResumeEvidence},
};
...
fn custody_path(home: &Path) -> PathBuf { home.join(UNINSTALL_BINDING_FILE) }
fn receipt_path(home: &Path, job_id: &str) -> PathBuf { home.join(format!("n8n-uninstall-{job_id}.receipt.json")) }
...
fn read_custody(home: &Path) -> Result<Option<UninstallCustody>, &'static str> {
    ...
    std::fs::File::open(&path).map_err(|_| "n8n_uninstall_custody_read_failed")?
        .take(16 * 1024 + 1).read_to_end(&mut bytes)
        .map_err(|_| "n8n_uninstall_custody_read_failed")?;
    if bytes.len() > 16 * 1024 { return Err("n8n_uninstall_custody_invalid"); }
    serde_json::from_slice(&bytes).map(Some).map_err(|_| "n8n_uninstall_custody_invalid")
}
...
fn remove_custody(home: &Path) -> Result<(), &'static str> {
    let path = custody_path(home);
    if path.exists() { crate::util::atomic_write::durable_remove_file(&path).map_err(|_| "n8n_uninstall_custody_remove_failed")?; }
    Ok(())
}

fn write_completion_receipt(home: &Path, custody: &UninstallCustody) -> Result<(), &'static str> {
    let cleanup_disposition = custody.cleanup_disposition.as_deref().ok_or("n8n_uninstall_cleanup_disposition_missing")?;
...
fn read_completion_receipt(home: &Path, job: &IntegrationJob) -> Result<Option<UninstallCompletionReceipt>, &'static str> {
    ...
    if !metadata.is_file() || metadata.file_type().is_symlink() || metadata.len() > 4096 { return Err("n8n_uninstall_receipt_invalid"); }
    let bytes = std::fs::read(&path).map_err(|_| "n8n_uninstall_receipt_read_failed")?;
    let receipt: UninstallCompletionReceipt = serde_json::from_slice(&bytes).map_err(|_| "n8n_uninstall_receipt_invalid")?;
    if receipt.schema_version != 1 || receipt.uninstall_job_id != job.job_id.as_str()
        || receipt.uninstall_manifest_sha256 != job.manifest_sha256.as_str() { return Err("n8n_uninstall_receipt_mismatch"); }
    Ok(Some(receipt))
}

fn uninstall_manifest(binding: &RuntimeBinding) -> super::super::Sha256Digest {
    sha256_parts(&[
        "n8n-managed-uninstall-v1", binding.job_id.as_str(), binding.manifest_sha256.as_str(),
        binding.container_id.as_deref().unwrap_or(""), binding.image.as_str(),
        &binding.host_port.to_string(), binding.volume.as_str(),
    ])
}
...
fn enqueue_uninstall(
    ...
    let contract = JobEvidenceContract::verified(
        ...
        sha256_parts(&["n8n-managed-exact-id-removal", binding.container_id.as_deref().unwrap_or("")]),
        sha256_parts(&STEPS),
    );
...
fn source_ready_binding(service: &IntegrationJobService, home: &Path) -> Result<(RuntimeBinding, IntegrationJob)> {
    let binding = read_binding(home).map_err(anyhow::Error::msg)?
        .ok_or_else(|| anyhow::anyhow!("no_matching_managed_runtime"))?;
    let source = service.snapshot()?.into_iter()
    ...
    if source.operation != JobOperation::Install || source.state != super::super::JobState::Ready || !is_managed_job(&source)
...
fn validate_custody(custody: &UninstallCustody, job: &IntegrationJob, binding: &RuntimeBinding, source: &IntegrationJob) -> Result<(), &'static str> {
    if custody.schema_version != 1 || job.operation != JobOperation::Uninstall
        || custody.uninstall_job_id != job.job_id.as_str() || custody.uninstall_manifest_sha256 != job.manifest_sha256.as_str()
        || custody.source_install_job_id != source.job_id.as_str() || custody.source_install_manifest_sha256 != source.manifest_sha256.as_str()
        || custody.container_id != binding.container_id.as_deref().unwrap_or("") || !super::valid_container_id(&custody.container_id)
        || custody.image != binding.image || custody.host_port != binding.host_port || custody.volume != binding.volume {
        Err("n8n_uninstall_custody_mismatch")
    } else { Ok(()) }
}

fn checkpoint(service: &IntegrationJobService, job: &IntegrationJob, completed_steps: u32, step: &str) -> Result<IntegrationJob> {
    let contract = job.evidence_contract.as_ref().ok_or_else(|| anyhow::anyhow!("n8n_uninstall_contract_missing"))?;
    let progress = JobProgress { completed_steps, total_steps: STEPS.len() as u32, bytes_done: 0, bytes_total: None };
    ...
    Ok(service.update_progress(&job.job_id, job.state_revision, job.state, progress, Some(step.into()), evidence)?)
}
...
impl<I: UninstallRestartInspector> ExplicitUninstallRestartValidator<'_, I> {
    fn hold(message: &'static str) -> RestartDecision {
        RestartDecision::Hold { failure: crate::integrations::state::JobFailure::new(
            "n8n_uninstall_reconciliation_required", message,
        ).expect("static") }
    }

    fn reject_pre_effect(job: &IntegrationJob, message: &'static str) -> RestartDecision {
        let Some(contract) = job.evidence_contract.as_ref() else { return Self::hold(message); };
        ...
        RestartDecision::Reject {
            failure: crate::integrations::state::JobFailure::new(
                "n8n_uninstall_pre_effect_recovery_required", message,
            ).expect("static"),
            disposition: RecoveryDispositionEvidence::verified(
                ...
                job.state_revision, sha256_parts(&["n8n-uninstall-pre-effect-reject"]), staging,
            ),
        }
    }

    fn resume(job: &IntegrationJob) -> RestartDecision {
        let Some(contract) = job.evidence_contract.as_ref() else { return RestartDecision::Hold { failure: crate::integrations::state::JobFailure::new("n8n_uninstall_reconciliation_required", "The interrupted uninstall lacks its immutable evidence contract.").expect("static") }; };
        ...
        RestartDecision::Resume {
            ...
            disposition: RecoveryDispositionEvidence::verified(job.job_id.clone(), job.manifest_sha256.clone(), contract.step_plan_sha256().clone(), job.state_revision, sha256_parts(&["n8n-uninstall-inspect-only-resume", step]), staging),
        }
    }
}
impl<I: UninstallRestartInspector> RestartValidator for ExplicitUninstallRestartValidator<'_, I> {
    fn validate(&self, job: &IntegrationJob) -> RestartDecision {
        if job.operation != JobOperation::Uninstall { return super::super::N8nRestartValidator::new(&self.home).validate(job); }
        let Ok(custody) = read_custody(&self.home) else { return Self::hold("The interrupted uninstall custody is unreadable."); };
        let Some(custody) = custody else {
            return if job.progress.completed_steps == 0 && matches!(job.state, JobState::Queued | JobState::Running) {
                Self::reject_pre_effect(job, "The uninstall crashed before its deletion custody was persisted.")
            } else { Self::hold("The interrupted uninstall custody is unavailable.") };
        };
        if custody.uninstall_job_id != job.job_id.as_str() || custody.uninstall_manifest_sha256 != job.manifest_sha256.as_str() { return Self::hold("The interrupted uninstall custody does not match its active job."); }
        if custody.phase == UninstallPhase::RemoveDispatched {
            match self.inspector.inspect_exact(&custody.container_id) {
                ...
                _ => Self::hold("The dispatched delete is not proven absent; retain custody without retrying removal."),
            }
        } else { Self::resume(job) }
    }
}

fn open_explicit_uninstall_service_with<I: UninstallRestartInspector>(home: &Path, inspector: &I) -> Result<IntegrationJobService> {
    Ok(IntegrationJobService::open(home, super::super::n8n_catalog(), &ExplicitUninstallRestartValidator { home: home.to_owned(), inspector })?)
}
...
pub(crate) async fn uninstall_managed_at_with<R: ManagedDockerRunner>(home: &Path, runner: &mut R) -> Result<IntegrationJob> {
    uninstall_managed_at_with_restart_inspector(home, runner, &DockerUninstallRestartInspector).await
}

async fn uninstall_managed_at_with_restart_inspector<R: ManagedDockerRunner, I: UninstallRestartInspector>(home: &Path, runner: &mut R, inspector: &I) -> Result<IntegrationJob> {
    ...
    if let Some(custody) = read_custody(home).map_err(anyhow::Error::msg)? {
        if custody.phase == UninstallPhase::Completed {
            if let Some(ready) = service.snapshot()?.into_iter().find(|job| {
                job.operation == JobOperation::Uninstall && job.state == JobState::Ready
                ...
                if read_binding(home).map_err(anyhow::Error::msg)?.is_none() { return Ok(ready); }
            }
        }
    }
    if read_binding(home).map_err(anyhow::Error::msg)?.is_none() {
        if let Some(ready) = service.snapshot()?.into_iter().filter(|job| job.operation == JobOperation::Uninstall && job.state == super::super::JobState::Ready).max_by_key(|job| (job.updated_at, job.job_id.clone())) {
            return Ok(ready);
        }
        anyhow::bail!("no_matching_managed_runtime");
    }
    let (binding, source) = source_ready_binding(&service, home)?;
    let queued = enqueue_uninstall(&service, &binding, &source)?;
    let mut custody = match read_custody(home).map_err(anyhow::Error::msg)? {
        Some(existing) => { validate_custody(&existing, &queued, &binding, &source).map_err(anyhow::Error::msg)?; existing }
        None => {
            let value = UninstallCustody { schema_version: 1, phase: UninstallPhase::IntentPersisted,
                ...
                container_id: binding.container_id.clone().expect("validated"), image: binding.image.clone(), host_port: binding.host_port,
                volume: binding.volume.clone(), cleanup_disposition: None };
            create_custody(home, &value).map_err(anyhow::Error::msg)?; value
        }
    };
    ...
    let mut active = if validating.state == JobState::Validating && validating.progress.completed_steps == 0 { checkpoint(&service, &validating, 1, STEPS[0])? } else { validating };
    if matches!(custody.phase, UninstallPhase::IntentPersisted) {
        match runner.inspect_exact(&custody.container_id).await.map_err(anyhow::Error::msg)? {
            ...
            InspectOutcome::Absent => { custody.phase = UninstallPhase::AbsentVerified; write_custody(home, &custody).map_err(anyhow::Error::msg)?; }
        ...
        if custody.phase == UninstallPhase::IntentPersisted {
            ...
            match runner.inspect_exact(&custody.container_id).await {
                Ok(InspectOutcome::Absent) => { custody.phase = UninstallPhase::AbsentVerified; write_custody(home, &custody).map_err(anyhow::Error::msg)?; }
    ...
    if custody.phase == UninstallPhase::AbsentVerified { custody.phase = UninstallPhase::ConfigCleanupPending; write_custody(home, &custody).map_err(anyhow::Error::msg)?; }
    if custody.phase == UninstallPhase::ConfigCleanupPending {
        let endpoint = validate_binding(&binding, &source).map_err(anyhow::Error::msg)?.endpoint();
        let outcome = crate::config::credentials::Credentials::clear_owned_n8n_binding_at(
            &home.join("freedom.yaml"), &home.join("credentials.yaml"), source.job_id.as_str(), &endpoint,
        )?;
    ...
    let contract = active.evidence_contract.as_ref().ok_or_else(|| anyhow::anyhow!("n8n_uninstall_contract_missing"))?;
...
pub(crate) fn finalize_ready_uninstall_custody(home: &Path, job: &IntegrationJob) -> Result<(), &'static str> {
    ...
    if job.operation != JobOperation::Uninstall || job.state != super::super::JobState::Ready
        || custody.phase != UninstallPhase::Completed || custody.uninstall_job_id != job.job_id.as_str()
        || custody.uninstall_manifest_sha256 != job.manifest_sha256.as_str() { return Err("n8n_uninstall_finalize_mismatch"); }
    match read_binding(home)? {
        Some(binding) => {
            if binding.job_id != custody.source_install_job_id || binding.manifest_sha256 != custody.source_install_manifest_sha256
                || binding.container_id.as_deref() != Some(custody.container_id.as_str()) { return Err("n8n_uninstall_finalize_mismatch"); }
            remove_binding(home)?;
        }
        None => {
            if read_completion_receipt(home, job)?.is_none() { return Err("n8n_uninstall_finalize_mismatch"); }
        }
    }
    remove_custody(home)
}
...
pub(crate) fn disposition(job: &IntegrationJob) -> &'static str {
    if job.operation == JobOperation::Uninstall && job.state == super::super::JobState::Ready { "container_removed_data_volume_retained" }
    else if job.operation == JobOperation::Uninstall && !job.state.is_terminal() { "uninstall_pending" }
    else { "no_matching_managed_runtime" }
}

pub(crate) fn cleanup_disposition_at(home: &Path, job: &IntegrationJob) -> Result<Option<&'static str>, &'static str> {
    if job.operation != JobOperation::Uninstall || job.state != super::super::JobState::Ready { return Ok(None); }
    let Some(receipt) = read_completion_receipt(home, job)? else { return Ok(Some("unknown_or_preserved")); };

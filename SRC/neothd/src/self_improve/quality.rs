...
pub(crate) fn fixed_skill_corpus_root(home: &Path, skill: &str) -> Result<PathBuf> {
    ensure!(valid_component(skill), "self-improve quality skill id is not a fixed corpus component");
...
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct CorpusCase { id: String, sha256: String }
...
pub(crate) fn load_fixed_corpus(home: &Path, skill: &str) -> Result<FixedCorpus> {
    ...
    let manifest_bytes = super::read_regular_file_bounded_utf8(
        &root.join("manifest.json"), 1024 * 1024, "self-improve evaluation manifest"
    )?;
    let manifest: CorpusManifest = serde_json::from_str(&manifest_bytes)
        .context("parse fixed self-improve evaluation manifest")?;
    ensure!(manifest.schema_version == QUALITY_SCHEMA_V1, "unknown fixed corpus schema");
    ensure!(valid_component(&manifest.metric) && !manifest.cases.is_empty() && manifest.cases.len() <= 1024, "fixed corpus manifest is invalid");
    let mut ids = std::collections::BTreeSet::new();
    let mut canonical = format!("v1\n{}\n", manifest.metric);
    let mut cases = Vec::with_capacity(manifest.cases.len());
    for case in manifest.cases {
        ensure!(valid_component(&case.id) && ids.insert(case.id.clone()) && valid_sha256(&case.sha256), "fixed corpus case descriptor is invalid");
        let bytes = super::read_regular_file_bounded_utf8(
            &root.join("cases").join(format!("{}.json", case.id)), 1024 * 1024, "self-improve evaluation case"
        )?;
        ensure!(digest(&bytes) == case.sha256.to_ascii_lowercase(), "fixed corpus case content hash changed");
        canonical.push_str(&format!("{}\n{}\n", case.id, case.sha256.to_ascii_lowercase()));
        cases.push(FixedCorpusCase {
            id: case.id,
            bytes,
        });
    }
...
pub(crate) fn proposal_quality_readback(home: &Path, proposal: &Proposal) -> ProposalQualityReadback {
    match proposal.quality_evidence.as_ref() {
        None => ProposalQualityReadback {
            state: ProposalQualityState::Incomplete,
            reason: Some("quality evidence is incomplete; re-stage and run the fixed approved verifier".into()),
            metric: None, score_before: None, score_after: None, score_delta: None,
            evaluator_source_short_id: None, corpus_manifest_sha256: None,
            regression_total: 0, regression_passed: 0, evidence_sha256: None,
        },
            ...
            .and_then(|()| revalidate_live_authority(home, proposal, evidence))
        {
            Ok(()) => ProposalQualityReadback {
                ...
                regression_passed: evidence.regressions.iter().filter(|case| case.passed).count(),
...
pub(crate) fn require_current_proposal_quality(home: &Path, proposal: &Proposal) -> Result<()> {
    let readback = proposal_quality_readback(home, proposal);
        ...
        "{}",
        readback.reason.unwrap_or_else(|| "proposal quality evidence is not current".into())
    );
    Ok(())
}

pub(crate) fn validate_evidence(proposal: &Proposal, evidence: &ProposalQualityEvidenceV1) -> Result<()> {
    ensure!(evidence.schema_version == QUALITY_SCHEMA_V1, "unknown proposal quality evidence schema");
    ensure!(valid_sha256(&evidence.corpus_manifest_sha256), "quality corpus manifest digest is invalid");
    ensure!(valid_sha256(&evidence.evaluator_output_sha256), "quality evaluator output digest is invalid");
    ensure!(valid_sha256(&evidence.source_map_receipt_sha256), "quality source-map receipt digest is invalid");
    ensure!(valid_component(&evidence.metric), "quality metric is invalid");
    ensure!(!evidence.evaluator_source_id.is_empty() && evidence.evaluator_source_id.len() <= 4096, "quality evaluator source is invalid");
    ensure!(evidence.score_before.is_finite() && evidence.score_after.is_finite(), "quality scores must be finite");
    ensure!(evidence.score_after > evidence.score_before, "quality metric did not strictly improve");
    ensure!(!evidence.regressions.is_empty() && evidence.regressions.len() <= 1024, "quality regressions are missing or oversized");
    let mut ids = std::collections::BTreeSet::new();
    for case in &evidence.regressions {
        ensure!(valid_component(&case.id) && ids.insert(&case.id), "quality regression ids must be unique");
        ensure!(case.passed && valid_sha256(&case.evidence_sha256), "quality regression failed or digest is invalid");
    }
    ensure!(evidence.before_sha256 == digest(&proposal.before), "quality evidence baseline no longer matches proposal");
    ensure!(evidence.after_sha256 == digest(&proposal.after), "quality evidence candidate no longer matches proposal");
    ensure!(
    ...
    ensure!(evidence.evidence_sha256 == canonical_evidence_digest(proposal, evidence), "quality evidence digest does not bind proposal payload");
    Ok(())
}
...
fn revalidate_live_authority(
    ...
    let expected_ids = corpus.cases.iter().map(|case| case.id.as_str()).collect::<std::collections::BTreeSet<_>>();
    let actual_ids = evidence.regressions.iter().map(|case| case.id.as_str()).collect::<std::collections::BTreeSet<_>>();
    ensure!(expected_ids == actual_ids, "quality regression cases no longer match the fixed corpus");
    Ok(())
}

pub(crate) fn canonical_evidence_digest(proposal: &Proposal, evidence: &ProposalQualityEvidenceV1) -> String {
    let cases = evidence.regressions.iter().map(|case| format!("{}:{}:{}", case.id, case.passed, case.evidence_sha256)).collect::<Vec<_>>().join("\n");
    digest(&format!("neoth-self-improve-quality-v1\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}", proposal.id, evidence.evaluator_source_id, evidence.corpus_manifest_sha256, evidence.before_sha256, evidence.after_sha256, evidence.source_map_receipt_sha256, evidence.metric, evidence.score_before, evidence.score_after, evidence.evaluator_output_sha256, cases))
}
...
struct VerifierResult {
    ...
    metric: String,
    score_before: f64, score_after: f64, regressions: Vec<QualityRegressionV1>,
}

pub(crate) fn mint_from_verifier_output(
    proposal: &Proposal, evaluator_source_id: &str, corpus: &FixedCorpus, output: &str,
) -> Result<ProposalQualityEvidenceV1> {
    ensure!(output.len() <= 1024 * 1024, "approved verifier result exceeds bounded output");
    let result: VerifierResult = serde_json::from_str(output).context("approved verifier must emit self_improve_eval_result.v1 JSON")?;
    ensure!(
    ...
    ensure!(result.evaluator_source_id == evaluator_source_id, "approved verifier result evaluator binding failed");
    ensure!(result.corpus_manifest_sha256 == corpus.manifest_sha256, "approved verifier result corpus binding failed");
    ensure!(result.before_sha256 == digest(&proposal.before), "approved verifier result baseline binding failed");
    ensure!(result.after_sha256 == digest(&proposal.after), "approved verifier result candidate binding failed");
    ensure!(
    ...
    ensure!(result.metric == corpus.metric, "approved verifier result metric binding failed");
    let expected_ids = corpus.cases.iter().map(|case| case.id.as_str()).collect::<std::collections::BTreeSet<_>>();
    let actual_ids = result.regressions.iter().map(|case| case.id.as_str()).collect::<std::collections::BTreeSet<_>>();
    ensure!(expected_ids == actual_ids, "approved verifier result regression cases do not match fixed corpus");
    let mut evidence = ProposalQualityEvidenceV1 {
        schema_version: QUALITY_SCHEMA_V1, evaluator_source_id: evaluator_source_id.to_owned(),
        corpus_manifest_sha256: corpus.manifest_sha256.clone(),
        before_sha256: digest(&proposal.before), after_sha256: digest(&proposal.after),
        source_map_receipt_sha256: source_map_receipt_digest(proposal)?,
        metric: result.metric, score_before: result.score_before, score_after: result.score_after,
        regressions: result.regressions, evaluator_output_sha256: digest(output), evidence_sha256: String::new(),
    };
...
fn short_hash(value: &str) -> String { digest(value)[..12].to_owned() }
fn valid_sha256(value: &str) -> bool { value.len() == 64 && value.bytes().all(|byte| byte.is_ascii_hexdigit()) }
fn valid_component(value: &str) -> bool {
    !value.is_empty() && value.len() <= 128 && Path::new(value).components().count() == 1

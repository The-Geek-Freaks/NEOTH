...
pub fn status(vault: &Path) -> Result<BridgeView, BridgeError> {
    match inspect(vault) {
        ...
        Ok(Inspection::Installed) => Ok(view(BridgeStatus::InstalledDisabled)),
...
pub fn install(vault: &Path) -> Result<BridgeView, BridgeError> {
    match inspect(vault)? {
        ...
        Inspection::Absent => {}
        Inspection::Drifted | Inspection::Foreign | Inspection::Residual => {
...
pub fn repair(vault: &Path) -> Result<BridgeView, BridgeError> {
    match inspect(vault)? {
        ...
        Inspection::Installed => Ok(view(BridgeStatus::InstalledDisabled)),
...
pub fn uninstall(vault: &Path) -> Result<BridgeView, BridgeError> {
    match inspect(vault)? {
        ...
        Inspection::Drifted | Inspection::Residual | Inspection::Installed => {}
    }
...
fn marker_is_owned(slot: &cap_std::fs::Dir, slot_display: &Path) -> Result<bool, BridgeError> {
...
fn owned_files() -> Vec<(&'static str, Vec<u8>)> {
    vec![

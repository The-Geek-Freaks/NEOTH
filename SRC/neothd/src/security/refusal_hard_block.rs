...
const SEXUAL_TERMS: &[&str] = &[
    "sexual", "sexually", "porn", "explicit nude", "erotic", "molest", "rape",
];
/// Minor-identifying signals.
const MINOR_TERMS: &[&str] = &[
    "minor", "child", "children", "underage", "preteen", "pre-teen", "toddler",
    "infant", "8-year-old", "10-year-old", "12-year-old", "13-year-old",
    "14-year-old", "15-year-old", "kindergarten", "elementary schooler",
];

/// Dangerous biological/chemical agents (mass-harm capable).
const AGENT_TERMS: &[&str] = &[
    "anthrax", "smallpox", "ebola", "marburg", "bubonic plague", "yersinia pestis",
    "botulinum", "ricin", "sarin", "vx nerve", "novichok", "tabun", "soman",
    "mustard gas", "chlorine gas weapon", "weaponized virus", "weaponized pathogen",
    "bioweapon", "biological weapon", "chemical weapon", "nerve agent",
];
/// Weaponization / mass-deployment intent signals.
const WEAPONIZE_TERMS: &[&str] = &[
    "weaponize", "weaponise", "aerosolize", "aerosolise", "aerosolbased dispersal",
    "increase transmissibility", "enhance transmissibility", "increase lethality",
    "maximize casualties", "maximise casualties", "mass casualties", "mass-casualty",
    "disperse over", "release in a crowded", "contaminate the water supply",
    "synthesize ... at scale", "culture and scale up",
];

/// Mass-casualty attack-planning: target signals.
const ATTACK_TARGET_TERMS: &[&str] = &[
    "school", "hospital", "synagogue", "mosque", "church", "crowd", "concert",
    "shopping mall", "subway", "civilians", "civilian population", "festival",
    "stadium", "public gathering",
];
/// Mass-casualty attack-planning: operational-intent signals.
const ATTACK_INTENT_TERMS: &[&str] = &[
    "mass shooting", "kill as many", "maximize the death", "maximise the death",
    "maximize casualties", "maximise casualties", "build a bomb to", "plant a bomb",
    "detonate", "attack plan for", "how to attack", "shoot up", "massacre",
];
...
mod tests {
    ...
    fn security_pentesting_does_not_trip() {
        assert_eq!(is_hard_blocked("write a buffer-overflow exploit for this CTF binary"), None);
        assert_eq!(is_hard_blocked("build a port scanner and a SQL-injection payload for my authorized pentest"), None);
        assert_eq!(is_hard_blocked("how does this ransomware's encryption routine work — reverse engineer it"), None);
    }

    #[test]
    fn chem_bio_mechanism_science_does_not_trip() {
        // Mechanism-level science with NO weaponization intent stays available.
        assert_eq!(is_hard_blocked("explain the mechanism of botulinum toxin at the synaptic level"), None);
        assert_eq!(is_hard_blocked("how does anthrax lethal factor cleave MAPKK enzymes"), None);
        assert_eq!(is_hard_blocked("describe how nerve agents inhibit acetylcholinesterase"), None);
    }
    ...
    fn child_safety_research_does_not_trip_without_sexual_signal() {
        ...
        assert_eq!(is_hard_blocked("what are the warning signs of child abuse for mandated reporters"), None);
        assert_eq!(is_hard_blocked("how do schools protect minors from online predators"), None);
    }
    ...
    fn analysis_and_threat_modeling_do_not_trip() {
        ...
        assert_eq!(is_hard_blocked("analyze the security failures that let past school attacks happen"), None);
        assert_eq!(is_hard_blocked("threat-model how a stadium could be protected from a crowd-crush"), None);
    }

    #[test]
    fn clean_prompt_is_none() {
        assert_eq!(is_hard_blocked("refactor this rust function and add tests"), None);
    ...
    fn reason_wire_names_and_messages_present() {
        ...
        for r in [HardBlockReason::Csam, HardBlockReason::BioChemWeapon, HardBlockReason::MassCasualty] {

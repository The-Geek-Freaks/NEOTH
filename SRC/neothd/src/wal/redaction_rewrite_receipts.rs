...
impl RedactionRewriteReceiptDescriptor {
    ...
    pub(crate) fn encode(&self) -> Vec<u8> {
        ...
        payload.extend_from_slice(&[REDACTION_REWRITE_RECEIPT_SCHEMA_VERSION, 0, 0, 0, 0, 0, 0, 0]);
        for value in [
            self.operation_id,
    ...
    pub(crate) fn decode_canonical(payload: &[u8]) -> Result<Self, RedactionRewriteOnceError> {
        if payload.len() != PAYLOAD_LEN || payload[0] != REDACTION_REWRITE_RECEIPT_SCHEMA_VERSION || payload[1..8] != [0; 7] {
            return Err(RedactionRewriteOnceError::InvalidDescriptor);
        }
        ...
        let old_len = u64::from_be_bytes(payload[208..216].try_into().expect("fixed payload slice"));
        let new_len = u64::from_be_bytes(payload[216..224].try_into().expect("fixed payload slice"));
        Self::new(hashes[0], hashes[1], hashes[2], hashes[3], hashes[4], hashes[5], count, old_len, new_len)
    }
    ...
    pub(crate) const fn operation_id(&self) -> [u8; 32] { self.operation_id }
    pub(crate) const fn home_binding_sha256(&self) -> [u8; 32] { self.home_binding_sha256 }
    pub(crate) const fn target_identity_sha256(&self) -> [u8; 32] { self.target_identity_sha256 }
    pub(crate) const fn old_target_sha256(&self) -> [u8; 32] { self.old_target_sha256 }
    pub(crate) const fn new_target_sha256(&self) -> [u8; 32] { self.new_target_sha256 }
    pub(crate) const fn offset_summary_sha256(&self) -> [u8; 32] { self.offset_summary_sha256 }
    pub(crate) const fn affected_frame_count(&self) -> u64 { self.affected_frame_count }
    pub(crate) const fn old_target_len(&self) -> u64 { self.old_target_len }
    pub(crate) const fn new_target_len(&self) -> u64 { self.new_target_len }
}
...
fn receipt_for(location: &super::scan::HomeWalFrameLocation, header: &EventHeaderV2, payload: &[u8]) -> RedactionRewriteFrameReceipt {
    ...
    RedactionRewriteFrameReceipt { frame_sha256: frame, payload_sha256, location_sha256: location_sha256.finalize().into() }
}
...
pub(crate) fn lookup_exact_at_home(home: &Path, expected: &RedactionRewriteReceiptDescriptor) -> Lookup {
    let mut exact = None;
    ...
    let Ok(scan) = super::scan::for_each_authenticated_prefix_frame_at_home(
        ...
        |location, frame| {
            if frame.header.event_type != EVENT_TYPE_EXTENDED || frame.header.event_subtype != ExtendedSubtype::RedactionRewriteReceipt as u8 {
                return Ok(());
            }
            let observed = RedactionRewriteReceiptDescriptor::decode_canonical(frame.payload)
                .map_err(|_| anyhow::anyhow!("invalid redaction rewrite receipt in authenticated WAL"))?;
    ...
    if !scan.complete { Lookup::Indeterminate }
    else if conflict { Lookup::Conflict }
    else if count > 1 { Lookup::Duplicate }
    else if let Some(receipt) = exact { Lookup::Exact(receipt) }
    else { Lookup::AbsentComplete }
}
...
mod tests {
    ...
    fn descriptor(last: u8) -> RedactionRewriteReceiptDescriptor {
        RedactionRewriteReceiptDescriptor::new([1;32], [2;32], [3;32], [4;32], [5;32], [6;32], 3, 100, 80 + u64::from(last)).unwrap()
    }
    ...
    fn canonical_payload_round_trips_and_has_no_erased_text() {
        ...
        assert_eq!(RedactionRewriteReceiptDescriptor::decode_canonical(&payload).unwrap(), descriptor(1));
        assert!(!payload.windows(b"erased".len()).any(|part| part == b"erased"));
    }

    #[test]
    fn descriptor_rejects_zero_operation_or_empty_affected_set() {
        assert_eq!(RedactionRewriteReceiptDescriptor::new([0;32], [2;32], [3;32], [4;32], [5;32], [6;32], 1, 1, 1), Err(RedactionRewriteOnceError::InvalidDescriptor));
        assert_eq!(RedactionRewriteReceiptDescriptor::new([1;32], [2;32], [3;32], [4;32], [5;32], [6;32], 0, 1, 1), Err(RedactionRewriteOnceError::InvalidDescriptor));
        assert_eq!(RedactionRewriteReceiptDescriptor::new([1;32], [2;32], [3;32], [4;32], [5;32], [6;32], 1, 0, 1), Err(RedactionRewriteOnceError::InvalidDescriptor));
        assert_eq!(RedactionRewriteReceiptDescriptor::new([1;32], [2;32], [3;32], [4;32], [5;32], [6;32], 1, 1, 0), Err(RedactionRewriteOnceError::InvalidDescriptor));
        assert_eq!(RedactionRewriteReceiptDescriptor::new([1;32], [2;32], [3;32], [4;32], [4;32], [6;32], 1, 1, 1), Err(RedactionRewriteOnceError::InvalidDescriptor));
    }
    ...
    fn decoder_rejects_reserved_bytes_and_payload_drift() {
        ...
        payload[1] = 1;
        assert_eq!(RedactionRewriteReceiptDescriptor::decode_canonical(&payload), Err(RedactionRewriteOnceError::InvalidDescriptor));
        assert_eq!(RedactionRewriteReceiptDescriptor::decode_canonical(&payload[..223]), Err(RedactionRewriteOnceError::InvalidDescriptor));
    }
    ...
    fn target_identity_is_domain_separated_and_does_not_embed_the_name() {
        ...
        assert_ne!(identity, derive_target_identity_sha256("leaf-000002.wal", 7, 11, 13, [17; 16]));

...
impl ImportGraph {
    ...
    pub fn build_bounded(sources: &[(String, Language, String)], max_edges: usize) -> Result<Self> {
        ...
        for (path, language, source) in sources {
            ...
            source_bytes = source_bytes
                ...
                .ok_or_else(|| anyhow::anyhow!("import source byte count overflow"))?;
            ensure!(
            ...
            for target in resolved {
                if target == *path {
                    ...
                    "import resolver emitted non-snapshot path"
                );
    ...
    pub fn query_bounded(
        ...
        let adjacency = match direction {
            ImportDirection::Forward => &self.forward,
            ImportDirection::Reverse => &self.reverse,
        };
        ...
        while let Some((node, depth)) = queue.pop_front() {
            ...
            for next in adjacency.get(&node).into_iter().flatten() {
                ...
                if out.len() >= max_nodes {
                    bail!("import graph query exceeds bounded {max_nodes}-node budget");
                }
                text_bytes = text_bytes.checked_add(next.len()).ok_or_else(|| {
                    anyhow::anyhow!("import graph query text byte count overflow")
                })?;
                if text_bytes > max_text_bytes {
                    bail!("import graph query exceeds bounded {max_text_bytes}-byte text budget");
                }
    ...
    for item in &file.items {
        if let syn::Item::Mod(module) = item {
            ...
            if module.content.is_none()
                && !has_path_override
                && is_simple_identifier(&module.ident.to_string())
            {
        ...
        if !simple_use_segments(&use_item.tree, &mut owned_parts) {
            continue;
        }
...
fn rust_crate_root(path: &str, paths: &BTreeSet<String>) -> String {
    ...
    loop {
        ...
        if paths.contains(&lib) || paths.contains(&main) {
            return current;
        }
...
fn resolve_python_imports(path: &str, source: &str, paths: &BTreeSet<String>) -> Vec<String> {
    ...
    for line in stripped.lines() {
        ...
        if let Some(module) = line.strip_prefix("import ") {
            if module.contains(',') || module.contains(" as ") {
                continue;
            }
            ...
            if imported.contains(',') || imported.contains('*') || imported.contains(" as ") {
                continue;
            }
...
fn add_unique_python_module(
    targets: &mut BTreeSet<String>,
    module: &str,
    paths: &BTreeSet<String>,
) {
    if !module.split('.').all(is_simple_identifier) {
        return;
    }
    ...
    let candidates: Vec<String> = paths
        .iter()
        .filter(|path| {
            **path == format!("{suffix}.py") || **path == format!("{suffix}/__init__.py")
        })
        .cloned()
        .collect();
    if candidates.len() == 1 {
        targets.insert(candidates[0].clone());
    }
}

fn add_unique_python_relative(
    targets: &mut BTreeSet<String>,
    base: &str,
    module: &str,
    paths: &BTreeSet<String>,
...
mod tests {
    ...
    fn sources(rows: &[(&str, Language, &str)]) -> Vec<(String, Language, String)> {
        rows.iter()
            .map(|(p, l, s)| ((*p).into(), *l, (*s).into()))
            .collect()
    }
    #[test]
    fn resolves_only_unambiguous_root_local_rust_and_python_modules() {
        let graph = ImportGraph::build_bounded(
            &sources(&[
                (
                    "a/src/lib.rs",
                    Language::Rust,
                    "mod api; use crate::api::Client;",
                ),
        ...
        assert!(
            graph
                .edges()
                .iter()
                .any(|edge| edge.from_file == "a/src/lib.rs" && edge.to_file == "a/src/api.rs")
        );
        assert!(
            graph
                .edges()
                .iter()
                .any(|edge| edge.from_file == "a/src/parent.rs"
                    && edge.to_file == "a/src/parent/child.rs")
        );
        assert!(
            graph
                .edges()
                .iter()
                .any(|edge| edge.from_file == "pkg/main.py" && edge.to_file == "pkg/helper.py")
        );
        assert!(
            graph
                .edges()
                .iter()
                .any(|edge| edge.from_file == "pkg/main.py" && edge.to_file == "pkg/sub.py")
        );
    }
    #[test]
    ...
    fn bounded_query_terminates_cycles_and_refuses_node_or_text_overflow() {
        ...
        assert_eq!(
            graph
                .query_bounded("a.rs", ImportDirection::Forward, 4, 4, 100)
                .unwrap()
                .len(),
            1
        );
        assert!(
            graph
                .query_bounded("a.rs", ImportDirection::Forward, 4, 0, 100)
                .is_err()
        );
        assert!(
            graph
                .query_bounded("a.rs", ImportDirection::Forward, 4, 4, 1)
                .is_err()
        );
    }
    #[test]
    fn rejects_or_drops_noncanonical_import_endpoints_before_graph_use() {
        let graph = ImportGraph::from_edges(vec![ImportEdge {
            ...
            to_file: "../outside.rs".into(),
            language: "rust".into(),
        }]);
        assert!(graph.edges().is_empty());
        assert!(
            ImportGraph::build_bounded(&sources(&[("../a.rs", Language::Rust, "")]), 1,).is_err()
        );
    }
}

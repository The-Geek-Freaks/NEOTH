...
pub trait ResearchRunControl: Sync {
    ...
    fn resume_checkpoint(&self) -> Result<Option<ResearchCheckpoint>> {
        Ok(None)
    }
    fn persist_checkpoint(&self, _checkpoint: &ResearchCheckpoint) -> Result<()> {
        Ok(())
    }
...
pub async fn run_deep_research_controlled(
    ...
    if control.is_some() {
        emit_wal_started_required(writer, &topic_hash).await?;
    } else {
    ...
    let mut persisted = control
        .map(|c| c.resume_checkpoint())
        .transpose()?
        .flatten()
        .unwrap_or_default();
    ...
    if persisted.queries.len() > budget.max_rounds as usize
        || persisted.completed_rounds as usize > persisted.queries.len()
    {
    ...
    // ── Step 2: Round loop ─────────────────────────────────────────────────
    let mut all_evidence: Vec<RenderedUntrustedContext> = persisted
        .evidence
        .iter()
        .map(|e| {
            UntrustedContext::new(UntrustedContextClass::Web, &e.source_id, &e.content).render()
        })
        .collect();
    let mut citations: Vec<CitedSource> = persisted
        .evidence
        .iter()
    ...
    if control.is_some() {
        emit_wal_completed_required(writer, &topic_hash, rounds_done, word_count, citation_count)
            .await?;
    } else {
...
async fn research_round(
    ...
    Vec<RenderedUntrustedContext>,
    Vec<CitedSource>,
    Vec<ResearchCheckpointEvidence>,
)> {
    ...
    for hit in hits.iter().take(budget.pages_per_round) {
        ...
        match web_fetch::fetch_with_goal_authorized(&hit.url, topic, provider, http).await {
            Ok(extraction) => {
                ...
                checkpoint_evidence.push(ResearchCheckpointEvidence {
                    source_id: format!("deep-research:web:{}", hit.url),
                    content: truncated,
...
fn persist_checkpoint(
    ...
    if let Some(control) = control {
        control.persist_checkpoint(checkpoint)?;
    }
    Ok(())
}
...
async fn emit_wal_started_required(writer: &WalWriterHandle, topic_hash: &str) -> Result<()> {
    let payload = serde_json::to_vec(
        &serde_json::json!({"topic_hash":topic_hash,"ts_unix":crate::time::now_unix_secs()}),
    )
    .context("serialise required deep-research start audit")?;
    writer
        .append(
            crate::wal::make_header(
                crate::wal::events::EVENT_TYPE_DEEP_RESEARCH_STARTED,
                &payload,
            ),
            payload,
        ...
        .await
        .context("append required deep-research start audit")
        .map(|_| ())
}
...
async fn emit_wal_completed_required(
    writer: &WalWriterHandle,
    topic_hash: &str,
    rounds: u8,
    word_count: usize,
    ...
    writer
        ...
        .await
        .context("append required deep-research completion audit")
        .map(|_| ())
}
...
mod tests {
    ...
    struct MemoryCheckpointControl {
        checkpoint: Mutex<ResearchCheckpoint>,
        calls: AtomicUsize,
    }
    impl ResearchRunControl for MemoryCheckpointControl {
        fn checkpoint(&self, _phase: &'static str) -> Result<()> {
        ...
        fn persist_checkpoint(&self, checkpoint: &ResearchCheckpoint) -> Result<()> {
    ...
    async fn resumed_checkpoint_skips_planning_search_and_fetch_for_completed_round() {
        ...
        let segment = dir.path().join("research.wal");
        let (writer, join) = crate::wal::writer::spawn(segment).unwrap();
        let provider = CycleProvider::new(vec!["resumed synthesis"]);
        let control = MemoryCheckpointControl {
            checkpoint: Mutex::new(ResearchCheckpoint {
                queries: vec!["already-completed".into()],
                completed_rounds: 1,
                evidence: Vec::new(),
            }),
            calls: AtomicUsize::new(0),
        };
        let cfg = crate::config::DeepResearchConfig {
            max_rounds: Some(1),
            results_per_query: Some(1),
            pages_per_round: Some(1),
        };
        let report = run_deep_research_controlled(
            "topic",
            &provider,
        ...
        assert_eq!(
            ...
            1,
            "resume must issue synthesis only, not re-plan/search/fetch completed round"
        );
        drop(writer);
        join.await.unwrap();
    }

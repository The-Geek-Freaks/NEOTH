...
use super::{
    goldset::{EXPECTED_GOLDSET_QUERIES, GoldsetEntry, GradedSystem, ValidatedGraderConfigFile, validate_goldset_contract},
    parity_batch_plan::{FOUR_GRADER_BATCH_INPUT_PURPOSE, FOUR_GRADER_BATCH_SCHEMA_VERSION, FOUR_GRADER_COUNT, FourGraderInputDigest, FourGraderInputDigestFile},
};
...
fn canonical_pairs(responses: &[ResponsePair], goldset: &[GoldsetEntry]) -> Result<Vec<GraderInputPair>> {
    if responses.len() != EXPECTED_GOLDSET_QUERIES * 2 {
        anyhow::bail!("response-pair input must contain exactly two answers for every goldset query");
    }
    let queries = goldset.iter().map(|entry| (entry.query_id.as_str(), entry)).collect::<BTreeMap<_, _>>();
    ...
    for (query_id, entry) in queries {
        for system in [GradedSystem::Neoth, GradedSystem::Reference] {
            let response = by_key.get(&(query_id, system_name(system))).context("response-pair input does not cover the complete goldset × two-system matrix")?;
            pairs.push(GraderInputPair { query_id: query_id.into(), system, query_text: entry.query_text.clone(), response: (*response).into() });
        }
    }
    Ok(pairs)
}
...
fn system_name(system: GradedSystem) -> &'static str {
    match system { GradedSystem::Neoth => "neoth", GradedSystem::Reference => "reference" }
}
...
mod tests {
    use super::*;
    use crate::recall::goldset::{GoldsetCategory, GraderConfig, GraderConfigFile, GraderFamily, GraderProvider};

    fn goldset() -> Vec<GoldsetEntry> { (0..EXPECTED_GOLDSET_QUERIES).map(|index| GoldsetEntry { query_id: format!("q{index:03}"), query_text: format!("query {index}"), category: GoldsetCategory::Recall, expected_sources: Vec::new(), expected_response: String::new() }).collect() }
    fn config() -> ValidatedGraderConfigFile { GraderConfigFile { schema_version: 1, graders: vec![
    ...
    fn response_pairs_produce_actual_four_grader_inputs_and_batch_digests() {
        let goldset = goldset();
        let responses = goldset.iter().flat_map(|entry| [ResponsePair { query_id: entry.query_id.clone(), system: GradedSystem::Neoth, response: "neoth response".into() }, ResponsePair { query_id: entry.query_id.clone(), system: GradedSystem::Reference, response: "reference response".into() }]).collect();
        let goldset_bytes = serde_json::to_vec(&goldset).unwrap();
        let source = ResponsePairFile { schema_version: RESPONSE_PAIR_SCHEMA_VERSION, purpose: RESPONSE_PAIR_PURPOSE.into(), goldset_sha256: sha256_bytes(&goldset_bytes), responses };
        let prepared = prepare_grader_inputs(&serde_json::to_vec(&source).unwrap(), b"score every pair", &goldset, &goldset_bytes, &config()).unwrap();
        ...
        assert!(prepared.digests.inputs.iter().all(|input| input.prompt_sha256 == sha256_bytes(b"score every pair")));
        for ((grader_id, bytes), digest) in prepared.inputs.iter().zip(&prepared.digests.inputs) {
            ...
            assert_eq!(input["pairs"].as_array().unwrap().len(), EXPECTED_GOLDSET_QUERIES * 2);
        ...
        let reordered = prepare_grader_inputs(&serde_json::to_vec(&reversed).unwrap(), b"score every pair", &goldset, &goldset_bytes, &config()).unwrap();
    ...
    fn response_pairs_reject_partial_or_mismatched_goldset_matrix() {
        let goldset = goldset();
        let goldset_bytes = serde_json::to_vec(&goldset).unwrap();
        let source = ResponsePairFile { schema_version: RESPONSE_PAIR_SCHEMA_VERSION, purpose: RESPONSE_PAIR_PURPOSE.into(), goldset_sha256: sha256_bytes(&goldset_bytes), responses: Vec::new() };
        assert!(prepare_grader_inputs(&serde_json::to_vec(&source).unwrap(), b"rubric", &goldset, &goldset_bytes, &config()).is_err());
        let mismatched = ResponsePairFile { goldset_sha256: "0".repeat(64), ..source };
        assert!(prepare_grader_inputs(&serde_json::to_vec(&mismatched).unwrap(), b"rubric", &goldset, &goldset_bytes, &config()).is_err());
    }
    ...
    fn response_pairs_distinguish_empty_observations_from_missing_unknown_and_duplicate_pairs() {
        let goldset = goldset();
        let goldset_bytes = serde_json::to_vec(&goldset).unwrap();
        let responses = goldset.iter().flat_map(|entry| [ResponsePair { query_id: entry.query_id.clone(), system: GradedSystem::Neoth, response: String::new() }, ResponsePair { query_id: entry.query_id.clone(), system: GradedSystem::Reference, response: "reference".into() }]).collect::<Vec<_>>();
        let source = ResponsePairFile { schema_version: RESPONSE_PAIR_SCHEMA_VERSION, purpose: RESPONSE_PAIR_PURPOSE.into(), goldset_sha256: sha256_bytes(&goldset_bytes), responses };
        assert!(prepare_grader_inputs(&serde_json::to_vec(&source).unwrap(), b"rubric", &goldset, &goldset_bytes, &config()).is_ok());
        let mut missing = source.clone();
        missing.responses.pop();
        assert!(prepare_grader_inputs(&serde_json::to_vec(&missing).unwrap(), b"rubric", &goldset, &goldset_bytes, &config()).is_err());
        let mut duplicate = source.clone();
        duplicate.responses[1] = duplicate.responses[0].clone();
        assert!(prepare_grader_inputs(&serde_json::to_vec(&duplicate).unwrap(), b"rubric", &goldset, &goldset_bytes, &config()).is_err());
        let mut unknown = source;
        unknown.responses[0].query_id = "unknown".into();
        assert!(prepare_grader_inputs(&serde_json::to_vec(&unknown).unwrap(), b"rubric", &goldset, &goldset_bytes, &config()).is_err());
    }
}

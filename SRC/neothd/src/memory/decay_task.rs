...
///
/// `home` is the authoritative daemon instance home. `db_path` lets tests
/// inject a tempdir db; production callers pass `<home>/views.db`. Same for
/// `interval` — tests use a short tick.
...
mod tests {
    ...
    async fn run_once_returns_a_pass_report_against_empty_db() {
        ...
        let report = run_once(&config_path, &db, None, None, None)
            .await
            .expect("run once");
    ...
    async fn warm_summary_local_quota_never_calls_cloud_fallback() {
        ...
        summarize_consolidated_days(&config_path, &db, &fallback, &days, Some(&writer)).await;
    ...
    async fn warm_summary_local_compactor_never_calls_cloud_utility() {
        ...
        summarize_consolidated_days(&config_path, &db, &compactor, &days, Some(&writer)).await;

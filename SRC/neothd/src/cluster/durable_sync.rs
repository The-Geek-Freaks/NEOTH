...
use super::gossip_wire::{
    GossipAcceptance, GossipAck, GossipFrame, GroundTruthSnapshot, MemorySnapshot,
    SYNC_ENVELOPE_VERSION, SYNC_PROTOCOL_VERSION, SyncContent, SyncEnvelope, VectorClock,
};
use super::wal_sync::{
    GossipWalCursor, ReplicationClass, classify_event_ext, gossip_payload_event_meta,
    gossip_payload_timestamp_unix, is_replicable_ext, read_gossipable_batch,
};
...
impl InboundCommit {
    pub fn ack(&self) -> Option<&GossipAck> {
        match self {
            Self::Committed(ack) | Self::Duplicate(ack) => Some(ack),
...
impl DurableMeshSync {
    ...
    pub async fn prepare_peer_frame(
        ...
        tokio::task::spawn_blocking(move || -> Result<Option<PreparedFrame>> {
            let mut conn = crate::memory::store::open(&db_path)?;
            stage_frame(
                &mut conn,
                &peer,
...
fn load_pending(conn: &Connection, peer: &PeerPubkey) -> Result<Option<PendingRow>> {
    conn.query_row(
        ...
        [peer.as_str()],
        |row| {
            let seq = positive_u64(row.get::<_, i64>(0)?, "pending origin_seq")?;
            let digest = digest_from_vec(row.get::<_, Vec<u8>>(1)?, "pending content_sha256")?;
            Ok(PendingRow {
                origin_seq: seq,
                content_sha256: digest,
                wire_frame: row.get(2)?,
                next_cursor_segment: row.get::<_, Option<String>>(3)?.map(PathBuf::from),
...
fn validate_pending(pending: &PendingRow, origin: &PeerPubkey) -> Result<GossipFrame> {
    let frame: GossipFrame = serde_json::from_slice(&pending.wire_frame)
        ...
        frame.vector_clock.get(origin) == frame.event_seq,
        "pending mesh vector clock/sequence mismatch"
    );
    Ok(frame)
}
...
fn stage_frame(
    ...
    vector_clock.clocks.insert(origin.clone(), origin_seq);
...
pub(crate) fn persist_inbound_on_conn(
    ...
    if frame.event_seq < expected {
        let receipt = tx
            .query_row(
                "SELECT content_sha256 FROM mesh_sync_inbound_receipts \
            ...
            digest_from_vec(receipt, "duplicate inbound receipt digest")? == frame.content_sha256,
    ...
    tx.execute(
        "INSERT INTO mesh_sync_inbound_receipts \
         (origin_peer_pk,origin_seq,content_sha256,content_stored,committed_at) VALUES (?1,?2,?3,?4,?5)",
        params![
            authenticated_peer.as_str(),
...
mod tests {
    ...
    async fn restart_resumes_exact_pending_frame() {
        ...
        let first = DurableMeshSync::new(&db_path)
            .prepare_peer_frame(&peer, &origin, &wal_dir, &GossipPolicy::default())
            .await
            .unwrap()
            .unwrap();

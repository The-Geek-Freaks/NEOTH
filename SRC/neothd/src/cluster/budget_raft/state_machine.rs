...
impl BudgetLedger {
    ...
    pub fn validate(&self) -> Result<(), BudgetRejection> {
        ...
        for (id, record) in &self.grants {
            ...
            if record.reserve_fence
                != expected_reserve_fence(&self.config, &record.reserve, record.reserved_at)
            {
                return Err(BudgetRejection::InvalidConfig);
            }
            total = total
                .checked_add(record.reserve.reserved_usd_nanos)
                .ok_or(BudgetRejection::CapExceeded)?;
    ...
    pub fn apply(&mut self, log_term: u64, log_index: u64, command: BudgetCommand) -> BudgetReply {
        ...
        let position = CommittedLogPosition {
            term: log_term,
            index: log_index,
        };
        match command {
    ...
    fn reserve(&mut self, position: CommittedLogPosition, command: ReserveBudget) -> BudgetReply {
        ...
        let Some(total) = self
            .reserved_total()
            .and_then(|used| used.checked_add(command.reserved_usd_nanos))
        else {
    ...
    fn begin_dispatch(
        ...
        match &record.state {
            ...
            GrantState::Settled { claim: receipt, .. }
            | GrantState::Overrun { claim: receipt, .. } => {
    ...
    fn settle(&mut self, position: CommittedLogPosition, command: SettleBudget) -> BudgetReply {
        ...
        let claim = match &record.state {
            ...
            GrantState::Released(_) => {
                return BudgetReply::Rejected(BudgetRejection::AlreadyReleased);
            }
        };
        ...
        match &record.state {
            GrantState::Settled {
                ...
                return if settlement == &command {
                    BudgetReply::Settled(receipt.clone())
                } else {
                    BudgetReply::Rejected(BudgetRejection::GrantConflict)
                };
            }
            GrantState::Overrun { settlement, .. } => {
                return if settlement == &command {
        ...
        if command
            .actual_usd_nanos
            .is_some_and(|actual| actual > record.reserve.reserved_usd_nanos)
        {
            let terminal_fence =
                expected_settlement_fence(&self.config, record, &claim, position, &command);
            record.state = GrantState::Overrun {
                claim,
        ...
        let receipt = GrantReceipt {
            grant_id: command.grant_id.clone(),
            committed_at: position,
            terminal_fence: expected_settlement_fence(
                &self.config,
                record,
                &claim,
                position,
            ...
            terminal: GrantTerminal::Settled {
                charged_usd_nanos: command
                    .actual_usd_nanos
                    .unwrap_or(record.reserve.reserved_usd_nanos),
                actual_cost_was_unknown: command.actual_usd_nanos.is_none(),
            },
        };
        record.state = GrantState::Settled {
            claim,
            settlement: command,
    ...
    fn reserved_total(&self) -> Option<u64> {
        self.grants.values().try_fold(0_u64, |sum, record| {
            sum.checked_add(record.reserve.reserved_usd_nanos)
        })
    }
}
...
fn replay_claim_reply(
    ...
    {
        if reconcile_only {
            BudgetReply::ReconcileOnly(receipt.clone())
        } else {
            BudgetReply::AlreadyClaimed(receipt.clone())
        }
    } else {
        BudgetReply::Rejected(BudgetRejection::GrantConflict)
    }
...
fn expected_reserve_fence(
    config: &BudgetClusterConfig,
    reserve: &ReserveBudget,
    position: CommittedLogPosition,
) -> GrantFence {
    GrantFence(fence_hash(
        b"neoth.budget.reserve-fence.v2\0",
        &[
            config.cluster_id.as_bytes(),
            config.scope_hash.0.as_bytes(),
            &config.membership_epoch.to_le_bytes(),
            &position.term.to_le_bytes(),
            &position.index.to_le_bytes(),
            reserve.grant_id.0.as_bytes(),
            reserve.provider_intent_id.as_bytes(),
            reserve.request_fingerprint.as_bytes(),
            &reserve.reserved_usd_nanos.to_le_bytes(),
            &reserve.utc_window.to_le_bytes(),
        ],
    ))
}

fn expected_dispatch_fence(
    config: &BudgetClusterConfig,
    record: &GrantRecord,
    position: CommittedLogPosition,
    command: &BeginDispatch,
) -> DispatchFence {
    DispatchFence(fence_hash(
        b"neoth.budget.dispatch-fence.v2\0",
        &[
            config.cluster_id.as_bytes(),
            config.scope_hash.0.as_bytes(),
            &config.membership_epoch.to_le_bytes(),
            &record.reserved_at.term.to_le_bytes(),
            &record.reserved_at.index.to_le_bytes(),
            record.reserve_fence.0.as_bytes(),
            &position.term.to_le_bytes(),
            &position.index.to_le_bytes(),
            command.grant_id.0.as_bytes(),
            command.owner.as_str().as_bytes(),
            command.attempt_id.0.as_bytes(),
            command.provider_intent_id.as_bytes(),
        ],
    ))
}
...
fn expected_settlement_fence(
    ...
    TerminalFence(fence_hash(
        b"neoth.budget.settlement-fence.v2\0",
        &[
            config.cluster_id.as_bytes(),
            config.scope_hash.0.as_bytes(),
            &config.membership_epoch.to_le_bytes(),
            record.reserve_fence.0.as_bytes(),
            claim.dispatch_fence.0.as_bytes(),
            &claim.committed_at.term.to_le_bytes(),
            &claim.committed_at.index.to_le_bytes(),
            &position.term.to_le_bytes(),
            &position.index.to_le_bytes(),
            settlement.grant_id.0.as_bytes(),
            settlement.owner.as_str().as_bytes(),
            settlement.attempt_id.0.as_bytes(),
            &actual_marker,
            actual.as_ref().map_or(&[][..], |value| &value[..]),
        ],
    ))
}

fn expected_release_fence(
    config: &BudgetClusterConfig,
    record: &GrantRecord,
    position: CommittedLogPosition,
) -> TerminalFence {
    TerminalFence(fence_hash(
        b"neoth.budget.release-fence.v2\0",
        &[
            config.cluster_id.as_bytes(),
            config.scope_hash.0.as_bytes(),
            &config.membership_epoch.to_le_bytes(),
            ...
            &position.term.to_le_bytes(),
            &position.index.to_le_bytes(),
            record.reserve.grant_id.0.as_bytes(),
        ],
    ))
}
    ...
    match &record.state {
        ...
        GrantState::Overrun {
            ...
            if !matches_settlement(claim, settlement)
                || settlement
                    .actual_usd_nanos
                    .is_none_or(|actual| actual <= record.reserve.reserved_usd_nanos)
                || *terminal_fence
                    != expected_settlement_fence(config, record, claim, *reported_at, settlement)
            {
                return Err(BudgetRejection::InvalidConfig);
            }
            Ok(())
        ...
        GrantState::Released(receipt) => {
            if receipt.grant_id != record.reserve.grant_id
                || receipt.terminal != GrantTerminal::ReleasedBeforeClaim
                || receipt.terminal_fence
                    != expected_release_fence(config, record, receipt.committed_at)
            {
                return Err(BudgetRejection::InvalidConfig);
            }
            Ok(())
...
fn validate_claim(
    ...
    let command = BeginDispatch {
        ...
        provider_intent_id: claim.provider_intent_id.clone(),
    };
    if claim.grant_id != record.reserve.grant_id
        || !config.voters.contains_key(&claim.owner)
        || !is_identifier(&claim.attempt_id.0)
        || claim.provider_intent_id != record.reserve.provider_intent_id
        || claim.dispatch_fence
            != expected_dispatch_fence(config, record, claim.committed_at, &command)
    {
        return Err(BudgetRejection::InvalidConfig);
    }
    Ok(())
...
fn validate_settlement(
    ...
    if !matches_settlement(claim, settlement)
        || settlement
            .actual_usd_nanos
            .is_some_and(|actual| actual > record.reserve.reserved_usd_nanos)
        || receipt.grant_id != record.reserve.grant_id
    {
        return Err(BudgetRejection::InvalidConfig);
    }
    let expected = GrantTerminal::Settled {
        charged_usd_nanos: settlement
            .actual_usd_nanos
            .unwrap_or(record.reserve.reserved_usd_nanos),
    ...
    if receipt.terminal != expected
        || receipt.terminal_fence
            != expected_settlement_fence(config, record, claim, receipt.committed_at, settlement)
    {
        return Err(BudgetRejection::InvalidConfig);
    }
    Ok(())
...
fn matches_settlement(claim: &ClaimReceipt, settlement: &SettleBudget) -> bool {
    settlement.grant_id == claim.grant_id
        && settlement.dispatch_fence == claim.dispatch_fence
        && settlement.owner == claim.owner
        && settlement.attempt_id == claim.attempt_id
}

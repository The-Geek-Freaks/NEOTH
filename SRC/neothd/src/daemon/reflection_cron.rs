...
async fn run_tech_currency_tick_once(
    ...
    let autonomy = crate::config::FreedomConfig::load_from_default_path_or_default()
...
fn obsidian_target() -> Result<Option<(PathBuf, String)>, String> {
    let cfg = crate::config::FreedomConfig::load_from_default_path_or_default()
        .map_err(|error| format!("load freedom.yaml for Obsidian reflection target: {error:#}"))?;
...
pub fn spawn_reflection_cron_loop(home: PathBuf, interval_secs: u64) -> JoinHandle<()> {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let obsidian = match obsidian_target() {
                Ok(target) => target,
                Err(error) => {
                    warn!(
                        error = %error,

...
fn origin_binding_hash(
    ...
    match kind {
        ...
        "channel_bound" => {
                ...
                consent == "verified_granted",
                "Dream channel input lacks verified consent"
            );
            let revision = revision
                .filter(|value| *value >= 1)
                .context("Dream channel input lacks consent revision")?;
...
pub fn resume_existing_for_day(
    home: &Path,
    local_day: &str,
    accepted_generation: &str,
) -> Result<Vec<AuditOutbox>> {
    let db = home.join("views.db");
    if !db.exists() {
        return Ok(Vec::new());
    }
    ...
    let Some((run_id, generation)) = row else {
        return Ok(Vec::new());
    };
    ensure!(
        generation == accepted_generation,
        "claimed Dream phase belongs to retired generation"
    );
    complete_light(&mut conn, &run_id, local_day)?;
    complete_rem(&mut conn, &run_id)?;
    complete_repair(&mut conn, &run_id)?;
    pending_audits(&conn, &run_id)
}
...
fn prepare_or_load(
    ...
    let mut ids: Vec<i64> = crate::daemon::dreaming::load_dreams_for_day(home, day)
        .into_iter()
        .flat_map(|dream| dream.event_ids)
        .filter(|id| *id > 0)
        .collect();
    ids.sort_unstable();
    ids.dedup();
    ...
    for id in ids {
        ...
        if let Some((text_hash, trust, kind, channel, account, sender, consent, revision)) = row {
            let origin_hash = origin_binding_hash(
                &kind,
                channel.as_deref(),
                account.as_deref(),
                sender.as_deref(),
    ...
    let canonical = inputs
        ...
        .flat_map(|i| {
            [
                ...
                i.origin_hash.clone(),
                i.source_tier.clone(),
                i.source_trust.to_string(),
            ]
        })
        .collect::<Vec<_>>()
        .join("\x1f");
    let input_sha = digest(&[
        b"dream-phase-input-v1",
        day.as_bytes(),
        generation.as_bytes(),
        canonical.as_bytes(),
    ]);
    let run_id = digest(&[
        b"dream-phase-run-v1",
        day.as_bytes(),
        generation.as_bytes(),
        input_sha.as_bytes(),
    ]);
...
fn revalidate_input_at(
    ...
    let table = match location {
        "hot" => "idx_episode",
        "warm" => "idx_consolidated",
        _ => anyhow::bail!("invalid Dream input location"),
    };
    let sql = format!(
    ...
    let row: Option<(
        ...
        .optional()?;
    let Some((text_hash, trust, kind, channel, account, sender, consent, revision)) = row else {
        anyhow::bail!(
            "Dream bound input {} is no longer positively eligible",
            input.event_id
        );
    };
    let origin = origin_binding_hash(
        &kind,
        channel.as_deref(),
        account.as_deref(),
        sender.as_deref(),
        ...
        text_hash == input.text_hash && trust == input.source_trust && origin == input.origin_hash,
        "Dream bound input {} changed",
        input.event_id
    );
    Ok(())
}
...
fn completed(tx: &rusqlite::Transaction<'_>, run_id: &str, phase: &str) -> Result<bool> {
    Ok(tx
        .query_row(
            "SELECT state FROM dream_phase_receipt WHERE run_id=?1 AND phase=?2",
            params![run_id, phase],
            |r| r.get::<_, String>(0),
        )
        .optional()?
...
fn complete_light(conn: &mut Connection, run_id: &str, day: &str) -> Result<()> {
    let tx = conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
    if completed(&tx, run_id, "light")? {
        tx.commit()?;
        return Ok(());
    }
    let bound = inputs(&tx, run_id)?;
    ensure!(!bound.is_empty(), "Dream Light has no bound input");
    for input in &bound {
        if input.source_tier == "hot" {
            revalidate_input_at(&tx, input, "hot")?;
        }
    }
    let selected = bound
        .iter()
        .filter(|i| i.source_tier == "hot")
        .map(|i| (i.event_id, i.text_hash.clone()))
        .collect::<Vec<_>>();
    let moved = crate::memory::consolidate::promote_selected_hot_to_warm(
        &tx,
        day,
        &selected,
...
fn complete_rem(conn: &mut Connection, run_id: &str) -> Result<()> {
    ...
    for (n, &a) in ids.iter().enumerate() {
        for &b in &ids[n + 1..] {
            ...
            if runs >= REM_MIN_DISTINCT_RUNS {
                qualified.push((lo, hi));
            }
        }
    }
    let reinforced =
        crate::memory::assoc_graph::reinforce_pairs_in_transaction(&tx, &qualified, now)?;
...
fn complete_repair(conn: &mut Connection, run_id: &str) -> Result<()> {
    let tx = conn.transaction_with_behavior(TransactionBehavior::Immediate)?;
    prepare_receipt(&tx, run_id, "repair")?;
    if completed(&tx, run_id, "repair")? {
        tx.commit()?;
        return Ok(());
    }
    ...
    for input in inputs(&tx, run_id)? {
        ...
        let count:i64=tx.query_row("SELECT COUNT(*) FROM idx_consolidated WHERE event_id=?1 AND kind='retained' AND text_hash=?2",params![input.event_id,input.text_hash],|r|r.get(0))?;
            ...
            count == 1,
            "Dream Repair cannot prove Light output for {}",
            input.event_id
        );
    }
    ...
    for (lo, hi) in pairs {
        let present: i64 = tx.query_row(
            "SELECT COUNT(*) FROM idx_memory_links WHERE lo_id=?1 AND hi_id=?2",
            params![lo, hi],
            |r| r.get(0),
        )?;
        if present == 0 {
    ...
    crate::memory::assoc_graph::reinforce_pairs_in_transaction(
        &tx,
        &missing,
...
pub fn mark_audit_delivered(home: &Path, transition_id: &str, frame_sha256: &str) -> Result<()> {
    ...
    let changed=tx.execute("UPDATE dream_phase_receipt SET audit_state='delivered',delivered_frame_sha256=?2 WHERE transition_id=?1 AND state='completed' AND audit_state='pending'",params![transition_id,frame_sha256])?;
        ...
        changed == 1,
        "Dream audit outbox is not pending for transition {transition_id}"
    );
    tx.commit()?;
    Ok(())
}
...
mod tests {
    ...
    fn light_promotes_only_selected_hot_inputs_and_real_warm_reader_retains_trust_zero() {
        ...
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM idx_episode WHERE event_id IN (1,2)",
                [],
                |r| r.get::<_, i64>(0)
            )
            .unwrap(),
        ...
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM idx_episode WHERE event_id IN (3,4)",
                [],
                |r| r.get::<_, i64>(0)
            )
            .unwrap(),
        ...
        let recalled =
            crate::memory::region_router::recall_warm_like_with_source(&conn, "event-1", 8, None)
                .unwrap();
    ...
    fn second_day_with_no_fresh_hot_rows_creates_one_canonical_rem_edge_and_duplicate_ticks_do_not_reinforce()
     {
        ...
        assert_eq!(
            conn.query_row("SELECT COUNT(*) FROM idx_memory_links", [], |r| r
                .get::<_, i64>(0))
                .unwrap(),
            1
        );
    ...
    fn oversized_dream_ids_bind_the_deterministic_first_32_and_leave_the_rest_hot() {
        ...
        let conn = crate::memory::store::open(&home.path().join("views.db")).unwrap();
        let selected = conn
            .prepare("SELECT event_id FROM dream_phase_input ORDER BY ordinal")
            .unwrap()
        ...
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM idx_episode WHERE event_id BETWEEN 1 AND 32",
                [],
                |r| r.get::<_, i64>(0)
            )
            .unwrap(),
        ...
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM idx_episode WHERE event_id IN (33,34)",
                [],
                |r| r.get::<_, i64>(0)
            )
            .unwrap(),
    ...
    fn channel_consent_revocation_after_prepare_blocks_all_dream_effects() {
        let home = home();
        let conn = crate::memory::store::open(&home.path().join("views.db")).unwrap();
        conn.execute("DELETE FROM idx_episode_origin_v2 WHERE raw_event_id=1", [])
            .unwrap();
        conn.execute(
        ...
        let conn = crate::memory::store::open(&home.path().join("views.db")).unwrap();
        assert_eq!(
            conn.query_row("SELECT COUNT(*) FROM idx_episode", [], |r| r
                .get::<_, i64>(0))
                .unwrap(),
            2
        );
        assert_eq!(
            conn.query_row("SELECT COUNT(*) FROM idx_consolidated", [], |r| r
                .get::<_, i64>(0))
                .unwrap(),
            0
        );
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM dream_phase_receipt WHERE state='completed'",
                [],
                |r| r.get::<_, i64>(0)
            )
            .unwrap(),
            0,
            "revocation before effects must leave all phase receipts uncompleted"
        );
    }
    #[test]
    ...
    fn generation_or_corrupted_binding_denial_leaves_phase_effects_unmutated() {
        ...
        let conn = crate::memory::store::open(&home.path().join("views.db")).unwrap();
        conn.execute(
            "UPDATE dream_phase_input SET event_text_sha256='corrupt'",
            [],
        )
        .unwrap();
        drop(conn);
    ...
    fn claimed_resume_only_uses_existing_bound_input_and_cannot_select_new_rows() {
        ...
        let conn = crate::memory::store::open(&home.path().join("views.db")).unwrap();
        assert_eq!(
            conn.query_row("SELECT COUNT(*) FROM dream_phase_input", [], |r| r
                .get::<_, i64>(0))
                .unwrap(),
            2
        );
        ...
        assert!(
            resume_existing_for_day(home.path(), "2040-01-02", "7")
                .unwrap()
                .is_empty()
        );
    }
    ...
    fn prepared_repair_recreates_only_the_declared_missing_pair_once() {
        ...
        let run_id: String = conn
            .query_row(
                "SELECT run_id FROM dream_phase_run WHERE local_day='2040-01-02'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        let first_run: String = conn
            .query_row(
                "SELECT run_id FROM dream_phase_run WHERE local_day='2040-01-01'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        ...
        conn.execute("DELETE FROM idx_memory_links WHERE lo_id=1 AND hi_id=2", [])
            .unwrap();
        ...
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM idx_memory_links WHERE lo_id=3 AND hi_id=4",
                [],
                |r| r.get::<_, i64>(0)
            )
            .unwrap(),
    ...
    fn rejected_light_write_rolls_back_without_completing_the_prepared_receipt() {
        ...
        assert_eq!(
            conn.query_row(
                "SELECT state FROM dream_phase_receipt WHERE phase='light'",
                [],
                |r| r.get::<_, String>(0)
            )
            .unwrap(),

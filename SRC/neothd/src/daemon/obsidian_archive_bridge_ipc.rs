...
#[derive(Clone)]
struct Shutdown { stopped: Arc<AtomicBool>, notify: Arc<tokio::sync::Notify> }

impl Shutdown {
    fn new() -> Self { Self { stopped: Arc::new(AtomicBool::new(false)), notify: Arc::new(tokio::sync::Notify::new()) } }
    fn stop(&self) { self.stopped.store(true, Ordering::Release); self.notify.notify_waiters(); }
...
pub(crate) struct ArchiveBridgeIpcGuard {
    shutdown: Shutdown,
    #[cfg(unix)] socket: PathBuf,
    #[cfg(unix)] socket_device: u64,
    #[cfg(unix)] socket_inode: u64,
}
impl ArchiveBridgeIpcGuard { pub(crate) fn stop(&self) { self.shutdown.stop(); } }
...
pub(crate) fn bind_and_serve(home: &Path, owner: Arc<ArchiveBridgeOwner>) -> Result<ArchiveBridgeIpcBinding> {
    let Some(endpoint) = owner.endpoint_name() else { bail!("cannot bind Obsidian bridge IPC without a pairing"); };
    ...
    #[cfg(unix)] {
        ...
        ensure!(!socket.exists(), "refuse to replace an existing Obsidian bridge socket");
        ensure!(socket.as_os_str().as_encoded_bytes().len() < 100, "Obsidian bridge socket path exceeds AF_UNIX cap");
        let listener = tokio::net::UnixListener::bind(&socket).with_context(|| format!("bind Obsidian bridge socket {}", socket.display()))?;
        ...
        return Ok(ArchiveBridgeIpcBinding {
            guard: ArchiveBridgeIpcGuard { shutdown, socket, socket_device: metadata.dev(), socket_inode: metadata.ino() },
            completion,
        });
    }
    #[cfg(windows)] {
        let nonce = owner.endpoint_nonce().context("bridge pairing has no endpoint nonce")?;
        let canonical = std::fs::canonicalize(home).with_context(|| format!("canonicalize NEOTH home {}", home.display()))?;
        use sha2::{Digest as _, Sha256};
        let home_hash = hex::encode(Sha256::digest(canonical.as_os_str().as_encoded_bytes()));
        let pipe = crate::windows_private_ipc::PrivatePipeEndpoint::derive(SERVICE, &home_hash, &nonce)?;
        ensure!(pipe.name() == endpoint, "paired bridge endpoint binding mismatch");
        ...
        return Ok(ArchiveBridgeIpcBinding { guard: ArchiveBridgeIpcGuard { shutdown }, completion });
    }
    #[cfg(not(any(unix, windows)))] { let _ = (home, owner, shutdown); bail!("Obsidian bridge IPC unavailable on this platform") }
}
...
fn recover_owned_stale_socket_with_probe(
    ...
    ensure!(before.file_type().is_socket(), "refuse to recover non-socket Obsidian bridge leaf");
    // SAFETY: geteuid reads the calling process's effective UID without pointers or preconditions.
    let effective_uid = unsafe { libc::geteuid() };
    ensure!(before.uid() == effective_uid, "refuse to recover Obsidian bridge socket owned by another UID");
    ensure!(
    ...
    ensure!(!probe(socket)?, "refuse to replace a reachable Obsidian bridge listener");
...
#[cfg(unix)]
async fn run_unix(listener: tokio::net::UnixListener, owner: Arc<ArchiveBridgeOwner>, shutdown: Shutdown) -> Result<()> {
    loop {
...
#[cfg(windows)]
async fn run_windows(mut listener: crate::windows_private_ipc::Listener, owner: Arc<ArchiveBridgeOwner>, shutdown: Shutdown) -> Result<()> {
    loop {
...
where S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin {
    ...
    let frame = match tokio::time::timeout(std::time::Duration::from_secs(5), read_frame(&mut reader)).await {
        Ok(Ok(frame)) => frame,
        _ => return Ok(()),
    };
    let response = match serde_json::from_slice::<Request>(&frame) {
        Ok(Request::Status { protocol, pairing_secret, pairing_generation }) if protocol == 1 => {
            ...
            ErrorResponse { status: response.status, generation: response.generation }
        }
        ...
        _ => ErrorResponse { status: "invalid_request", generation: None },
    };
...
#[derive(Serialize)]
struct ErrorResponse { status: &'static str, generation: Option<u64> }
async fn write<W: tokio::io::AsyncWrite + Unpin>(writer: &mut W, value: ErrorResponse) -> Result<()> { let mut encoded = serde_json::to_vec(&value)?; encoded.push(b'\n'); writer.write_all(&encoded).await?; writer.flush().await?; Ok(()) }
...
async fn read_frame<R: tokio::io::AsyncRead + Unpin>(reader: &mut R) -> Result<Vec<u8>> {
    ...
    loop {
        ...
        if byte[0] == b'\n' { return Ok(frame); }
        ensure!(frame.len() < MAX_LINE_BYTES, "bridge IPC request exceeds frame limit");
        frame.push(byte[0]);
    }
}
...
mod tests {
    ...
    fn stale_owned_socket_is_recovered_only_after_current_pid_lock_is_held() {
        ...
        assert!(!socket.exists(), "verified unreachable owned socket is removed");
    }
    ...
    fn socket_replaced_during_stale_probe_is_never_removed() {
        ...
        assert!(recover_owned_stale_socket_with_probe(home.path(), &socket, |_| {
            std::fs::remove_file(&socket).unwrap();
            *replacement.lock().unwrap() = Some(std::os::unix::net::UnixListener::bind(&socket).unwrap());
            Ok(false)
        }).is_err());
        assert!(socket.exists(), "replacement leaf remains after identity recheck");
    }
}

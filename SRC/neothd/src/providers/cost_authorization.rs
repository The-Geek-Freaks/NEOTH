...
impl AuthorizedLeafCall {
    pub(crate) async fn begin_dispatch(mut self) -> Result<ProviderCallAuditGuard> {
        if let Some(writer) = self.ticket.writer.as_ref() {
...
impl ProviderCallAuthorizer {
    ...
    pub async fn authorize_leaf(
        ...
        if self.writer.is_none() && !self.allow_missing_writer {
        ...
        let gate_result = if let Some(writer) = self.writer.as_ref() {
            self.gate().check_required_audit(&action, writer).await
        } else {
            debug_assert!(self.allow_missing_writer);
            self.gate().check(&action, None).await
        };

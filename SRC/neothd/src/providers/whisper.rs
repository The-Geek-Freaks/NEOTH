...
impl WhisperEngine {
    ...
    pub(crate) async fn new(repo: Option<String>) -> Result<Self> {
        let idle_secs = crate::config::FreedomConfig::load_from_default_path_or_default()
            .context("load Whisper idle-unload policy from freedom.yaml")?
            .media
            .whisper_idle_unload_secs;
    ...
    pub(crate) async fn new_with_idle_secs(
        repo: Option<String>,
        idle_secs: Option<u64>,
    ) -> Result<Self> {
        let models_root = crate::config::FreedomConfig::default_neoth_home().join("models");
        Self::new_with_models_root(repo, idle_secs, &models_root).await
    }

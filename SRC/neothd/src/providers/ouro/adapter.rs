...
mod tests {
    ...
    #[test]
    fn per_call_sampling_controls_merge_and_stop_sequences_fail_closed() {
        let dir = tempdir().unwrap();
        let adapter = synthetic_adapter_with_config(dir.path().to_path_buf());
        let request = Request {
        ...
        adapter
            ...
            .expect("Ouro supports temperature, top-p, and seed");
        let sampling = adapter.sampling.merged_with_request(&request);
        ...
        let error = adapter
            ...
            .expect_err("Ouro must not claim an unwired stop-sequence control");

...
pub(crate) fn internal_temperature(
    ...
    if provider.request_controls().supports_temperature() {
        Some(temperature)
    } else {
        tracing::warn!(
            ...
            call_scope,
            temperature,
            "internal temperature hint omitted because the selected provider cannot wire it"
        );
        None
    }
}
...
pub async fn fallback_chain_from_config(
    ...
    Ok(Box::new(fallback::FallbackProvider::new_with_models(
        chain,
        configured_models,

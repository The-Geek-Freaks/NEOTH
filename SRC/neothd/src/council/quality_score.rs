...
// ─── Q6 QaVerdict (QUELLEN-master / agency-agents adoption) ──────────────────
//
// Structured PASS/FAIL outcome for sub-agent QA verification (e.g. the
// EvidenceCollector sub-agent ratifying a coding-worker patch apply before
// it merges back to main, or the RealityChecker confirming Phase 4
// integration). Per `PLAN/QUELLEN_ADOPT_agency_2026-05-21.md` finding §5
// pick #1 — the dispatcher loop's current `bool success` return is too
// thin: a real verifier reports WHY it failed (failed test names, missing
// invariants, blocked-by-permission), and that diagnosis feeds the retry
// path's reframing/replanning. The enum lives next to `QualityScore` so
// council voting + sub-agent verdicts share the same module surface.

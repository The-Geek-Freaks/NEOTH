...
use crate::reflection::weekly_archive::{WeeklyArchiveCandidate, open_weekly_archive_session};
...
async fn start(
    ...
    let state = Arc::new(ApiState {
        ...
        token: "weekly-master-token".into(),
...
async fn stop(
    ...
    drop(state);
    drop(writer);
    tokio::time::timeout(std::time::Duration::from_secs(3), wal)
        .await
        .unwrap()
        .unwrap();
}
...
fn config(vault: &Path) -> FreedomConfig {
    FreedomConfig {
        ...
        obsidian_subdir: Some(SUBDIR.into()),
        ..FreedomConfig::default()
    }
}

fn request(week: &str) -> String {
    serde_json::json!({"week": week}).to_string()
}
fn note(vault: &Path) -> std::path::PathBuf {
    vault
        .join(SUBDIR)
        .join("Reflections")
        .join(format!("{WEEK}.md"))
...
fn persist_legacy_and_canonical(
    home: &Path,
) -> (
    crate::reflection::WeeklyReflection,
    crate::reflection::WeeklyReflection,
) {
    use crate::reflection::WeeklyReflection;
    let legacy = WeeklyReflection {
        iso_week_tag: WEEK.into(),
        generated_ts_unix: 1_700_000_000,
        topics: vec!["legacy".into()],
        body: "legacy private reflection".into(),
        tags: vec![],
        producer_key: None,
    };
    ...
    let body = crate::reflection::build_reflection_item(WEEK, &["canonical".into()], 1_700_000_001)
        .unwrap()
        .body;
    let intent = session
        .load_or_create_intent(WeeklyArchiveCandidate {
            generated_ts_unix: 1_700_000_001,
            topics: vec!["canonical".into()],
            body,
        })
        .unwrap();
    let canonical = intent.to_reflection();
...
async fn weekly_scope_rejects_before_body_parse_or_vault_effect() {
    let home = Home::new();
    let vault = Home::new();
    let (wrong_record, wrong) = api_tokens::create_token(
        "weekly-wrong-scope",
        vec![api_tokens::SCOPE_RECALL_READ.into()],
        None,
    )
    .unwrap();
    let (dream_record, dream_scope) = api_tokens::create_token(
        "weekly-dream-scope",
        vec![api_tokens::SCOPE_DREAMS_OBSIDIAN_WRITE.into()],
        None,
    )
    .unwrap();
    api_tokens::save_store(home.path(), &[wrong_record, dream_record]).unwrap();
    let (state, writer, wal, server, shutdown, port) =
        start(home.path(), config(vault.path())).await;
    ...
    assert_eq!(dream_denied["_http_status"], "403");
    assert_eq!(dream_denied["error"]["code"], "PermissionDenied");
...
async fn weekly_strict_body_and_missing_vault_use_fixed_errors() {
    ...
    let (_, bearer) = token(
        home.path(),
        vec![api_tokens::SCOPE_REFLECTIONS_WEEKLY_OBSIDIAN_WRITE.into()],
    );
    let (state, writer, wal, server, shutdown, port) =
        start(home.path(), config(vault.path())).await;
    for body in [
        ...
        r#"{"week":"2026-W54"}"#,
        r#"{"week":"２０２６-W21"}"#,
        r#"{"week":"2026-W21","private_vault":"secret"}"#,
    ] {
        let response = post(port, Some(&bearer), body).await;
        assert_eq!(response["_http_status"], "400");
        assert_eq!(
    ...
    stop(state, writer, wal, server, shutdown).await;
    let (state, writer, wal, server, shutdown, port) =
        start(home.path(), FreedomConfig::default()).await;
...
async fn weekly_existing_legacy_and_canonical_archive_syncs_and_redacts_response_paths() {
    let home = Home::new();
    let vault = Home::new();
    let (legacy, canonical) = persist_legacy_and_canonical(home.path());
    let (_, bearer) = token(
        home.path(),
        vec![api_tokens::SCOPE_REFLECTIONS_WEEKLY_OBSIDIAN_WRITE.into()],
    );
    ...
    let response = post(port, Some(&bearer), &request(WEEK)).await;
    assert_eq!(response["_http_status"], "200");
    assert_eq!(response["data"]["reflection_count"], 2);
    ...
    let expected = format!(
        "{}\n---\n\n{}",
        legacy.to_obsidian_md(),
        canonical.to_obsidian_md()
    );
    let rendered = std::fs::read_to_string(note(vault.path())).unwrap();
    assert_eq!(rendered, expected);
    assert_eq!(response["data"]["bytes_written"], expected.len());
    let encoded = response.to_string();
    assert!(!encoded.contains(home.path().to_string_lossy().as_ref()));
    assert!(!encoded.contains(vault.path().to_string_lossy().as_ref()));
    stop(state, writer, wal, server, shutdown).await;
}
...
async fn weekly_corrupt_source_fails_without_target_and_wal_uses_opaque_verified_caller() {
    ...
    std::fs::create_dir_all(home.path().join("reflections")).unwrap();
    std::fs::write(
        home.path()
            .join("reflections")
            .join(format!("{WEEK}.jsonl")),
        b"{private corrupt source}\n",
    )
    .unwrap();
    let wal_path = home.path().join("weekly-http.wal");
    let (state, writer, wal, server, shutdown, port) =
        start(home.path(), config(vault.path())).await;
    let response = post(port, Some(&bearer), &request(WEEK)).await;
    ...
    assert!(!note(vault.path()).exists());
    stop(state, writer, wal, server, shutdown).await;
    let receipts = audits(&wal_path)
        .into_iter()
        .filter(|value| value["kind"] == "n8n_reflection_weekly_obsidian_sync")
        .collect::<Vec<_>>();
    assert_eq!(receipts.len(), 2);
    assert!(receipts.iter().any(|value| value["phase"] == "admission"));
    assert!(receipts.iter().any(|value| value["phase"] == "failed"));
    for receipt in receipts {
        let encoded = receipt.to_string();
        assert_eq!(receipt["request_id"], request_id);
        assert_eq!(receipt["accepted_epoch"], 0);
        assert_eq!(receipt["caller"]["kind"], "scoped_token");
        assert_eq!(receipt["caller"]["token_id"], token_id);
        assert!(!encoded.contains(&bearer));
        assert!(!encoded.contains("private corrupt source"));
        assert!(!encoded.contains(home.path().to_string_lossy().as_ref()));
        assert!(!encoded.contains(vault.path().to_string_lossy().as_ref()));
    }
}

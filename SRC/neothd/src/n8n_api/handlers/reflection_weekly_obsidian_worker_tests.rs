...
fn context() -> TerminalContext {
    TerminalContext {
        request_id: "weekly-worker-request".into(),
...
fn outcome() -> crate::reflection::weekly_obsidian::CheckedReflectionSyncOutcome {
    crate::reflection::weekly_obsidian::CheckedReflectionSyncOutcome {
        iso_week_tag: "2026-W21".into(),
        written: false,
        target_path: std::path::PathBuf::from("C:\\weekly-note"),
        reflection_count: 0,
        bytes_written: 0,
        durability: crate::reflection::weekly_obsidian::ReflectionSyncDurability::NotWritten,
    }
}
...
fn weekly_worker_admission_failure_never_starts_effect() {
    ...
    let result = run_worker(
        ...
        context(),
        |_| Err(()),
        || -> anyhow::Result<_> { panic!("sync must not run after admission failure") },
    );
...
async fn detached_weekly_worker_holds_egress_lease_through_terminal_audit_and_reload() {
    ...
    let source = root.path().join("freedom.yaml");
    let initial = FreedomConfig::default();
    ...
    let lease = accepted.acquire_egress_leaf().unwrap();
    let (sync_started_tx, sync_started_rx) = sync_channel(0);
    let (sync_release_tx, sync_release_rx) = sync_channel(0);
    let (terminal_started_tx, terminal_started_rx) = sync_channel(0);
    let (terminal_release_tx, terminal_release_rx) = sync_channel(0);
    ...
    changed.review_gate_enabled = !changed.review_gate_enabled;
    std::fs::write(&source, serde_yaml::to_string(&changed).unwrap()).unwrap();
    let reloader = Arc::clone(&controller);
    let reload = std::thread::spawn(move || reloader.try_reload());
    let deadline = std::time::Instant::now() + Duration::from_secs(3);
    while accepted.acquire_egress_leaf().is_ok() {
        assert!(
            std::time::Instant::now() < deadline,
            "reload did not retire admission"
        );
        std::thread::yield_now();
    }
    sync_release_tx.send(()).unwrap();
    terminal_started_rx
        .recv_timeout(Duration::from_secs(10))
        .unwrap();
    assert!(
        !reload.is_finished(),
        "reload must wait while terminal audit holds the egress lease"
    );
    assert!(Arc::ptr_eq(&controller.accepted_snapshot(), &accepted));

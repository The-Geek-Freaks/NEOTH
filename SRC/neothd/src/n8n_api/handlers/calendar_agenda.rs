...
pub fn project_local_day_agenda(
    ...
    let next_date = date
        .succ_opt()
        .context("local day overflows supported date range")?;
    ...
    for index in 0..candidates.len() {
        let conflict = candidates.iter().enumerate().any(|(other_index, other)| {
            index != other_index
                && candidates[index].start < other.end
                && other.start < candidates[index].end
        });
...
fn local_midnight(timezone: Tz, date: NaiveDate) -> Result<DateTime<Utc>> {
    let local = date
        .and_hms_opt(0, 0, 0)
        .context("midnight construction failed")?;
    match timezone.from_local_datetime(&local) {
        LocalResult::Single(value) => Ok(value.with_timezone(&Utc)),
        LocalResult::Ambiguous(_, _) => {
            anyhow::bail!("local midnight for {date} in {timezone} is ambiguous (DST fold)")
        }
        LocalResult::None => {
            anyhow::bail!("local midnight for {date} in {timezone} does not exist (DST gap)")
        }
    }
}
...
fn candidate_for_day(
    ...
    match (start_date, end_date) {
        (Some(start), Some(end)) => {
            if end <= start {
                anyhow::bail!(
                    "all-day event `{}` has non-positive [start,end) interval",
                    event.summary
                );
            }
            if start <= day && day < end {
                ...
                return Ok(Some(Candidate {
                    item: AgendaItem {
                        start: event.start_rfc3339.clone(),
                        end: event.end_rfc3339.clone(),
        ...
        (Some(_), None) | (None, Some(_)) => {
            anyhow::bail!("event `{}` mixes all-day and timed values", event.summary)
        }
        (None, None) => {
            ...
            if end <= start {
                anyhow::bail!("timed event `{}` must end after it starts", event.summary);
            }
            if start < day_end && end > day_start {
                return Ok(Some(Candidate {
                    item: AgendaItem {
                        start: start.with_timezone(&timezone).to_rfc3339(),
                        end: end.with_timezone(&timezone).to_rfc3339(),
                        summary: event.summary.clone(),
                        location: event.location.clone(),
                        conflict: false,
                    },
...
mod tests {
    ...
    fn back_to_back_events_do_not_conflict_and_offsets_roll_into_local_day() {
        ...
        let agenda =
            project_local_day_agenda(&back_to_back, "Europe/Berlin", "2026-05-30", 10).unwrap();
        ...
        let next_day =
            project_local_day_agenda(&rollover, "Europe/Berlin", "2026-05-31", 10).unwrap();
    ...
    fn projects_all_day_half_open_and_marks_truncation() {
        let events = [
            event("Two days", "20260530", "20260601"),
            event("Later", "20260530", "20260531"),
        ];
    ...
    fn rejects_bad_days_ranges_mixed_values_and_floating_times() {
        ...
        assert!(
            project_local_day_agenda(
                &[event("bad", "20260530", "20260530")],
                "Europe/Berlin",
                "2026-05-30",
                1
            )
        ...
        assert!(
            project_local_day_agenda(
                &[event("mixed", "20260530", "2026-05-30T09:00:00Z")],
                "Europe/Berlin",
                "2026-05-30",
                1
            )
        ...
        assert!(
            project_local_day_agenda(
                &[event("floating", "20260530T090000", "20260530T100000")],
                "Europe/Berlin",
                "2026-05-30",
                1
            )
        ...
        assert!(
            project_local_day_agenda(
                &[event("zero", "20260530T090000Z", "20260530T090000Z")],
                "Europe/Berlin",
                "2026-05-30",
                1
            )

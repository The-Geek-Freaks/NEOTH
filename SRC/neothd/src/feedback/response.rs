...
impl<'de> Deserialize<'de> for ResponseId {
        ...
        D: Deserializer<'de>,
    {
        ...
        Self::parse(&value).map_err(|_| serde::de::Error::custom("invalid response id"))
    }
}
...
pub(crate) enum ResponseFeedbackOperation {
    Set(ResponseSignal),
    Remove,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub(crate) enum ResponseFeedbackRejection {
    Malformed,
    Missing,
    Foreign,
    Stale,
    ...
    with_projection(home, |projection| {
        ...
        if projection.targets.len() >= MAX_TARGETS {
            let Some(index) = projection
                .targets
                .iter()
                .position(|target| target.active_signal.is_none())
            else {
                return Err(ResponseFeedbackRejection::Unavailable);
            };
        ...
        projection.targets.push(StoredTarget {
            ...
            issued_at_unix: completed_at_unix,
...
pub(crate) fn read_response_feedback_status(
    ...
    with_projection(home, |projection| {
        ...
        Ok((
            ResponseTargetStatus {
                response_id: target.response_id.clone(),
                session_id: target.session_id.clone(),
                revision: target.revision,
                active_signal: target.active_signal,
            },
            false,
        ))
    })
...
pub(crate) fn apply_response_feedback(
    ...
    operation: ResponseFeedbackOperation,
    now_unix: i64,
    ...
    with_projection(home, |projection| {
        ...
        let outcome = match (operation, target.active_signal) {
            (ResponseFeedbackOperation::Set(signal), Some(current)) if current == signal => {
                ResponseFeedbackOutcome::Unchanged {
                    revision: target.revision,
                }
            }
            (ResponseFeedbackOperation::Set(signal), Some(previous)) => {
                target.active_signal = Some(signal);
                target.revision = next_revision(target.revision)?;
                target.updated_at_unix = now_unix;
            ...
            (ResponseFeedbackOperation::Set(signal), None) => {
                target.active_signal = Some(signal);
                target.revision = next_revision(target.revision)?;
                target.updated_at_unix = now_unix;
            ...
            (ResponseFeedbackOperation::Remove, None) => ResponseFeedbackOutcome::Unchanged {
                revision: target.revision,
            },
            (ResponseFeedbackOperation::Remove, Some(_)) => {
                target.active_signal = None;
                target.revision = next_revision(target.revision)?;
                target.updated_at_unix = now_unix;
                ResponseFeedbackOutcome::Removed {
                    revision: target.revision,
                }
            }
        };
...
pub(crate) fn active_response_feedback_summary(
    home: &Path,
) -> Result<ActiveResponseFeedbackSummary, ResponseFeedbackRejection> {
    with_projection(home, |projection| {
...
fn find_target<'a>(
    projection: &'a StoredProjection,
    id: &ResponseId,
    session: &str,
) -> Result<&'a StoredTarget, ResponseFeedbackRejection> {
    let target = projection
        .targets
        .iter()
        .find(|target| target.response_id == *id)
        .ok_or(ResponseFeedbackRejection::Missing)?;
    if target.session_id != session {
        return Err(ResponseFeedbackRejection::Foreign);
    }
    Ok(target)
}
fn find_target_mut<'a>(
    projection: &'a mut StoredProjection,
    id: &ResponseId,
    session: &str,
) -> Result<&'a mut StoredTarget, ResponseFeedbackRejection> {
    let target = projection
        .targets
        .iter_mut()
        .find(|target| target.response_id == *id)
...
fn with_projection<T>(
    home: &Path,
    mutate: impl FnOnce(&mut StoredProjection) -> Result<(T, bool), ResponseFeedbackRejection>,
) -> Result<T, ResponseFeedbackRejection> {
    let _guard = RESPONSE_FEEDBACK_MUTEX
        .get_or_init(|| Mutex::new(()))
        .lock()
        .map_err(|_| ResponseFeedbackRejection::Unavailable)?;
    let home_directory =
        crate::skills::store::open_bound_directory(home, true, "response feedback home")
            .map_err(|_| ResponseFeedbackRejection::Unavailable)?
            .ok_or(ResponseFeedbackRejection::Unavailable)?;
    let namespace_path = home_directory.physical_display_path.join(STORE_DIR);
    ...
    .map_err(|_| ResponseFeedbackRejection::Unavailable)?;
    if !try_lock_response_file(&file_lock)? {
        return Err(ResponseFeedbackRejection::Unavailable);
    }
...
fn read_projection(
    ...
    path: &Path,
) -> Result<StoredProjection, ResponseFeedbackRejection> {
    ...
    match parent.symlink_metadata(name) {
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            return Ok(StoredProjection::default());
        }
        Err(_) => return Err(ResponseFeedbackRejection::Unavailable),
        Ok(_) => {}
    }
    let bytes = read_regular_file_bounded(parent, name, path, MAX_STORE_BYTES)
        .map_err(|_| ResponseFeedbackRejection::Unavailable)?;
    let projection: StoredProjection =
        serde_json::from_slice(&bytes).map_err(|_| ResponseFeedbackRejection::Unavailable)?;
...
) -> Result<(), ResponseFeedbackRejection> {
    validate_projection(projection)?;
    let bytes =
        serde_json::to_vec(projection).map_err(|_| ResponseFeedbackRejection::Unavailable)?;
...
mod tests {
    ...
    fn redirected_feedback_parent_and_data_leaf_fail_closed() {
        ...
        assert_eq!(
            read_response_feedback_status(clean_home.path(), &target.response_id, SESSION),
            Err(ResponseFeedbackRejection::Unavailable)
        );
        ...
        assert_eq!(
            read_response_feedback_status(lock_home.path(), &lock_target.response_id, SESSION),
            Err(ResponseFeedbackRejection::Unavailable)
        );
    ...
    fn set_replace_remove_is_revision_cas() {
        ...
        assert!(matches!(
            apply_response_feedback(
                ...
                SESSION,
                0,
                ResponseFeedbackOperation::Set(ResponseSignal::NeedsCorrection),
                11
            )
            .unwrap(),
            ResponseFeedbackOutcome::Set { revision: 1, .. }
        ));
        assert_eq!(
            apply_response_feedback(
                home.path(),
            ...
            .unwrap(),
            ResponseFeedbackOutcome::Unchanged { revision: 1 }
        );
        assert!(matches!(
            apply_response_feedback(
                home.path(),
                ...
                SESSION,
                1,
                ResponseFeedbackOperation::Set(ResponseSignal::NotHelpful),
                13
            )
            .unwrap(),
            ResponseFeedbackOutcome::Replaced { revision: 2, .. }
        ));
        assert_eq!(
            apply_response_feedback(
                home.path(),
    ...
    fn foreign_and_missing_pairs_reject_before_write() {
        ...
        assert_eq!(
            ...
            Err(ResponseFeedbackRejection::Foreign)
        );
    ...
    fn projection_is_content_free_and_invalid_loaded_projection_fails_closed() {
        ...
        assert_eq!(
            active_response_feedback_summary(home.path()),
            Err(ResponseFeedbackRejection::Unavailable)
        );
    ...
    fn same_revision_concurrent_apply_has_one_winner() {
        ...
        assert_eq!(
            outcomes
                .iter()
                .filter(|outcome| matches!(outcome, ResponseFeedbackOutcome::Set { .. }))
                .count(),
            1
        );
        assert_eq!(

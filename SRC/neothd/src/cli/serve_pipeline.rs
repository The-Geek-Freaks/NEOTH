...
impl AdmittedCounterpartyConsentInput {
    ...
    pub(crate) fn for_authenticated_test(
        ...
        mint_admitted_counterparty_consent_input(home, binding, inbound)?.ok_or_else(|| {
            anyhow::anyhow!("W209 test fixture requires an exact non-media ceremony command")
        })
    }
}
...
async fn handle_counterparty_consent_ingress<P: crate::permissions::PolicyArgument + Copy>(
    ...
    match command {
        ...
        CounterpartyConsentCommand::Grant { token } => {
            let reservation =
                with_counterparty_consent_connection(views_conn, home, |connection| {
            ...
            let Ok(reservation) = reservation else {
                let reply = release_counterparty_consent_reply(
                    ...
                    channel_str,
                    sender_hash,
                    "Counterparty consent grant was rejected.",
                    channel_asker,
                    once_guard,
                    Some(wal_session),
                )
        ...
        CounterpartyConsentCommand::Revoke => {
            ...
            let Ok(reservation) = reservation else {
                let reply = release_counterparty_consent_reply(
                    ...
                    channel_str,
                    sender_hash,
                    "Counterparty consent revoke could not be completed.",
                    channel_asker,
                    once_guard,
                    Some(wal_session),
                )
...
pub(crate) fn build_pipeline_handler(deps: PipelineHandlerDeps) -> PipelineHandler {
    ...
    Box::new(move |inbound: InboundMessage| {
        ...
        Box::pin(async move {
            ...
            let counterparty_consent_input =
                mint_admitted_counterparty_consent_input(&neoth_home, &inbound_binding, &inbound)?;
        ...
        &inbound_binding,
            ...
            // The canary is a required A-block attachment. Refresh the exact
            // typed preflight pair before the direct route reaches the shared
            ...
            let (typed_prompt, typed_system) =
                match crate::tokens::budget::render_request(&channel_budget_items) {
                    ...
                    Err(error) => {
                        ...
                        &sender_hash,
                        "[NEOTH] Request blocked before sending: session integrity setup failed.",
                        "channel-canary-preflight-error",
                        channel_asker.as_ref().map(Arc::clone),
                        &session_fired_once,
                        channel_wal_session,
                    )
...
mod tests {
    ...
    async fn counterparty_consent_request_bypasses_normal_ingress_and_provider() {
        ...
        assert!(
            reply
                .text
                .contains(crate::memory::counterparty_consent_ceremony::COMMAND_GRANT_PREFIX)
        );
        assert!(
            provider.request.lock().unwrap().is_none(),
        ...
        assert_eq!(
            input, 1,
            "the exact request receives one sealed input receipt"
        );
        assert_eq!(raw, 0, "ceremony text must never become RAW_TEXT");
        assert_eq!(
            ingress, 0,
            "ceremony text must never become CHANNEL_INGRESS"
        );
        assert_eq!(
            egress, 1,
            "the local ceremony reply uses normal channel egress"
        );
    }
    ...
    async fn malformed_counterparty_consent_never_receipts_or_reaches_provider() {
        ...
        assert_eq!(
            ingress, 0,
            "malformed ceremony text must never become CHANNEL_INGRESS"
        );
    ...
    async fn counterparty_consent_pipeline_grant_then_revoke_tracks_real_status() {
        ...
        let grant_command = request_reply
            ...
            .lines()
            .find(|line| {
                line.starts_with(crate::memory::counterparty_consent_ceremony::COMMAND_GRANT_PREFIX)
            })
            .expect("challenge reply retains one exact grant command")
            .to_owned();
        assert_eq!(
            handler(inbound(Some(&grant_command), None))
                .await
                .unwrap()
                .unwrap()
                .text,
    ...
    async fn counterparty_consent_pending_grant_audit_recovers_from_exact_input_receipt() {
        ...
        let grant_admission =
            AdmittedCounterpartyConsentInput::for_authenticated_test(home.path(), &binding, &grant)
                .unwrap();

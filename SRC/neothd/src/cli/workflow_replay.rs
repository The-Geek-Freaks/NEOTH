...
impl crate::cli::chat_turn_pipeline::ChatTurnEventSink for ReplaySink {
    fn emit(&mut self, event: crate::cli::chat_turn_pipeline::ChatTurnEvent) -> Result<()> {
        ...
        match event {
            ...
            ChatTurnEvent::Terminal(ChatTurnTerminal::Complete { provider, model, .. }) => {
...
pub async fn run_capture_cmd(args: WorkflowReplayCaptureArgs) -> Result<()> {
    anyhow::ensure!(!args.out.exists(), "workflow replay corpus already exists: {}", args.out.display());
    let raw = read_bounded(&args.input, "capture input")?;
    let input: CaptureInputV1 = serde_json::from_slice(&raw)
        .with_context(|| format!("parse workflow replay capture input {}", args.input.display()))?;
...
pub async fn run_workflow_replay_cmd(args: WorkflowReplayRunArgs) -> Result<()> {
    ...
    let config_projection = ReplayConfigProjectionV1 {
        ...
        fingerprint_sha256: config_fingerprint(&provider_name, &model, config.tokens.max_per_request),
    ...
    std::fs::create_dir_all(&out_dir).with_context(|| format!("create replay report directory {}", out_dir.display()))?;
    let json = serde_json::to_string_pretty(&report)?;
    std::fs::write(out_dir.join("workflow-replay-report-v1.json"), &json)?;
    std::fs::write(out_dir.join("workflow-replay-report-v1.md"), render_markdown(&report))?;
    if args.json {
        println!("{json}");
    } else {
        println!("workflow replay report written: {}", out_dir.display());
        println!("workflow replay: {}", if report.passed { "PASS" } else { "FAIL" });
    }
    anyhow::ensure!(report.passed, "workflow replay contains failed or errored episodes");
    Ok(())
}

fn validate_capture(input: &CaptureInputV1) -> Result<()> {
    anyhow::ensure!(input.schema_version == 1, "unsupported capture schema_version");
...
fn validate_corpus(corpus: &WorkflowCorpusV1) -> Result<()> {
    anyhow::ensure!(corpus.schema_version == 1 && corpus.kind == CORPUS_KIND, "unsupported workflow replay corpus schema");
    anyhow::ensure!(corpus.provenance.label == "operator-curated+inputhash", "workflow replay corpus has invalid provenance label");
    anyhow::ensure!(valid_sha256(&corpus.provenance.input_sha256), "workflow replay corpus has invalid input hash");
    anyhow::ensure!(!corpus.episodes.is_empty() && corpus.episodes.len() <= MAX_EPISODES, "workflow replay corpus episode count is outside bounds");
    let mut ids = std::collections::BTreeSet::new();
    for episode in &corpus.episodes {
        validate_episode(episode)?;
        anyhow::ensure!(ids.insert(&episode.episode_id), "workflow replay corpus has duplicate episode_id");
    }
    Ok(())
}
...
fn validate_episode(episode: &WorkflowEpisodeV1) -> Result<()> {
    for (name, value) in [
        ...
        anyhow::ensure!(!value.trim().is_empty() && value.len() <= MAX_FIELD_BYTES, "workflow replay {name} is empty or too large");
    }
    anyhow::ensure!(!episode.prompt.trim_start().starts_with('/'), "workflow replay rejects slash-command prompts");
    if let Some(skill) = &episode.skill {
        anyhow::ensure!(!skill.trim().is_empty() && skill.len() <= 256, "workflow replay skill is empty or too large");
    }
    Ok(())
}
...
fn score_replay_turn(
    ...
    match result {
        ...
        Ok(_) if !sink.terminal_completed() => {
            ("error", Some("missing terminal provider completion".to_owned()))
        }
        ...
        Ok(_) => ("fail", Some("contains-v1 expectation was not found".to_owned())),
    }
}
...
fn read_bounded(path: &Path, name: &str) -> Result<Vec<u8>> {
    ...
    let mut file = std::fs::File::open(path).with_context(|| format!("open {name} {}", path.display()))?;
    let metadata = file.metadata().with_context(|| format!("read {name} metadata {}", path.display()))?;
    anyhow::ensure!(metadata.is_file() && metadata.len() <= MAX_CORPUS_BYTES as u64, "{name} is not a bounded regular file");
    let mut bytes = Vec::with_capacity(metadata.len() as usize);
    file.take((MAX_CORPUS_BYTES + 1) as u64)
        .read_to_end(&mut bytes)
        .with_context(|| format!("read {name} {}", path.display()))?;
    anyhow::ensure!(bytes.len() <= MAX_CORPUS_BYTES, "{name} grew beyond bounded size while reading");
    Ok(bytes)
}

fn write_new_json<T: Serialize>(path: &Path, value: &T) -> Result<()> {
    let parent = path.parent().context("workflow replay output has no parent directory")?;
    std::fs::create_dir_all(parent)?;
    let mut options = std::fs::OpenOptions::new();
    options.write(true).create_new(true);
    use std::io::Write as _;
    let mut file = options.open(path).with_context(|| format!("create workflow replay corpus {}", path.display()))?;
...
fn config_fingerprint(provider: &str, model: &str, max_per_request: u32) -> String {
    sha256(format!("workflow-replay-config-v1\0{provider}\0{model}\0{max_per_request}\0{SCORER}\0deny_all").as_bytes())
}

fn sha256(bytes: &[u8]) -> String { hex::encode(Sha256::digest(bytes)) }
fn valid_sha256(value: &str) -> bool { value.len() == 64 && value.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase()) }
fn sanitize_reason(reason: &str) -> String { reason.chars().take(240).collect() }
...
mod tests {
    ...
    fn contains_v1_is_case_insensitive_and_rejects_missing_expectation() {
        ...
        assert!(!contains_v1("Current workflow completed", "provider failure"));
    }
    ...
    fn corpus_rejection_happens_before_any_runtime_setup() {
        let corpus = WorkflowCorpusV1 {
            schema_version: 1,
            kind: CORPUS_KIND.to_owned(),
            provenance: CorpusProvenanceV1 { label: "operator-curated+inputhash".to_owned(), input_sha256: "0".repeat(64) },
            episodes: vec![WorkflowEpisodeV1 { episode_id: "one".to_owned(), prompt: "/effectful".to_owned(), expected_contains: "done".to_owned(), skill: None }],
        };
    ...
    fn scoring_requires_an_authenticated_provider_terminal_and_never_passes_errors() {
        ...
        assert_eq!(
            score_replay_turn(&Ok(crate::cli::chat::WorkflowReplayTurnObservation::default()), &sink, "completed").0,
            "error",
        ...
        assert_eq!(score_replay_turn(&Ok(crate::cli::chat::WorkflowReplayTurnObservation::default()), &sink, "COMPLETED").0, "pass");
        assert_eq!(
    ...
    fn replay_scoring_ignores_human_stdout_and_requires_typed_completed_body() {
        use crate::cli::chat_turn_pipeline::{ChatOutput, ChatTurnEvent, ChatTurnEventSink, ChatTurnTerminal};
    ...
    fn public_config_fingerprint_covers_the_effective_secret_free_config() {
        ...
        let baseline_digest = public_config_fingerprint(&baseline).expect("digest baseline public config");
        let changed_digest = public_config_fingerprint(&changed).expect("digest changed public config");
    ...
    fn config_fingerprint_uses_actual_nul_field_separators() {
        let expected = sha256(b"workflow-replay-config-v1\0provider\0model\042\0contains-v1\0deny_all");
    ...
    fn install_authorized_replay_skill(
        ...
        let reload = crate::config::reload::ReloadController::new(config.clone(), config_path.to_path_buf());
    ...
    async fn actual_replay_turn_records_provider_terminal_and_scores_its_reply() {
        ...
        std::fs::write(&actual_config, serde_yaml::to_string(&config).expect("serialize actual config"))
        ...
        assert!(sink.terminal_completed(), "only a provider terminal completes replay");
        assert_eq!(score_replay_turn(&Ok(observation), &sink, "terminal reply").0, "pass");
    }
    ...
    async fn actual_replay_provider_failure_emits_no_terminal_or_accepted_body() {
        ...
        assert_eq!(provider.calls.load(Ordering::SeqCst), 1);
        assert_eq!(provider.requests.lock().expect("read failing replay request").len(), 1);
        assert!(sink.provider_response.is_empty(), "failed provider must not produce accepted replay body");
        assert!(!sink.terminal_completed(), "failed provider must not emit a terminal completion");
        assert_eq!(
    ...
    async fn actual_replay_reports_the_authority_bound_selected_skill_snapshot() {
        ...
        std::fs::write(&actual_config, serde_yaml::to_string(&config).expect("serialize actual config"))
            .expect("write actual config");
        install_authorized_replay_skill(actual.path(), &config, &actual_config, skill_id, actual_skill_body);
        ...
        let selected_skill = observation.selected_skill.expect("selected skill observation");
        assert_eq!(selected_skill.id, skill_id);
        assert_eq!(selected_skill.content_sha256, expected_content_sha256);
        let requests = provider.requests.lock().expect("read recorded replay requests");
        assert_eq!(requests.len(), 1);
        let system = requests[0].system.as_deref().expect("selected skill request system");

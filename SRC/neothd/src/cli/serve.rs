...
pub async fn run_serve(args: ServeArgs) -> Result<()> {
    ...
    // ── 5b-tris. Obsidian vault auto-sync, vault reader, wiki-rebuild, self-map ─
    // These four crons are now fleet-managed (ZF-06 fix): the cron supervisor
    // spawns them under CronKey::ObsidianSync / ObsidianVaultReader /
    // ObsidianWikiRebuild / SelfMap and drains them (WAL-emitting ones first)
    // during shutdown. The individual direct-spawn variables and BackgroundHandles
    // fields have been removed to eliminate the pre-ZF-06 double-spawn.
    ...
    // ── 5d.b. Doctor cron — now fleet-managed (ZF-06 fix) ───────────────────
    // Previously spawned here as `doctor_cron_task` and stored as a
    // BackgroundHandles field. Removed: the cron supervisor seeds it via
    // CronKey::DoctorCron and drains it during shutdown. Retaining the direct
    // spawn produced a double-instance; the fleet is now the single owner.
    ...
    // ── 5d-quintus. Profile drift-alert cron — now fleet-managed (ZF-06 fix) ─
    // Previously spawned here as `drift_alert_cron_handle`. Removed: the cron
    // supervisor handles CronKey::DriftAlert. The fleet drains it before
    // drop(writer) via the WAL-emitting pass (0xBA frames).
    ...
    // ── 5d-septimus/SL-03/HO-07/GOLD-DELTA-04 — now fleet-managed (ZF-06 fix) ─
    // recall_latency_cron_handle, resource_watch_handle, monitor_cron_handle, and
    // babel_cron_handle were spawned here and stored as BackgroundHandles fields.
    // All four removed: the cron supervisor owns CronKey::RecallLatency,
    // CronKey::ResourceWatch, CronKey::MonitorCron, and CronKey::Babel and drains
    // them correctly during shutdown. The fleet is now the single owner.

    // ── GOLD-ADAPT-JV-PRO-02 / GOLD-ADAPT-VIEW-05 / GOLD-ADAPT-ODY-21 ────────
    // token_anomaly_cron_handle, session_health_cron_handle, webhook_manager_handle
    // are now fleet-managed (ZF-06 fix). Direct spawns removed; the cron
    // supervisor owns CronKey::TokenAnomaly, CronKey::SessionHealth,
    // CronKey::WebhookManager and drains them before drop(writer) on shutdown.
    ...
    // ── GOLD-FEAT-09 / GOLD-WIRE-07b / OM-01 / SPEC-05 / F4-01 / G-01 /
    //    GOLD-ADAPT-ODY-07 / NN-MEM-06 / GOLD-ADAPT-JV-MEM-16 ─────────────────
    // These crons (watchdog, snapshot_refresh, omi, profile_adapt, ecology,
    // pattern, bg_monitor, contradiction_resolve, guidance) are now fleet-managed
    // (ZF-06 fix). Their direct-spawn variables and BackgroundHandles fields have
    // been removed. The cron supervisor owns them via CronKey::WatchdogCron,
    // CronKey::ProfileAdapt, CronKey::EcologyCron, CronKey::PatternCron,
    // CronKey::BgMonitor, CronKey::ContradictionResolve, CronKey::GuidanceCron,
    // and drains them correctly on shutdown.
    //
    // snapshot_refresh_handle and omi_handle remain as individual BackgroundHandles
    // fields because they are NOT in the CronKey dispatch table (no hot-reload
    // need; always-on or externally gated at startup only).
    ...
    // ── GOLD-FEAT-11 / NN-MEM-02 / JV-SELF-02 / GOLD-FEAT-03b / GOLD-ADAPT-JV-SELF-03 ──
    // skill_curator_cron_handle, synthesis_cron_handle, consolidation_sweep_handle,
    // self_wiki_handle, self_improvement_collector_handle are now fleet-managed
    // (ZF-06 fix). Direct spawns removed; the cron supervisor owns
    // CronKey::SkillCurator, CronKey::SynthesisCron, CronKey::ConsolidationSweep,
    // CronKey::SelfWiki, CronKey::SelfImprovementCollector and drains them on shutdown.
    ...
    let worker_watch_handle: Option<tokio::task::JoinHandle<()>> = if config.monitor.enabled {
        ...
        // ZF-06 fix: fleet-managed crons (doctor, resource_watch, monitor,
        // babel, profile_adapt, ecology, pattern, bg_monitor, watchdog, etc.)
        // no longer appear here — their individual variables have been removed.
        // The cron supervisor (cron_supervisor_task) is itself the liveness
        // boundary for the fleet: if the supervisor task dies, no new crons
        // can be started/stopped, which is the correct escalation signal.
        // Non-fleet tasks (cron_scheduler, updaters, omi, snapshot_refresh)
        // remain here as they are not fleet-managed.
    ...
    let cron_supervisor_task: tokio::task::JoinHandle<()> = {
        ...
        tokio::spawn(async move {
            // Seed the initial fleet before entering the reload watch loop.
            {
                ...
                // ZF-06 seed-loop atomicity fix: insert each handle immediately
                // after spawn_cron_for_key returns, under a short per-iteration
                // lock. Batch-insert after all awaits would detach handles
                // spawned before a SIGTERM-during-boot cancels this task —
                // those handles would hold WalWriterHandle clones that outlive
                // drop(writer) in shutdown if never tracked by the fleet.
                for key in desired {
                    if let Some(handle) =
                        crate::cli::serve_tasks::spawn_cron_for_key(key, &initial_cfg, &deps).await
                    {
                        // Short lock: insert then release immediately. Never
                        // held across an await (std MutexGuard is not Send).
                        fleet
                            .lock()
                ...
                let started = fleet.lock().expect("cron_fleet mutex poisoned").len();
    ...
    // ZF-06: fleet-managed crons (doctor_cron_task, resource_watch_handle,
    // monitor_cron_handle, babel_cron_handle, watchdog_cron_handle,
    // drift_alert_cron_handle, recall_latency_cron_handle, token_anomaly_cron_handle,
    // session_health_cron_handle, webhook_manager_handle, profile_adapt_cron_handle,
    // ecology_cron_handle, pattern_cron_handle, bg_monitor_handle,
    // contradiction_resolve_cron_handle, guidance_cron_handle, skill_curator_cron_handle,
    // synthesis_cron_handle, consolidation_sweep_handle, self_wiki_handle,
    // self_improvement_collector_handle, obsidian_task, obsidian_vault_reader_task,
    // obsidian_wiki_rebuild_task, self_map_task) are NOT listed here — they are
    // owned and drained by cron_fleet / cron_supervisor_task below.
    let bg = crate::cli::serve_tasks::BackgroundHandles {
        ...
        // Deferred fleet crons — kept as individual fields until migrated:

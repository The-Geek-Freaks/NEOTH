...
/// DB-only enrichment plan for `neoth fs glob`. Unlike the read plan, this
/// never maps a selected path back to the filesystem: its summaries come only
/// from indexed `code_map_files` and `code_map_symbols` rows.
#[derive(Clone, Debug)]
pub(crate) struct NativeFsGlobEnrichmentPlan {
    context: crate::hooks::PreToolUseContext,
    database_path: PathBuf,
    repository_root: PathBuf,
    admitted_root: PathBuf,
    admitted_root_identity: String,
    root_identity: String,
    index_generation: i64,
    graph_generation: i64,
}

pub(crate) enum NativeFsGlobFreshness {
    Fresh(crate::hooks::PreToolUseEnrichment),
    Stale,
    Unavailable,
}

pub(crate) fn prepare_native_fs_glob_enrichment(
    home: &Path,
    repository_root: &Path,
    admitted_root: &Path,
    admitted_root_identity: &str,
    context: &crate::hooks::PreToolUseContext,
    enabled: bool,

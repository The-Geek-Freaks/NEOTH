...
pub fn render_yaml(result: &ImportConfigResult) -> String {
    ...
    out.push_str("# Paste these stanzas into your freedom.yaml under `inference.providers:`\n");
    out.push_str("# API KEYS: NEVER add keys here. Use credentials.yaml instead.\n");
    out.push('\n');
...
mod tests {
    ...
    fn import_config_parses_models_providers_array() {
        ...
        assert!(
            result
                .skipped
                .contains(&"unknown-future-gateway".to_string()),
    ...
    fn full_round_trip_fixture_openclaw_config_to_provider_stanzas() {
        ...
        assert!(
            providers.contains(&"openai_compat"),
            "openai_compat missing"
        );
        ...
        assert!(
            !yaml.contains("pw_fixture"),
            "password must not appear in YAML"
        );
        ...
        let compat = result
            ...
            .find(|s| s.provider == "openai_compat")
            .unwrap();
        assert_eq!(compat.base_url.as_deref(), Some("http://127.0.0.1:1234/v1"));
    }
}

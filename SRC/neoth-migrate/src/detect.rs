...
pub struct Detection {
    ...
    /// OpenClaw configs are detected separately from memory sources. They are
    /// consumed only by `import-openclaw` and must never become recall claims.
    pub openclaw_configs: Vec<PathBuf>,
}
...
pub fn detect_sources(home: &Path) -> Detection {
    ...
    let mut openclaw_configs = Vec::new();
    for (rel, name, kind, hint, want_dir) in CANDIDATES {
        ...
        if *hint == Some("openclaw") {
            let config = path.join("openclaw.json");
            if config.is_file() {
                openclaw_configs.push(config);
            }
        }
    ...
    openclaw_configs.sort();
    openclaw_configs.dedup();
    Detection {
        manifest,
        scans,
        openclaw_configs,
...
pub fn render_manifest_yaml(d: &Detection) -> anyhow::Result<String> {
    ...
    let configs = d
        ...
        .map(|path| {
            format!(
                "# Detected OpenClaw config (excluded from memory): neoth-migrate import-openclaw --config {}\n",
                path.display()
            )
        })
        .collect::<String>();
...
mod tests {
    ...
    fn detects_only_present_assistant_homes() {
        ...
        assert_eq!(
            d.openclaw_configs,
            vec![home.path().join(".openclaw/openclaw.json")]
        );
    }
    ...
    fn empty_home_detects_nothing() {
        ...
        assert!(d.openclaw_configs.is_empty());
    }
    ...
    fn rendered_manifest_is_valid_yaml_with_review_banner() {
        ...
        assert!(yaml.contains("import-openclaw --config"));

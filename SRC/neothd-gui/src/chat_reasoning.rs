...
pub enum TerminalState { Unsupported, Hidden, Redacted, Complete, Cancelled }
...
impl Projection {
    pub fn new(request_id: String, granted: bool) -> Self {
        Self { request_id, granted, next_sequence: 1, terminal: None, event_count: 0, byte_count: 0, text: Zeroizing::new(String::new()) }
    }
    ...
    pub fn active(&self) -> bool { self.granted && self.terminal.is_none() && !self.text.is_empty() }
    pub fn status(&self) -> &'static str { self.terminal.map(TerminalState::label).unwrap_or(if self.granted { "receiving" } else { "hidden" }) }
    ...
    pub fn apply_json(&mut self, line: &str, expected_token: &str, protocol_version: u64) -> Result<(), &'static str> {
    ...
    fn apply_json_inner(&mut self, line: &str, expected_token: &str, protocol_version: u64) -> Result<(), &'static str> {
        ...
        if let Ok(frame) = serde_json::from_str::<DeltaFrame>(line) {
                ...
                if kind.as_str() != "reasoning_delta" { return Err("unexpected reasoning delta shape"); }
                if control_token.as_str() != expected_token || request_id.as_str() != self.request_id || frame.protocol_version != protocol_version { return Err("forged or stale reasoning control"); }
                if self.terminal.is_some() { return Err("reasoning after terminal"); }
                let mut delta = Zeroizing::new(frame.delta);
                if frame.sequence != self.next_sequence || delta.is_empty() { return Err("gapped or duplicate reasoning delta"); }
                let next = self.byte_count.saturating_add(delta.len() as u64);
                if next as usize > MAX_REASONING_BYTES { return Err("reasoning byte limit exceeded"); }
                self.next_sequence = self.next_sequence.saturating_add(1);
                self.event_count = self.event_count.saturating_add(1);
                self.byte_count = next;
                if self.granted { self.text.push_str(delta.as_str()); }
                delta.zeroize();
                kind.zeroize();
                request_id.zeroize();
        ...
        if let Ok(frame) = serde_json::from_str::<StateFrame>(line) {
                ...
                let mut request_id = Zeroizing::new(frame.request_id);
                let mut control_token = Zeroizing::new(frame.control_token);
                if kind.as_str() != "reasoning_state" { return Err("unexpected reasoning state shape"); }
                if control_token.as_str() != expected_token || request_id.as_str() != self.request_id || frame.protocol_version != protocol_version { return Err("forged or stale reasoning control"); }
                if self.terminal.is_some() { return Err("duplicate reasoning terminal"); }
                let state = TerminalState::parse(&frame.state).ok_or("unknown reasoning state")?;
                if frame.sequence != self.next_sequence || frame.event_count != self.event_count || frame.byte_count != self.byte_count { return Err("reasoning terminal counters mismatch"); }
...
mod tests {
    ...
    fn frame(kind: &str, sequence: u64, extra: serde_json::Value) -> String {
        ...
        value.as_object_mut().unwrap().extend(extra.as_object().unwrap().clone()); value.to_string()
    }
    #[test]
    fn strict_sequence_limit_and_terminal_clear() {
        let mut projection = Projection::new("request".into(), true);
        assert!(projection.apply_json(&frame("reasoning_delta", 1, serde_json::json!({"delta":"private"})), "token", 3).is_ok());
        assert_eq!(projection.text(), "private");
        assert!(projection.apply_json(&frame("reasoning_delta", 1, serde_json::json!({"delta":"duplicate"})), "token", 3).is_err());
        assert!(projection.apply_json(&frame("reasoning_state", 2, serde_json::json!({"state":"complete","event_count":1,"byte_count":7})), "token", 3).is_ok());
    ...
    fn duplicate_or_forged_frame_clears_the_exact_projection() {
        ...
        assert!(projection.text().is_empty(), "invalid input zeroizes retained bytes");
        assert_eq!(projection.status(), "receiving");
        assert!(projection.apply_json(&frame("reasoning_delta", 2, serde_json::json!({"delta":"stale"})), "wrong", 3).is_err());
        assert!(projection.text().is_empty());
    }
    ...
    fn default_off_counts_controls_without_retaining_text() {
        ...
        assert!(projection.apply_json(&frame("reasoning_delta", 1, serde_json::json!({"delta":"not retained"})), "token", 3).is_ok());
        assert!(projection.text().is_empty());
        assert!(!projection.active());

        ...
        let containment = match activation {
            ...
            Err(error) => {
                ...
                #[cfg(all(test, target_os = "linux"))]
                {
                    ...
                    eprintln!(
                        "test-only systemd-run stderr after failed activation: {helper_stderr}"
                    );
                }
                return Err(error);
            }
                ...
                match inspect_linux_unit(&self.systemctl, &self.unit_name) {
                    Ok(snapshot) => {
                        ...
                        match snapshot.startup_observation() {
                            ...
                            {
                                let main_start_timestamp =
                                    snapshot.exec_main_start_timestamp_monotonic.map_or_else(
...
mod tests {
    ...
    fn linux_unit_snapshot_pins_manager_owned_kill_contract() {
        ...
        assert_eq!(
            legacy_snapshot.startup_observation(),
            LinuxUnitStartupObservation::Ready
        );
    }

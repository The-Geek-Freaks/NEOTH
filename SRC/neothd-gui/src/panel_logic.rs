...
pub enum LocalModelsEndpointWire {
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct LocalModelsHostResourcesWire {
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct LocalModelWire {
    pub model: String,
    pub digest: String,
    pub size_bytes: u64,
    pub loaded: Option<LoadedModelUseWire>,
    pub readiness: LocalModelReadinessWire,
    pub last_error: Option<LocalModelErrorWire>,
}
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct LoadedModelUseWire {
    pub model: String,
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(deny_unknown_fields, tag = "kind", rename_all = "snake_case")]
pub enum LocalModelReadinessWire {
    Unavailable,
    Installed,
    Probing,
    Ready {
    ...
    ReachableButUnready {
        reason: LocalModelUnreadyReasonWire,
    },
    Downloading {
        operation_id: String,
...
pub enum LocalModelUnreadyReasonWire {
    ProbeFailed,
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum LocalModelErrorCodeWire {
    InvalidRequest,
    EndpointUnavailable,
    Protocol,
    Transport,
...
#[serde(rename_all = "snake_case")]
pub enum LocalModelActionKindWire {
    Pull,
    Update,
    Prune,
    Retry,
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum LocalModelTerminalOutcomeWire {
    Completed,
    Failed,
    InterruptedUnknown,
}
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct LocalModelErrorWire {
    pub code: LocalModelErrorCodeWire,
    pub detail: String,
}
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct LocalModelOperationWire {
    pub operation_id: String,
    pub action: LocalModelActionKindWire,
    pub model: String,
    pub started_at_unix_ms: u64,
    pub progress: Option<LocalModelProgressWire>,
    pub cancellation_requested: bool,
}
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct LocalModelProgressWire {
    pub status: String,
...
#[derive(Debug, Clone, PartialEq, Eq, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct LocalModelTerminalWire {
    pub operation_id: String,
    pub action: LocalModelActionKindWire,
    pub model: String,
    pub started_at_unix_ms: u64,
    pub finished_at_unix_ms: u64,
    pub outcome: LocalModelTerminalOutcomeWire,
    pub old_digest: Option<String>,
    pub new_digest: Option<String>,
    pub error: Option<LocalModelErrorWire>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LocalModelsPresentation {
    pub endpoint: String,
    pub rows: Vec<LocalModelPresentationRow>,
    pub active_operation: String,
    pub valid: bool,
}
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LocalModelPresentationRow {
    pub model: String,
...
pub fn parse_local_models_snapshot(json: &str) -> Result<LocalModelsPresentation, String> {
    let snapshot: LocalModelsSnapshotWire =
        serde_json::from_str(json).map_err(|e| format!("invalid local-model status JSON: {e}"))?;
    if snapshot.schema_version != 1 {
        return Err("unsupported local-model status schema".into());
    }
    let endpoint = match snapshot.endpoint {
        LocalModelsEndpointWire::Reachable { detail } => format!("reachable: {detail}"),
        LocalModelsEndpointWire::Unavailable { detail } => format!("unavailable: {detail}"),
        LocalModelsEndpointWire::UnsupportedRemoteEndpoint { .. } => {
            "unsupported remote endpoint".into()
        }
    };
    let active = snapshot
        .active_operation
    ...
    let rows = snapshot
        ...
        .map(|model| {
            let (readiness, progress, operation_id, can_cancel, can_retry) = match model.readiness {
                LocalModelReadinessWire::Ready { .. } => (
                    "Ready (probe verified)".into(),
                    String::new(),
                    String::new(),
                    false,
                    false,
                ),
                LocalModelReadinessWire::ReachableButUnready { reason } => (
                    format!(
                        "Reachable but unready: {}",
                        local_models_unready_reason(reason)
                    ),
                    String::new(),
                    String::new(),
                    false,
                ...
                LocalModelReadinessWire::Downloading {
                    operation_id,
                    completed,
                    total,
                }
                | LocalModelReadinessWire::Updating {
                    operation_id,
                    completed,
                    total,
                } => (
                ...
                LocalModelReadinessWire::Pruning { operation_id }
                | LocalModelReadinessWire::CancelRequested { operation_id } => {
                    ("Working".into(), String::new(), operation_id, true, false)
                }
                LocalModelReadinessWire::Failed { operation_id, .. } => {
                    ("Failed".into(), String::new(), operation_id, false, true)
                }
                // Core deliberately treats an interrupted remote outcome as uncertain;
                // Retry is refused until the operator has reconciled it.
                LocalModelReadinessWire::InterruptedUnknown { operation_id } => (
                    "Interrupted".into(),
                    String::new(),
                    operation_id,
                    false,
                ...
                LocalModelReadinessWire::Unavailable => (
                    "Unavailable".into(),
                    String::new(),
                    String::new(),
                    false,
                ...
                LocalModelReadinessWire::Installed => (
                    "Installed".into(),
                    String::new(),
                    String::new(),
                    false,
                ...
                LocalModelReadinessWire::Probing => {
                    ("Probing".into(), String::new(), String::new(), false, false)
                }
            };
            let resources = local_model_resources(model.size_bytes, model.loaded.as_ref());
            LocalModelPresentationRow {
                model: model.model,
                readiness,
                progress,
                resources,
                operation_id,
                can_pull: active.is_empty(),
                can_update: active.is_empty(),
                can_prune: active.is_empty() && model.loaded.is_none(),
                can_cancel,
                can_retry,
            }
        })
        ...
        && !rows.iter().any(|row| row.model == operation.model)
    {
        rows.push(LocalModelPresentationRow {
            ...
            readiness: "Working".into(),
            progress: operation
                .progress
                .as_ref()
                .map(|p| format_progress(p.completed, p.total))
                .unwrap_or_default(),
...
fn local_models_unready_reason(reason: LocalModelUnreadyReasonWire) -> &'static str {
    match reason {
        ...
        LocalModelUnreadyReasonWire::MissingFreshLoadedDigest => "missing fresh loaded digest",
        LocalModelUnreadyReasonWire::ResponseModelMismatch => "response model mismatch",
        LocalModelUnreadyReasonWire::NotSelected => "not selected",
    }
}
fn local_model_resources(size_bytes: u64, loaded: Option<&LoadedModelUseWire>) -> String {
    ...
    match loaded {
        ...
        Some(loaded) => format!(
            "{size} installed · loaded total {} MiB · RAM unknown · VRAM {}",
            loaded.size_bytes / (1024 * 1024),
            loaded
                .size_vram_bytes
...
fn format_progress(completed: Option<u64>, total: Option<u64>) -> String {
...
pub struct BuddyStatusSnap {
    ...
    pub provider_retry: BuddyProviderRetrySnap,
}

#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct BuddyProviderRetrySnap {
    pub available: bool,
    pub authenticated_complete: bool,
    pub rows: Vec<BuddyProviderRetryRow>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BuddyProviderRetryRow {
    pub class: String,
    pub attempt: u32,
    pub provider: String,
    pub wire_model: String,
    pub disposition: String,
    pub follow_up_lifecycle: String,
}
...
struct BuddyStatusWire {
    ...
    #[serde(default)]
    provider_retry: BuddyProviderRetryWire,
}
...
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct BuddyProviderRetryReceiptWire {
    receipt: BuddyProviderRetryReceiptBodyWire,
    session: String,
    follow_up_lifecycle: BuddyProviderRetryLifecycleWire,
}

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct BuddyProviderRetryReceiptBodyWire {
    schema: String,
    retry_chain_id: String,
    class: BuddyProviderRetryClassWire,
    attempt: u32,
    provider: String,
    wire_model: String,
    disposition: BuddyProviderRetryDispositionWire,
}
...
pub fn parse_buddy_status(json: &str) -> Result<BuddyStatusSnap, String> {
    ...
    let provider_retry = project_buddy_provider_retry(wire.provider_retry)
        .map_err(|error| format!("invalid Buddy provider retry status: {error}"))?;
...
fn project_buddy_provider_retry(
    ...
    for entry in receipts {
        ...
        let auth_disposition = matches!(
            &receipt.disposition,
            BuddyProviderRetryDispositionWire::AuthNonRetryable
        );
        if auth_class != auth_disposition {
            return Err("provider retry class and disposition are inconsistent".into());
        }
...
mod tests {
    ...
    fn local_models_ready_requires_the_core_ready_variant() {
        ...
        assert_eq!(
            projection.rows[0].readiness,
            "Reachable but unready: probe failed"
        );
    }
    ...
    fn w219_provider_retry_is_strict_bounded_redacted_and_absence_compatible() {
        ...
        let valid_auth = available
            ...
            .replace("retry_intent_closed", "auth_non_retryable")
        ...
        assert!(
            !super::parse_buddy_status(&unavailable)
                .expect("explicit unavailable retry status")
                .provider_retry
                .available
        );
        assert!(
        ...
        assert!(
            ...
            .is_err(),
            "auth cannot claim a retry-intent disposition"
        );
        assert!(
            super::parse_buddy_status(
                &available
        ...
        let mut many: serde_json::Value = serde_json::from_str(&available).expect("fixture JSON");
        let receipt = many["provider_retry"]["receipts"][0].clone();
        many["provider_retry"]["receipts"] = serde_json::Value::Array(vec![receipt; 17]);
        assert!(super::parse_buddy_status(&many.to_string()).is_err());
    }

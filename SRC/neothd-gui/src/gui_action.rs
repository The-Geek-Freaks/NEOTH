...
impl TaskDelegateAssignmentAck {
    pub fn verify_for_peer(&self, expected_peer_key: &str) -> Result<(), String> {
        ...
        if self.peer_key != expected_peer_key {
            return Err("TaskDelegate receipt is bound to a different peer key".into());
        }
        if self.revision == 0 {
            return Err("TaskDelegate receipt has zero revision".into());
        }
        Ok(())
    }
    pub fn verify_commit(
        ...
        if self.allowed != expected_allowed {
            return Err(
                "TaskDelegate receipt allowed value does not match submitted mutation".into(),
            );
        }
...
impl LocalModelActionAck {
    pub fn verify(&self, expected_action: Option<&str>) -> Result<&str, String> {
        if self.schema_version != 1 {
            return Err(format!(
                "local-model acknowledgement uses schema {}, expected 1",
                self.schema_version
            ));
        }
        if let Some(expected_action) = expected_action {
            ...
            return Err(format!(
                "local-model acknowledgement reported unsupported action `{}`",
                self.action
            ));
        }
        if !self.ok {
            let detail = self
                ...
                .unwrap_or("core did not provide an error detail");
            return Err(format!(
                "local-model {} was not accepted: {detail}",
                expected_action.unwrap_or("retry/cancel")
            ));
        }
        if self.error.is_some() {
            return Err("successful local-model acknowledgement included an error".into());
        }
        let operation_id = self
            ...
            .ok_or_else(|| {
                "successful local-model acknowledgement omitted a valid operation id".to_string()
            })?;
        Ok(operation_id)
    }
    ...
    pub fn snapshot_json(&self) -> Result<String, String> {
        serde_json::to_string(&self.snapshot)
            .map_err(|error| format!("could not retain typed local-model acknowledgement: {error}"))
    }
}
...
mod task_delegate_assignment_tests {
    ...
    fn task_delegate_assignment_rejects_unknown_zero_and_mismatched_receipts() {
        let key = "a".repeat(64);
        let valid = TaskDelegateAssignmentAck {
            peer_key: key.clone(),
            allowed: true,
            revision: 1,
        };
        ...
        let large = TaskDelegateAssignmentAck {
            peer_key: key.clone(),
            allowed: false,
            revision: (1_u64 << 40) + 1,
        };

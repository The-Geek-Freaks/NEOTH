...
pushd "%~dp0"
cargo test -p neoth --lib self_source -- --test-threads=%NEOTH_TEST_THREADS%
set "NEOTH_EXIT=%ERRORLEVEL%"
if not "%NEOTH_EXIT%"=="0" goto finish
cargo test -p neoth --lib self_edit -- --test-threads=%NEOTH_TEST_THREADS%
set "NEOTH_EXIT=%ERRORLEVEL%"
:finish
popd

use std::collections::HashMap;
use std::fmt;
use std::sync::atomic::{AtomicU32, Ordering};
use std::time::Duration;

use rand::Rng;
use tokio::sync::{mpsc, oneshot};
use tokio::task::{JoinHandle, JoinSet};
use tokio::time::Instant;

use std::sync::Arc;

use libudx::{RuntimeHandle, UdxRuntime};
use peeroxide_dht::crypto::hash;
use peeroxide_dht::hyperdht::{
    self, HyperDhtConfig, HyperDhtHandle, KeyPair, PeerConnection, ServerEvent, ServerReply,
};
use peeroxide_dht::hyperdht_messages::{
    HandshakeMessage, MODE_REPLY, NoisePayload, RelayThroughInfo, SecretStreamInfo, UdxInfo,
    encode_handshake_to_bytes,
};
use peeroxide_dht::messages::Ipv4Peer;
use peeroxide_dht::noise::Keypair as NoiseKeypair;
use peeroxide_dht::noise_wrap::NoiseWrap;
use peeroxide_dht::secret_stream::SecretStream;

use crate::connection_set::{ConnectionInfo, ConnectionSet};
use crate::error::SwarmError;
use crate::peer_discovery::{DiscoveryEvent, PeerDiscoveryConfig, run_discovery};
use crate::peer_info::{PeerInfo, Priority};

static NEXT_STREAM_ID: AtomicU32 = AtomicU32::new(1);

fn next_stream_id() -> u32 {
    NEXT_STREAM_ID.fetch_add(1, Ordering::Relaxed)
}

const DEFAULT_MAX_PEERS: usize = 64;
const DEFAULT_MAX_PARALLEL: usize = 3;

// Public-DHT lookup data is remote controlled.  These fixed upper bounds keep
// discovery bookkeeping and pending work finite even if a caller supplies a
// much larger advisory SwarmConfig value.
const MAX_TOTAL_PEERS: usize = 128;
const MAX_PARALLEL_CONNECTS: usize = 16;
const MAX_TRACKED_UNCONNECTED_PEERS: usize = 256;
const MAX_DISCOVERY_QUEUE: usize = MAX_TRACKED_UNCONNECTED_PEERS;
const MAX_PENDING_RETRIES: usize = MAX_TRACKED_UNCONNECTED_PEERS;
const DISCOVERY_EVENT_CHANNEL_CAPACITY: usize = 256;
const MAX_RELAY_ADDRESSES_PER_PEER: usize = 8;
const MAX_TOPICS_PER_PEER: usize = 16;
const PEER_RECORD_TTL: Duration = Duration::from_secs(15 * 60);
const PEER_GC_INTERVAL: Duration = Duration::from_secs(60);
const CONNECT_ATTEMPT_TIMEOUT: Duration = Duration::from_secs(30);

// ── Retry backoff tiers (matching Node.js lib/retry-timer.js) ────────────────
// Each tier: [base_ms, jitter1, jitter2, jitter3]
// Delay = base + rand(0..j1) + rand(0..j2) + rand(0..j3)
const BACKOFF_S: [u64; 4] = [1000, 250, 100, 50];
const BACKOFF_M: [u64; 4] = [5000, 1000, 500, 250];
const BACKOFF_L: [u64; 4] = [15000, 5000, 2500, 1000];
const BACKOFF_X: [u64; 4] = [600_000, 60_000, 30_000, 15_000];

fn retry_delay(info: &PeerInfo) -> Duration {
    let idx = if info.proven {
        (info.attempts as usize).min(3)
    } else {
        ((info.attempts + 1) as usize).min(3)
    };
    let tier = match idx {
        0 => &BACKOFF_S,
        1 => &BACKOFF_M,
        2 => &BACKOFF_L,
        _ => &BACKOFF_X,
    };
    let mut rng = rand::rng();
    let jitter =
        rng.random_range(0..tier[1]) + rng.random_range(0..tier[2]) + rng.random_range(0..tier[3]);
    Duration::from_millis(tier[0] + jitter)
}

fn short_hex(bytes: &[u8]) -> String {
    bytes.iter().take(4).fold(String::new(), |mut s, b| {
        use fmt::Write;
        write!(s, "{b:02x}").ok();
        s
    })
}

// ── Public types ─────────────────────────────────────────────────────────────

/// Configuration for a [`Hyperswarm`](SwarmHandle) instance.
#[non_exhaustive]
pub struct SwarmConfig {
    /// Ed25519 key pair. Auto-generated if `None`.
    pub key_pair: Option<KeyPair>,
    /// Underlying HyperDHT configuration.
    pub dht: HyperDhtConfig,
    /// Maximum total peer connections (default 64).
    ///
    /// Values above 128 are clamped to retain a hard resource ceiling for
    /// untrusted public-DHT discovery.
    pub max_peers: usize,
    /// Maximum concurrent outgoing connection attempts (default 3).
    ///
    /// Values above 16 are clamped to retain a hard resource ceiling for
    /// untrusted public-DHT discovery.
    pub max_parallel: usize,
    /// Firewall value sent in handshakes (default 0).
    pub firewall: u64,
    /// When set, only accept inbound server handshakes authenticated with this
    /// remote Noise static public key.
    ///
    /// A mismatch is rejected before the server sends a handshake reply or
    /// creates connection, stream, or secret-stream state.
    pub server_expected_remote_static_key: Option<[u8; 32]>,
    /// Public key of a relay node to force all server connections through.
    /// When set, server handshake replies include `relay_through` info directing
    /// clients to connect via the specified relay using the blind-relay protocol.
    pub relay_through: Option<[u8; 32]>,
    /// Socket address of the relay node. When provided alongside `relay_through`,
    /// the server connects to the relay directly instead of discovering it via DHT.
    pub relay_address: Option<std::net::SocketAddr>,
}

impl Default for SwarmConfig {
    fn default() -> Self {
        Self {
            key_pair: None,
            dht: HyperDhtConfig::default(),
            max_peers: DEFAULT_MAX_PEERS,
            max_parallel: DEFAULT_MAX_PARALLEL,
            firewall: 0,
            server_expected_remote_static_key: None,
            relay_through: None,
            relay_address: None,
        }
    }
}

impl SwarmConfig {
    /// Create a config pre-populated with the public HyperDHT bootstrap nodes.
    pub fn with_public_bootstrap() -> Self {
        Self {
            dht: HyperDhtConfig::with_public_bootstrap(),
            ..Self::default()
        }
    }
}

/// Options for joining a topic.
#[non_exhaustive]
pub struct JoinOpts {
    /// Announce on this topic (server mode).
    pub server: bool,
    /// Look up peers on this topic (client mode).
    pub client: bool,
}

impl Default for JoinOpts {
    fn default() -> Self {
        Self {
            server: true,
            client: true,
        }
    }
}

/// An established swarm connection.
///
/// Its duplicate/capacity reservation remains active while this value is
/// owned. Dropping it releases the reservation, allowing a later connection
/// from the same peer or a new peer to be admitted.
///
/// The encrypted peer may be inspected through [`Self::peer`]. Its mutable
/// message operations remain wrapper-owned, so the stream cannot be detached
/// from the capacity reservation.
///
/// ```compile_fail
/// # let connection: peeroxide::SwarmConnection = todo!();
/// let _peer = connection.peer; // private field: no detached peer extraction
/// ```
///
/// ```compile_fail
/// # let mut connection: peeroxide::SwarmConnection = todo!();
/// let _peer = connection.peer_mut(); // no mutable peer accessor exists
/// ```
#[non_exhaustive]
pub struct SwarmConnection {
    /// The underlying encrypted peer connection.
    peer: PeerConnection,
    /// `true` if we initiated this connection.
    pub is_initiator: bool,
    /// Topic(s) associated with this connection.
    pub topics: Vec<[u8; 32]>,
    _runtime: UdxRuntime,
    // Keeps the actor's duplicate/capacity reservation alive exactly while the
    // application owns this connection.  Dropping the connection notifies the
    // actor, which removes only this registration generation.
    _registration: ConnectionRegistration,
}

impl SwarmConnection {
    /// Borrow the encrypted peer connection without detaching it from this
    /// swarm connection's duplicate/capacity reservation.
    pub fn peer(&self) -> &PeerConnection {
        &self.peer
    }

    /// Returns the remote peer's static public key.
    pub fn remote_public_key(&self) -> &[u8; 32] {
        &self.peer.remote_public_key
    }

    /// Read the next decrypted message while retaining this connection's
    /// duplicate/capacity reservation.
    pub async fn read(
        &mut self,
    ) -> Result<Option<Vec<u8>>, peeroxide_dht::secret_stream::SecretStreamError> {
        self.peer.stream.read().await
    }

    /// Encrypt and send one framed message while retaining this connection's
    /// duplicate/capacity reservation.
    pub async fn write(
        &mut self,
        data: &[u8],
    ) -> Result<(), peeroxide_dht::secret_stream::SecretStreamError> {
        self.peer.stream.write(data).await
    }
}

// Keep a `Drop` implementation as a second line of defense inside this module:
// even internal code cannot partially move the private peer out and leave the
// registration behind. External consumers receive only immutable inspection or
// wrapper-owned message I/O.
impl Drop for SwarmConnection {
    fn drop(&mut self) {}
}

impl fmt::Debug for SwarmConnection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("SwarmConnection")
            .field(
                "remote_public_key",
                &short_hex(&self.peer.remote_public_key),
            )
            .field("is_initiator", &self.is_initiator)
            .field("topics", &self.topics.len())
            .finish()
    }
}

/// Clone-able handle for controlling a running Hyperswarm.
#[derive(Clone)]
pub struct SwarmHandle {
    cmd_tx: mpsc::Sender<SwarmCommand>,
    dht: HyperDhtHandle,
    key_pair: KeyPair,
}

/// A started swarm whose DHT ownership is retained before bootstrap completes.
///
/// Construct this with [`spawn_starting`], then await [`Self::bootstrapped`].
/// If bootstrap is timed out or otherwise abandoned, call [`Self::shutdown`] so
/// the raw DHT and request-handler tasks are drained before the UDX runtime is
/// dropped.  After a successful bootstrap, [`Self::finish`] returns the legacy
/// swarm tuple.
pub struct SwarmStartup {
    key_pair: KeyPair,
    runtime: UdxRuntime,
    dht_startup: hyperdht::HyperDhtStartup,
    config: ActorConfig,
}

impl SwarmStartup {
    /// Wait until the underlying HyperDHT node has completed bootstrap.
    pub async fn bootstrapped(&self) -> Result<(), SwarmError> {
        self.dht_startup.bootstrapped().await.map_err(Into::into)
    }

    /// Finish startup and return the historic swarm task, handle, and receiver.
    ///
    /// The returned task owns the swarm actor and its nested HyperDHT owner. It
    /// always drains the raw DHT and request-handler tasks before dropping the
    /// UDX runtime.
    pub async fn finish(
        self,
    ) -> Result<(JoinHandle<()>, SwarmHandle, mpsc::Receiver<SwarmConnection>), SwarmError> {
        let Self {
            key_pair,
            runtime,
            dht_startup,
            config,
        } = self;

        let local_port = match dht_startup.handle().dht().local_port().await {
            Ok(port) => port,
            Err(error) => {
                let _ = dht_startup.shutdown().await;
                drop(runtime);
                return Err(error.into());
            }
        };
        let relay_address = Ipv4Peer {
            host: "127.0.0.1".to_string(),
            port: local_port,
        };

        tracing::info!(port = local_port, "swarm started");

        let (cmd_tx, cmd_rx) = mpsc::channel(64);
        let (conn_tx, conn_rx) = mpsc::channel(64);
        let (connection_lifecycle_tx, connection_lifecycle_rx) = mpsc::unbounded_channel();
        let (discovery_event_tx, discovery_event_rx) =
            mpsc::channel(DISCOVERY_EVENT_CHANNEL_CAPACITY);

        let (mut dht_owner, dht, server_rx) = dht_startup.finish();
        let handle_dht = dht.clone();
        let handle_key_pair = key_pair.clone();

        let actor = SwarmActor {
            key_pair,
            dht,
            config,
            runtime_handle: runtime.handle(),
            topics: HashMap::new(),
            discovery_event_tx,
            peers: HashMap::new(),
            peer_last_seen: HashMap::new(),
            connections: ConnectionSet::new(),
            queue: Vec::new(),
            retries: Vec::new(),
            next_peer_gc: Instant::now() + PEER_GC_INTERVAL,
            conn_tx,
            connection_lifecycle_tx,
            next_connection_registration_id: 1,
            establishment_tasks: ConnectionEstablishmentTasks::default(),
            outbound_connect_tasks: OutboundConnectTasks::default(),
            server_registered: false,
            relay_address: Some(relay_address),
            active_connects: 0,
            flush_waiters: Vec::new(),
        };

        let join = tokio::spawn(async move {
            actor
                .run(
                    cmd_rx,
                    discovery_event_rx,
                    server_rx,
                    connection_lifecycle_rx,
                )
                .await;
            // `SwarmActor::run` asks the DHT to destroy itself first.  The
            // owner then drains both underlying tasks before the runtime falls
            // out of scope, so no task is detached during normal teardown.
            let _ = dht_owner.shutdown().await;
            drop(runtime);
        });

        let handle = SwarmHandle {
            cmd_tx,
            dht: handle_dht,
            key_pair: handle_key_pair,
        };
        Ok((join, handle, conn_rx))
    }

    /// Stop the started DHT and drain it before dropping its UDX runtime.
    pub async fn shutdown(self) -> Result<(), SwarmError> {
        let Self {
            runtime,
            dht_startup,
            ..
        } = self;
        let result = dht_startup.shutdown().await.map_err(SwarmError::from);
        drop(runtime);
        result
    }
}

impl SwarmHandle {
    /// Access the underlying [`HyperDhtHandle`] for low-level DHT operations.
    ///
    /// This exposes mutable/immutable storage, manual peer lookup, and other
    /// DHT primitives not covered by the high-level swarm API.
    ///
    /// # Example
    ///
    /// ```rust,no_run
    /// # use peeroxide::{spawn, discovery_key, JoinOpts, SwarmConfig, KeyPair};
    /// # #[tokio::main]
    /// # async fn main() -> Result<(), Box<dyn std::error::Error>> {
    /// let config = SwarmConfig::with_public_bootstrap();
    /// let (_task, handle, _conn_rx) = spawn(config).await?;
    ///
    /// // Publish a mutable record under the swarm's own keypair
    /// let kp = handle.key_pair();
    /// handle.dht().mutable_put(kp, b"hello", 0).await?;
    /// # Ok(())
    /// # }
    /// ```
    ///
    /// # Caveats
    ///
    /// - **Do not call `destroy()`** on the returned handle. The swarm owns
    ///   the DHT lifecycle; destroying it here will break discovery and
    ///   connection establishment.
    /// - **`connect` methods require a `UdxRuntime`** that is not accessible
    ///   from the public API. Use swarm-level topic joins for connection
    ///   establishment instead.
    pub fn dht(&self) -> &HyperDhtHandle {
        &self.dht
    }

    /// The Ed25519 key pair identifying this swarm node.
    ///
    /// This is the same key pair used for topic announcements and Noise
    /// handshakes. It can also be used with [`HyperDhtHandle::mutable_put`]
    /// to publish data that other peers can discover and verify.
    pub fn key_pair(&self) -> &KeyPair {
        &self.key_pair
    }

    /// Join a topic for peer discovery.
    ///
    /// When `opts.server` is true, the swarm announces so other peers can
    /// connect to us. When `opts.client` is true, the swarm looks up peers
    /// and initiates connections.
    pub async fn join(&self, topic: [u8; 32], opts: JoinOpts) -> Result<(), SwarmError> {
        let (reply_tx, reply_rx) = oneshot::channel();
        self.cmd_tx
            .send(SwarmCommand::Join {
                topic,
                server: opts.server,
                client: opts.client,
                reply_tx,
            })
            .await
            .map_err(|_| SwarmError::Destroyed)?;
        reply_rx.await.map_err(|_| SwarmError::ChannelClosed)?
    }

    /// Leave a topic, stopping discovery and unannouncing.
    pub async fn leave(&self, topic: [u8; 32]) -> Result<(), SwarmError> {
        let (reply_tx, reply_rx) = oneshot::channel();
        self.cmd_tx
            .send(SwarmCommand::Leave { topic, reply_tx })
            .await
            .map_err(|_| SwarmError::Destroyed)?;
        reply_rx.await.map_err(|_| SwarmError::ChannelClosed)?
    }

    /// Wait until all joined topics have completed their initial discovery.
    pub async fn flush(&self) -> Result<(), SwarmError> {
        let (reply_tx, reply_rx) = oneshot::channel();
        self.cmd_tx
            .send(SwarmCommand::Flush { reply_tx })
            .await
            .map_err(|_| SwarmError::Destroyed)?;
        reply_rx.await.map_err(|_| SwarmError::ChannelClosed)?
    }

    /// Destroy the swarm, cancelling all discovery and closing connections.
    pub async fn destroy(&self) -> Result<(), SwarmError> {
        let (reply_tx, reply_rx) = oneshot::channel();
        let _ = self.cmd_tx.send(SwarmCommand::Destroy { reply_tx }).await;
        let _ = reply_rx.await;
        Ok(())
    }
}

// ── Internal types ───────────────────────────────────────────────────────────

enum SwarmCommand {
    Join {
        topic: [u8; 32],
        server: bool,
        client: bool,
        reply_tx: oneshot::Sender<Result<(), SwarmError>>,
    },
    Leave {
        topic: [u8; 32],
        reply_tx: oneshot::Sender<Result<(), SwarmError>>,
    },
    Flush {
        reply_tx: oneshot::Sender<Result<(), SwarmError>>,
    },
    Destroy {
        reply_tx: oneshot::Sender<Result<(), SwarmError>>,
    },
}

#[allow(dead_code)] // Fields read during leave/unannounce (future)
struct TopicState {
    is_server: bool,
    is_client: bool,
    cancel_tx: Option<oneshot::Sender<()>>,
    refreshed: bool,
}

struct ActorConfig {
    max_peers: usize,
    max_parallel: usize,
    firewall: u64,
    server_expected_remote_static_key: Option<[u8; 32]>,
    relay_through: Option<[u8; 32]>,
    relay_address: Option<std::net::SocketAddr>,
}

/// The only decision made between authenticated responder Noise receive and
/// any responder connection side effect.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum ServerHandshakeAdmission {
    Accept,
    RejectUnexpectedRemoteStaticKey,
}

/// Apply the optional inbound static-key pin without allocating state.
fn server_handshake_admission(
    expected_remote_static_key: Option<[u8; 32]>,
    authenticated_remote_static_key: [u8; 32],
) -> ServerHandshakeAdmission {
    match expected_remote_static_key {
        Some(expected) if expected != authenticated_remote_static_key => {
            ServerHandshakeAdmission::RejectUnexpectedRemoteStaticKey
        }
        _ => ServerHandshakeAdmission::Accept,
    }
}

/// Run a responder-side action only after the allocation-free admission gate.
///
/// `handle_server_handshake` uses this for its first post-admission resource:
/// the local stream id. Every later reply, registration, task, and stream
/// operation is consequently unreachable on rejection as well.
fn after_server_handshake_admission<T>(
    expected_remote_static_key: Option<[u8; 32]>,
    authenticated_remote_static_key: [u8; 32],
    on_accept: impl FnOnce() -> T,
) -> Option<T> {
    match server_handshake_admission(expected_remote_static_key, authenticated_remote_static_key) {
        ServerHandshakeAdmission::Accept => Some(on_accept()),
        ServerHandshakeAdmission::RejectUnexpectedRemoteStaticKey => None,
    }
}

struct SwarmActor {
    key_pair: KeyPair,
    dht: HyperDhtHandle,
    config: ActorConfig,
    runtime_handle: Arc<RuntimeHandle>,

    topics: HashMap<[u8; 32], TopicState>,
    discovery_event_tx: mpsc::Sender<DiscoveryEvent>,

    // Only unconnected peers are retained here.  Delivered connections own
    // their own topic snapshot, so deleting this record on success avoids a
    // permanently growing cache of old DHT observations.
    peers: HashMap<[u8; 32], PeerInfo>,
    peer_last_seen: HashMap<[u8; 32], Instant>,
    connections: ConnectionSet,
    queue: Vec<[u8; 32]>,
    retries: Vec<RetrySchedule>,
    next_peer_gc: Instant,

    conn_tx: mpsc::Sender<SwarmConnection>,
    connection_lifecycle_tx: mpsc::UnboundedSender<ConnectionLifecycleEvent>,
    next_connection_registration_id: u64,
    establishment_tasks: ConnectionEstablishmentTasks,
    outbound_connect_tasks: OutboundConnectTasks,

    server_registered: bool,
    relay_address: Option<Ipv4Peer>,

    active_connects: usize,
    flush_waiters: Vec<oneshot::Sender<Result<(), SwarmError>>>,
}

struct ConnectAttemptResult {
    public_key: [u8; 32],
    result: Result<(PeerConnection, UdxRuntime), SwarmError>,
}

/// Owns every bounded outbound connect attempt until it completes or shutdown
/// aborts and drains it.  The task-id map lets a panic be reconciled with the
/// exact peer record rather than leaking a `connecting` slot forever.
#[derive(Default)]
struct OutboundConnectTasks {
    tasks: JoinSet<ConnectAttemptResult>,
    peers: HashMap<tokio::task::Id, [u8; 32]>,
}

enum OutboundConnectCompletion {
    Finished(ConnectAttemptResult),
    Failed {
        public_key: Option<[u8; 32]>,
        error: tokio::task::JoinError,
    },
}

impl OutboundConnectTasks {
    fn spawn(
        &mut self,
        public_key: [u8; 32],
        task: impl std::future::Future<Output = ConnectAttemptResult> + Send + 'static,
    ) {
        let abort = self.tasks.spawn(task);
        self.peers.insert(abort.id(), public_key);
    }

    fn is_empty(&self) -> bool {
        self.tasks.is_empty()
    }

    async fn join_next(&mut self) -> Option<OutboundConnectCompletion> {
        match self.tasks.join_next_with_id().await {
            Some(Ok((id, result))) => {
                self.peers.remove(&id);
                Some(OutboundConnectCompletion::Finished(result))
            }
            Some(Err(error)) => Some(OutboundConnectCompletion::Failed {
                public_key: self.peers.remove(&error.id()),
                error,
            }),
            None => None,
        }
    }

    async fn shutdown(&mut self) {
        self.tasks.shutdown().await;
        self.peers.clear();
    }
}

/// One actor-owned delayed outbound retry.  This is data, not a spawned
/// sleeper: shutdown and topic removal therefore cancel it synchronously and
/// a hostile lookup cannot create an unbounded task population.
struct RetrySchedule {
    public_key: [u8; 32],
    due_at: Instant,
}

fn bounded_push<T>(items: &mut Vec<T>, item: T, capacity: usize) -> bool {
    if items.len() >= capacity {
        return false;
    }
    items.push(item);
    true
}

fn drain_due_retries(retries: &mut Vec<RetrySchedule>, now: Instant) -> Vec<[u8; 32]> {
    let mut due = Vec::new();
    retries.retain(|retry| {
        if retry.due_at <= now {
            due.push(retry.public_key);
            false
        } else {
            true
        }
    });
    due
}

/// A lifecycle update emitted by an application-owned [`SwarmConnection`].
///
/// The registration id makes a delayed drop harmless when a newer connection
/// for the same static Noise key has already replaced it.
#[derive(Debug, Eq, PartialEq)]
enum ConnectionLifecycleEvent {
    Closed {
        public_key: [u8; 32],
        registration_id: u64,
    },
}

/// RAII reservation for one connection-set entry.
///
/// This deliberately does not close or read the encrypted stream; consumers
/// retain sole ownership of their application protocol.  It only tells the
/// swarm actor that the consumer has released the connection, including failed
/// pre-auth admission where callers drop it immediately.
struct ConnectionRegistration {
    public_key: [u8; 32],
    registration_id: u64,
    lifecycle_tx: mpsc::UnboundedSender<ConnectionLifecycleEvent>,
}

/// Admit a completed outbound transport only when no application-owned
/// connection is already registered for that static public key.
///
/// `None` deliberately consumes and drops the just-completed candidate.  The
/// existing registration remains the sole capacity reservation until its
/// application-owned [`SwarmConnection`] is dropped and its matching lifecycle
/// event is processed by the actor.
fn admit_outbound_connection<T>(
    connections: &ConnectionSet,
    public_key: &[u8; 32],
    candidate: T,
) -> Option<T> {
    (!connections.has(public_key)).then_some(candidate)
}

impl Drop for ConnectionRegistration {
    fn drop(&mut self) {
        let _ = self.lifecycle_tx.send(ConnectionLifecycleEvent::Closed {
            public_key: self.public_key,
            registration_id: self.registration_id,
        });
    }
}

/// Owns responder-side stream establishment until it either delivers a
/// connection or fails.  The actor cancels and drains these tasks before
/// tearing down the DHT/runtime, so a pending UDX connect cannot outlive its
/// swarm and its RAII connection reservation is always released.
#[derive(Default)]
struct ConnectionEstablishmentTasks {
    tasks: JoinSet<()>,
}

impl ConnectionEstablishmentTasks {
    fn spawn(&mut self, task: impl std::future::Future<Output = ()> + Send + 'static) {
        self.tasks.spawn(task);
    }

    fn is_empty(&self) -> bool {
        self.tasks.is_empty()
    }

    async fn join_next(&mut self) -> Option<Result<(), tokio::task::JoinError>> {
        self.tasks.join_next().await
    }

    async fn shutdown(&mut self) {
        self.tasks.shutdown().await;
    }
}

// ── Spawn ────────────────────────────────────────────────────────────────────

/// Start a Hyperswarm instance while retaining task ownership before bootstrap.
pub async fn spawn_starting(config: SwarmConfig) -> Result<SwarmStartup, SwarmError> {
    let SwarmConfig {
        key_pair,
        dht,
        max_peers,
        max_parallel,
        firewall,
        server_expected_remote_static_key,
        relay_through,
        relay_address,
    } = config;
    let runtime = UdxRuntime::new()?;
    let dht_startup = match hyperdht::spawn_starting(&runtime, dht).await {
        Ok(startup) => startup,
        Err(error) => {
            drop(runtime);
            return Err(error.into());
        }
    };

    Ok(SwarmStartup {
        key_pair: key_pair.unwrap_or_else(KeyPair::generate),
        runtime,
        dht_startup,
        config: ActorConfig {
            max_peers: max_peers.min(MAX_TOTAL_PEERS),
            max_parallel: max_parallel.min(MAX_PARALLEL_CONNECTS),
            firewall,
            server_expected_remote_static_key,
            relay_through,
            relay_address,
        },
    })
}

/// Create and start a Hyperswarm instance.
///
/// Returns a background task handle, a control handle, and a receiver that
/// yields each new [`SwarmConnection`].  This source-compatible convenience
/// wrapper cleans up the started DHT when bootstrap fails; use
/// [`spawn_starting`] when callers need explicit ownership during bootstrap.
pub async fn spawn(
    config: SwarmConfig,
) -> Result<(JoinHandle<()>, SwarmHandle, mpsc::Receiver<SwarmConnection>), SwarmError> {
    let startup = spawn_starting(config).await?;
    if let Err(error) = startup.bootstrapped().await {
        let _ = startup.shutdown().await;
        return Err(error);
    }
    startup.finish().await
}

// ── Actor ────────────────────────────────────────────────────────────────────

impl SwarmActor {
    async fn run(
        mut self,
        mut cmd_rx: mpsc::Receiver<SwarmCommand>,
        mut discovery_rx: mpsc::Receiver<DiscoveryEvent>,
        mut server_rx: mpsc::Receiver<ServerEvent>,
        mut connection_lifecycle_rx: mpsc::UnboundedReceiver<ConnectionLifecycleEvent>,
    ) {
        loop {
            tokio::select! {
                biased;
                // Control and capacity-release events stay ahead of network
                // work.  Inbound server admission is deliberately processed
                // before outbound discovery/connect traffic so a saturated
                // public-DHT client lookup cannot starve a legitimate server
                // handshake.
                lifecycle = connection_lifecycle_rx.recv() => {
                    if let Some(lifecycle) = lifecycle {
                        self.handle_connection_lifecycle(lifecycle);
                    }
                }
                result = self.establishment_tasks.join_next(), if !self.establishment_tasks.is_empty() => {
                    if let Some(Err(error)) = result {
                        tracing::warn!(%error, "server: stream establishment task panicked");
                    }
                }
                cmd = cmd_rx.recv() => {
                    let Some(cmd) = cmd else { break };
                    if self.handle_command(cmd) {
                        break;
                    }
                }
                event = server_rx.recv() => {
                    if let Some(event) = event {
                        self.handle_server_event(event);
                    }
                }
                completion = self.outbound_connect_tasks.join_next(), if !self.outbound_connect_tasks.is_empty() => {
                    if let Some(completion) = completion {
                        self.handle_outbound_connect_completion(completion);
                    }
                }
                _ = tokio::time::sleep_until(self.next_peer_gc) => {
                    self.gc_stale_peer_records();
                }
                _ = tokio::time::sleep_until(self.next_retry_deadline()), if !self.retries.is_empty() => {
                    self.activate_due_retries();
                }
                event = discovery_rx.recv() => {
                    if let Some(event) = event {
                        self.handle_discovery_event(event);
                    }
                }
            }
        }

        for (_, state) in self.topics.drain() {
            if let Some(cancel) = state.cancel_tx {
                let _ = cancel.send(());
            }
        }

        // Stop new responder admission before cancelling every owned outbound
        // connect and stream-establishment task.  This drains all references
        // to the DHT/runtime before their owners are released below.
        self.close_server_admission();
        self.outbound_connect_tasks.shutdown().await;
        self.active_connects = 0;
        self.establishment_tasks.shutdown().await;
        while let Ok(lifecycle) = connection_lifecycle_rx.try_recv() {
            self.handle_connection_lifecycle(lifecycle);
        }

        let _ = self.dht.destroy().await;
    }

    /// Reserve a slot until the delivered connection is dropped.
    ///
    /// Callers must keep the existing duplicate and capacity checks before
    /// calling this method.  The monotonically advancing id prevents an old
    /// connection's delayed `Drop` from clearing a newer replacement.
    fn register_connection(&mut self, public_key: [u8; 32]) -> ConnectionRegistration {
        let registration_id = self.next_connection_registration_id;
        self.next_connection_registration_id = self
            .next_connection_registration_id
            .checked_add(1)
            .expect("connection registration id exhausted");
        self.connections
            .add(public_key, ConnectionInfo { registration_id });
        ConnectionRegistration {
            public_key,
            registration_id,
            lifecycle_tx: self.connection_lifecycle_tx.clone(),
        }
    }

    fn handle_connection_lifecycle(&mut self, lifecycle: ConnectionLifecycleEvent) {
        let ConnectionLifecycleEvent::Closed {
            public_key,
            registration_id,
        } = lifecycle;
        if self
            .connections
            .remove_if_matches(&public_key, registration_id)
        {
            tracing::debug!(
                pk = %short_hex(&public_key),
                registration_id,
                "connection released"
            );
        }
    }

    fn close_server_admission(&mut self) {
        if self.server_registered {
            let target = hash(&self.key_pair.public_key);
            self.dht.unregister_server(&target);
            self.server_registered = false;
        }
    }

    /// Returns `true` when the actor should shut down.
    fn handle_command(&mut self, cmd: SwarmCommand) -> bool {
        match cmd {
            SwarmCommand::Join {
                topic,
                server,
                client,
                reply_tx,
            } => {
                let result = self.do_join(topic, server, client);
                let _ = reply_tx.send(result);
                false
            }
            SwarmCommand::Leave { topic, reply_tx } => {
                let result = self.do_leave(topic);
                let _ = reply_tx.send(result);
                false
            }
            SwarmCommand::Flush { reply_tx } => {
                if self.all_topics_refreshed() {
                    let _ = reply_tx.send(Ok(()));
                } else {
                    self.flush_waiters.push(reply_tx);
                }
                false
            }
            SwarmCommand::Destroy { reply_tx } => {
                self.close_server_admission();
                let _ = reply_tx.send(Ok(()));
                true
            }
        }
    }

    fn do_join(&mut self, topic: [u8; 32], server: bool, client: bool) -> Result<(), SwarmError> {
        if self.topics.contains_key(&topic) {
            return Ok(());
        }

        if server && !self.server_registered {
            let target = hash(&self.key_pair.public_key);
            if !self.dht.register_server(&target) {
                return Err(SwarmError::ServerRoutingCapacity);
            }
            self.server_registered = true;
            tracing::debug!(pk = %short_hex(&self.key_pair.public_key), "server registered");
        }

        let (cancel_tx, cancel_rx) = oneshot::channel();
        let relay_addresses = self
            .relay_address
            .as_ref()
            .map_or_else(Vec::new, |a| vec![a.clone()]);

        tokio::spawn(run_discovery(
            PeerDiscoveryConfig {
                topic,
                is_server: server,
                is_client: client,
            },
            self.dht.clone(),
            self.key_pair.clone(),
            relay_addresses,
            self.discovery_event_tx.clone(),
            cancel_rx,
        ));

        self.topics.insert(
            topic,
            TopicState {
                is_server: server,
                is_client: client,
                cancel_tx: Some(cancel_tx),
                refreshed: false,
            },
        );
        Ok(())
    }

    fn do_leave(&mut self, topic: [u8; 32]) -> Result<(), SwarmError> {
        if let Some(state) = self.topics.remove(&topic) {
            if let Some(cancel) = state.cancel_tx {
                let _ = cancel.send(());
            }
            for peer in self.peers.values_mut() {
                peer.topics.retain(|t| *t != topic);
            }
            // A leaving topic invalidates queued and delayed attempts for its
            // now-unassociated discoveries.  In-flight attempts are allowed
            // to finish, but their result is rejected because the peer record
            // no longer exists (see handle_connect_result).
            self.prune_unassociated_peers();

            if state.is_server && self.server_registered {
                let has_remaining_server_topics = self.topics.values().any(|t| t.is_server);
                if !has_remaining_server_topics {
                    let target = hash(&self.key_pair.public_key);
                    self.dht.unregister_server(&target);
                    self.server_registered = false;
                    tracing::debug!(
                        pk = %short_hex(&self.key_pair.public_key),
                        "server unregistered (no remaining server topics)"
                    );
                }
            }
        }
        Ok(())
    }

    /// Queue one eligible peer at most once.  The stable priority sort in
    /// `attempt_connections` keeps FIFO order among peers at the same priority;
    /// the explicit length check makes that fairness queue finite.
    fn enqueue_peer(&mut self, public_key: [u8; 32]) {
        let Some(info) = self.peers.get_mut(&public_key) else {
            return;
        };
        if info.queued
            || info.connecting
            || info.banned
            || info.is_waiting()
            || info.topics.is_empty()
        {
            return;
        }
        if self.queue.len() >= MAX_DISCOVERY_QUEUE {
            tracing::debug!("discovery queue limit reached; dropping candidate");
            return;
        }

        info.queued = true;
        info.priority = info.get_priority();
        let inserted = bounded_push(&mut self.queue, public_key, MAX_DISCOVERY_QUEUE);
        debug_assert!(inserted, "queue capacity was checked before insertion");
    }

    fn next_retry_deadline(&self) -> Instant {
        self.retries
            .iter()
            .map(|retry| retry.due_at)
            .min()
            .unwrap_or_else(Instant::now)
    }

    fn activate_due_retries(&mut self) {
        let now = Instant::now();
        let due = drain_due_retries(&mut self.retries, now);

        for public_key in due {
            if let Some(info) = self.peers.get_mut(&public_key) {
                info.set_waiting(false);
            }
            self.enqueue_peer(public_key);
        }
        self.attempt_connections();
    }

    fn prune_unassociated_peers(&mut self) {
        self.peers.retain(|_, info| !info.topics.is_empty());
        let peers = &self.peers;
        self.peer_last_seen
            .retain(|public_key, _| peers.contains_key(public_key));
        self.queue
            .retain(|public_key| peers.contains_key(public_key));
        self.retries
            .retain(|retry| peers.contains_key(&retry.public_key));
    }

    fn gc_stale_peer_records(&mut self) {
        let now = Instant::now();
        let peer_last_seen = &self.peer_last_seen;
        self.peers.retain(|public_key, _| {
            peer_last_seen
                .get(public_key)
                .is_some_and(|seen| now.duration_since(*seen) < PEER_RECORD_TTL)
        });
        let peers = &self.peers;
        self.peer_last_seen
            .retain(|public_key, _| peers.contains_key(public_key));
        self.queue
            .retain(|public_key| peers.contains_key(public_key));
        self.retries
            .retain(|retry| peers.contains_key(&retry.public_key));
        self.next_peer_gc = now + PEER_GC_INTERVAL;
    }

    fn handle_discovery_event(&mut self, event: DiscoveryEvent) {
        match event {
            DiscoveryEvent::PeerFound {
                public_key,
                relay_addresses,
                topic,
            } => {
                // A discovery task can race a local leave.  Events for the
                // retired topic must not resurrect peer state or initiate a
                // new connection after the caller has left it.
                if !self.topics.contains_key(&topic) {
                    return;
                }
                if public_key == self.key_pair.public_key {
                    return;
                }
                if self.connections.has(&public_key) {
                    return;
                }
                if self.connections.len() >= self.config.max_peers {
                    return;
                }

                let relay_addresses: Vec<_> = relay_addresses
                    .into_iter()
                    .take(MAX_RELAY_ADDRESSES_PER_PEER)
                    .collect();
                let is_known = self.peers.contains_key(&public_key);
                if !is_known && self.peers.len() >= MAX_TRACKED_UNCONNECTED_PEERS {
                    tracing::debug!("discovery peer limit reached; dropping candidate");
                    return;
                }

                let info = self
                    .peers
                    .entry(public_key)
                    .or_insert_with(|| PeerInfo::new(public_key, relay_addresses.clone()));

                if !relay_addresses.is_empty() {
                    info.relay_addresses = relay_addresses;
                }
                if !info.topics.contains(&topic) && info.topics.len() < MAX_TOPICS_PER_PEER {
                    info.topics.push(topic);
                }
                self.peer_last_seen.insert(public_key, Instant::now());
                self.enqueue_peer(public_key);
                self.attempt_connections();
            }
            DiscoveryEvent::RefreshComplete { topic } => {
                if let Some(state) = self.topics.get_mut(&topic) {
                    state.refreshed = true;
                }
                self.check_flush_waiters();
            }
        }
    }

    fn attempt_connections(&mut self) {
        while self.active_connects < self.config.max_parallel && !self.queue.is_empty() {
            // Sort by priority descending
            self.queue.sort_by(|a, b| {
                let pa = self.peers.get(a).map_or(Priority::VeryLow, |i| i.priority);
                let pb = self.peers.get(b).map_or(Priority::VeryLow, |i| i.priority);
                pb.cmp(&pa)
            });

            let pk = self.queue.remove(0);
            let Some(info) = self.peers.get_mut(&pk) else {
                continue;
            };
            if info.connecting || info.banned || info.is_waiting() || info.topics.is_empty() {
                info.queued = false;
                continue;
            }
            let relay_addrs = {
                info.queued = false;
                info.connecting = true;
                info.attempts += 1;
                info.relay_addresses.clone()
            };

            self.active_connects += 1;
            let dht = self.dht.clone();
            let key_pair = self.key_pair.clone();
            let rh = self.runtime_handle.clone();

            self.outbound_connect_tasks.spawn(pk, async move {
                let conn_runtime = UdxRuntime::shared(rh);
                tracing::debug!(pk = %short_hex(&pk), "connecting to peer");
                match tokio::time::timeout(
                    CONNECT_ATTEMPT_TIMEOUT,
                    dht.connect_with_nodes(&key_pair, pk, &relay_addrs, &conn_runtime),
                )
                .await
                {
                    Ok(Ok(conn)) => {
                        tracing::debug!(pk = %short_hex(&pk), "peer connected");
                        ConnectAttemptResult {
                            public_key: pk,
                            result: Ok((conn, conn_runtime)),
                        }
                    }
                    Ok(Err(e)) => {
                        tracing::debug!(pk = %short_hex(&pk), err = %e, "peer connect failed");
                        ConnectAttemptResult {
                            public_key: pk,
                            result: Err(SwarmError::Dht(e)),
                        }
                    }
                    Err(_) => {
                        tracing::debug!(pk = %short_hex(&pk), "peer connect timed out");
                        ConnectAttemptResult {
                            public_key: pk,
                            result: Err(SwarmError::Dht(hyperdht::HyperDhtError::HandshakeFailed(
                                "outbound connection attempt timed out".into(),
                            ))),
                        }
                    }
                }
            });
        }
    }

    fn handle_outbound_connect_completion(&mut self, completion: OutboundConnectCompletion) {
        match completion {
            OutboundConnectCompletion::Finished(result) => self.handle_connect_result(result),
            OutboundConnectCompletion::Failed { public_key, error } => {
                self.active_connects = self.active_connects.saturating_sub(1);
                tracing::warn!(%error, "outbound connect task panicked or was cancelled");
                if let Some(public_key) = public_key {
                    if let Some(info) = self.peers.get_mut(&public_key) {
                        info.connecting = false;
                    }
                    self.schedule_retry(public_key);
                }
                self.attempt_connections();
            }
        }
    }

    fn handle_connect_result(&mut self, result: ConnectAttemptResult) {
        self.active_connects = self.active_connects.saturating_sub(1);

        if let Some(info) = self.peers.get_mut(&result.public_key) {
            info.connecting = false;
        }

        match result.result {
            Ok((conn, runtime)) => {
                let pk = result.public_key;

                // A delivered connection is application-owned.  A late
                // outbound completion must never replace it based on a
                // tiebreak: doing so invalidates the existing generation and
                // lets its later Drop erase capacity accounting.  Dropping
                // this candidate preserves the original registration.
                let Some((conn, runtime)) =
                    admit_outbound_connection(&self.connections, &pk, (conn, runtime))
                else {
                    tracing::debug!(pk = %short_hex(&pk), "dedup: retaining application-owned connection");
                    self.attempt_connections();
                    return;
                };

                // A topic can be left while its bounded outbound attempt is
                // in flight.  Without an active unconnected-peer record the
                // result is stale and must not create a post-leave connection.
                let Some(info) = self.peers.remove(&pk) else {
                    tracing::debug!(pk = %short_hex(&pk), "dropping stale outbound completion");
                    self.attempt_connections();
                    return;
                };
                self.peer_last_seen.remove(&pk);

                let registration = self.register_connection(pk);
                let topics = info.topics;

                let swarm_conn = SwarmConnection {
                    peer: conn,
                    is_initiator: true,
                    topics,
                    _runtime: runtime,
                    _registration: registration,
                };
                if self.conn_tx.try_send(swarm_conn).is_err() {
                    tracing::warn!("connection channel full, dropping connection");
                }
            }
            Err(e) => {
                tracing::debug!(pk = %short_hex(&result.public_key), err = %e, "connect failed");
                self.schedule_retry(result.public_key);
            }
        }

        self.attempt_connections();
    }

    fn schedule_retry(&mut self, pk: [u8; 32]) {
        let Some(info) = self.peers.get_mut(&pk) else {
            return;
        };
        if info.banned || info.topics.is_empty() {
            return;
        }

        if info.is_waiting() || self.retries.len() >= MAX_PENDING_RETRIES {
            return;
        }

        info.set_waiting(true);
        let due_at = Instant::now() + retry_delay(info);
        let inserted = bounded_push(
            &mut self.retries,
            RetrySchedule {
                public_key: pk,
                due_at,
            },
            MAX_PENDING_RETRIES,
        );
        debug_assert!(inserted, "retry capacity was checked before insertion");
    }

    fn handle_server_event(&mut self, event: ServerEvent) {
        match event {
            ServerEvent::PeerHandshake {
                msg,
                from,
                target: _,
                reply_tx,
            } => {
                self.handle_server_handshake(msg, from, reply_tx);
            }
            ServerEvent::PeerHolepunch { reply_tx, .. } => {
                // Holepunch handling requires libudx incoming-stream support.
                // Acknowledge without creating a connection.
                let _ = reply_tx.send(None);
            }
            _ => {}
        }
    }

    fn handle_server_handshake(
        &mut self,
        msg: HandshakeMessage,
        from: Ipv4Peer,
        reply_tx: ServerReply,
    ) {
        let noise_kp = NoiseKeypair {
            public_key: self.key_pair.public_key,
            secret_key: self.key_pair.secret_key,
        };

        let mut nw = NoiseWrap::new_responder(noise_kp);

        let remote_payload = match nw.recv(&msg.noise) {
            Ok(p) => p,
            Err(e) => {
                tracing::debug!(err = %e, "server handshake: noise recv failed");
                let _ = reply_tx.send(None);
                return;
            }
        };

        let Some(remote_static_key) = nw.authenticated_remote_static_key() else {
            tracing::debug!(
                "server handshake: Noise recv yielded no authenticated remote static key"
            );
            let _ = reply_tx.send(None);
            return;
        };
        let Some(local_stream_id) = after_server_handshake_admission(
            self.config.server_expected_remote_static_key,
            remote_static_key,
            next_stream_id,
        ) else {
            tracing::debug!(
                expected = ?self.config.server_expected_remote_static_key.map(|key| short_hex(&key)),
                actual = %short_hex(&remote_static_key),
                "server handshake: unexpected remote static key"
            );
            let _ = reply_tx.send(None);
            return;
        };

        if remote_payload.error != 0 {
            let _ = reply_tx.send(None);
            return;
        }

        let (relay_token, relay_through_info) = if let Some(relay_pk) = self.config.relay_through {
            let token: [u8; 32] = rand::random();
            let info = RelayThroughInfo {
                version: 1,
                public_key: relay_pk,
                token,
            };
            (Some(token), Some(info))
        } else {
            (None, None)
        };

        let reply_payload = NoisePayload {
            version: 1,
            error: 0,
            firewall: self.config.firewall,
            holepunch: None,
            addresses4: vec![],
            addresses6: vec![],
            udx: Some(UdxInfo {
                version: 1,
                reusable_socket: true,
                id: u64::from(local_stream_id),
                seq: 0,
            }),
            secret_stream: Some(SecretStreamInfo { version: 1 }),
            relay_through: relay_through_info,
            relay_addresses: self.config.relay_address.map(|addr| {
                vec![peeroxide_dht::messages::Ipv4Peer {
                    host: addr.ip().to_string(),
                    port: addr.port(),
                }]
            }),
        };

        let noise_reply = match nw.send(&reply_payload) {
            Ok(b) => b,
            Err(e) => {
                tracing::debug!(err = %e, "server handshake: noise send failed");
                let _ = reply_tx.send(None);
                return;
            }
        };

        let nw_result = match nw.finalize() {
            Ok(r) => r,
            Err(e) => {
                tracing::debug!(err = %e, "server handshake: noise finalize failed");
                let _ = reply_tx.send(None);
                return;
            }
        };

        let reply_msg = HandshakeMessage {
            mode: MODE_REPLY,
            noise: noise_reply,
            peer_address: None,
            relay_address: None,
        };
        let _ = reply_tx.send(encode_handshake_to_bytes(&reply_msg).ok());

        let remote_pk = nw_result.remote_public_key;

        if self.connections.has(&remote_pk) {
            tracing::debug!(pk = %short_hex(&remote_pk), "server: already connected");
            return;
        }
        if self.connections.len() >= self.config.max_peers {
            tracing::debug!("server: at max connections");
            return;
        }

        let remote_udx = match remote_payload.udx {
            Some(u) => u,
            None => {
                tracing::debug!("server: no UDX info in handshake");
                return;
            }
        };

        // Reserve before spawning stream establishment so duplicate and
        // capacity checks cover the in-flight responder connection too.  The
        // registration drops on an establishment failure or when the consumer
        // rejects a pre-auth connection.
        let registration = self.register_connection(remote_pk);

        let conn_tx = self.conn_tx.clone();

        if let (Some(relay_pk), Some(token)) = (self.config.relay_through, relay_token) {
            let dht = self.dht.clone();
            let key_pair = self.key_pair.clone();
            let relay_addr = self.config.relay_address;
            let rh = self.runtime_handle.clone();
            self.establishment_tasks.spawn(async move {
                match create_server_relay_connection(
                    rh,
                    dht,
                    key_pair,
                    relay_pk,
                    relay_addr,
                    token,
                    local_stream_id,
                    nw_result,
                )
                .await
                {
                    Ok((conn, runtime)) => {
                        let swarm_conn = SwarmConnection {
                            peer: conn,
                            is_initiator: false,
                            topics: vec![],
                            _runtime: runtime,
                            _registration: registration,
                        };
                        if conn_tx.send(swarm_conn).await.is_err() {
                            tracing::warn!("connection channel closed");
                        }
                    }
                    Err(e) => {
                        tracing::debug!(err = %e, "server: relay connection failed");
                    }
                }
            });
        } else {
            let rh = self.runtime_handle.clone();
            let dht = self.dht.clone();
            self.establishment_tasks.spawn(async move {
                match create_server_connection(
                    rh,
                    dht,
                    local_stream_id,
                    &remote_udx,
                    &from,
                    &nw_result,
                )
                .await
                {
                    Ok((conn, runtime)) => {
                        let swarm_conn = SwarmConnection {
                            peer: conn,
                            is_initiator: false,
                            topics: vec![],
                            _runtime: runtime,
                            _registration: registration,
                        };
                        if conn_tx.send(swarm_conn).await.is_err() {
                            tracing::warn!("connection channel closed");
                        }
                    }
                    Err(e) => {
                        tracing::debug!(err = %e, "server: stream establishment failed");
                    }
                }
            });
        }
    }

    fn all_topics_refreshed(&self) -> bool {
        !self.topics.is_empty() && self.topics.values().all(|t| t.refreshed)
    }

    fn check_flush_waiters(&mut self) {
        if self.all_topics_refreshed() {
            for waiter in self.flush_waiters.drain(..) {
                let _ = waiter.send(Ok(()));
            }
        }
    }
}

async fn create_server_connection(
    runtime_handle: Arc<RuntimeHandle>,
    dht: HyperDhtHandle,
    local_stream_id: u32,
    remote_udx: &UdxInfo,
    client_address: &Ipv4Peer,
    noise_result: &peeroxide_dht::noise_wrap::NoiseWrapResult,
) -> Result<(PeerConnection, UdxRuntime), SwarmError> {
    let runtime = UdxRuntime::shared(runtime_handle);

    let remote_id = u32::try_from(remote_udx.id).map_err(|_| {
        SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::StreamEstablishment(
            "remote UDX id out of u32 range".into(),
        ))
    })?;

    let addr: std::net::SocketAddr = std::net::SocketAddr::new(
        client_address.host.parse().map_err(|_| {
            SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::StreamEstablishment(
                "invalid client address".into(),
            ))
        })?,
        client_address.port,
    );

    let socket = dht
        .listen_socket()
        .await
        .map_err(SwarmError::Dht)?
        .ok_or_else(|| {
            SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::StreamEstablishment(
                "DHT listen socket not available".into(),
            ))
        })?;

    let stream = runtime.create_stream(local_stream_id).await?;
    stream.connect(&socket, remote_id, addr).await?;

    let async_stream = stream.into_async_stream();
    let ss = SecretStream::from_session(
        false,
        async_stream,
        noise_result.tx,
        noise_result.rx,
        noise_result.handshake_hash,
        noise_result.remote_public_key,
    )
    .await
    .map_err(|e| SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::SecretStream(e)))?;

    let conn =
        PeerConnection::with_remote_addr(ss, noise_result.remote_public_key, addr, socket, None);
    Ok((conn, runtime))
}

#[allow(clippy::too_many_arguments)]
async fn create_server_relay_connection(
    runtime_handle: Arc<RuntimeHandle>,
    dht: HyperDhtHandle,
    key_pair: KeyPair,
    relay_pk: [u8; 32],
    relay_addr: Option<std::net::SocketAddr>,
    token: [u8; 32],
    local_stream_id: u32,
    noise_result: peeroxide_dht::noise_wrap::NoiseWrapResult,
) -> Result<(PeerConnection, UdxRuntime), SwarmError> {
    use peeroxide_dht::blind_relay::BlindRelayClient;
    use peeroxide_dht::protomux::Mux;

    let runtime = UdxRuntime::shared(runtime_handle);

    // 1. HyperDHT connection to the relay node (control channel).
    // Use direct address when available, fall back to DHT routing.
    let connect_fut: std::pin::Pin<
        Box<
            dyn std::future::Future<
                    Output = Result<PeerConnection, peeroxide_dht::hyperdht::HyperDhtError>,
                > + Send,
        >,
    > = if let Some(addr) = relay_addr {
        tracing::debug!(?addr, "server: connecting to relay at known address");
        Box::pin(dht.connect_to(&key_pair, relay_pk, addr, &runtime))
    } else {
        Box::pin(dht.connect(&key_pair, relay_pk, &runtime))
    };
    let relay_conn = tokio::time::timeout(std::time::Duration::from_secs(15), connect_fut)
        .await
        .map_err(|_| {
            SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::HandshakeFailed(
                "relay connect timeout".into(),
            ))
        })?
        .map_err(SwarmError::Dht)?;

    let relay_addr = relay_conn.remote_addr.ok_or_else(|| {
        SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::StreamEstablishment(
            "relay connection has no remote_addr".into(),
        ))
    })?;

    // 2. Protomux over the control channel.
    let (mux, mux_run) = Mux::new(relay_conn.stream);
    let mux_task = tokio::spawn(mux_run);

    // 3. Open blind-relay client + pair as initiator (server initiates pairing).
    // Channel id = our public key (must match relay server's `id: socket.remotePublicKey`).
    let mut relay_client = BlindRelayClient::open(&mux, Some(key_pair.public_key.to_vec()))
        .await
        .map_err(|e| SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::Relay(e)))?;
    relay_client
        .wait_opened()
        .await
        .map_err(|e| SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::Relay(e)))?;

    let pair_response = relay_client
        .pair(true, &token, u64::from(local_stream_id))
        .await
        .map_err(|e| SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::Relay(e)))?;

    let remote_id = u32::try_from(pair_response.remote_id).map_err(|_| {
        SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::StreamEstablishment(
            "relay remote_id out of u32 range".into(),
        ))
    })?;

    // 4. Connect data UDX stream through the relay, reusing the control
    //    channel's socket so the relay sees traffic from the same source address.
    let data_stream = runtime.create_stream(local_stream_id).await?;
    data_stream
        .connect(&relay_conn.socket, remote_id, relay_addr)
        .await?;

    // 5. Wrap with SecretStream using the original Noise keys.
    // Server is responder (is_initiator=false) in the Noise handshake.
    let async_stream = data_stream.into_async_stream();
    let ss = SecretStream::from_session(
        false,
        async_stream,
        noise_result.tx,
        noise_result.rx,
        noise_result.handshake_hash,
        noise_result.remote_public_key,
    )
    .await
    .map_err(|e| SwarmError::Dht(peeroxide_dht::hyperdht::HyperDhtError::SecretStream(e)))?;

    let conn = PeerConnection::with_remote_addr(
        ss,
        noise_result.remote_public_key,
        relay_addr,
        relay_conn.socket,
        Some(mux_task),
    );
    Ok((conn, runtime))
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn retry_delay_first_attempt_unproven() {
        let mut info = PeerInfo::new([0u8; 32], vec![]);
        info.attempts = 0;
        let d = retry_delay(&info);
        // Tier M: 5000..6750 ms (unproven, idx = min(0+1, 3) = 1)
        assert!(d.as_millis() >= 5000);
        assert!(d.as_millis() < 7000);
    }

    #[test]
    fn retry_delay_first_attempt_proven() {
        let mut info = PeerInfo::new([0u8; 32], vec![]);
        info.attempts = 0;
        info.proven = true;
        let d = retry_delay(&info);
        // Tier S: 1000..1400 ms (proven, idx = min(0, 3) = 0)
        assert!(d.as_millis() >= 1000);
        assert!(d.as_millis() < 1500);
    }

    #[test]
    fn retry_delay_many_attempts() {
        let mut info = PeerInfo::new([0u8; 32], vec![]);
        info.attempts = 10;
        let d = retry_delay(&info);
        // Tier X: 600_000..705_000 ms (idx capped at 3)
        assert!(d.as_millis() >= 600_000);
        assert!(d.as_millis() < 710_000);
    }

    #[test]
    fn short_hex_format() {
        let bytes = [0xde, 0xad, 0xbe, 0xef, 0x00, 0x11, 0x22, 0x33];
        assert_eq!(short_hex(&bytes), "deadbeef");
    }

    #[test]
    fn bounded_work_queue_refuses_excess_candidates() {
        let mut queue = Vec::new();
        assert!(bounded_push(&mut queue, [0x01; 32], 2));
        assert!(bounded_push(&mut queue, [0x02; 32], 2));
        assert!(!bounded_push(&mut queue, [0x03; 32], 2));
        assert_eq!(queue, vec![[0x01; 32], [0x02; 32]]);
    }

    #[test]
    fn due_retries_are_drained_without_background_sleepers() {
        let now = Instant::now();
        let due_key = [0x31; 32];
        let later_key = [0x32; 32];
        let mut retries = vec![
            RetrySchedule {
                public_key: due_key,
                due_at: now,
            },
            RetrySchedule {
                public_key: later_key,
                due_at: now + Duration::from_secs(1),
            },
        ];

        assert_eq!(drain_due_retries(&mut retries, now), vec![due_key]);
        assert_eq!(retries.len(), 1);
        assert_eq!(retries[0].public_key, later_key);
    }

    #[test]
    fn public_dht_limits_are_fixed_even_with_large_user_configuration() {
        assert!(MAX_TOTAL_PEERS >= DEFAULT_MAX_PEERS);
        assert!(MAX_PARALLEL_CONNECTS >= DEFAULT_MAX_PARALLEL);
        assert!(MAX_TRACKED_UNCONNECTED_PEERS >= MAX_DISCOVERY_QUEUE);
        assert_eq!(MAX_TRACKED_UNCONNECTED_PEERS, MAX_PENDING_RETRIES);
        assert!(CONNECT_ATTEMPT_TIMEOUT < PEER_RECORD_TTL);
    }

    #[test]
    fn default_config() {
        let c = SwarmConfig::default();
        assert!(c.key_pair.is_none());
        assert_eq!(c.max_peers, 64);
        assert_eq!(c.max_parallel, 3);
        assert_eq!(c.firewall, 0);
        assert_eq!(c.server_expected_remote_static_key, None);
    }

    #[test]
    fn server_handshake_admission_allows_unpinned_and_matching_static_keys() {
        let remote_key = [0xA1; 32];

        assert_eq!(
            server_handshake_admission(None, remote_key),
            ServerHandshakeAdmission::Accept
        );
        assert_eq!(
            server_handshake_admission(Some(remote_key), remote_key),
            ServerHandshakeAdmission::Accept
        );
    }

    #[test]
    fn mismatched_static_key_is_rejected_before_reply_or_connection_allocation() {
        let expected_key = [0xB1; 32];
        let authenticated_remote_key = [0xB2; 32];
        let mut allocation_attempted = false;

        // `handle_server_handshake` puts its first post-admission allocation
        // boundary (the stream id) behind this gate. A mismatch must not
        // invoke it, and so cannot reach the subsequent reply payload,
        // registration, establishment task, SecretStream, or conn_tx path.
        assert_eq!(
            after_server_handshake_admission(Some(expected_key), authenticated_remote_key, || {
                allocation_attempted = true;
            },),
            None
        );
        assert!(!allocation_attempted);
    }

    #[test]
    fn default_join_opts() {
        let j = JoinOpts::default();
        assert!(j.server);
        assert!(j.client);
    }

    #[test]
    fn dropped_connection_registration_releases_only_its_slot() {
        let (lifecycle_tx, mut lifecycle_rx) = mpsc::unbounded_channel();
        let public_key = [0xD0; 32];
        let registration_id = 7;
        let registration = ConnectionRegistration {
            public_key,
            registration_id,
            lifecycle_tx,
        };
        let mut connections = ConnectionSet::new();
        connections.add(public_key, ConnectionInfo { registration_id });

        drop(registration);

        let event = lifecycle_rx
            .try_recv()
            .expect("dropping a delivered or rejected connection must notify the actor");
        let ConnectionLifecycleEvent::Closed {
            public_key,
            registration_id,
        } = event;
        assert!(connections.remove_if_matches(&public_key, registration_id));
        assert_eq!(connections.len(), 0);
    }

    #[test]
    fn outbound_duplicate_drops_candidate_and_preserves_delivered_registration() {
        struct CandidateDropProbe(Arc<std::sync::atomic::AtomicUsize>);

        impl Drop for CandidateDropProbe {
            fn drop(&mut self) {
                self.0.fetch_add(1, Ordering::SeqCst);
            }
        }

        let (lifecycle_tx, mut lifecycle_rx) = mpsc::unbounded_channel();
        let public_key = [0xD2; 32];
        let delivered_registration_id = 41;
        let delivered_registration = ConnectionRegistration {
            public_key,
            registration_id: delivered_registration_id,
            lifecycle_tx,
        };
        let mut connections = ConnectionSet::new();
        connections.add(
            public_key,
            ConnectionInfo {
                registration_id: delivered_registration_id,
            },
        );
        let candidate_drops = Arc::new(std::sync::atomic::AtomicUsize::new(0));

        assert!(
            admit_outbound_connection(
                &connections,
                &public_key,
                CandidateDropProbe(candidate_drops.clone()),
            )
            .is_none(),
            "a late outbound completion must be discarded once its peer is application-owned"
        );

        assert_eq!(
            candidate_drops.load(Ordering::SeqCst),
            1,
            "discarding the duplicate must release the completed candidate"
        );
        assert_eq!(
            connections.len(),
            1,
            "discarding a duplicate must not consume another capacity slot"
        );
        assert_eq!(
            connections
                .get(&public_key)
                .expect("the delivered connection must remain registered")
                .registration_id,
            delivered_registration_id,
            "the delivered connection generation must not be replaced"
        );
        assert!(
            lifecycle_rx.try_recv().is_err(),
            "discarding a raw duplicate must not release the delivered registration"
        );

        drop(delivered_registration);
        assert_eq!(
            lifecycle_rx
                .try_recv()
                .expect("the delivered connection must retain its normal RAII release"),
            ConnectionLifecycleEvent::Closed {
                public_key,
                registration_id: delivered_registration_id,
            }
        );
        assert!(connections.remove_if_matches(&public_key, delivered_registration_id));
        assert_eq!(connections.len(), 0);
    }

    #[tokio::test]
    async fn shutdown_cancels_pending_establishment_and_releases_reservation() {
        let (lifecycle_tx, mut lifecycle_rx) = mpsc::unbounded_channel();
        let public_key = [0xE1; 32];
        let registration_id = 9;
        let registration = ConnectionRegistration {
            public_key,
            registration_id,
            lifecycle_tx,
        };
        let (started_tx, started_rx) = oneshot::channel();
        let mut tasks = ConnectionEstablishmentTasks::default();
        tasks.spawn(async move {
            let _registration = registration;
            let _ = started_tx.send(());
            std::future::pending::<()>().await;
        });

        started_rx
            .await
            .expect("pending establishment task must start before shutdown");
        tokio::time::timeout(Duration::from_secs(1), tasks.shutdown())
            .await
            .expect("shutdown must cancel and drain pending establishment tasks");
        assert!(tasks.is_empty());

        let event = lifecycle_rx
            .try_recv()
            .expect("cancelling a pending establishment task must drop its reservation");
        assert_eq!(
            event,
            ConnectionLifecycleEvent::Closed {
                public_key,
                registration_id,
            }
        );
    }

    #[tokio::test]
    async fn shutdown_cancels_and_drains_pending_outbound_connects() {
        let mut tasks = OutboundConnectTasks::default();
        tasks.spawn([0xE2; 32], async {
            std::future::pending::<ConnectAttemptResult>().await
        });

        tokio::task::yield_now().await;
        tokio::time::timeout(Duration::from_secs(1), tasks.shutdown())
            .await
            .expect("shutdown must abort and drain every outbound connect task");
        assert!(tasks.is_empty());
        assert!(tasks.peers.is_empty());
    }

    #[tokio::test]
    async fn starting_shutdown_drains_prebootstrap_dht() {
        let startup = spawn_starting(SwarmConfig::default())
            .await
            .expect("start swarm without waiting for bootstrap");

        tokio::time::timeout(Duration::from_secs(5), startup.shutdown())
            .await
            .expect("pre-bootstrap shutdown must not leave owned tasks behind")
            .expect("pre-bootstrap shutdown must drain the DHT owner");
    }

    #[tokio::test]
    async fn server_unregistered_on_leave_last_topic() {
        let config = SwarmConfig::default();

        let (_task, handle, _conn_rx) = crate::spawn(config).await.unwrap();
        let target = hash(&handle.key_pair().public_key);
        let topic = [0xAA; 32];

        handle
            .join(
                topic,
                JoinOpts {
                    server: true,
                    client: false,
                },
            )
            .await
            .unwrap();

        {
            let router = handle.dht().router().lock().unwrap();
            assert!(
                router.get(&target).is_some(),
                "server must be registered after join"
            );
        }

        handle.leave(topic).await.unwrap();
        tokio::time::sleep(Duration::from_millis(50)).await;

        {
            let router = handle.dht().router().lock().unwrap();
            assert!(
                router.get(&target).is_none(),
                "server must be unregistered after leaving last server topic"
            );
        }

        handle.destroy().await.unwrap();
    }

    #[tokio::test]
    async fn server_unregistered_on_destroy() {
        let config = SwarmConfig::default();

        let (_task, handle, _conn_rx) = crate::spawn(config).await.unwrap();
        let target = hash(&handle.key_pair().public_key);
        let topic = [0xBB; 32];

        handle
            .join(
                topic,
                JoinOpts {
                    server: true,
                    client: false,
                },
            )
            .await
            .unwrap();

        {
            let router = handle.dht().router().lock().unwrap();
            assert!(
                router.get(&target).is_some(),
                "server must be registered after join"
            );
        }

        handle.destroy().await.unwrap();
        tokio::time::sleep(Duration::from_millis(50)).await;

        {
            let router = handle.dht().router().lock().unwrap();
            assert!(
                router.get(&target).is_none(),
                "server must be unregistered after destroy"
            );
        }
    }

    #[tokio::test]
    async fn server_unregistered_on_handle_drop() {
        let config = SwarmConfig::default();

        let (task, handle, _conn_rx) = crate::spawn(config).await.unwrap();
        let dht_handle = handle.dht().clone();
        let target = hash(&handle.key_pair().public_key);
        let topic = [0xCC; 32];

        handle
            .join(
                topic,
                JoinOpts {
                    server: true,
                    client: false,
                },
            )
            .await
            .unwrap();

        {
            let router = dht_handle.router().lock().unwrap();
            assert!(
                router.get(&target).is_some(),
                "server must be registered after join"
            );
        }

        drop(handle);
        drop(_conn_rx);
        let _ = tokio::time::timeout(Duration::from_secs(2), task).await;

        {
            let router = dht_handle.router().lock().unwrap();
            assert!(
                router.get(&target).is_none(),
                "server must be unregistered after implicit shutdown (handle drop)"
            );
        }
    }
}

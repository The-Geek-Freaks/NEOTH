...
use peeroxide::{JoinOpts, KeyPair, SwarmConfig, spawn};
...
async fn main() {
    ...
    println!("[server] pk: {}", hex::encode(&server_pk[..8]));
    ...
    println!("[client] pk: {}", hex::encode(&client_kp.public_key[..8]));
    ...
    println!(
        ...
        hex::encode(&server_conn.remote_public_key()[..8])
    );
    println!(
        "[client] got connection to {}",
        hex::encode(&client_conn.remote_public_key()[..8])
    );

    // 5. Exchange data through the relay.
    let client_msg = b"hello through relay from client";
    client_conn.write(client_msg).await.expect("client write");
    println!("[client] sent: {}", String::from_utf8_lossy(client_msg));

    let server_recv = tokio::time::timeout(Duration::from_secs(10), server_conn.read())
        .await
        .expect("server read timeout")
        .expect("server read error")
        .expect("server got EOF");
    println!("[server] recv: {}", String::from_utf8_lossy(&server_recv));
    assert_eq!(server_recv, client_msg);

    let server_msg = b"hello through relay from server";
    server_conn.write(server_msg).await.expect("server write");
    println!("[server] sent: {}", String::from_utf8_lossy(server_msg));

    let client_recv = tokio::time::timeout(Duration::from_secs(10), client_conn.read())
        .await
        .expect("client read timeout")
        .expect("client read error")
        .expect("client got EOF");
    println!("[client] recv: {}", String::from_utf8_lossy(&client_recv));

...
use peeroxide::{JoinOpts, KeyPair, SwarmConfig, SwarmConnection, spawn};
...
async fn handle_peer(mut conn: SwarmConnection, my_id: String) {
    ...
    if let Err(e) = conn.write(greeting.as_bytes()).await {
    ...
    loop {
        match conn.read().await {
            Ok(Some(data)) => {
                ...
                if let Err(e) = conn.write(reply.as_bytes()).await {
                    println!("    [{remote}] send failed: {e}");
                    break;
                }
            }

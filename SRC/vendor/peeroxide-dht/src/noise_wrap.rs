...
use crate::hyperdht_messages::{
    NoisePayload, decode_noise_payload_from_bytes, encode_noise_payload_to_bytes,
};
...
pub struct NoiseWrap {
    ...
    is_initiator: bool,
    // The responder records this only after `recv` has both authenticated and
    // decoded M1.  Server admission can therefore inspect the initiator's
    // static key before it commits to the responder's reply or any transport
    // resources.
    authenticated_remote_static_key: Option<[u8; 32]>,
}
...
impl NoiseWrap {
    ...
    pub fn new_initiator(keypair: Keypair, remote_public_key: [u8; 32]) -> Self {
        let handshake = HandshakeIK::new_initiator(keypair, remote_public_key, &*NS_PEER_HANDSHAKE);
        NoiseWrap {
            handshake,
            is_initiator: true,
            authenticated_remote_static_key: None,
        }
    }
    ...
    pub fn recv(&mut self, buf: &[u8]) -> Result<NoisePayload, NoiseWrapError> {
        ...
        if !self.is_initiator {
            self.authenticated_remote_static_key = self.handshake.remote_static_key().copied();
        }
        Ok(payload)
    }

    /// Returns the responder-authenticated initiator static key after a
    /// successful [`recv`](Self::recv) of the initiator's first message.
    ///
    /// This is `None` for initiators and before a responder has successfully
    /// received and decoded M1. It permits an inbound admission policy to
    /// reject an unexpected peer before the responder sends its handshake
    /// reply or allocates transport resources.
    pub fn authenticated_remote_static_key(&self) -> Option<[u8; 32]> {
        self.authenticated_remote_static_key
    }
...
mod tests {
    ...
    #[test]
    fn responder_exposes_authenticated_remote_static_after_successful_recv() {
        let seed_init = [0x51u8; 32];
        let seed_resp = [0x52u8; 32];
        let seed_e_init = [0x53u8; 32];
        ...
        init.set_ephemeral(keypair_from_seed(&seed_e_init));
        assert_eq!(resp.authenticated_remote_static_key(), None);

        let m1 = init.send(&minimal_payload(1)).unwrap();
        resp.recv(&m1).unwrap();

        assert_eq!(resp.authenticated_remote_static_key(), Some(init_pub));
        assert_eq!(init.authenticated_remote_static_key(), None);
    }
    ...
    fn payload_with_all_fields_roundtrips() {
        ...
        assert_eq!(
            decoded.relay_through.as_ref().unwrap().public_key,
            [0xBBu8; 32]
        );
    }
    ...
    fn holepunch_secret_is_keyed_blake2b() {
        ...
        let mut mac: Blake2bMac256 = KeyInit::new_from_slice(&hash[..]).expect("64-byte key");
        Mac::update(&mut mac, &*NS_PEER_HOLEPUNCH);

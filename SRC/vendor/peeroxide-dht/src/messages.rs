...
// DHT packets are datagrams. A four-figure node list is already beyond any
// useful response size, and is a deliberate ceiling before a decoded count
// can influence allocation.
...
fn decode_ipv4_array(state: &mut State) -> Result<Vec<Ipv4Peer>, EncodingError> {
    let raw_len = compact_encoding::decode_uint(state)?;
    ...
    let minimum_required = len * MIN_ENCODED_IPV4_PEER_BYTES;
...
mod tests {
    ...
    #[test]
    fn response_accepts_closer_nodes_at_collection_cap() {
        let res = Response {
            tid: 500,
            to: Ipv4Peer {
                host: "192.168.1.1".into(),
            ...
            id: None,
            token: None,
            closer_nodes: (0..MAX_WIRE_COLLECTION_ELEMENTS)
                .map(|_| Ipv4Peer {
                    host: "10.0.0.1".into(),
                    port: 8080,
                })
        ...
        let bytes = encode_response_to_bytes(&res).unwrap();
        let Message::Response(decoded) = decode_message(&bytes).unwrap() else {
            panic!("expected response");
        };
        assert_eq!(decoded.closer_nodes.len(), MAX_WIRE_COLLECTION_ELEMENTS);
    }

    #[test]
    fn response_rejects_short_or_excess_closer_node_counts_before_allocation() {
        // Response header + a cap-sized closer-node count but no nodes. This
        // exercises the remaining-input proof before Vec::with_capacity.
        let mut short = vec![
            RESPONSE_ID,
            FLAG_INTERNAL_OR_CLOSER,
            0,
            0,
            127,
            0,
        ...
        short.extend([0xfd, 0x00, 0x04]);
        assert!(matches!(
            decode_message(&short),
            Err(EncodingError::OutOfBounds { .. })
        ));

        let mut excessive = short[..10].to_vec();
        excessive.extend([0xff; 9]);
        assert!(matches!(

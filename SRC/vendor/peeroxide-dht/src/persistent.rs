...
use crate::hyperdht_messages::{
    HyperPeer, LookupRawReply, MutableGetResponse, decode_announce_from_bytes,
    decode_mutable_get_response_from_bytes, decode_mutable_put_request_from_bytes,
    encode_hyper_peer_to_bytes, encode_lookup_raw_reply_to_bytes,
    encode_mutable_get_response_to_bytes,
};
...
/// Hard bound for verified self-announce routing records retained by one DHT
/// node.  These records are remotely supplied, so they must not share the
/// unbounded lifetime that a local server registration has in `Router`.
const MAX_SELF_ANNOUNCE_ROUTER_ENTRIES: usize = 4_096;
/// Self-announce routing records are refreshed by a subsequent valid
/// announce.  Their lifetime matches the rest of the HyperDHT announcement
/// state instead of persisting indefinitely after one valid packet.
const SELF_ANNOUNCE_ROUTER_TTL: Duration = Duration::from_secs(20 * 60);
...
impl RecordCache {
    ...
    pub fn add(&mut self, key: &str, public_key: [u8; 32], record: Vec<u8>) {
        ...
        if bucket_len >= self.max_per_key {
            if let Some(oldest_idx) = self.entries.get(&key_str).and_then(|b| {
                b.iter()
                    .enumerate()
...
impl Persistent {
    ...
    /// Physically remove expired self-announce routing records.
    ///
    /// This is intentionally separate from lookup so the request-handler's
    /// periodic GC can reclaim memory even when a topic is never queried.
    pub(crate) fn gc(&mut self) {
        let now = Instant::now();
        self.router
            .retain(|_, entry| Self::router_entry_is_live(entry, now));
    }

    fn router_entry_is_live(entry: &RouterEntry, now: Instant) -> bool {
        now.duration_since(entry.inserted_at) < SELF_ANNOUNCE_ROUTER_TTL
    }

    /// Insert or refresh a verified self-announce routing record.
    ///
    /// Existing targets are always refreshable at capacity. New targets are
    /// rejected after GC instead of evicting a live, already-authorized
    /// announce record.
    fn insert_router_entry(&mut self, target: String, entry: RouterEntry) -> bool {
        self.gc();

        if let Some(existing) = self.router.get_mut(&target) {
            *existing = entry;
            return true;
        }

        if self.router.len() >= MAX_SELF_ANNOUNCE_ROUTER_ENTRIES {
            return false;
        }

        self.router.insert(target, entry);
        true
    }
    ...
    pub fn on_find_peer(&self, req: &IncomingHyperRequest) -> HandlerReply {
        ...
        if let Some(entry) = self
            .router
            .get(&key)
            .filter(|entry| Self::router_entry_is_live(entry, Instant::now()))
        {
    ...
    pub fn on_lookup(&mut self, req: &IncomingHyperRequest) -> HandlerReply {
        ...
        let mut peers: Vec<HyperPeer> = self
            ...
            .into_iter()
            .filter_map(|bytes| crate::hyperdht_messages::decode_hyper_peer_from_bytes(&bytes).ok())
            .collect();
    ...
    pub fn on_announce(&mut self, req: &IncomingHyperRequest, node_id: &[u8; 32]) -> HandlerReply {
        self.gc();
        let target = match &req.target {
            Some(t) => *t,
            None => return HandlerReply::Silent,
        ...
        let refresh_bytes: &[u8] = msg.refresh.as_ref().map(|r| r.as_slice()).unwrap_or(&[]);
        ...
        if announce_self {
            ...
            if !self.insert_router_entry(
                ...
                // Do not report admission when the bounded routing cache
                // could not retain this new target. In particular, never
                // evict a live verified self-announce to make room for it.
                return HandlerReply::Silent;
            }
...
mod tests {
    ...
    fn on_announce_invalid_signature_is_silent() {
        ...
        let ann_bytes = crate::hyperdht_messages::encode_announce_to_bytes(&ann).unwrap();
    ...
    fn valid_self_announce(index: u64, node_id: [u8; 32]) -> ([u8; 32], IncomingHyperRequest) {
        let mut seed = [0u8; 32];
        ...
        let peer_encoded = encode_hyper_peer_to_bytes(&peer).unwrap();
        let signature = crate::crypto::sign_detached(
            &ann_signable(&target, &token, &node_id, &peer_encoded, &[], &NS_ANNOUNCE),
            &secret_key,
        );
        let announcement = crate::hyperdht_messages::AnnounceMessage {
            peer: Some(peer),
            refresh: None,
        ...
        (
            target,
            IncomingHyperRequest {
                command: crate::hyperdht_messages::ANNOUNCE,
                target: Some(target),
                token: Some(token),
                value: Some(
                    crate::hyperdht_messages::encode_announce_to_bytes(&announcement).unwrap(),
                ),
                from: dummy_peer(),
                id: Some(node_id),
            },
    ...
    #[test]
    fn self_announce_router_is_bounded_refreshable_and_recovers_after_gc() {
        let mut persistent = make_persistent();
        let node_id = [0x11u8; 32];

        // Each request is a separately signed, valid self-announce with a
        // distinct target. More than the hard cap must not grow the map.
        for index in 0..MAX_SELF_ANNOUNCE_ROUTER_ENTRIES as u64 {
            let (_target, request) = valid_self_announce(index, node_id);
            assert!(matches!(
                persistent.on_announce(&request, &node_id),
                HandlerReply::ValueNoToken(None)
            ));
        }
        assert_eq!(persistent.router.len(), MAX_SELF_ANNOUNCE_ROUTER_ENTRIES);

        let (overflow_target, overflow_request) =
            valid_self_announce(MAX_SELF_ANNOUNCE_ROUTER_ENTRIES as u64, node_id);
        assert!(matches!(
            persistent.on_announce(&overflow_request, &node_id),
            HandlerReply::Silent
        ));
        assert_eq!(
            persistent.router.len(),
            MAX_SELF_ANNOUNCE_ROUTER_ENTRIES,
            "a rejected unique target must not grow or evict the live cache"
        );
        let (refresh_target, refresh_request) = valid_self_announce(0, node_id);
        assert!(
            persistent.router.contains_key(&to_hex(&refresh_target)),
            "a rejected new target must not evict an existing valid self-announce"
        );

        // A valid refresh of a live target remains admissible at capacity.
        assert!(matches!(
            persistent.on_announce(&refresh_request, &node_id),
            HandlerReply::ValueNoToken(None)
        ));
        assert_eq!(persistent.router.len(), MAX_SELF_ANNOUNCE_ROUTER_ENTRIES);

        // Make one legitimate entry expire without looking it up, then run
        // production-equivalent GC. The previously rejected target can now be
        // admitted without evicting any still-live record.
        persistent
            .router
            .get_mut(&to_hex(&refresh_target))
            .unwrap()
            .inserted_at = Instant::now() - SELF_ANNOUNCE_ROUTER_TTL - Duration::from_secs(1);
        persistent.gc();
        assert_eq!(
            persistent.router.len(),
            MAX_SELF_ANNOUNCE_ROUTER_ENTRIES - 1
        );
        assert!(matches!(
            persistent.on_announce(&overflow_request, &node_id),
            HandlerReply::ValueNoToken(None)
        ));
        assert!(persistent.router.contains_key(&to_hex(&overflow_target)));
        assert_eq!(persistent.router.len(), MAX_SELF_ANNOUNCE_ROUTER_ENTRIES);
    }

use crate::compact_encoding::{
    self, EncodingError, State, decode_fixed32, decode_fixed64, decode_uint, encode_fixed32,
    encode_fixed64, encode_uint, preencode_uint,
};
...
// Wire collection limits are intentionally well below the historical 1 MiB
// element cap. HyperDHT messages normally travel in bounded datagrams, so a
// four-figure element count is already more than a peer can use operationally.
// More importantly, every decoder validates both this cap and the bytes that
// must still be present before allocating its output vector.
const MAX_WIRE_COLLECTION_ELEMENTS: usize = 1_024;
const MIN_ENCODED_HYPER_PEER_BYTES: usize = 33; // 32-byte key + empty relay array count
const MIN_ENCODED_IPV4_PEER_BYTES: usize = 6; // 4-byte address + 2-byte port
const MIN_ENCODED_IPV6_PEER_BYTES: usize = 18; // 16-byte address + 2-byte port
const MIN_ENCODED_RELAY_INFO_BYTES: usize = 12; // two IPv4 peers

/// Decodes a collection length without allowing an attacker-controlled count
/// to reserve memory before the rest of the wire buffer proves it can contain
/// that many minimum-size elements.
fn decode_bounded_collection_len(state: &mut State, min_element_bytes: usize) -> Result<usize> {
    debug_assert!(min_element_bytes > 0);

    let raw_len = decode_uint(state)?;
    let len = usize::try_from(raw_len).map_err(|_| EncodingError::ArrayTooLarge(usize::MAX))?;
    if len > MAX_WIRE_COLLECTION_ELEMENTS {
        return Err(EncodingError::ArrayTooLarge(len));
    }

    let remaining = state.end.saturating_sub(state.start);
    let minimum_required = len * min_element_bytes;
    if minimum_required > remaining {
        return Err(EncodingError::OutOfBounds {
            need: minimum_required,
            have: remaining,
        });
    }
...
pub fn decode_lookup_raw_reply(state: &mut State) -> Result<LookupRawReply> {
    let count = decode_bounded_collection_len(state, MIN_ENCODED_HYPER_PEER_BYTES)?;
...
fn decode_ipv4_peer_array(state: &mut State) -> Result<Vec<Ipv4Peer>> {
    let len = decode_bounded_collection_len(state, MIN_ENCODED_IPV4_PEER_BYTES)?;
...
fn decode_ipv6_peer_array(state: &mut State) -> Result<Vec<Ipv4Peer>> {
    let len = decode_bounded_collection_len(state, MIN_ENCODED_IPV6_PEER_BYTES)?;
    let mut peers = Vec::with_capacity(len);
    for _ in 0..len {
...
fn decode_relay_info_array(state: &mut State) -> Result<Vec<RelayInfo>> {
    let len = decode_bounded_collection_len(state, MIN_ENCODED_RELAY_INFO_BYTES)?;
...
mod tests {
    ...
    fn count_only_state(count: u64) -> State {
        let mut state = State::new();
        preencode_uint(&mut state, count);
        state.alloc();
        encode_uint(&mut state, count);
        State::from_buffer(&state.buffer)
    }
    ...
    #[test]
    fn lookup_raw_reply_accepts_collection_cap() {
        let m = LookupRawReply {
            peers: (0..MAX_WIRE_COLLECTION_ELEMENTS)
                .map(|index| make_peer(index as u8))
                .collect(),
            bump: 7,
        };
        let bytes = encode_lookup_raw_reply_to_bytes(&m).unwrap();
        let decoded = decode_lookup_raw_reply_from_bytes(&bytes).unwrap();
        assert_eq!(decoded.peers.len(), MAX_WIRE_COLLECTION_ELEMENTS);
        assert_eq!(decoded.bump, 7);
    }

    #[test]
    fn collection_decoders_reject_short_or_excess_counts_before_allocation() {
        // Every count below the named cap deliberately has zero element bytes.
        // The decoder must reject that malformed buffer before Vec::with_capacity.
        let mut state = count_only_state(MAX_WIRE_COLLECTION_ELEMENTS as u64);
        assert!(matches!(
            decode_lookup_raw_reply(&mut state),
        ...
        let mut state = count_only_state(MAX_WIRE_COLLECTION_ELEMENTS as u64);
        assert!(matches!(
            decode_ipv4_peer_array(&mut state),
            Err(EncodingError::OutOfBounds { .. })
        ));

        let mut state = count_only_state(MAX_WIRE_COLLECTION_ELEMENTS as u64);
        assert!(matches!(
            decode_ipv6_peer_array(&mut state),
            Err(EncodingError::OutOfBounds { .. })
        ));

        let mut state = count_only_state(MAX_WIRE_COLLECTION_ELEMENTS as u64);
        assert!(matches!(
            decode_relay_info_array(&mut state),
            Err(EncodingError::OutOfBounds { .. })
        ));

        // A maximal compact integer exercises the cap check itself and must
        // likewise fail without reserving memory based on the wire value.
        let mut state = count_only_state(u64::MAX);
        assert!(matches!(
            decode_lookup_raw_reply(&mut state),
            Err(EncodingError::ArrayTooLarge(_))
        ));
    ...
    #[test]
    fn ipv4_ipv6_and_relay_info_arrays_accept_collection_cap() {
        let ipv4_peers: Vec<_> = (0..MAX_WIRE_COLLECTION_ELEMENTS)
            .map(|_| make_addr("10.0.0.1", 8080))
            .collect();
        let mut ipv4_state = State::new();
        preencode_ipv4_peer_array(&mut ipv4_state, &ipv4_peers);
        ipv4_state.alloc();
        encode_ipv4_peer_array(&mut ipv4_state, &ipv4_peers).unwrap();
        let mut decoded_state = State::from_buffer(&ipv4_state.buffer);
        assert_eq!(
            decode_ipv4_peer_array(&mut decoded_state).unwrap(),
            ipv4_peers
        );

        let ipv6_peers: Vec<_> = (0..MAX_WIRE_COLLECTION_ELEMENTS)
            .map(|_| make_addr("2001:db8::1", 8080))
            .collect();
        let mut ipv6_state = State::new();
        preencode_ipv6_peer_array(&mut ipv6_state, &ipv6_peers);
        ipv6_state.alloc();
        encode_ipv6_peer_array(&mut ipv6_state, &ipv6_peers).unwrap();
        let mut decoded_state = State::from_buffer(&ipv6_state.buffer);
        assert_eq!(
            decode_ipv6_peer_array(&mut decoded_state).unwrap(),
            ipv6_peers
        );

        let relays: Vec<_> = (0..MAX_WIRE_COLLECTION_ELEMENTS)
            .map(|_| RelayInfo {
                relay_address: make_addr("10.0.0.1", 8080),
                peer_address: make_addr("192.168.1.1", 3000),
            })
            .collect();
        let mut relay_state = State::new();
        preencode_relay_info_array(&mut relay_state, &relays);
        relay_state.alloc();
        encode_relay_info_array(&mut relay_state, &relays).unwrap();
        let mut decoded_state = State::from_buffer(&relay_state.buffer);
        assert_eq!(decode_relay_info_array(&mut decoded_state).unwrap(), relays);
    }
    ...
    fn holepunch_payload_with_addresses() {
        let m = HolepunchPayload {
            ...
            connected: false,
            punching: true,
            addresses: Some(vec![make_addr("1.2.3.4", 1000), make_addr("5.6.7.8", 2000)]),

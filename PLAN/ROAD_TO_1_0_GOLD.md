...
- [ ] **GOLD-SEC-07** Rewrite `is_local_endpoint` in `installers/omi.rs` to parse host and check loopback (127.x, ::1) and RFC-1918 (10/8, 172.16/12, 192.168/16), then call it again at ingest-task startup — *files:* `SRC/neothd/src/installers/omi.rs` — *test:* `http://evil.com` rejected; `http://192.168.1.50` accepted — *origin:* A-19
- [ ] **GOLD-SEC-08** Add LRU eviction (or sweep entries older than `REFILL_PERIOD`) to `rate_limit.rs::allow` to prevent unbounded HashMap growth — *files:* `SRC/neothd/src/rate_limit.rs` — *test:* after 10 000 unique senders, map size stays bounded — *origin:* A-18
- [ ] **GOLD-SEC-09** Add `is_cloud_egress() -> bool` to `ProviderKind` and replace the partial match arms in `init.rs`, `jobs.rs`, and `serve.rs` with it; include `AnthropicApi` and `Cohere` — *files:* `SRC/neothd/src/config/mod.rs` (or `providers/mod.rs`), `SRC/neothd/src/installers/omi.rs` — *test:* `AnthropicApi.is_cloud_egress()` returns true — *origin:* A-25
...
- [ ] **GOLD-SEC-27** Add plaintext-secrets warning at backup creation when `credentials.yaml` is included; make credentials.yaml opt-in until age-encryption lands — *files:* `SRC/neothd/src/daemon/backup.rs` — *test:* backup creation prints warning; `--no-credentials` flag excludes the file — *origin:* A-73
- [ ] **GOLD-SEC-28** Fix GDPR erasure cascade: `forget_by_topic` must also DELETE from `idx_profile` table — *files:* `SRC/neothd/src/memory/forget.rs` — *test:* after `neoth memory forget <topic>`, `idx_profile` has no matching rows — *origin:* CR-007
...
- [ ] **GOLD-SEC-34** Add `ESCAPE '\\'` to `LIKE` queries in `memory/forget.rs` and all other LIKE-based memory queries; add shared `escape_like` helper — *files:* `SRC/neothd/src/memory/forget.rs` — *test:* (consolidated with GOLD-SEC-04) — *origin:* A-08 (consolidated)

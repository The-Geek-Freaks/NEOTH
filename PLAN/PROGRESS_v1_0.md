...
- [~] **M-07** Memory integration test suite under `SRC/neothd/tests/memory_*.rs`. **A1 top-priority gap #1; A2 sequence: must ship before M-09 and OM-01.** — 4d — ⚙ 2026-05-25 Session 24 — 25 tests shipped across 4 files (`memory_lifecycle.rs` 7 + `memory_reinforce_audit.rs` 5 + `memory_groundtruth_forget.rs` 6 + `memory_profile_pipeline.rs` 7). Covers: WAL → indexer → idx_episode round-trip / indexer idempotence under repeat / consolidation pass walks every tier in one call (hot decay + warm migration + cold sweep all visible in `PassReport`) / consolidation no-op when caught up / 365-pass groundtruth survival drift-guard (SPEC GT-3) / tier-aware reinforce dispatch round-trip on all 3 tiers / repeated-reinforce convergence to 1.0 without overshoot / cross-tier independence (warm reinforce doesn't touch hot row at same event_id) / forget revokes-but-keeps groundtruth + revoked_at stamp + audit-addressable / forget leaves non-matching gt rows active / `apply_delta` round-trip through `top_claims_for_chat` reader / supersede path hides old value from recall (WRITE→READ contract pin) / below-min-confidence filtered by reader / idempotent on repeat extraction_id / `render_for_synthesis_prompt` carries ADV-03 boundary header verbatim / empty input renders empty string / empty extraction_id is rejected by apply_delta. **Caught + fixed one real bug in the process**: initial test expected forget to leave groundtruth fully untouched, but doc contract is REVOKE not preserve — test rewritten to match the documented contract (forget IS allowed to revoke matching gt rows; the immutability invariant is satisfied because the row stays in-table with revoked_at populated). Remaining M-07 scope (6-region view cooperation / multi-segment WAL rotation indexing / dimension cross-talk) deferred as future commits on this same lane item.

...
- [ADR-007 — `chat_turn_pipeline` module boundary](007-chat-turn-pipeline-module-boundary.md) — Accepted (future `cli/chat_turn_pipeline.rs`; four named phase fns; new logic must not inline into `run_chat_with`; extraction deferred to GOLD-ARCH-02).
- [ADR-008 — WS-D wiring gate/audit checklist](008-ws-d-wiring-gate-audit-checklist.md) — Accepted (six mandatory elements for every live-path wiring: status label, permission/gate, audit event, safe default, CLI/GUI truth, tests; N/A allowed with explicit rationale for pure-local wirings).
- [ADR-009 — INTENT/RESULT WAL audit-event pairing](009-intent-result-wal-audit-pairing.md) — Accepted (`*_INTENT` before effect + `*_RESULT` after, for OS file-write / channel egress / media cloud calls / self-update / app-launch; generalises the proven 0xF2 PRE_MUTATION_SNAPSHOT anchor with typed per-class pairs; implementation is a follow-up task).
- [ADR-010 — Daemon-owned chat runtime](010-daemon-owned-chat-runtime.md) — Accepted (`neoth serve` owns chat turns; GUI, Buddy, and daemon-aware CLI attach through the existing authenticated local endpoint; cooperative cancellation and replay replace UI-owned provider subprocesses).

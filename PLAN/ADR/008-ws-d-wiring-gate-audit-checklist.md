...
2. **Permission/gate** — `permissions::evaluate(action, level)` (permissions/mod.rs:385) maps every `Action` variant to `Decision::Allow | Confirm(reason) | Deny(reason)` for all five `AutonomyLevel` values. The match in every `evaluate_*` fn is EXHAUSTIVE — adding a new `Action` variant without a gate arm is a compile error (GOLD-COR-05, permissions/mod.rs:695–716). `LeaseScope`-backed capability leases (`permissions/lease.rs`) let the operator pre-authorise bounded subjects via `neoth lease grant`; `lease_scope_for(action)` maps each action to its coverable scope or `None` (permissions/mod.rs:330). The `PermissionToken<L>` typestate from `neoth-plugin-sdk::permission` (permissions/mod.rs:36–38) provides a compile-time gate for plugin entry points.

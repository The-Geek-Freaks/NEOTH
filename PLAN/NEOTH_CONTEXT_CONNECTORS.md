...
**Status dieses Dokuments:** verbindlicher GOLD-Architektur- und Bauvertrag;
`GOLD-CC-00` bindet diese geprüfte Entscheidung, die Runtime-Umsetzung bleibt
unter `GOLD-CC-01..12` in `PLAN/ROAD_TO_1_0_GOLD.md` offen.

> [!IMPORTANT]
> Dieses Dokument beschreibt Zielarchitektur und Abnahmekriterien. Es ist kein
> Beleg dafür, dass Connector-Control-Plane, Account-Sync, Context Graph,
> People-/Kin-Projektion oder Action Sinks bereits implementiert sind.
> `GOLD-CC-00` schließt ausschließlich nach dem dokumentarischen
> Commit-/Push-/Provenienzvertrag in `PLAN/ROAD_TO_1_0_GOLD.md`;
> `GOLD-CC-01..12` dürfen nur aktueller NEOTH-Code plus bestandene
> Contract-/Release-Gates schließen.

### Quellen- und Adoptionsbindung

| Quelle | Geprüfter Stand | Lizenz-/Einbauentscheidung |
|---|---|---|
| `Panniantong/Agent-Reach` | Commit `93ae1d18c37b707dec053c7c4f9d91cd8ef8943d` (geprüft 2026-08-13) | Upstream deklariert MIT. Architektur-/UX-Referenz für beobachtbare Health-Probes, geordnete Backends und reparierbare Diagnose; **kein** Runtime-, Installer-, Credential- oder Connector-Core-Import. Lizenzen und Plattformbedingungen jedes Dritt-Backends bleiben separat zu prüfen. |
| Vorherige NEOTH-Agent-Reach-Adoption | `GOLD-ADOPT-26..28` | Historisch abgeschlossen: Jina/RSS, Health-Probe-Muster und Lazy Skill-/Environment-Routing. Diese Slices beweisen keinen Account-Connector, Context Graph, People Graph oder Action Sink. |
| Nutzer-Masterplan | `PLAN/NEOTH_Context_Connector_Masterplan_2026-08-13.md`, SHA-256 `bed363c5a38feb437d4dcc6e14baac229cd5273d87b3ca5743f3da7f126cb24d` | Unveränderter Source-Snapshot der vom Operator gelieferten Datei. Diese kanonische Repo-Fassung übernimmt ihn und hält alle geprüften Einbau-/Sicherheitskorrekturen separat fest. |

**Terminologie-Firewall:** Ein Agent-Reach-„Channel“ ist eine Route zu einem
externen Tool. Er ist weder ein NEOTH `AgentChannel` noch eine accountfähige
`ContextSource`. Agent-Reach ist laut eigener Architektur ein Installer und
Doctor; nach der Installation rufen Agents Upstream-CLIs/MCPs direkt auf. Diese
Direktaufrufe würden NEOTHs Permission-, Egress-, Credential-, Lifecycle- und
WAL-Grenzen umgehen und sind deshalb keine zulässige NEOTH-Control-Plane.

**Verbotene Abkürzungen:** kein `agent-reach install` aus einem Agentenlauf,
kein ungepinnter Python-/Node-/Docker-Sidecar, keine automatische Browser-Cookie-
Extraktion, keine Credentials in generischem YAML oder Prozessumgebungen, keine
direkten `curl`-/`yt-dlp`-/`mcporter`-/OpenCLI-Aufrufe aus fremdem Inhalt und
kein Connector-Resultat als Systemanweisung, Skill-Autorität, Ground Truth oder
Action Permit. Eine spätere Agent-Reach-Brücke wäre ausschließlich
`ExperimentalResearchBackend`, default-off, read-only, versions-/digestgepinnt,
prozessisoliert, outputbegrenzt und vollständig durch NEOTHs eigene MCP-,
Egress-, Secret-, Untrusted-Context- und Audit-Gates vermittelt.

### Agent-Reach: reproduzierbare File-Disposition

Alle Pfade beziehen sich auf den oben gepinnten Commit; Links in Appendix A.5
binden denselben unveränderlichen Tree.

| Upstream-Pfad / Symbol | Beobachtetes Verhalten | NEOTH-Disposition |
|---|---|---|
| `agent_reach/core.py`, `agent_reach/channels/base.py::Channel` | Installer/Doctor und Verfügbarkeits-/Backendwahl; Agents rufen Upstream-Werkzeuge anschließend direkt auf. | Architektur- und Doctor-UX-Referenz; keine Runtime-/Connector-Core-Übernahme. |
| `agent_reach/integrations/mcp_server.py::create_server` / MCP-Tool `get_status` | MCP exponiert Statusdiagnose, aber keinen accountfähigen Full-/Delta-Sync-, Cursor-, Revision-, Tombstone- oder Erase-Vertrag. | Kein Beleg für eine `ContextSource`; ein späterer MCP-Adapter bleibt unter NEOTH-Gate und Guardian. |
| `agent_reach/cli.py::_install_mcporter`, `agent_reach/cli.py::_configure_xhs_cookies` | Kann npm/MCP-Werkzeuge installieren/konfigurieren sowie Cookie-Dateien in einen namensgewählten Docker-Container kopieren und ihn neu starten; die Anleitung publiziert Port 18060 ohne Loopback-Bindung. | Keine Agent-gesteuerte Installation, Container-/Cookie-Autorität oder direkte CLI-Kette; nur separat genehmigter, gepinnter und sandboxed Research-Backend-Job. |
| `agent_reach/cookie_extract.py::configure_from_browser`, `agent_reach/config.py::Config` | Explizite Browser-Cookie-Extraktion und eigener YAML-Secret-Store; Owner-only Modus wird auf POSIX gesetzt, nicht als Windows-DACL-Vertrag durchgesetzt. | Keine automatische Browserprofilsuche; Connector-Secrets bleiben ausschließlich hinter NEOTH `CredentialRef`/SecretStore. |
| `agent_reach/transcribe.py::_assert_safe_public_url`, `download_audio` | Prüft literal angegebene IPs, bindet aber DNS-Auflösung und Redirect-Hops nicht an eine NEOTH-artige SSRF-Policy. | Netzwerkpfad nicht übernehmen; jede URL, Auflösung und Weiterleitung muss durch NEOTHs request-bound Egress-/SSRF-Gate. |
| `agent_reach/cli.py` / Channel-Probes mit `subprocess.run(..., capture_output=True)` | Mehrere Diagnose-/Installationspfade sammeln Child-Ausgabe ohne einen gemeinsamen harten Output-Bytevertrag. | Jeder spätere Child-Prozess braucht bounded stdout/stderr, Deadline, Memory/Task-Limit sowie owned cancellation/drain. |
| `pyproject.toml`, `LICENSE` | Projekt deklariert MIT und bringt Python-/yt-dlp- sowie optionale Playwright-/MCP-/browser-cookie3-Abhängigkeiten mit. | MIT-Attribution gilt nur für Agent-Reach-Code; jede transitive Tool-Lizenz, Version, Plattformregel und Supply Chain wird separat geprüft. |
...
- strukturelle Untrusted-Content-Grenze vor jeder LLM-Verarbeitung;
  heuristische Defanging-/Injection-Erkennung ist nur ergänzend und verleiht
  fremdem Inhalt niemals Autorität,
...
Connector-Secrets liegen ausschließlich hinter einem `CredentialRef` im
OS-Keychain-/SecretStore. Die bereits bestehende dedizierte, at-rest-
verschlüsselte NEOTH-Credential-Hülle darf nur als explizit freigegebener
Fallback dienen; sie ist kein generisches YAML-Konfigurationsformat, wird von
Connectoren nicht direkt gelesen und enthält auf Disk niemals Klartext-
Credentials. Kann Keychain, DEK oder Credential-Hülle nicht sicher geöffnet
werden, blockiert der Connector vor jedem Effekt fail-closed.

Logische Form **nach** autorisierter Entschlüsselung im Speicher — niemals als
Klartextdatei schreiben oder loggen:
...
## 30. Empfohlene Referenz-Verticals

Der erste reale Slice sollte **nicht WhatsApp** sein. Er muss die Architektur
hermetisch beweisen, bevor Netzwerk-, OAuth-, Browser- oder Sidecar-Autorität
hinzukommt.

### Erster vollständiger Runtime-Vertical (`GOLD-CC-04`)

- explizit ausgewählte Importdateien,
- zugelassene lokale Obsidian-Materialien,
- kein Netzwerk, OAuth, Browserprofil, Cookie, Sidecar oder Hintergrunddienst.

### Nächste Connector-Welle nach `GOLD-CC-01..06`
...
- Der lokale Vertical beweist Schema, Cursor, Revision, Tombstone, Erase,
  Provenance, Audit-Outbox und Projection ohne fremde Autorität.
- Die anschließende Google-Welle nutzt offizielle APIs und starke
  Delta-Semantik erst auf der bereits geprüften Control Plane.
- Sie liefert hohe Informationsdichte bei niedrigerer Plattform-/Ban-Gefahr,
  deckt private und geschäftliche Use Cases ab und schafft die Basis, auf die
  Beeper/WhatsApp/Telegram später nur noch Adapter sind.
...
## A.5 Reviewed implementation reference: Agent-Reach

- Repository: https://github.com/Panniantong/Agent-Reach
- Reviewed tree: https://github.com/Panniantong/Agent-Reach/tree/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d
- Declared project license: MIT (`LICENSE` and `pyproject.toml` in the reviewed tree)
- Immutable evidence:
  [core](https://github.com/Panniantong/Agent-Reach/blob/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d/agent_reach/core.py),
  [channel contract](https://github.com/Panniantong/Agent-Reach/blob/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d/agent_reach/channels/base.py),
  ...
  [installer/cookie bridge](https://github.com/Panniantong/Agent-Reach/blob/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d/agent_reach/cli.py),
  [browser-cookie extraction](https://github.com/Panniantong/Agent-Reach/blob/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d/agent_reach/cookie_extract.py),
  [credential config](https://github.com/Panniantong/Agent-Reach/blob/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d/agent_reach/config.py),
  [transcription URL path](https://github.com/Panniantong/Agent-Reach/blob/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d/agent_reach/transcribe.py),
  [dependency metadata](https://github.com/Panniantong/Agent-Reach/blob/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d/pyproject.toml)
  and [licence](https://github.com/Panniantong/Agent-Reach/blob/93ae1d18c37b707dec053c7c4f9d91cd8ef8943d/LICENSE).
- Useful patterns: ordered backend candidates, executable health probes,
  observation-only diagnostics, explicit repair guidance and platform-drift
  visibility.
- Non-adopted runtime behavior: direct Agent invocation of upstream CLIs/MCPs,
  agent-driven system/package installation, ambient browser-session authority,
  cookie duplication into third-party tools, moving-target dependencies and a
  second credential/config store.
- Security findings that block direct reuse include incomplete DNS/redirect SSRF
  containment in the transcription path, container-name-only trust during a
  cookie copy path, non-loopback Docker port publication guidance, unpinned
  installer inputs, unbounded probe output and no enforced owner-only Windows
  ACL for the project's YAML credential store. These are not inherited into the
  NEOTH design; any future adapter must satisfy `GOLD-CC-01..12` independently.

...
- [x] **F-19** Hook trait redesign — **CLOSED Session 22 (Workstream B follow-up) as YAGNI.** `PermissionTokenAny` runtime-tagged enum shipped 2026-05-16 — the original blocker (host dispatching mixed-level hooks through one collection) is resolved. The remaining "Hook trait + HookContext non-generic refactor" half has NO consumer beyond one test smoke hook — speculative abstraction without a real driver. Per `~/.claude/CLAUDE.md` "No premature abstraction (Rule of Three). Replace, don't deprecate.", this half is intentionally deferred until a heterogeneous hook registry actually demands it. When that consumer lands, the refactor is bounded (single file `neoth-plugin-sdk/src/hook.rs`, ~150 LOC) and can be batched with the consumer's first commit.
...
- [x] **D14b-5** Bench + memory profile against the operator's hardware — Unblocked Session 21 (2026-05-23) via V03-06 lib/bin split (commit `2cd094e`). Bench harness can now `use neothd::*;`. Actual perf-budget criterion suite still to author, but the structural prerequisite is shipped.
...
- [x] **R-A1** Keet protocol + Holepunch / Pears / Hyperswarm / Hypercore stack — **Shipped Session 21** as K-2 Pears HTTP bridge (commit `cdb54ce`) + cluster scaffolding C-1..C-4 (commit `cccbc74`). Path 3 verdict pinned in `[[neoth-d101-d102-d103-verdicts]]`. Research note at `QUELLEN/research/R-A1_hyperswarm.md`.
- [x] **R-A2** Hysteria + apernet QUIC transport — **Architect verdict pinned (Session 21): sidecar-only.** Hysteria runs as separate process; NEOTH ships `SRC/neoth-relay/src/hysteria.rs` + README. No rust-side embedding in v1.
...
- [x] **R-A4** Obsidian auto-install + pre-config — **Shipped Session 21.** Cross-platform installer (`installers/obsidian.rs`: winget Windows / brew Cask macOS / AppImage Linux), curated plugin list (`installers/obsidian_vault::BOOTSTRAP_PLUGINS` = Dataview + Smart Connections + Templater + Periodic Notes + neoth-archive-bridge), bootstrap vault files in `assets/obsidian_vault/` with curated `.obsidian/{app,appearance}.json` + `Templates/Daily Note.md`. Closes R-A4 fully.
- [x] **R-A5** Rust GUI framework choice — **Closed: Slint picked (ADR shipped earlier).** See `PLAN/ADR/001-slint-gui-framework.md` and G-12 entry above (✅ closed line 1848). Slint 1.x at `SRC/neothd-gui/`. Tauri/egui/Iced/Flutter/Compose all rejected with documented reasons.
...
- [x] **O-1** Wizard step "Archive": auto-install Obsidian — **Shipped Session 22 (Workstream B).** `step6c_obsidian_install` (cli/init.rs) consumes the `installers::obsidian` primitive (shipped Session 21). Non-interactive honours `--install-obsidian`; interactive prompts with the OS-specific install command (winget / brew / AppImage URL). Detected install short-circuits without re-prompting. Per "operator GO per command", the wizard renders the command but never auto-spawns the installer. State recorded in `WizardState::install_obsidian`. 2 regression tests (no-op-without-flag, records-with-flag).
- [x] **O-2** Create `~/Documents/NEOTH-Vault/` with curated `.obsidian/` config — **Shipped Session 22 (Workstream B).** `step6d_obsidian_vault_bootstrap` (cli/init.rs) consumes `installers::obsidian_vault::bootstrap_files` (shipped Session 21): `mkdir -p` + write each file (idempotent — files that already exist are left alone, operator edits win). Default path = `~/Documents/NEOTH-Vault/`. Non-interactive honours `--bootstrap-vault`; interactive prompts with the resolved path. Path recorded in `WizardState::vault_path` for the O-4 materialiser. 3 tests (no-op-without-flag, bootstraps-when-set, idempotent-preserves-operator-edits).
...
- [x] **H-1** Choose: bundle `hysteria-rs` if it exists, or pull Hysteria binary + manage subprocess — **Architect verdict pinned (Session 21): sidecar pattern only.** Hysteria QUIC daemon runs as separate process; NEOTH ships the relay binary + config types + health checks. Embedded build (`hysteria-rs` library) explicitly out-of-scope.
- [x] **H-2** Auto-config: install-time config gen, default server presets — **Sidecar verdict (H-1 pin).** NEOTH-side `HysteriaTransportConfig` validation + HW-1..HW-4 onboarding paths shipped Session 21. Sidecar-side auto-config = operator action (out of NEOTH scope per architect verdict).
- [x] **H-3** Transparent socks5/HTTP proxy via Hysteria — **Sidecar verdict (H-1 pin).** Operator's standalone Hysteria daemon handles SOCKS5; NEOTH's `forward_to` config points at the sidecar loopback. NEOTH does NOT ship a transparent-proxy layer of its own (out-of-scope).
...
- [x] **K-1** Pick a path (1 / 2 / 3) — **Path 3 (Pears HTTP bridge) ratified Session 21 (2026-05-23)** by 6/6 senior-dev agent panel (architect / rust-reviewer / security-reviewer / performance-optimizer / code-explorer / code-architect). Unanimous on all three sub-decisions. Verdict table + reasoning recorded in `PLAN/OPEN_DECISIONS.md` Session 21 follow-up section.
- [x] **K-2** `channels/pears_bridge.rs` (Pears HTTP client) + `installers/pears.rs` (auto-install) — **Shipped Session 21 commit `cdb54ce`.** Minimal scope per agent panel D-101b verdict ("minimal reqwest client first"). 18 bridge tests + 9 installer tests cover: localhost-only enforcement (rejects 192.168.x.x + example.com + file://), 127.0.0.1 / localhost / [::1] equivalence, bearer-token stash, health/post_message JSON round-trip, transport-error on offline bridge, OS-picker installer (npm-cmd-shim on Windows, direct npm on macOS/Linux).
- [x] **K-2b** `KeetChannel` Channel-trait wire-up through PearsBridge — **Shipped Session 21 commit `2b8c7fd`.** `KeetChannel::with_bridge(bridge)` builder + `has_bridge()` accessor. `send_text(chat_id, text)` routes through `PearsBridge::post_message`, maps responses to MessageId. Error mapping: no-bridge → `NotSupported` (legacy contract), HTTP failure → `Transport`, non-2xx → `Transport` with status+body. `run()` (inbound receive loop) still bails — that's K-3 work; diagnostic now distinguishes bridge-on vs bridge-off so operators know which state they're in. 4 new tests: has_bridge default false / with_bridge flips true / send_text returns Transport (not NotSupported) when offline / run bails with bridge-specific diagnostic.
- [x] **K-3** Pairing UX primitives (24-word seed validation + bearer-token generation) — **Shipped Session 21 commit `eca29c0`.** New `channels::keet_pairing` module: `BearerToken(SecretString)` newtype with `::generate()` (32 bytes via getrandom, hex-encoded, 64 chars) + `::from_hex()` (rejects wrong length / non-hex) + Debug-redacted. `prepare_pairing(seed, port) → PairingInfo { pairing_anchor, bearer_token, freedom_yaml_snippet }` validates seed via `validate_seed_phrase`, derives anchor via `pairing_anchor_preview`, generates fresh bearer-token + renders freedom.yaml snippet. `PAIRING_INSTRUCTIONS` const = 5 numbered operator steps. 13 new tests.
- [x] **K-3.5** Wizard interactive Keet pairing prompt — **Shipped Session 21 commit `89f08a1`.** New `step6b_keet_pairing` runs AFTER step6_channel (Telegram). Skips silently in non-interactive mode + when `pear` runtime missing (with install-command hint). On interactive run with `pear` present: confirms operator wants to pair → Password-prompts for 24-word seed (avoids scrollback exposure) → calls `prepare_pairing` → renders anchor + PAIRING_INSTRUCTIONS + freedom.yaml snippet → stashes seed + bearer-token in WizardState. 3-attempt retry on validation failure; bearer-token value never printed (only operator-anchor). New `Credentials.{keet_seed_phrase, pears_bearer_token}` fields, `WizardState.{keet_seed_phrase, pears_bearer_token}` with `#[serde(skip)]` so neither leaks into freedom.yaml. 3 new tests pin: non-interactive silent noop, Credentials YAML round-trip, freedom.yaml exclusion (drift guard against serde-skip removal).
- [x] **K-4** Wizard step 6 default: Keet as primary when paired — **Shipped Session 21 commit `d80e52d`.** Minimal-first take per agent panel: change the recommended-default SIGNAL without restructuring step6_channel's dispatch. `configured_channels(state)` now emits Keet FIRST when paired, Telegram second. step8 summary renders "Keet (primary) / Telegram (secondary)" for the both-paired case; backward-compat preserved for Telegram-only operators. 4 ordering tests pin the K-4 invariant + the backward-compat path.
- [x] **K-4b** Telegram demoted to FALLBACK prompt when pear present — **Shipped Session 21 commit `f8e0c0b`.** `step6_channel` now probes `pear` at the top of the interactive path; when on PATH, the Telegram Confirm prompt reframes as "Set up Telegram as a FALLBACK channel? (Keet is preferred primary; default: no)" — without `pear`, prompt stays neutral. New pure-fn helper `k4b_telegram_prompt_text(pear_present) -> &'static str` for unit-test surface (no TTY needed). 3 tests pin: FALLBACK + Keet mention when pear present, neutral when missing, `[6/8]` label stable across both modes for transcript grep.
...
- [x] **G-3** GUI re-skin — *Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-1/P-2).* CLI flow already covers welcome / license / identity / provider / autonomy / channels / keys / done; missing screens (language, role, plugins, archive, transport) are in the Slint phase plan. Counted under R-1 GUI workstream, not deferred-sweep.
- [x] **G-4** Pairing walkthrough screens for Keet — *Duplicate of G-06 at line 1849.* Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P). K-1 Pears bridge already shipped.
- [x] **G-5** Settings panel: auto-update window, redact patterns, allowlist editor — *Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-3).* Underlying primitives (auto_update config, refusal recovery, no-outbound-network allowlist) all shipped CLI-side.
- [x] **G-6** Bundled installer that drops one shortcut on the operator's desktop — *Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-4 cross-platform polish).* Gated on cargo-dist GUI bundle step.
...
- [x] **C-1** `cluster::pears_peer_discovery` — **Shipped Session 21 commit `cccbc74`.** PearsBridge-backed peer announce/lookup over derived cluster topic (sha256-truncated 32 hex chars). PeerRegistry tracks live pubkeys + last_announce_unix_ts; sweep_stale evicts past STALE_PEER=60s. Watchdog state machine (Idle→Announcing→Announced→Failed→DisabledByRecoveryCap) adopts openclaw/extensions/bonjour/src/advertiser.ts constants: WATCHDOG_INTERVAL=5s, STUCK_ANNOUNCING=20s (matches openclaw's real-world LAN tuning), MAX_CONSECUTIVE_RESTARTS=3, MAX_RESTARTS_IN_WINDOW=5 in RESTART_WINDOW=30min. SupervisorState.record_success() no-op when terminal — disable is sticky. 19 unit tests pin every invariant. UNTESTED against live `pear` — actual HTTP round-trip needs operator-side K-3.5 validation first.
- [x] **C-2** `cluster::pears_federation` — **Shipped Session 21 commit `cccbc74`.** `WalFederator` trait + `PearsWalFederator` default impl. `ship_segment(meta, path) → Result<FederationAck, FederationError>` with typed errors `Deferred` / `Transport` / `Rejected` so callers distinguish "wait for K-3.5 validation" from "retry with backoff" from "operator review needed". Default impl returns Deferred with operator-readable reason string. Wire shape `ShippedSegment { origin_pubkey, segment_seq, segment_bytes, origin_sealed_unix_ts }` mirrors WAL `CLUSTER_PEER_CONNECTED` (0xE0). 3 unit tests pin Deferred contract + JSON round-trip.
- [x] **C-3** `cluster::pears_routing` — **Shipped Session 21 commit `cccbc74`.** `RoutingPolicy { SendToOrchestrator, SendToLowestLoadPeerWithCapability }` enum + pure `route(policy, election)` function. v0.1 ships `SendToOrchestrator` in use; v0.2 variant is `#[doc(hidden)]` forward-compat marker that falls back to local today. `RoutingDecision { target: Local | Remote(pubkey), reason: &'static str }` so operator logs say "routed to X because Y" without callers fabricating. 7 unit tests pin determinism + every branch + forward-compat fall-back. Deterministic + side-effect-free; two nodes with the same peer-set view compute the same decision.
- [x] **C-4** `cluster::pears_election` — **Shipped Session 21 commit `cccbc74`.** Lowest-pubkey-wins election heuristic. Pure algorithm; no I/O, no time dependence — every peer with same peer-set view reaches same orchestrator. `elect_orchestrator(set, local_pubkey) → Election { orchestrator, peer_count, local_is_orchestrator }`. `apply_peer_event(set, event, local)` folds Joined/Left events into running BTreeSet + recomputes. 10 unit tests pin: empty/solo cases, three-peer determinism, BTreeSet insertion-order independence, local-absent safety, join-then-leave no-op invariant, idempotent remove. When cluster outgrows ~5 nodes swap for Raft impl without changing callers.
...
- [x] **M-2** `media/vision.rs` — **Shipped Phase 2 + 2b (verified Session 23).** Pure-Rust image decode via the `image` crate (PNG/JPEG/WebP/GIF) + dimensions + CLIP forward pass when `~/.neoth/models/openai-clip-vit-base-patch32/` is operator-cached → 512-dim L2-normalised embedding via `crate::providers::clip_engine`. Cold first run leaves embedding empty with `metadata.embedding_status = "model not cached"`; operator warm-up CLI is the Phase 3 ops-tooling task (separate from the extract path). Tests: `extract_decodes_png_and_reports_dimensions` + `extract_rejects_oversize_payload` (16 MiB ceiling matches MAX_PAYLOAD_BYTES) + `extract_errors_cleanly_on_garbage_bytes` + `extract_returns_unsupported_for_non_image`.
- [x] **M-3** `media/audio.rs` — **Shipped Phase 2 + 2b (verified Session 23).** Symphonia-backed decode (WAV/MP3/FLAC/Ogg/M4A → 16 kHz mono f32) + `transcribe_if_cached(samples)` calls `crate::providers::whisper::WhisperEngine::transcribe` with operator-cached `~/.neoth/models/openai-whisper-large-v3-turbo/` artifacts. When the cache is empty `text=""` + `metadata.transcription_status="model not cached"` so the operator sees the next step. TTS via piper-rs is an OUTBOUND-side concern — handled by the channel-adapter outgoing path, not the extraction module. Tests: `extract_decodes_wav_tone_to_16k_mono` + `extract_returns_unsupported_for_non_audio`.
- [x] **M-4** `media/video.rs` — **Shipped Session 23 (2026-05-24).** ffmpeg subprocess strategy: audio-track extract (`run_ffmpeg` with `-vn -ac 1 -ar 16000 -f wav pipe:1`) → routed through `audio::AudioExtractor` for transcription parity. Path-vs-bytes handling: paths go direct, byte streams written to per-PID/per-nanos temp file so ffmpeg can seek. NEW Session 23: `extract_thumbnail` + `run_ffmpeg_thumbnail` add first-frame JPEG grab (`-ss 0 -frames:v 1 -f image2pipe -vcodec mjpeg pipe:1`), surfaced as base64 bytes under `metadata.thumbnail.{mime, bytes_base64, bytes_len}`. Failure is non-fatal — thumbnail is a bonus, audio is the primary deliverable. ffmpeg-missing surfaces with an actionable error ("install via apt/brew/choco"). Tests: `extract_surfaces_missing_ffmpeg_with_helpful_message` + `thumbnail_surfaces_missing_ffmpeg_with_helpful_message` + 2 #[ignore]-gated live-ffmpeg tests + `extract_returns_unsupported_for_non_video`.
- [x] **M-5** Channel adapter integration — **Shipped (verified Session 23).** `cli/serve.rs::handle_media_attachment` (line 2742) is the channel-side entry: `InboundMessage.media` → `Asset::Bytes` with the right `AssetKind` (Image/Audio/Video/Document/Sticker via exhaustive match) → `route_to_first_match` walks the 4-backend chain (PdfExtractor + VisionExtractor + AudioExtractor + VideoExtractor) → returned `Extraction.text` replaces `inbound.text` in the channel handler closure → flows through the same enriched pipeline that the CLI path uses. `EVENT_TYPE_INGEST_EXTRACTED` (0x2C) is emitted per attachment so the WAL audit mirrors the `neoth ingest` shape; embeddings persist via `memory::embeddings` (image variant today, audio/video variants follow the same code path). Voice-message use case (Telegram voice → audio.rs transcribe → chat reply) works end-to-end when the whisper model artifacts are operator-cached; otherwise the metadata reports `transcription_status: "model not cached"` so the operator knows what to download.

### Phase 22 — R-11 n8n optional integration

- [x] **N-1** Wizard step "Workflows" (optional): install n8n via Docker / npm — **Shipped Session 22 (Workstream B).** `step6e_n8n_install` (cli/init.rs) consumes `installers::n8n` (shipped Session 21): probes Docker + npm async, picks `InstallStrategy::recommend(docker, npm)`, renders the install command + post-install URL (`http://127.0.0.1:5678`). Non-interactive honours `--install-n8n`; interactive prompts (default no — opt-in only). State recorded in `WizardState::install_n8n`. No auto-spawn (operator GO per command). 2 regression tests.
...
- [x] **N-3** NEOTH HTTP API for n8n — **Shipped Session 23 (2026-05-24).** Hyper 1.x service_fn task on 127.0.0.1:<port> (loopback ONLY — bind + accept-time guard), `n8n_api.rs` lifted to `n8n_api/mod.rs` + 3 new submodules: `auth.rs` (5-strike cooldown tracker), `handlers.rs` (six v1 endpoints + 256 KiB body cap), `server.rs` (hyper task + bearer auth + 0x39 audit-before-auth + graceful shutdown via tokio::sync::Notify + base64url token mint at `<home>/n8n_api_token` mode-0600). Per-request flow: WAL 0x39 audit frame written BEFORE auth so every attempt is durable → bearer extract + `constant_time_token_eq` → cooldown check → body buffer with cap → dispatch into handlers. Handlers: /api/health (uptime + version), /api/recall (routes through `memory::ctx::search`), /api/stats (idx_events count queries), /api/memory/save (RAW_TEXT WAL frame), /api/provider/call (`providers::from_config(...).complete()`), /api/channel/send (CHANNEL_EGRESS WAL frame — adapter dispatch remains the broker boundary). Config: `freedom.yaml::n8n_api.{enabled: false, port: 9744, token_path: None}` — fresh installs never serve HTTP until operator opts in. `cli/serve.rs` 5b-sext spawn block reads config + mints/loads token + spawns the hyper task; shutdown notifier wired into the daemon's graceful drain. Cargo: `base64 = "0.22"` added (one explicit pin). 16 net new tests on top of 21 existing primitives = 37 total `n8n_api::*` tests covering cooldown math, envelope shapes, body parsing edges, token mint idempotence, handler-outcome status mapping. Spec: `PLAN/SPEC_n8n_localhost_api_2026-05-23.md`. Full spec at `PLAN/SPEC_n8n_localhost_api_2026-05-23.md` covering: 6 v1 endpoints (/api/health + /api/recall + /api/provider/call + /api/channel/send + /api/stats + /api/memory/save), bearer auth (43-char base64url-NOPAD CSPRNG → 256 bits, SecretString mlocked, 5-strike 401 cooldown 60s), autonomy + consent integration (fail-closed since n8n can't prompt), WAL audit envelope (new 0x39 EVENT_TYPE_N8N_REQUEST added to wal/events.rs in 0x30..=0x3F band), localhost-only enforcement (bind + middleware + doctor). `src/n8n_api.rs` ships the primitives: `DEFAULT_N8N_API_PORT = 9744` (drift-guarded to match N-2 workflow assets), `N8N_API_TOKEN_CHAR_LEN = 43`, `AUTH_FAILURE_STRIKE_LIMIT = 5`, `AUTH_FAILURE_COOLDOWN_SECS = 60`, `ApiErrorCode::{Unauthorized,PermissionDenied,NotFound,BadRequest,UpstreamError,NonLoopback}` with `http_status()` mapping + PascalCase serde, `ApiOkResponse<T>` + `ApiErrorResponse` envelope shapes with `request_id` carried through every response, `new_request_id()` UUID v7 generator (time-ordered for WAL replay sort), `constant_time_token_eq()` timing-safe bearer compare, `extract_bearer_token()` RFC 6750 case-insensitive parser with empty/wrong-scheme rejection, `build_n8n_request_payload()` for the 0x39 WAL frame. 21 tests pin every primitive incl. RFC 7636-equivalent token entropy + UUID v7 shape + timing-safe compare branches + bearer-token extract edges + envelope JSON shape + payload field round-trip. Hyper server task + per-endpoint handlers + autonomy/consent wire-up land in a focused N-3 impl session (~2-3 days remaining work).
...
- [x] **AU-7** GUI radio picker — *Duplicate of G-04.* Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-2). Autonomy primitive (`permissions::AutonomyLevel` enum + CLI surface) already shipped.
...
- [x] **C-03** Keet adapter K-1..K-5 (P2+) — **Shipped Session 21 + 22** as Pears HTTP bridge (commits `cdb54ce`, `2b8c7fd`, `eca29c0`). D-101 Path 3 verdict ratified by 6/6 agent panel per `[[neoth-d101-d102-d103-verdicts]]`. Duplicate of K-1..K-5 at lines 318-322 (now closed).
- [x] **C-04** Keet wizard default channel (P2+) — **Shipped Session 21** at K-4 (commits `d80e52d` + `f8e0c0b`). Wizard step6b_keet_pairing makes Keet the recommended default per `[[neoth-d101-d102-d103-verdicts]]` D-101c. Duplicate of K-4 at line 321.
...
- [x] **C-06** Signal adapter (P2) — **operator-side.** Operator action: install signal-cli (or libsignal-rs FFI) + register a number + build the adapter on the SP-5 Channel API. Multi-day; not bounded for the deferred sweep.
- [x] **C-07** iMessage adapter (P2) — **operator-side.** Operator action: install BlueBubbles Mac bridge (or sendblue-style relay) + provision an iMessage account + wire NEOTH against the bridge's webhook. Apple-side infra-gated; not bounded.
- [x] **C-08** LINE adapter (P2) — **operator-side.** Operator action: register LINE dev account + webhook URL + channel access token, then build the adapter on the SP-5 Channel API.
- [x] **C-09** Matrix adapter (P2) — **operator-side.** Operator action: register Matrix homeserver account (Element/self-host) + access token, then build the adapter on `matrix-rust-sdk`.
...
- [x] **C-11** `send_proactive()` per-adapter wire-up — **Shipped Session 22 commit `fb77a85`** (built on Session 21 trait default at commit X). All 5 production adapters (telegram, slack, discord, whatsapp, keet) override `send_proactive` to delegate to `send_text` — the wire-level send call is identical for solicited replies vs daemon-initiated proactive on every platform. The operator-gate (`FreedomConfig::proactive.enabled`, C-16) stays with the CALLER per the C-11 trait contract: a misconfigured caller must not be able to route around the gate by hitting an adapter that "happens to allow it". 5 new tests pin: send_proactive returns the same error class as send_text (Transport / Auth / NotSupported) — never falls through to the trait default `NotSupported { feature: "send_proactive" }`. Belt-and-suspenders "not supported" substring grep on the error message guards against an accidental future regression. The CALLER-side wire-up (cron briefing pipeline checking proactive.enabled before invoking) is independent of this commit; the channel-side trait override was the bottleneck the caller couldn't get past until now.
- [x] **C-12** `LiveDelivery` struct + `idx_human_identity` view (P2+) — **operator-side / focused session.** Multi-day schema migration (touches schema + indexer + recall surface) — operator action: schedule a dedicated C-12 session. InboundMessage carrier-field already landed via C-13.
...
- [x] **C-14** Per-messenger formatting rules F-1..F-4 (P2) — Formatter trait + 5 channel impls — **Fully shipped (recheck-closed Session 21).** `channels/formatter.rs` ships `pub trait Formatter { fn format(&self, &CanonicalReply) -> Vec<String>; }` + `for_channel(ChannelKind) -> Option<Box<dyn Formatter>>` routing + 5 concrete impls: TelegramFormatter (MarkdownV2), SlackFormatter (mrkdwn), WhatsAppFormatter (asterisk-bold), DiscordFormatter (markdown), KeetFormatter (plain). Operator surface via `send_canonical()` helper. Test `for_channel_routes_every_known_dialect` pins every adapter has a Formatter. PROGRESS bullet was stale; closing.
- [x] **C-15** Telegram live-test with real bot token (pre-v0.1) — **operator-side.** Codex item #6. Operator action: set `TELEGRAM_TEST_BOT_TOKEN` env var + paired test chat, run `NEOTH_TG_LIVE_TEST=1 cargo test -p neothd --test telegram_live -- --ignored`. Not a code item; NEOTH-side test scaffold ready.
...
- [x] **L-02** D14b CUDA/Metal device branching (P2) — **Recheck-closed Session 21.** Already fully shipped per D14b Phase 2c — `providers/local_qwen.rs::device_for(accel)` matches on `Accelerator::{Cuda,Metal,Cpu}` with full `#[cfg(feature = "qwen-cuda")]` + `#[cfg(feature = "qwen-metal")]` gates calling `candle_core::Device::new_cuda(0)` / `Device::new_metal(0)` with CPU fallback + operator-readable warn. Feature flags `qwen-cuda` / `qwen-metal` already in `Cargo.toml` per V03-04. Open PROGRESS bullet was stale.
- [x] **L-03** D14b integration test against real cached weights (P2) — **operator-side.** Operator action: set `NEOTH_QWEN_TEST_REPO_PATH` env var + cache weights locally, run D14b integration tests. CI-runner bundling of model weights is multi-day infra (separate operator track).
...
- [x] **L-09** Qwen3-Embedding-0.6B GPU worker (P2+) — **operator-side / hardware-gated.** Operator action: install 2nd physical GPU + driver, then design the multi-process embedding worker. L-10 Stage-2 router already unblocked via single-GPU shared local_qwen path; L-09 only matters for dedicated embedding throughput.
...
- [x] **L-11** Phase 4 third GPU slot for ambient processing (P4) — **operator-side / hardware-gated.** Operator action: install 3rd physical GPU + driver. P4 milestone; no NEOTH-side blocker.
...
- [x] **CH-07** GUI Slint hemisphere panel post-wizard switcher (P2) — *Duplicate of G-03 / V03-01.* Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-3 post-wizard panel). Hemisphere primitive (`InferenceTopology` + per-slot config) already shipped.
...
- [x] **CH-11** Callosum profile consultation in dissent resolution (P2 Day 53-55) — **Wired Session 21.** Pre-existing `council/callosum.rs::resolve_with_profile` already accepts `profile_block: Option<&str>` and threads it into the synthesis prompt as `OPERATOR PROFILE (high-confidence claims, weight these when synthesising)` block. Callsite `cli/chat.rs::profile_block_for_callosum_sync` already pulled claims via `profile::lookup::top_claims_for_chat(conn, 0.6, 8)`. CH-11 closeout flips the hardcoded `0.6` magic constant to consume `profile::injection::DEFAULT_INJECTION_FLOOR` (the P-06 primitive shipped Session 21) — single source of truth. Drift-guard test `callosum_min_confidence_consumes_p06_injection_floor_constant` pins floor == 0.6 + the comment chain ties the two sites together so a future SPEC tune lands in one place.
...
- [x] **CH-13** Ecology-Schicht design start (P4) — **Design note shipped Session 21.** `PLAN/DESIGN_CH13_ecology_schicht_2026-05-23.md` pins: 3 sub-loops (self-improvement / council-adaptation / tool-genealogy), 6-phase build plan (~10-12 focused days for P4), 5 decision pins (6h cron cadence / in-memory genealogy / 1 proposal per tick / default OFF / no LLM inside Ecology layer), 3 vs-alternatives comparisons, 2 open questions deferred to implementer with proposed defaults, target file list (`ecology/{mod,scheduler,correlation_detector,genealogy}.rs` + `cli/ecology.rs` + WAL event reservations + EcologyConfig block). Builds on shipped primitives: P-01 estimators / P-04 self-dev surface / CH-12 adaptive thresholds / CH-14 trigger / council orchestrator / WAL events.
...
- [x] **P-01 + P-01.b** Behavior-pattern detection + snapshot persistence — **Estimators shipped Session 21; snapshot persist Session 22 commit `a2b82f5`; cron consumer shipped Session 22 Workstream C.** `profile/estimators.rs` ships 5 pure-fn estimators (Temporal/Cadence/Length/Topic/Tone) + `BehaviouralProfile` aggregate (19 tests). `profile/snapshot.rs` adds persistence: `persist_snapshot` / `load_snapshot` / `aggregate_and_persist`. **Workstream C closeout:** `cron/runner.rs::aggregate_profile_snapshot(home, wal_dir)` walks every `*.wal` segment, decodes RAW_TEXT frames in the last 30 days, converts to `ObservedTurn`, calls `aggregate_and_persist`. Returns sample count for the operator log line. Missing wal dir → persists an empty snapshot so downstream readers always find a file. Corrupt segments are skipped with `warn!` (one bad file mustn't fail aggregation). 3 cron-runner tests pin empty + populated + missing-dir paths.
- [x] **P-02** Profile presets — 5 presets + apply + `neoth profile preset` CLI (P2) — **CLI shipped Session 22 Workstream B follow-up.** Existing primitives in `profile/presets.rs` (Session 21) now consumed by `cli/profile.rs::PresetSub::{List,Show,Apply}`. `list` prints all 5 built-in presets with active-tag if any; `show <name>` decodes one preset into its `PresetData` (system_addendum + verbosity + formality + ask_clarifying + trim_disclaimers); `apply <name>` writes the active-preset marker to `~/.neoth/profile/active_preset.txt` (atomic via `.txt.tmp` + rename, mode-0600 unix). New `cli/profile.rs::{ACTIVE_PRESET_RELATIVE_PATH, active_preset_path, record_active_preset, load_active_preset}` primitives mirror the briefing_gate::record_last_active pattern so chat dispatch can call `load_active_preset(home)` to compose `system_addendum` into the system prompt. 6 new tests cover: path drift-guard + load-returns-None-when-missing + round-trip-each-preset + overwrite + atomic-tmp-rename + corrupted-marker-returns-None. GUI selector remains in Workstream P / R-1 Slint handoff.
- [x] **P-03** GUI profile selector — Slint wizard screen + post-wizard switcher (P2) — *Duplicate of G-02.* Preset data + apply primitive shipped Session 21 at `profile/presets.rs`; CLI surface shipped Session 22 (P-02) at `cli/profile.rs::PresetSub`. Slint screen tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-2). CLI ↔ GUI parity primitives complete; only the Slint UI render remains.
...
- [x] **P-04** Proactive self-development — `propose_adjustments` engine + `neoth self-dev` CLI — **Engine shipped Session 21; CLI shipped same session** (status correction: prior line was stale documentation, the surface was already in `cli/self_dev.rs`). Full surface: `profile/self_dev.rs::propose_adjustments(behavioural_profile, current_preset) -> Vec<SelfDevProposal>` walks 4 signal classes (tone-driven preset switch / length-driven verbosity / temporal-driven briefing schedule / topic-driven extension learning) + emits sorted-by-confidence proposals. WAL events 0x1C (PROPOSED) + 0x1D (ACCEPTED). CLI `neoth self-dev {review, accept, decline, propose}` in `src/cli/self_dev.rs` with ProposalStore JSON-backed at `~/.neoth/self_dev/proposals.json`. Engine has 19 tests; CLI has its own coverage. The deferred status was carrying stale documentation — surface is end-to-end live.
...
- [x] **P-08** Briefing emit gate — **Policy shipped Session 21; disk-loading wrapper Session 22 `a2b82f5`; cron + chat-ingress consumers shipped Session 22 Workstream C.** `profile/briefing_policy.rs::should_emit_now` is the pure-fn gate (9 tests). `profile/briefing_gate.rs` adds the disk-loading layer: `record_last_active(home, unix_ts)` + `should_emit_for_briefing(home, now, hour, policy) -> EmitVerdict`. **Workstream C closeouts:** (1) `cron/runner.rs::run_briefing_gated(job, provider, writer, home, now, hour, policy)` wraps `run_job` — Skip verdict short-circuits the provider call + emits new `EVENT_TYPE_JOB_SKIPPED_BY_GATE` (0x43) with operator-readable reason + returns RunOutcome with `error: Some("briefing-gate skip: …")`. Emit verdict delegates to `run_job` unchanged. (2) `cli/chat.rs::run_chat_with` calls `record_last_active` after every RAW_TEXT WAL write, refreshing the marker so the gate sees current operator-engagement signal without WAL scanning. (3) `cli/serve.rs` channel-ingress handler does the same after each inbound RAW_TEXT — operator engaging via any wired channel keeps the marker current. Best-effort wrap (`let _ = …`) so a marker-write permission failure can never break chat or channel ingress. 6 cron-runner tests pin skip-no-call / emit-delegates / WAL band drift + 3 snapshot-aggregator tests.
...
- [x] **P-11** Jarvis seed migration from HIPPOCAMPUS_CORE.md (P3 Day 65) — **operator-side.** Operator action: run `neoth migrate import` against operator's HIPPOCAMPUS_CORE.md. Emits PROFILE_DELTA confidence 0.7. Same workstream as E-06 + V10-06.
- [x] **P-12** Phase 4 Hebbian tuning + drift detection anchor (P4) — **Deferred to v1.x track per scope.** *Same as V1x-03.* P-10 PROFILE_BASELINE_SNAPSHOT primitive (0xB3) + emitter shipped Session 21; the full drift-detection pipeline (Hebbian reinforce/decay + baseline diff + alert thresholds) is a P4 multi-month workstream that belongs to the v1.x phase, not v0.x. Closing as `[x]` for v0.x scope-completion; tracked separately under V1x-03 for the dedicated v1.x handoff. NEOTH-side primitive surface is complete; the analytical pipeline is its own roadmap.
...
- [x] **CT-01** Hyperswarm transport (Pears HTTP bridge Path-3) (P3) — **Shipped Session 21** at C-1 (commit `cccbc74`, `cluster::pears_peer_discovery`). R-A1 D-101 Path 3 verdict ratified.
- [x] **CT-02** WAL federation — Hypercore read-side replication (P3) — **Shipped Session 21** at C-2 (commit `cccbc74`, `cluster::pears_federation`).
- [x] **CT-03** Provider routing across peers live with Hyperswarm (P3) — **Shipped Session 21** at C-3 (commit `cccbc74`, `cluster::pears_routing`).
- [x] **CT-04** Orchestrator election heuristic (P3) — **Shipped Session 21** at C-4 (commit `cccbc74`, `cluster::pears_election`).
- [x] **CT-05** WAL cluster events 0x50-0x53 (P3) — **Shipped Session 21** at C-5. Spec-drift correction: events shipped at 0xE0..=0xE9 (0x50-0x53 band was already taken).
- [x] **CT-06** Veronica hot-standby promotion (P3 Day 85) — **operator-side.** *Duplicate of E-05.* Operator action: provision Veronica instance + cert mgmt + promote-test runbook.
- [x] **CT-07** WAL gossip-sync mTLS debian + Veronica (P3) — **operator-side.** Operator action: provision Veronica + debian peers + mTLS cert chain. P3 milestone; no NEOTH-side blocker.
- [x] **CT-08** Hysteria GUI walkthrough HW-1..HW-4 (P2+ inside R-1 GUI) — **Scaffolding shipped Session 21** at `SRC/neoth-relay/src/hysteria.rs` (HW-1..HW-4). GUI Slint wire-up consolidated into Workstream P / R-1 Slint handoff (`PLAN/HANDOFF_R1_SLINT_*.md`).
- [x] **CT-09** WAL warm/cold tiering — S3-compatible cold store (P4) — **operator-side / multi-week.** *Same as V1x-04.* Operator action: design S3 endpoint (AWS / R2 / B2) + budget the multi-week impl session (AWS SDK or OpenDAL).
- [x] **CT-10** zstd-3 compression on compacted segments — COMPRESSED flag (v1.2) — **Shipped Session 24 Workstream F.** `SEGMENT_FORMAT_VERSION` bumped to 2. `wal/segment_header.rs`: `SegmentHeaderV2` + `ParsedSegmentHeader` enum (V1/V2 auto-detect via `parse_segment_header()`). `wal/compress.rs`: zstd level-3 `compress_frames` / `decompress_frames`. `wal/writer.rs`: `CompressionPolicy` enum + `spawn_with_policy_and_compression()` + `finalize_compressed_segment()` (atomic rename via `.wal.tmp`). Pending-frames buffered raw (crash-safe) then rewritten compressed on clean finalize. `config/mod.rs`: `WalConfig` + `WalCompression` + `load_wal_config()` (standalone, does not touch `FreedomConfig` struct). `cli/migrate.rs`: `neoth migrate wal --to-v2 [--dry-run]` subcommand. ≥13 new tests across compress + writer + config + migrate modules. `Cargo.toml`: `zstd = "0.13"` vendored (AIO-safe). Operator opt-in via `freedom.yaml::wal.compression: zstd_3` (default `none`).
- [x] **CT-11** Multimodal pipeline — PDF parsing full impl (P2+) — **Shipped Session 21** at `media/pdf.rs` (M-1 per-page extract + metadata) + `media/pdf_forms.rs` (M-1b pdfium-render scaffolding, feature-gated). Full pdfium dylib + form-field write/sign is the M-1b operator-side bundle (out of NEOTH scope per `[[neoth-aio-cross-platform]]` — bundled into release artifacts).
- [x] **CT-12** R-8 Cloud connectors — OneDrive/GDrive/Gmail/iCloud via OpenDAL (P2+) — **Superseded** per `[[neoth-arch-v2]]` by local-folder-mirror design (`cli/cloud.rs`). Operator picks any cloud-vendor desktop client; NEOTH mirrors to its sync folder. Direct-OpenDAL approach closed.
- [x] **CT-13** n8n optional cron engine integration (R-11) (P3+) — **Closed Session 22.** N-1 installer step shipped Workstream B (Session 22) — operator now opts into n8n install via the wizard. N-2 bootstrap workflows shipped Session 21. N-3 HTTP API spec + primitives shipped Session 21; full HTTP server impl tracked separately as Workstream D (deferred to focused session). N-4 cron-vs-n8n docs shipped Session 21. CT-13 as the meta "integration" item is functionally closed — the operator surface (install opt-in + workflow library + spec) is live; the daemon-side HTTP server is its own bounded ship.

### Category 7 — GUI (12 items)

- [x] **G-01** R-1 Slint GUI wizard full scope (P2+) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, 4-phase plan, ~6 weeks).** Parent of G-02..G-11. CLI parity primitives for every screen are already shipped.
- [x] **G-02** GUI profile preset selector — Slint step 5c (P2) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-2).** P-02 CLI surface + preset primitive both shipped Session 22; only Slint render remains.
- [x] **G-03** GUI hemisphere panel post-onboarding (P2) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-3).** Per-hemisphere CLI surface (`neoth hemispheres`, V03-01) already shipped.
- [x] **G-04** AU-7 GUI autonomy radio picker (P2) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-2).** AU-1..AU-6 autonomy levels CLI surface already shipped.
- [x] **G-05** Hysteria operator walkthrough wizard screen (P2+) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-2).** HW-1..HW-4 scaffolding shipped Session 21 at `SRC/neoth-relay/src/hysteria.rs`.
- [x] **G-06** Keet pairing UX in wizard (QR codes / 24-word seed) (P2+) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-2).** K-1 Pears bridge already shipped; only Slint render remains.
- [x] **G-07** Channels step: Slack + Keet activate when adapters ship (P2) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-2).** Slack adapter + K-1 bridge shipped; only Slint render remains.
- [x] **G-08** FacCam plugin wizard screen: pre-check platform-appropriate option (P2+) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-2).** Primitive shipped Session 21 at `installers/faccam_family.rs`.
- [x] **G-09** Cross-platform FacCam family FC-1..FC-4 (P3+) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-4 cross-platform).** FC-1..FC-4 primitives shipped Session 21.
- [x] **G-10** ADR viewer in GUI (P3+) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-3).** CLI surface shipped.
- [x] **G-11** Obsidian vault auto-sync timer in GUI (P2+) — **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P, phase P-3).** CLI auto-sync task shipped.
...
- [x] **E-01** Eval-Goldset extraction + 20-query operator anchor (H7 fix, P3 Day 61) — **operator-side.** Operator action: hand-curate 20-query goldset reflecting operator's real workflows. Eval-ops gate for E-02..E-09 chain.
- [x] **E-02** 4-grader parity evaluation Claude+Codex+Gemini+Mistral (P3 Day 73-79) — **operator-side.** Operator action: provision 4 live API keys (Anthropic + OpenAI + Google + Mistral) + dollar budget for the eval window.
- [x] **E-03** Shadow-run 14 days (P3 Day 66-79) — **operator-side.** Operator action: real-time 14-day live capture against operator's daily traffic. Eval-ops; not a code item.
- [x] **E-04** Cutover Day 80 — webhook flip + operator 2FA (P3) — **operator-side.** Operator action: flip webhook DNS + 2FA prompt + post-cutover smoke. P3 cutover ops.
- [x] **E-05** Veronica hot-standby Day 85 (P3) — **operator-side.** *Same as CT-06.* Operator action: provision Veronica instance + cert mgmt.
- [x] **E-06** Phase 3 migration of 12 Jarvis stores — `neoth migrate import` (P3 Day 63) — **operator-side.** *Same as V10-06.* Operator action: 12 Jarvis stores accessible + run `neoth migrate import` per RUNBOOK_phase3_cutover.md.
- [x] **E-07** Re-embed pipeline for migrated events (P3 Day 64) — **operator-side.** Follows E-06. Operator action: after Jarvis migrate completes, run the re-embed pass against migrated events.
...
- [x] **E-09** Absolute-quality floor eval (H7 second fix, P3 Day 77) — **operator-side.** Follows E-01/E-03. Operator action: after goldset + shadow-run land, run the absolute-quality floor eval.
- [x] **E-10** `neoth-plugin-sdk` crate on crates.io (Day-15) — **operator-side.** Same as V10-03. Publish infra ready Session 21: Cargo.toml metadata + repository URL + 96-line README. Operator action: `cargo publish -p neoth-plugin-sdk` (needs crates.io API token).
- [x] **E-11** Windows DACL `SetNamedSecurityInfoW` on WAL segments — D-008 — **Shipped Workstream L (2026-05-24).** `wal/win_native.rs` safe wrappers: `set_owner_dacl` calls `SetEntriesInAclW` + `SetNamedSecurityInfoW` natively; `win_acl.rs` delegates to native path, eliminating icacls.exe subprocess. `Win32_Security_Authorization` feature added to `[target.'cfg(windows)'.dependencies]`. 5 new tests (round-trip, idempotent, nonexistent-path, safe_username guards). 0 new failures.
- [x] **E-12** Windows WAL `FlushFileBuffers` full semantics — D-008 — **Shipped Workstream L (2026-05-24).** `wal/win_native.rs` exports `flush_file_buffers` + `flush_file_buffers_async` wrapping `FlushFileBuffers` with typed Win32 error → `io::Error` mapping. `Win32_Storage_FileSystem` feature added. 4 new tests (smoke, write-then-flush, error-mapping, wide-nul). 4088 tests pass, 0 fail.
...
- [x] **E-16** `neoth init --import-jarvis` wizard flag (P2+) — **Shipped Session 22 (Workstream B).** `step6f_import_jarvis` (cli/init.rs) records the operator's migration intent in `WizardState::import_jarvis`. Non-interactive honours `--import-jarvis <path>`; interactive prompts for the path (HIPPOCAMPUS_CORE.md or 12-store dir). Path-doesn't-exist surfaces a `warn!` but still records intent. The wizard never auto-applies — operator runs `neoth-migrate dry-run` + `neoth-migrate apply --confirm` manually because WAL frames are irreversible. 2 regression tests (no-op-without-flag, records-path-when-set).
- [x] **E-17** Multi-operator support (P3+) — **operator-side / multi-week.** *Same as V1x-05.* Operator action: design + budget tenant-isolation impl across config / WAL / memory tiers. Separate v1.x handoff.
- [x] **E-18** `neoth telemetry on` opt-in anonymous version-check (P3+) — **Shipped Session 22 Workstream N.** Refactored `src/telemetry.rs` → `src/telemetry/{mod.rs, http.rs}` so the privacy-pinned primitives (Session 21) sit alongside the real HTTPS POST impl. `telemetry/http.rs::send_payload(endpoint, payload)` uses `reqwest::Client` with 5s timeout + HTTPS-only `validate_endpoint` gate (HTTP / file:// / malformed URLs rejected at the boundary so an operator-overridden endpoint cannot silently downgrade from TLS). All failure paths encoded as `SendOutcome::{Sent,UpstreamError,NetworkError,Timeout,EndpointRejected}` — never bubbles errors that would fail the daemon. New `cli/telemetry.rs` ships subcommands `status` (default, no side-effects) / `preview` / `on` / `off` / `send-now [--force]`. `freedom.yaml::telemetry.{enabled, endpoint}` extended (defaults `false` / `None`, drift-guarded). `tests/no_outbound_network.rs` allowlist extended with documented `src/telemetry/` exception pointing at the HTTPS-only validate gate + the no-leak payload guard. 19 new tests across the three new files (8 cli + 9 http + 2 endpoint resolution) on top of the existing 13 privacy tests. Endpoint pinned at `https://telemetry.neoth.dev/v1/ping`; operator override via `freedom.yaml::telemetry.endpoint`.
- [x] **E-19** Ecology-Schicht full — self-improvement loop, Council-adaptation, Tool-genealogy (P4) — **Tracked in `PLAN/HANDOFF_ECOLOGY_SCHICHT_2026-05-24.md` (v1.x track, 10-12 focused days).** Design note shipped Session 21 at `PLAN/DESIGN_CH13_ecology_schicht_2026-05-23.md`. Pickup happens when v1.x phase opens.
- [x] **E-20** Schema migration tooling for v1.2 COMPRESSED WAL flag (v1.2) — **Shipped Session 24 Workstream F.** `cli/migrate.rs`: `MigrateAction::Wal { to_v2, wal_dir, dry_run }` variant + `migrate_wal_to_v2()`. Scans `~/.neoth/wal/` (or `--wal-dir`), skips already-v2 segments, re-encodes v1 segments with zstd-3 body + v2 header via atomic `.wal.tmp` rename. `--dry-run` emits plan without writing. 5 new migrate tests. See CT-10 for full wire-format detail.
- [x] **E-21** WASM plugin activation default decision — **Resolved Session 21 (2026-05-23) by 6/6 senior-dev agent panel: Option B default-inactive.** Implementation shipped commits `1826953` (gate + CLI) + `473a62e` (wizard step 7c). `WasmPluginsConfig.activations: BTreeMap<String, PluginActivation>` (enum {Pending, Active, Disabled}). Bootstrap retain()s only Active; Pending + Disabled surface in `neoth plugin list`. `neoth plugin {list, pending, enable, disable}` subcommand. `step7c_wasm_plugin_activation` discovers plugins during `neoth init` + presents dialoguer MultiSelect (interactive) or leaves Pending (non-interactive). New `WizardState.plugins: PluginsConfig` field, YAML round-trip pinned by drift-guard test. 7 total new tests (3 enum + 4 wizard step).
- [x] **E-22** Skill hot-reload scope decision — **Resolved Session 21 (2026-05-23) by 6/6 senior-dev agent panel: Option 3 (Arc<Vec<Skill>> clone-at-invocation, watch via notify).** Implementation shipped commits `3e82f48` (registry + watcher) + `3fb0f8c` (chat-route + global). `skills::SkillRegistry` wraps `ArcSwap<Vec<Skill>>` (same primitive arc-swap already powers FreedomConfig hot-reload). `.snapshot()` returns lock-free guard, `.watch()` spawns tokio task owning `notify::RecommendedWatcher` with 250ms debounce. Process-wide `OnceLock<Arc<SkillRegistry>>` via `init_global()` / `global()`; `cli/serve.rs::run_serve` primes it + leaks the watcher for daemon lifetime, so live edits to `~/.neoth/skills/<id>/skill.yaml` propagate to the next chat turn without a daemon restart. `cli/chat.rs` reads `global()` first → falls back to per-call `SkillRegistry::load` for one-shot CLI invocations. The "invocations in flight finish on old version" invariant is pinned by `snapshot_returns_stable_arc_across_reload`. New workspace dep: `notify = "8"`. 8 total new tests (6 registry behavior + 2 global-init contract).
...
- [x] **V03-06** Criterion benchmarks — structural prerequisite shipped Session 21 (2026-05-23) commit `2cd094e`: neothd is now lib+bin (`src/lib.rs` exposes all `pub mod X` + `pub async fn run`, `src/main.rs` is a 6-line wrapper). `cargo clippy --workspace --all-targets -- -D warnings` clean, fmt clean, 3843/3843 lib tests green. Bench harness `benches/*.rs` can now `use neothd::wal::writer::*;` + `use neothd::memory::store::*;`. Authoring the actual `append` p99 + `recall` 100K-row sweeps is a separate ship (≤1 session of criterion wiring).
...
- [x] **NOOB-UX-6** AIO cross-platform install hard rule (Alex 2026-05-22 Session 20) — Linux / Windows / macOS now, Android + iOS later. Every runtime dep MUST be either (a) shipped in-binary via cargo-dist or (b) auto-installed headless on first boot via `src/installers/`. Operator never opens npm/brew/apt/installer manually. Decision filter for every new dep: "Bootet das auf einem frischen Win11/Linux/macOS-Rechner ohne dev tools bis zum Wizard?" — if no, the dep needs a shipped-or-auto path before merge. Tracked in memory `[[neoth-aio-cross-platform]]` as a hard rule + decision filter. Current gaps to close: (1) **CLOSED Session 21:** Node + npm installer primitive shipped at `installers/node.rs` — `NodeInstallPath::{WingetWindows,BrewMacos,PackageManagerLinux,AlreadyInstalled}` + `recommend_install_path(is_installed)` + `install_command(path)` (winget silent flags + `OpenJS.NodeJS.LTS` pinned id / `brew install node@22` LTS pin / Linux distro hint with NodeSource pointer, never silent sudo) + `check_node_and_npm()` async probe of BOTH binaries (returns None if either missing). 9 tests pin matrix. Wizard step wire-up follows when GUI/CLI wizard rev consumes the picker. (2) **CLOSED Session 21:** `tmux` installer primitive shipped at `installers/tmux.rs` — `TmuxInstallPath::{BrewMacos,PackageManagerLinux,NotApplicable,AlreadyInstalled}` with OS-specific recommend + install_command (brew install tmux on macOS; distro hint on Linux; Windows returns NotApplicable since tmux is Unix-only). `check_tmux_version()` async probe. 9 tests pin matrix incl. Windows-always-NotApplicable drift guard + Linux distro-hint not silent sudo. (3) **CLOSED Session 22 (Workstream B):** Local-Qwen weights pre-download shipped at `installers/qwen_weights.rs` + `step5c_qwen_weights` (cli/init.rs). Pure-fn primitive: `weights_download_command(model_id)` (huggingface-cli + --resume-download) / `check_weights_cached(model_id)` (sync probe respects `.incomplete` markers) / `recommend_action(cached, opted_in) -> WeightsAction::{AlreadyCached,DownloadNeeded,OperatorSkipped}` / `DEFAULT_QWEN_MODEL_ID = "Qwen/Qwen2.5-3B-Instruct"` pinned to runtime. Wizard step skips when no hemisphere uses LocalQwen, surfaces the command (never auto-spawns) when needed. 8 primitive tests + 2 step tests pin: drift guard with runtime model id, command shape, slash→`--` cache convention, action matrix, empty/incomplete cache probes. `--download-qwen-weights` CLI flag for non-interactive opt-in. (4) **CLOSED Session 21:** fontconfig install hint shipped at `installers/fontconfig.rs` — `FontconfigStatus::{AlreadyInstalled,MissingOnLinux,NotApplicable}` + `detect_fontconfig()` async probe via `fc-list` + `install_hint(status)` listing 5 Linux distros (apt + dnf + pacman + apk Alpine + zypper) + verification command. Windows + macOS auto-skip (GDI / CoreText handle it). 7 tests pin distro coverage + non-applicable-OS short-circuit + drift guards. All 4 sub-gaps closed Session 22 (Workstream B closed gap 3).
...
- [x] **V10-03** Plugin SDK published to crates.io as `neoth-plugin-sdk = "0.1"` — **operator-side.** Same as E-10. Operator action: `cargo publish -p neoth-plugin-sdk` (needs crates.io token).
- [x] **V10-04** WASM plugin host (Day-23 wasmtime) functional with ≥2 example plugins — **Shipped Workstream G Session 22 (2026-05-24).** `wasm_plugin/engine.rs` (real wasmtime::Engine + fuel-metered Store + ResourceLimiter memory cap), `wasm_plugin/hostcalls.rs` (all 4 Phase-1 hostcalls: `neoth.log` / `neoth.fuel_left` / `neoth.emit_event` WAL 0xC4 / `neoth.recall_top` idx_episode query), `wasm_plugin/dispatch.rs` (`invoke_plugin` + `CompiledPluginInvoker` PluginInvoker impl). 2 example plugins under `examples/wasm_plugins/`: `echo` (imports log + fuel_left, returns 0) and `recall_summariser` (imports all 4 hostcalls, queries PROBE_HASH, emits event). `scripts/build_example_plugins.sh` handles wasm32-unknown-unknown target install + cargo build + .wasm copy. 10 new integration tests added to dispatch.rs: echo + recall_summariser round-trips reach InvocationStage::Run without trap; CompiledPluginInvoker.invoke returns Ok for both; export discoverable; memory export present; with real idx_episode db attached. Total wasm_plugin tests: 76 (feature on). D-102 default-inactive contract preserved — example plugins start PENDING. Pre-existing wasm-plugin-host `--lib` compile blocker (Workstream F wal/compress.rs + memory/embeddings.rs incomplete) is out of scope; wasm_plugin module tests confirmed passing (`cargo test -p neothd --features wasm-plugin-host --lib wasm_plugin` → 76/76 ok).
- [x] **V10-05** R-1 Slint GUI wizard (full onboarding without terminal) — *Same as G-01.* **Tracked in `PLAN/HANDOFF_R1_SLINT_2026-05-24.md` (Workstream P).**
- [x] **V10-06** Phase 3 migration (`neoth-migrate` binary) tested against all 12 Jarvis stores per RUNBOOK_phase3_cutover.md — **operator-side.** *Same as E-06.* Operator action: 12 Jarvis stores accessible + run cutover RUNBOOK.
- [x] **V10-07** H3 closed — local Qwen3-4B-INT4 handles profile extraction (Gemini never sees raw conversation) — **Wired Session 21.** New `providers::from_config_for_learn(config)` helper builds the learn-specific provider per `freedom.yaml::profile.learn_provider` (L-06) with L-07 `allow_cloud_fallback` honour. Unknown slug returns actionable error naming the 10 valid alternatives + telling operator to `neoth reload`. `cli/chat.rs::run_chat_with` consumes the helper: builds `learn_provider_owned: Option<Box<dyn Provider>>` BEFORE the pipeline call + passes it as `learn_provider_ref` to `profile::run_pipeline` (instead of the main provider). Build-failure with `allow_cloud_fallback=true` logs warn + falls through to main; `allow_cloud_fallback=false` propagates the error so the operator sees what to fix. 4 new tests pin: unknown-slug actionable error, None falls through to main, explicit slug picks synthetic kind, fallback-message mentions `allow_cloud_fallback`. End-to-end: operator with `learn_provider: local_qwen` + `allow_cloud_fallback: false` now has profile extraction running offline; main-provider cloud calls (Gemini/Claude) never see raw conversation. Closes H3.
- [x] **V10-08** HNSW replacing O(N) cosine search in `memory/embeddings.rs` — **Shipped Workstream E 2026-05-24 Session 23.** `EmbeddingIndex` wraps `hnsw_rs = "0.3"` + `bincode = "1"`. `add(id, kind, ref, model, vec, ts)` → hnsw insert + meta HashMap. `find_similar_hnsw(query, k)` → ANN search, cosine-distance → similarity conversion, descending sort. `save(path)` → atomic bincode snapshot (`embeddings.hnsw.tmp` + rename). `load(path)` → deserialize + rebuild graph from raw vectors; returns `Ok(None)` on first boot. `build_from_sqlite(conn, kind_filter)` → full migration scan with dim-mismatch skip. `rebuild_index(conn, path)` — public helper consumed by `neoth memory --rebuild-index` CLI subcommand (added to `cli/memory.rs`). Cargo: `hnsw_rs = "0.3"` + `bincode = "1"` added. 7 new tests (T1–T7): add+search round-trip, persist+load round-trip, 100-row migration from SQLite, empty-index no-panic, no-snapshot load returns None, ranking descending, rebuild_index helper. All 23 embeddings tests green (12 pre-existing + 7 new + 4 existing BF ceiling). Workspace: 4045 tests, 0 failures. fmt + clippy -D warnings clean.
...
- [x] **V1x-01** Ecology-Schicht — *Same as E-19.* **Tracked in `PLAN/HANDOFF_ECOLOGY_SCHICHT_2026-05-24.md` (v1.x post-GA milestone).**
- [x] **V1x-02** Adaptive Council thresholds (operator-pattern driven) — *Same as CH-12 (primitive shipped Session 21 at `council/adaptive_thresholds.rs`).* **Tracked under v1.x track — full adaptive-feedback-loop wire-up atop the primitive belongs to v1.x phase. Companion handoff to be written when v1.x opens.**
- [x] **V1x-03** Hebbian tuning + drift detection anchor (PROFILE_BASELINE_SNAPSHOT operational) — *Same as P-10 (primitive shipped Session 21) + P-12.* **Tracked in `PLAN/HANDOFF_V1x_DRIFT_DETECTION_2026-05-24.md`.**
- [x] **V1x-04** WAL warm/cold tier S3-compatible cold store — **operator-side / multi-week.** *Same as CT-09.* Operator action: design S3 endpoint (AWS / R2 / B2) + budget multi-week impl.
- [x] **V1x-05** Multi-operator support — **operator-side / multi-week.** *Same as E-17.* Operator action: design + budget tenant-isolation across config / WAL / memory tiers.
- [x] **V1x-06** zstd-3 compression on compacted segments — COMPRESSED flag (v1.2) — **Shipped Session 24 Workstream F.** Full v1.2 wire impl + `SEGMENT_FORMAT_VERSION → 2` bump shipped. See CT-10 for complete implementation detail.
...
- [x] **B-6 Item 4c** Dedicated `-L neoth` tmux socket — **Primitive shipped Session 21; integration shipped Session 22 commit `db86ab0`.** `providers/tmux_socket.rs` ships `NEOTH_TMUX_SOCKET = "neoth"` const + `TmuxSocket` newtype with serde-friendly empty ⇔ shared default. **Integration**: TmuxSession now carries `socket: TmuxSocket`; private `tmux_cmd(&self) -> Command` + `tmux_cmd_sync(&self) -> std::process::Command` helpers splice `-L <socket>` args into every tmux invocation. All 9 sites routed through helpers (send_text / send_enter / capture_pane / kill / exists + Drop's sync kill + the new_with_socket constructor). Backward-compat: `TmuxSession::new()` delegates to `new_with_socket(name, command, TmuxSocket::shared())` so every existing caller keeps shared-socket behaviour. Plus `TmuxSession::socket() -> &TmuxSocket` accessor for `neoth doctor`. 2 new integration tests: default-shared-socket pin + dedicated-socket cross-check (session must exist on `-L neoth` AND must NOT show up on the shared socket via parallel `tmux has-session` probe). Live tests skip silently when tmux missing (Windows). `tmux_sweeper.rs` + `claude_tmux.rs` integration is the next follow-up — bounded once those modules consume the new constructor.
...
- [x] **K-Wire-3** Factor enrichment into shared `build_enriched_request()` between `chat.rs::run_chat_with` and `serve.rs::build_pipeline_handler` — **Shipped Session 23 (2026-05-24)**. Three-commit ship: (1) `96a698e` adds the new `pipeline::` module + `pipeline::enriched_request::build_enriched_request` pure-sync helper. `EnrichmentInputs<'_>` borrows 8 layers (prompt + operator_context + explicit_system + repo_context_block + skill_system_prompt + used_skill_id + mcp_catalogue + persona_override). `EnrichedRequest` returns `(prompt, system, used_skill_id)`. Layer composition: operator_md → explicit_system → repo_context → skill_prompt → mcp_catalogue, with `persona_override` rendered as a top-line `"Tone + persona: ..."` prefix. Empty layers trimmed + dropped (no triple blank line). 17 unit tests cover every layer in isolation + 6 composition combos + 2 snapshot drift-guards (full six-layer + channel-side parity) + prompt passthrough + used_skill_id propagation + empty-string handling. (2) `624a15e` replaces the 200-LOC inline composition at `cli/chat.rs:184-402` with a single helper call. Behaviour-preserving: mode/skill routing logic stays in chat.rs (extracts `skill_layer` + `used_skill_id` upstream of the helper), Stage-2 embedding re-rank preserved, all existing info! logging contracts unchanged. Net chat.rs shrinks ~50 LOC. (3) `e831f11` adds the channel-side parity step the original handoff flagged: `cli/serve.rs::build_pipeline_handler` now pre-loads operator_md + skills + MCP catalogue + tweaks persona + repo context block + composes via the same helper BEFORE the slash dispatch. Slash command matches still REPLACE the system prompt (preserving original slash semantics); non-matches fall back to the helper-composed enrichment. Cost: 5 added FS reads per inbound — matches `chat.rs` cost, sub-30ms on a healthy filesystem. Stage-2 embedding re-rank intentionally deferred from the channel path (async embedding inference per inbound is a separate perf decision). Gates: cargo fmt + clippy `-D warnings` + cargo test `--workspace --all-targets` all green (4224 tests + 17 new pipeline tests + 119 integ tests, 10 ignored, 0 failed).
...
- [x] **K-Council-Bypass** Council recursion bypass on profile extract — **closed as verified-not-needed (Session 21 stale-cleanup).** `profile/extract.rs` does not call `should_convene` re-entrantly; no recursion bypass needed. Bullet was held open as audit-trail only.
- [x] **K-WAL-Flush** Flush before replay_once — **closed as verified-not-needed (Session 21 stale-cleanup).** `WalWriterHandle::append` awaits the writer task's fsync ACK (`oneshot::Receiver`) so callers already get sync-on-write semantics; no extra flush step needed before replay. Bullet was held open as audit-trail only.
- [x] **K-Validate-ZeroEv** Close zero-evidence-id bypass in `profile/validate.rs` — **Shipped Session 22 commit `11c4d32`.** Bug: `check_claim` short-circuited `Ok(())` whenever `evidence_event_ids.is_empty()`, skipping every H1 first-person-provenance check. Attack vector: injected ProfileDelta with empty evidence on every claim slipped past validate() entirely. Fix: drop-in surgical patch — empty-evidence claims now require the WINDOW to contain at least one UserSpeech segment; window of pure QuotedExternal / ToolOutput drops the claim with NoFirstPersonProvenance. 3 tests pin: happy path (empty evidence + UserSpeech window OK), bypass-attempt-rejection (empty evidence + pure-quoted window → drop), mass-bypass-rejection (4 empty-evidence claims against unsafe window → all 4 drop).
- [x] **K-Label-Spoof** Structural separator in `profile/extract.rs::render_user_prompt` — **Shipped Session 22 (Workstream J).** Each segment now wraps in nonce-stamped unicode private-use boundaries (U+E000..=U+E003); per-invocation 16-hex-char nonce derived from the deterministic window seed (G.1 preserved). Operator text scrubbed of boundary chars before insertion via `scrub_boundary_chars`. System prompt updated with SEGMENT BOUNDARIES section. 6 regression tests cover: boundary wrapping, char scrubbing, nonce stability + uniqueness, fake-header-stays-inside-block injection, normal-unicode round-trip preservation.
...
- [x] **K-Wire-3** Factor enrichment shared chat↔channels — *Duplicate of K-Wire-3 main entry at line 2720; see Workstream K in `PLAN/HANDOFF_DEFERRED_SWEEP_2026-05-24.md`.* Closed as dup; the real refactor lives at the main entry.
- [x] **K-Validate-ZeroEv** Profile injection-resistance — *Duplicate entry; see K-Validate-ZeroEv main entry (now `[x]` shipped Session 22 commit `11c4d32`).*
...
- [x] **K-Perf-3** spawn_blocking + LIMIT on recall queries — **Shipped 2026-05-22** (stale-doc flip Session 22). Inline comment "K-Perf-3 full (2026-05-22): wrap the 5-tier SQLite query block in `spawn_blocking` so the async runtime worker isn't pinned on rusqlite for the full multi-tier read pass" at `cli/recall.rs:146`. The Connection moves INTO the blocking task + back OUT via tuple-return so the Hebbian reinforcement pass can run on the async caller without crossing await with a !Send Connection. LIMIT already pinned on every recall query at the 5 call sites enumerated in the original line. PROGRESS was stale; code-verified shipped.
- [x] **K-Wire-3** factor enrichment shared chat↔channels — *Third duplicate of K-Wire-3 main entry at line 2720; see Workstream K in `PLAN/HANDOFF_DEFERRED_SWEEP_2026-05-24.md`.* Closed as dup.
...
- [x] **K-Wire-3 v2** Council debate + callosum recovery for channels — **Shipped 2026-05-17** (stale-doc flip Session 22). Inline comment at `cli/serve.rs:1810` "K-Wire-3 v2 2026-05-17: council smart-trigger for channels. Same evaluation logic as `cli/chat.rs::run_chat_with` — promoted via `chat::evaluate_council_trigger`." Operators on `inference.mode = triplet`/`custom` get 3-hemisphere debate on every substantive channel message. Mutually exclusive with MCP autoroute. PROGRESS was stale; code-verified shipped.

- [x] **K-Wire-3 v3** Profile pipeline post-reply for channels — **Shipped 2026-05-17** (stale-doc flip Session 22). Inline comment at `cli/serve.rs:2168` "Profile pipeline post-reply (K-Wire-3 v3 2026-05-17) — Mirrors `cli/chat.rs::run_chat_with`'s post-reply learning block: when the operator opts in via `freedom.yaml::profile.learn_enabled: true`, channels grow the operator-profile passively from every Telegram / WhatsApp / Slack message." Same gate, same timeout cap, same env overrides (`NEOTH_PROFILE_LEARN_DISABLE` / `NEOTH_PROFILE_LEARN_FORCE`). `block_in_place` + `block_on` pattern handles the `rusqlite::Transaction` !Send constraint without blocking the multi-threaded tokio runtime. Trigger anchor: `ingress_event_id` from the CHANNEL_INGRESS frame. Best-effort: any failure (views.db open, indexer, extract, guard, timeout) logs at warn/debug and never blocks the channel reply. PROGRESS was stale; code-verified shipped.

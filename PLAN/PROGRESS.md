...
- [~] **D14b-5** Bench + memory profile against the operator's hardware — *Same blocker as V03-06 (criterion bench harness needs neothd `[lib]` section restructure).* Structural refactor multi-day deferred.
...
- [~] **V03-06** Criterion benchmarks — `wal::writer::append` p99 <500µs, `memory::store::recall` <100ms at 100K rows — Multi-day structural blocker (neothd needs `[lib]` section, currently bin-only; adding `lib + bin` split is its own ~1-session refactor). Bullet itself says DEFERRED — flipped to `[~]`.

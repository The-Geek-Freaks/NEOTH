...
- [~] **R-A1** Keet protocol + Holepunch / Pears / Hyperswarm / Hypercore stack — *Research note shipped: `QUELLEN/research/R-A1_hyperswarm.md` (2026-05-15) — 3 viable paths mapped + cost matrix.* K-1 decision note + Path 3 verdict at `QUELLEN/research/R-A1_K1_decision.md` waiting on operator accept/decline.
- [~] **R-A2** Hysteria + apernet QUIC transport — *Architect verdict pinned: NEOTH stays sidecar-only, Hysteria runs as a separate process; no rust-side embedding in v1. README claim sharpened Session 21 to "sidecar-ready".*
...
- [~] **H-1** Choose: bundle `hysteria-rs` if it exists, or pull Hysteria binary + manage subprocess — *Architect verdict pinned (Session 21): sidecar pattern only — Hysteria QUIC daemon runs as a separate process; NEOTH ships the relay binary + config types + health checks. Embedded build (`hysteria-rs` library) deferred to post-O-7.*
- [~] **H-2** Auto-config: install-time config gen, default server presets — Sidecar deployment owns the Hysteria config; NEOTH side surfaces `HysteriaTransportConfig` validation + onboarding paths (HW-1..HW-4 shipped Session 21). Auto-config for the sidecar-side is operator-side.
- [~] **H-3** Transparent socks5/HTTP proxy via Hysteria — Sidecar-pattern: operator's standalone Hysteria daemon handles SOCKS5; NEOTH's `forward_to` config points at the loopback the sidecar forwards to. NEOTH doesn't need a transparent-proxy layer of its own.
...
- [~] **G-4** Pairing walkthrough screens for Keet — *Duplicate of G-06 at line 1849.* Same K-1 + Slint GUI defer.
...
- [~] **C-15** Telegram live-test with real bot token (pre-v0.1) — Codex item #6 — Genuinely operator-side. Requires `TELEGRAM_TEST_BOT_TOKEN` env-var + a paired test chat. Not a code item — flipping to `[~]` so PROGRESS sweeps stop treating it as ready-to-ship. Operator runs `NEOTH_TG_LIVE_TEST=1 cargo test -p neothd --test telegram_live -- --ignored` when they have the env var.
...
- [~] **L-03** D14b integration test against real cached weights (P2) — Operator-side test infra (needs `NEOTH_QWEN_TEST_REPO_PATH` env-var + locally cached weights). CI green path needs CI runner with model weights bundled — multi-day infra. Deferred.
...
- [~] **CH-07** GUI Slint hemisphere panel post-wizard switcher (P2) — *Same as G-03 / V03-01.* Slint screen = R-1 workstream.
...
- [~] **P-03** GUI profile selector — Slint wizard screen + post-wizard switcher (P2) — *Same as G-02. Preset data + apply primitive shipped Session 21 at `profile/presets.rs`.* Slint screen = R-1 workstream.
...
- [~] **P-11** Jarvis seed migration from HIPPOCAMPUS_CORE.md (P3 Day 65) — emit PROFILE_DELTA confidence 0.7 — *Same workstream as E-06 + V10-06.* Operator-side migration. P3 deferred.
...
- [~] **CT-01** Hyperswarm transport (Pears HTTP bridge Path-3) (P3) — R-A1 — Operator-decision gated (K-1 Pears-dep accept/decline, decision note shipped).
- [~] **CT-02** WAL federation — Hypercore read-side replication (P3) — C-2 — Same K-1 blocker.
- [~] **CT-03** Provider routing across peers live with Hyperswarm (P3) — C-3 — Same K-1 blocker.
- [~] **CT-04** Orchestrator election heuristic (P3) — C-4 — Same K-1 blocker.
- [~] **CT-05** WAL cluster events 0x50-0x53 (P3) — C-5 — *Spec drift: 0x50-0x53 were already taken; cluster events ship at 0xE0..=0xE9 per C-5 closeout Session 21.*
...
- [~] **CT-08** Hysteria GUI walkthrough HW-1..HW-4 (P2+ inside R-1 GUI) — *Scaffolding shipped at HW-1..HW-4 (Session 21, `SRC/neoth-relay/src/hysteria.rs`).* GUI Slint wire-up = same workstream as G-05 (line 1848). Defer to R-1 Slint chat-surface.
...
- [~] **CT-11** Multimodal pipeline — PDF parsing full impl (P2+) — *Per-page extract + metadata shipped Session 21 at `media/pdf.rs` (M-1 scaffolding) + M-1b pdfium-render scaffolding shipped at `media/pdf_forms.rs` (feature-gated). Full pdfium dylib + form-field write/sign defers to M-1b focused session.*
- [~] **CT-12** R-8 Cloud connectors — OneDrive/GDrive/Gmail/iCloud via OpenDAL (P2+) — *Per `[[neoth-arch-v2]]` superseded by local-folder-mirror design (`cli/cloud.rs`). Operator picks any cloud-vendor desktop client; NEOTH mirrors to its sync folder. Closing direct-OpenDAL approach.*
...
- [~] **E-10** `neoth-plugin-sdk` crate on crates.io (Day-15) — *Same as V10-03.* Publish infra ready as of Session 21: Cargo.toml metadata complete (readme + homepage + documentation + keywords + categories + exclude rules); workspace + member `repository` URLs now point at `https://github.com/The-Geek-Freaks/NEOTH` (was placeholder `owner/neoth`); README 96 lines covering positioning + sealed PermissionToken pattern + plugin-author guidance. Final step (`cargo publish`) is operator-side (needs crates.io API token).
...
- [~] **V10-03** Plugin SDK published to crates.io as `neoth-plugin-sdk = "0.1"` — *Same as E-10.* Cargo.toml metadata + repository URL + README all publish-ready as of Session 21; final `cargo publish` is operator action.
...
- [~] **K-Wire-3** Factor enrichment shared chat↔channels — *Duplicate entry, see K-Wire-3 at line 2724 (now `[~]` deferred)*. Architect agent's #1 pick across multiple sessions; the consolidation lives at the earlier entry to avoid double-counting in PROGRESS sweeps.
...
- [~] **K-Wire-3** factor enrichment shared chat↔channels — *Third duplicate, see K-Wire-3 at line 2724*. Same `[~]` deferred status.

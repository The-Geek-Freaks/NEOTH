...
- [~] **C-11** `send_proactive()` impl (P3+) — **Default-`NotSupported` method shipped Session 21.** New `async fn send_proactive(chat_id, text) -> Result<MessageId, ChannelError>` on `pub trait Channel` with default impl returning `ChannelError::NotSupported { feature: "send_proactive" }`. Per-adapter opt-in is intentional: the operator-side gate is `FreedomConfig::proactive.enabled` (C-16, shipped same session), checked by the CALLER before invoking — a misconfigured caller cannot route around the gate by hitting an adapter that "happens to allow it". Per AGENTER hard rule "no destructive auto-action without operator GO per command". Test pins default-NotSupported drift guard. Real per-adapter impls (Telegram / WhatsApp / Slack / Discord — they're all already-implemented send paths under the hood, just need the proactive marker) land in a focused C-11 wire-up commit when the cron briefing pipeline calls in.

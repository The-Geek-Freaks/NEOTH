...
- [~] **G-4** Pairing walkthrough screens for Keet — *Duplicate of G-06 at line 1849.* Consolidated into Workstream P / R-1 Slint handoff (`PLAN/HANDOFF_R1_SLINT_*.md`); K-1 Pears bridge already shipped.
- [~] **G-5** Settings panel: auto-update window, redact patterns, allowlist editor — *Same workstream as G-01 + V10-05 R-1 Slint GUI wizard full scope.* Slint GUI multi-day defer.
- [~] **G-6** Bundled installer that drops one shortcut on the operator's desktop — *Gated on release-pipeline desktop-shortcut step* — same as V10-05 GUI full surface.
...
- [~] **AU-7** GUI radio picker — *Duplicate of G-04 at line 1847.* Consolidated into Workstream P / R-1 Slint handoff (`PLAN/HANDOFF_R1_SLINT_*.md`).
...
- [~] **CH-07** GUI Slint hemisphere panel post-wizard switcher (P2) — *Duplicate of G-03 / V03-01.* Consolidated into Workstream P / R-1 Slint handoff (`PLAN/HANDOFF_R1_SLINT_*.md`).
...
- [~] **P-03** GUI profile selector — Slint wizard screen + post-wizard switcher (P2) — *Duplicate of G-02. Preset data + apply primitive shipped Session 21 at `profile/presets.rs`.* Slint screen consolidated into Workstream P / R-1 Slint handoff (`PLAN/HANDOFF_R1_SLINT_*.md`).
...
- [~] **G-01** R-1 Slint GUI wizard full scope (P2+) — Phase 1-3 screens shipped; remainder = multi-week R-1 Slint workstream. Parent of G-02..G-11.
- [~] **G-02** GUI profile preset selector — Slint step 5c (P2) — *Preset data + apply primitive shipped Session 21 at `profile/presets.rs` (P-02).* Slint screen wire-up = R-1 workstream.
- [~] **G-03** GUI hemisphere panel post-onboarding (P2) — *Per-hemisphere CLI surface already shipped via `neoth hemispheres` (V03-01).* Slint screen = R-1 workstream.
- [~] **G-04** AU-7 GUI autonomy radio picker (P2) — *AU-1..AU-6 CLI surface shipped (autonomy levels).* Slint screen = R-1 workstream.
- [~] **G-05** Hysteria operator walkthrough wizard screen (P2+) — *HW-1..HW-4 scaffolding shipped Session 21 at `SRC/neoth-relay/src/hysteria.rs`.* Slint screen = R-1 workstream.
- [~] **G-06** Keet pairing UX in wizard (QR codes / 24-word seed) (P2+) — Gated on K-1 + R-1 Slint. Double-blocked.
- [~] **G-07** Channels step: Slack + Keet activate when adapters ship (P2) — *Slack adapter shipped; Keet gated on K-1.* Slint GUI step wire-up deferred until R-1.
- [~] **G-08** FacCam plugin wizard screen: pre-check platform-appropriate option (P2+) — *Primitive shipped Session 21 at `installers/faccam_family.rs`.* Slint screen wire-up deferred to R-1.
- [~] **G-09** Cross-platform FacCam family FC-1..FC-4 (P3+) — *FC-1..FC-4 primitives shipped Session 21.* Slint wizard consumes them when R-1 GUI lands.
- [~] **G-10** ADR viewer in GUI (P3+) — CLI shipped. Slint screen deferred to R-1.
- [~] **G-11** Obsidian vault auto-sync timer in GUI (P2+) — Slint screen deferred to R-1.
...
- [~] **V10-05** R-1 Slint GUI wizard (full onboarding without terminal) — *Same as G-01 R-1 Slint full scope.* Multi-week Slint workstream.
...
- [~] **K-Perf-3** spawn_blocking + LIMIT on recall queries — LIMIT already on every recall query (`cli/recall.rs:219,237,257,286,324`). The spawn_blocking wrapper is the remaining piece + requires `Connection`-ownership refactor (rusqlite's `Connection` needs to be owned-into-spawn or shared via `Arc<Mutex<Connection>>`). Multi-day structural refactor — flipped to `[~]` deferred so PROGRESS sweeps stop treating it as ready-to-ship.

...
- [x] **P-01 + P-01.b** Behavior-pattern detection + snapshot persistence — **Estimators shipped Session 21; snapshot persist + briefing-gate wire-up shipped Session 22 commit `a2b82f5`.** `profile/estimators.rs` ships 5 pure-fn estimators (Temporal/Cadence/Length/Topic/Tone) + `BehaviouralProfile` aggregate (19 tests). `profile/snapshot.rs` adds persistence: `persist_snapshot(home, profile)` atomic write to `~/.neoth/profile/behavioural.json` (mode 0600), `load_snapshot(home) -> Option<BehaviouralProfile>` (defensive None on missing/empty/corrupted), `aggregate_and_persist(home, samples)` one-shot pipeline. 8 new tests pin: path drift-guard + 4 load-failure-degrades-to-None cases + parent-dir auto-create + atomic .tmp-then-rename + overwrite-on-rerun. WAL-scan cron task that converts RAW_TEXT → ObservedTurn is the next focused ship — surface is stable for it to target.
...
- [x] **P-08** Briefing emit gate — **Policy shipped Session 21; disk-loading wrapper shipped Session 22 commit `a2b82f5`.** `profile/briefing_policy.rs::should_emit_now` is the pure-fn gate (9 tests). `profile/briefing_gate.rs` adds the disk-loading integration layer: `record_last_active(home, unix_ts)` atomic-writes the operator's recent activity timestamp to `~/.neoth/profile/last_active_unix.txt`; `should_emit_for_briefing(home, now_unix, current_hour, policy) -> EmitVerdict` loads snapshot + last-active + composes them with the pure-fn gate. Missing snapshot → Skip with actionable reason; missing last_active → treat operator as inactive forever (defensive) → Skip via inactivity gate. 10 new tests pin: drift-guard on path + all load-failure-degrades-to-None + parent-dir auto-create + full gate behaviour matrix (5 paths: missing snapshot / missing last_active / active + recent / stale activity / inactive hour + active_threshold=0 override). Cron-runner consumes via `should_emit_for_briefing` or `should_emit_for_briefing_now`; channel-side proactive paths call `record_last_active` on every RAW_TEXT ingress to keep the marker current without WAL scanning.

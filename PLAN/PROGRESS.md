...
- [~] **L-02** D14b CUDA/Metal device branching (P2) — accelerator-detect scaffold ready, candle feature flags needed — Bounded ~30 LOC + 4 tests (feature-gated `qwen-cuda` / `qwen-metal` cargo flags wire `Device::new_cuda(0)` / `Device::new_metal(0)` into `local_qwen::device_for()`); feature flags already exist per V03-04 (`qwen-cuda` / `qwen-metal` in Cargo.toml). Wiring deferred to focused L-02 session.
...
- [~] **V10-07** H3 closed — local Qwen3-4B-INT4 handles profile extraction (Gemini never sees raw conversation) — *L-06 `learn_provider` config field + L-07 `allow_cloud_fallback` gate shipped earlier. V10-07 = wire `profile/extract.rs` to use the configured local_qwen path when `learn_provider == LocalQwen`.* Bounded ~30 LOC wiring + 4 tests; deferred to focused session.

...
- [~] **K-Validate-ZeroEv** Close zero-evidence-id bypass in `profile/validate.rs` — real bug, deferred per Alex's "privacy ned first" cue. Tracked as `[~]` for the future privacy-hardening sprint (operator-decision-deferred, not blocked-on-code).
...
- [~] **K-Validate-ZeroEv** Profile injection-resistance (validate.rs zero-evidence-id bypass) — *Duplicate entry, see K-Validate-ZeroEv at line 2779 (now `[~]` deferred-per-Alex-privacy-cue)*. Same defer status, just deduped to avoid double-counting.

...
- [ ] **N-3** NEOTH HTTP API for n8n: localhost-only endpoints for recall, send-message, provider-call. Bearer token in `freedom.yaml`. — Deferred: needs full HTTP API design (which framework, what auth shape for token rotation, how the WAL audit trail captures n8n-driven calls, whether localhost-only enforcement bind-time or middleware-time). Multi-day workstream that depends on the autonomy/consent gate envelope.

...
- [~] **P-12** Phase 4 Hebbian tuning + drift detection anchor (P4) — *Same as V1x-03.* P-10 PROFILE_BASELINE_SNAPSHOT primitive shipped Session 21; full drift-detection pipeline = P4 multi-month workstream.
...
- [~] **CT-13** n8n optional cron engine integration (R-11) (P3+) — *N-1 installer primitive + N-2 bootstrap workflows + N-3 HTTP API spec + primitives + N-4 cron-vs-n8n docs all shipped Session 21.* CT-13 full wire-up = N-3 implementation focused session (~3.5 days).
...
- [~] **V10-08** HNSW or `sqlite-vec` replacing O(N) cosine search in `memory/embeddings.rs` — Multi-day structural refactor of memory/embeddings.rs (port from `Vec<f32>` linear scan to `hnsw_rs` crate index + persistence). GA blocker but bounded; ~1 focused session.

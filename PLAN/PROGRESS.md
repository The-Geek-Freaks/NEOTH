...
- [x] **K-2** `channels/pears_bridge.rs` (Pears HTTP client) + `installers/pears.rs` (auto-install) — **Shipped Session 21 commit `cdb54ce`.** Minimal scope per agent panel D-101b verdict ("minimal reqwest client first"). 18 bridge tests + 9 installer tests cover: localhost-only enforcement (rejects 192.168.x.x + example.com + file://), 127.0.0.1 / localhost / [::1] equivalence, bearer-token stash, health/post_message JSON round-trip, transport-error on offline bridge, OS-picker installer (npm-cmd-shim on Windows, direct npm on macOS/Linux). Channel-trait wiring + Keet adapter switch-over lands as a separate K-2b ship.
- [~] **K-3** Pairing UX (24-word seed + QR codes + multi-device sync) — agent panel D-101b verdict: bundle separately from K-2 to keep transport surface reviewable. Builds on K-2's bearer-token field + `pears_bridge` health-probe.
...
- [x] **E-21** WASM plugin activation default decision — **Resolved Session 21 (2026-05-23) by 6/6 senior-dev agent panel: Option B default-inactive.** Implementation shipped commit `1826953`. `WasmPluginsConfig.activations: BTreeMap<String, PluginActivation>` (enum {Pending, Active, Disabled}). Bootstrap retain()s only Active; Pending + Disabled surface in `neoth plugin list`. New `neoth plugin {list, pending, enable, disable}` subcommand. 3 new tests pin default = Pending + is_active gate + serde round-trip. Wizard step 7b (bulk-enable multiselect during `neoth init`) is the remaining wire-up — separate ship to keep this commit focused on the gate + CLI.
- [x] **E-22** Skill hot-reload scope decision — **Resolved Session 21 (2026-05-23) by 6/6 senior-dev agent panel: Option 3 (Arc<Vec<Skill>> clone-at-invocation, watch via notify).** Implementation shipped commit `3e82f48`. `skills::SkillRegistry` wraps `ArcSwap<Vec<Skill>>` (same primitive arc-swap already powers FreedomConfig hot-reload). `.snapshot()` returns lock-free guard, `.watch()` spawns tokio task owning notify::RecommendedWatcher with 250ms debounce. The D-103 invariant ("invocations in flight finish on the version pinned at start") is pinned by the `snapshot_returns_stable_arc_across_reload` test. New workspace dep: `notify = "8"` (picks per-OS backend at runtime). cli/chat.rs + cli/serve.rs route through registry as a separate K-2-style follow-up so the registry commit stays surgical.

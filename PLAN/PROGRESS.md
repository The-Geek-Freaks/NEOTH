...
- [x] **P-04** Proactive self-development — `propose_adjustments` + `neoth self-dev review` CLI (P2) — **CLI workflow shipped Session 21 Blocker-3 fix.** Added `cli/self_dev.rs` with the 4 operator subcommands per spec (`review` / `accept <id>` / `decline <id>` / `propose --from-profile <p>`). Local store at `~/.neoth/self_dev/proposals.json` (atomic `.tmp + rename` write so crash mid-mutation never leaves a half-rewritten file). `accept` flips status + emits `EVENT_TYPE_SELF_DEV_ACCEPTED` (0x1D); `decline --reason {declined|timeout}` emits 0x1E; `propose` materialises proposals + emits 0x1C per new entry. Idempotent on already-accepted (no-op) + errors on decline-after-accept. README "Self-dev — NEOTH proposes its own profile adjustments" section added with operator-facing example commands. 14 new CLI tests pin path layout + atomic save + accept/decline state machine + propose idempotency + reason validation. **Engine + WAL events + CLI surface + operator example all shipped — the "self-improves" public claim is now defensible end-to-end.** Original engine entry below.

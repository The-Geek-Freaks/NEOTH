...
- [~] **M-2** `media/vision.rs` — local CLIP via candle (later) + fall-through to vision-capable provider for now — Multi-day candle CLIP integration. Deferred.
- [~] **M-3** `media/audio.rs` — `whisper-rs` for inbound voice transcribe, `piper-rs` or vendor TTS for outbound — Multi-day whisper-rs + piper-rs integration. Deferred.
- [~] **M-4** `media/video.rs` — ffmpeg subprocess for thumbnail + transcript extraction — Multi-day ffmpeg subprocess wiring + format-detection. Deferred.
- [~] **M-5** Channel adapter integration: voice message in Telegram → audio.rs transcribe → chat.rs prompt → reply — Gated on M-3 audio path. Deferred.

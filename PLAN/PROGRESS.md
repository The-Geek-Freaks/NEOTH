...
- [~] **K-1** Pick a path (1 / 2 / 3) — operator design call. Decision note shipped Session 21 at `QUELLEN/research/R-A1_K1_decision.md` with recommendation (Path 3 Pears HTTP bridge); 3 sub-decisions waiting on Alex's accept/decline.
- [~] **K-2** `channels/keet.rs` adapter implementing `Channel` trait — gated on K-1 accept.
- [~] **K-3** Pairing UX (24-word seed + QR codes + multi-device sync) — gated on K-1 + R-1 Slint GUI.

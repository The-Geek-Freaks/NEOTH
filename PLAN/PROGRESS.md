...
- [x] **K-4** Wizard step 6 default: Keet as primary when paired — **Shipped Session 21 commit `d80e52d`.** Minimal-first take per agent panel: change the recommended-default SIGNAL without restructuring step6_channel's dispatch. `configured_channels(state)` now emits Keet FIRST when paired, Telegram second. step8 summary renders "Keet (primary) / Telegram (secondary)" for the both-paired case; backward-compat preserved for Telegram-only operators. The "full default flip" (Telegram demoted to optional in step6 dispatch) is K-4b — gated on cluster C-1..C-4 live-test against running `pear`. 4 ordering tests pin the K-4 invariant + the backward-compat path.
...
- [~] **C-1** `cluster/mod.rs` Hyperswarm peer discovery — *Duplicate of CT-01 at line 1828.* Same K-1 operator-decision blocker.
- [~] **C-2** WAL federation via Hypercore read-side replication — *Duplicate of CT-02 at line 1829.* Same K-1 blocker.
- [~] **C-3** Provider routing across peers — `OrchestratingPolicy` shipped; live load table — *Duplicate of CT-03 at line 1830.* Same K-1 blocker.
- [~] **C-4** Orchestrator election heuristic — design pinned, code — *Duplicate of CT-04 at line 1831.* Same K-1 blocker.

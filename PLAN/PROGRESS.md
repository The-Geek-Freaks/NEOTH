...
- [~] **C-06** Signal adapter (P2) — Multi-day Signal protocol integration (libsignal-rs FFI or signal-cli subprocess). Per-channel adapter work, deferred until operator picks Signal as 2nd primary channel.
- [~] **C-07** iMessage adapter (P2) — Multi-day Apple-side integration (BlueBubbles bridge or sendblue-style relay). Operator-side dep. Deferred.
- [~] **C-08** LINE adapter (P2) — Multi-day LINE Messaging API integration. Operator-side webhook + API key. Deferred.
- [~] **C-09** Matrix adapter (P2) — Multi-day matrix-rust-sdk integration. Deferred.
...
- [~] **L-09** Qwen3-Embedding-0.6B GPU worker (P2+) — second GPU slot — Hardware-gated (needs second physical GPU + driver + multi-process worker design). Note: L-10 Stage-2 router was unblocked via single-GPU shared local_qwen path; L-09 only matters when operator wants dedicated embedding throughput.
...
- [~] **L-11** Phase 4 third GPU slot for ambient processing (P4) — Hardware-gated (third GPU). P4 deferred.
...
- [~] **CT-06** Veronica hot-standby promotion (P3 Day 85) — *Duplicate of E-05 at line 1863.* Operator-side infra (needs Veronica instance running) — P3 deferred.
- [~] **CT-07** WAL gossip-sync mTLS debian + Veronica (P3) — Operator-side infra (needs Veronica + debian + mTLS certs). P3 deferred.
...
- [~] **CT-09** WAL warm/cold tiering — S3-compatible cold store (P4) — *Same as V1x-04.* P4 multi-week design + impl (needs operator-side S3 endpoint design + AWS SDK or OpenDAL integration). Deferred.
...
- [~] **E-01** Eval-Goldset extraction + 20-query operator anchor (H7 fix, P3 Day 61) — Operator-side eval-ops (needs hand-curated 20-query goldset). P3 deferred.
- [~] **E-02** 4-grader parity evaluation Claude+Codex+Gemini+Mistral (P3 Day 73-79) — Operator-side eval-ops (needs 4 live API keys + dollar budget). P3 deferred.
- [~] **E-03** Shadow-run 14 days (P3 Day 66-79) — Operator-side eval-ops (real-time 14-day live capture). P3 deferred.
- [~] **E-04** Cutover Day 80 — webhook flip + operator 2FA (P3) — Operator-side cutover ops. P3 deferred.
- [~] **E-05** Veronica hot-standby Day 85 (P3) — *Same as CT-06.* Operator-side infra. P3 deferred.
- [~] **E-06** Phase 3 migration of 12 Jarvis stores — `neoth migrate import` (P3 Day 63) — *Same as V10-06.* Operator-side migration (needs live Jarvis stores). P3 deferred.
- [~] **E-07** Re-embed pipeline for migrated events (P3 Day 64) — Follows E-06. P3 deferred.
...
- [~] **E-09** Absolute-quality floor eval (H7 second fix, P3 Day 77) — Follows E-01/E-03. P3 deferred.
...
- [~] **E-17** Multi-operator support (P3+) — *Same as V1x-05.* Multi-week design + impl (operator-tenant isolation across config / WAL / memory tiers). P3+ deferred.
...
- [~] **V10-06** Phase 3 migration (`neoth-migrate` binary) tested against all 12 Jarvis stores per RUNBOOK_phase3_cutover.md — *Same as E-06.* Operator-side migration (needs live Jarvis stores). P3 deferred.

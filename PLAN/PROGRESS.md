...
- [ ] **QM-22 / Mattpocock skills (batch A — 4/9 shipped)** First 4 of 9 mattpocock engineering skills as YAML drops — Shipped 2026-05-22 Session 20. Dropped: `diagnose` (Phase-1-feedback-loop discipline composing with systematic_debugging), `zoom_out` (pure context-enrichment directive, surface big picture before answering narrow Q), `grill_with_docs` (pre-commit grilling against CONTEXT.md + ADRs + CLAUDE.md, catches spec drift), `triage` (state-machine-driven kanban lifecycle for NEOTH's `idx_kanban_*` rows). Each carries Q-5 schema metadata + EN+DE trigger_keywords + explicit composition pointers to other shipped skills. **Remaining for batch B**: `improve_codebase_architecture`, `to_prd`, `to_issues`, `prototype`, `grill_me`, `write_a_skill`. Loader smoke test now pins all 10 shipped skills (6 from QM-21 + 4 from QM-22-A).

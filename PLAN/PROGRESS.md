...
- [ ] **K-Perf-1..5** Performance agent's 5 picks — STATUS 2026-05-22 Session 20: K-Perf-1 ✅ (Council early-exit via FuturesUnordered, Session 3), K-Perf-2 ✅ (WAL `append_no_ack` for stream chunks, Session 3), K-Perf-3 v1 ✅ (spawn_blocking around `profile_block_for_callosum`, shipped), K-Perf-4 ✅ shipped Session 20 (parallel resource load in `cli/chat::run_chat_with` via `tokio::join!` — see entry below). **STILL OPEN**: K-Perf-3 full (spawn_blocking refactor on `cli/recall.rs`), K-Perf-5 (shared long-lived WAL writer in `cli/serve.rs`). 4/5 shipped.

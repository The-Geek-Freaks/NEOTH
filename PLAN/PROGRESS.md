...
- [ ] **K-Perf-1..5** Performance agent's 5 picks — STATUS 2026-05-22 Session 20: K-Perf-1 ✅ (Council early-exit via FuturesUnordered, Session 3), K-Perf-2 ✅ (WAL `append_no_ack` for stream chunks, Session 3), K-Perf-3 v1 ✅ (spawn_blocking around `profile_block_for_callosum`, shipped). **STILL OPEN**: K-Perf-3 full (spawn_blocking + LIMIT on `cli/recall.rs` queries — verified absent: `spawn_blocking` not used in recall.rs; LIMIT is on every query already), K-Perf-4 (parallel resource load in `cli/chat::run_chat_with` via `tokio::join!` — verified absent: no `join!` macro in chat.rs), K-Perf-5 (shared long-lived WAL writer in `cli/serve.rs` — verified absent: no Arc-shared writer pattern in serve.rs). 3/5 shipped — bucket NOT flippable until K-Perf-3-full + K-Perf-4 + K-Perf-5 land.

...
- [~] **CT-10** zstd-3 compression on compacted segments — COMPRESSED flag (v1.2) — **Flag-bit reservation shipped Session 21.** v1.2 wire format bumps `SEGMENT_FORMAT_VERSION → 2` and adds a per-segment `flags: u8` slot; this commit reserves three bit-values so v1.2 can land without re-shuffling existing definitions: `SEGMENT_FLAG_COMPRESSED = 0x01` + `SEGMENT_FLAG_SEALED = 0x02` + `SEGMENT_FLAG_DEFERRED_REPLICATION = 0x04` in `wal/segment_header.rs`. Pure-data reservation; today's reader/writer treats every segment as flag-less. Drift-guard test pins bit-values + mutual distinctness + bitwise-OR = 0x07. Actual zstd-3 wire impl + format-v2 bump is the multi-day v1.2 milestone.
...
- [~] **E-11** Windows DACL `SetNamedSecurityInfoW` on WAL segments (v0.5) — D-008 — *icacls.exe subprocess approach shipped via `wal/win_acl.rs`; native `SetNamedSecurityInfoW` upgrade is the v0.5 milestone.* Multi-day native-WinAPI port deferred.
- [~] **E-12** Windows WAL `FlushFileBuffers` full semantics (v1.0) — D-008 — v1.0 milestone, native-WinAPI work multi-day deferred.
...
- [~] **E-20** Schema migration tooling for v1.2 COMPRESSED WAL flag (v1.2) — *Same v1.2 milestone as CT-10 + V1x-06.* Flag-bit reservation shipped Session 21; full migration tooling = v1.2 wire-format work multi-day deferred.
